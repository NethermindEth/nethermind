...
public class KBucketTree<TNode> : IRoutingTable<TNode> where TNode : notnull
{
    private class TreeNode
    {
        ...
        {
            return Bucket.Count != 0 ? $"{Bucket.Count}({Left}|{Right})" : "";
        }
    }

    public override string ToString()
    {
        return $"KBucketTree({_root})"; // string.Join(",", _buckets.Select((b, i) => (b, i)).Where(b => b.b.Count is not 0).Select(kv => $"[{kv.i}]={kv.b.Count}"));
    }
    ...
    public BucketAddResult TryAddOrRefresh(in ValueHash256 nodeHash, TNode node, out TNode? toRefresh)
    {
        ...
        var logDistance = Hash256XorUtils.MaxDistance - Hash256XorUtils.CalculateDistance(_currentNodeHash, nodeHash);
        var depth = 0;
        while (true)
        {
            if (current.IsLeaf)
            ...
            var goRight = GetBit(nodeHash, depth);
    ...
    {
        var shouldSplit = depth < 256 && targetLogDistance + _b >= depth;
    ...
    {
        var targetDepth = Hash256XorUtils.MaxDistance - distance;
        if (node.IsLeaf)
        {
        ...
        else
        {
            if (depth < targetDepth)
            {
                var goRight = GetBit(_currentNodeHash, depth);
                if (goRight)
    ...
    public TNode[] GetKNearestNeighbour(ValueHash256 hash, ValueHash256? exclude, bool excludeSelf)
    {
        ...
        var shouldNotContainExcludedNode = exclude == null || !firstBucket.ContainsNode(exclude.Value);
        var shouldNotContainSelf = excludeSelf == false || !firstBucket.ContainsNode(_currentNodeHash);
