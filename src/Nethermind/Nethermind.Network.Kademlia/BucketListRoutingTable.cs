...
public class BucketListRoutingTable<TNode> : IRoutingTable<TNode> where TNode : notnull
{
    ...
    {
        return string.Join(",", _buckets.Select((b, i) => (b, i)).Where(b => b.b.Count is not 0).Select(kv => $"[{kv.i}]={kv.b.Count}"));
    }
    ...
    {
        var idx = Hash256XorUtils.CalculateDistance(hash, _currentNodeIdAsHash);
        return _buckets[idx];
    }
    ...
    {
        var startingDistance = Hash256XorUtils.CalculateDistance(_currentNodeIdAsHash, hash);
    ...
    public TNode[] GetKNearestNeighbour(ValueHash256 hash, ValueHash256? exclude, bool excludeSelf)
    {
        ...
        var shouldNotContainSelf = excludeSelf == false || !firstBucket.ContainsNode(_currentNodeIdAsHash);
    ...
    private IEnumerable<KBucket<TNode>> EnumerateBucket(int startingDistance)
    {
        ...
        var left = startingDistance - 1;
        var right = startingDistance + 1;
