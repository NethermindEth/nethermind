    ...
    public static class Hashes
    {
        ...
        static Hashes()
        {
            All = (from hf in Assembly.GetAssembly(typeof(IHash)).GetTypes()
                   ...
                   where hf.GetTypeInfo().ImplementedInterfaces.Contains(typeof(IHash))
                   where !hf.IsNested
            ...
            AllUnique = (from hf in All
                         where !(hf.GetTypeInfo().IsDerivedFrom(typeof(HashLib.Hash32.DotNet)))
                         where !x2.Contains(hf)
                         where !hf.IsNested
                         select hf).ToList().AsReadOnly();

            Hash32 = (from hf in All
                      where hf.GetTypeInfo().IsImplementInterface(typeof(IHash32))
                      where !hf.GetTypeInfo().IsImplementInterface(typeof(IChecksum))
                      select hf).ToList().AsReadOnly();

            Hash64 = (from hf in All
                      where hf.GetTypeInfo().IsImplementInterface(typeof(IHash64))
                      where !hf.GetTypeInfo().IsImplementInterface(typeof(IChecksum))
                      select hf).ToList().AsReadOnly();

            Hash128 = (from hf in All
                       where hf.GetTypeInfo().IsImplementInterface(typeof(IHash128))
                       where !hf.GetTypeInfo().IsImplementInterface(typeof(IChecksum))
                       select hf).ToList().AsReadOnly();
            ...
            FastComputes = (from hf in All
                            where hf.GetTypeInfo().IsImplementInterface(typeof(IFastHash32))
                            select hf).ToList().AsReadOnly();

            NonBlock = (from hf in All
                        where hf.GetTypeInfo().IsImplementInterface(typeof(INonBlockHash))
                        select hf).ToList().AsReadOnly();

            WithKey = (from hf in All
                       where hf.GetTypeInfo().IsImplementInterface(typeof(IWithKey))
                       select hf).ToList().AsReadOnly();

            CryptoAll = (from hf in All
                         where hf.GetTypeInfo().IsImplementInterface(typeof(ICrypto))
                         select hf).ToList().AsReadOnly();

            CryptoNotBuildIn = (from hf in CryptoAll
                                where hf.GetTypeInfo().IsImplementInterface(typeof(ICryptoNotBuildIn))
                                select hf).ToList().AsReadOnly();

            CryptoBuildIn = (from hf in CryptoAll
                             where hf.GetTypeInfo().IsImplementInterface(typeof(ICryptoBuildIn))
                             select hf).ToList().AsReadOnly();

            HasHMACBuildIn = (from hf in CryptoBuildIn
                              where hf.GetTypeInfo().IsImplementInterface(typeof(IHasHMACBuildIn))
                              select hf).ToList().AsReadOnly();
        }
    }
}
