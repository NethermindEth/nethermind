    ...
    : PacedSweep(db, persistenceManager, "OrphanStorageSwept", "OrphanStorageSweepProgress", TallyLength, pacer, logManager.GetClassLogger<OrphanStorageSweep>())
{
    ...
    protected override bool Scan(bool repair, ReadOnlySpan<byte> start, long maxUnits, TimeSpan budget, out byte[]? next, CancellationToken token)
    {
        ...
        long[] tallyBefore = (long[])Tally.Clone();
        ...
        if (repair && orphans.Count > 0)
        {
            ...
            if (!Delete(Flat.GetColumnDb(FlatDbColumns.Account), orphans, token))
            {
                tallyBefore.CopyTo(Tally, 0);
                Deferred = true;
                next = Cursor(startPrefix);
                return false;
            }
        }
    ...
    private bool Delete(IReadOnlyKeyValueStore accounts, List<ValueHash256> orphans, CancellationToken token)
    {
        int next = 0;
        while (next < orphans.Count)
        {
            token.ThrowIfCancellationRequested();
            int before = next;
            bool applied = Persistence.RunMaintenance(batch =>
            {
                if (!Drained(0)) return;
            ...
            if (!Drained(0)) StopForClearedBase();
            if (!applied) return false;
            if (next == before) token.ThrowIfCancellationRequested();
        }

        return true;
    }

    private void StopForClearedBase()
    {
        StampSwept();
