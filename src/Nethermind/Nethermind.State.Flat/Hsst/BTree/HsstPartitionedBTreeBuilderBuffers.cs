...
/// </summary>
/// <remarks>
/// <see cref="Inner"/> backs the per-partition inner B-tree builds and the final directory build
/// (used strictly sequentially, so one instance suffices). <see cref="AccumHashes"/> /
...
/// accumulate the directory entries (first key + serialized Hashtable node) across all partitions.
/// </remarks>
public struct HsstPartitionedBTreeBuilderBuffers : IDisposable
{
    // new(16) (not new()) so the primary-ctor field initializers of the inner buffers run; new()
    // would bind the zero-init parameterless struct ctor and leave the native lists null.
...
/// <see cref="HsstBTreeBuilderBuffersContainer"/>: the ref property lets the buffers be handed to
/// the partitioned builder's borrowed-buffers constructor.
/// </summary>
