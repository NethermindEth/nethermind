    ...
    where TPin : struct, IBufferPin, allows ref struct
{
    ...
    public void Add(scoped ReadOnlySpan<byte> key, scoped ReadOnlySpan<byte> value, out long entryStart) =>
        AddImpl(key, value, out entryStart);

    private void AddImpl(scoped ReadOnlySpan<byte> key, scoped ReadOnlySpan<byte> value, out long entryStart)
    {
        entryStart = -1;
