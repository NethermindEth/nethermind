...
/// <summary>
/// On-disk layout, hash, and bucket/way codec for the per-partition hashtable carried by a
/// <see cref="BTreeNodeKind.Hashtable"/> node — the single source of truth shared by the builder
/// that fills the buckets (<see cref="HsstPartitionedBTreeBuilder{TWriter,TReader,TPin}"/>) and the
/// <see cref="BTreeNodeKind.Hashtable"/> dispatch in the reader/enumerator.
/// </summary>
/// <remarks>
/// A hashtable is an array of cache-line buckets. Each bucket is <see cref="BucketBytes"/> (64)
/// bytes holding <see cref="WaysPerBucket"/> (8) ways struct-of-arrays so the tags scan with one
/// SIMD compare: <c>[Tag_0..Tag_7: 8×u16 LE][Offset_0..Offset_7: 8×u48 LE]</c> (tags in
/// <c>[0,16)</c>, offsets in <c>[16,64)</c>, no padding). <c>Tag == 0</c> marks an empty way; a
/// live placement forces the tag to be ≥ 1 so the equality scan never matches an empty way.
/// <c>Offset_i</c> is the entry's flag-byte position stored as the <b>forward</b> distance from
/// the partition's data-section start (<c>Offset = EntryOffset − DataRegionStart</c>), recovered as
/// <c>entry_abs = bound.Offset + DataRegionStart + Offset</c>. The 6-byte offset bounds a
/// partition's data section to <see cref="MaxOffset"/> + 1 (256 TiB). The table is best-effort: a
/// key whose bucket already holds 8 live tags is dropped, and the reader falls back to the
...
internal static class HsstPartitionHashtable
{
    ...
    /// <summary>Byte offset of the offsets section within a bucket (tags fill <c>[0, 16)</c>).</summary>
    ...
    /// after the node's flag byte, before the variable inner-root prefix):
    ...
    /// <summary>Hard cap on the bucket count — a runaway guard that fits the u24 record field
    /// (≈ 16 M buckets); real partitions are bounded by the key-bytes threshold well below this.</summary>
    internal const int MaxBucketCount = (1 << 24) - 1;

    /// <summary>Target hashtable load factor (keys / total ways), as a percent.</summary>
    ...
    /// keys: <c>ceil(keyCount / <see cref="TargetKeysPerBucket"/>)</c>, at least 1, capped at
    /// <see cref="MaxBucketCount"/>. Lookup-time selection uses Lemire reduction
    /// (<see cref="BucketIndex"/>), so any count works.
    /// </summary>
    ...
    internal static ulong Hash(scoped ReadOnlySpan<byte> key)
    {
        ...
        tail *= prime1;
        h ^= tail;
        // fmix64 finalizer — full avalanche across the bucket (low 32) / tag (high 16) split.
        h ^= h >> 33;
    ...
    /// buckets via Lemire's multiply-shift reduction — <c>(low32 · bucketCount) >> 32</c> maps the
    /// uniform low 32 bits into <c>[0, bucketCount)</c> with one multiply and works for any count.
    /// The low 32 bits are disjoint from the tag's high 16.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Insert <paramref name="offset"/> (= EntryOffset − DataRegionStart) into
    /// <paramref name="buckets"/> for <paramref name="hash"/>. Returns false on bucket overflow
    /// (8 live tags) or u48 offset overflow — the caller drops the key (best-effort; the reader
    /// falls back to the inner B-tree).
    /// </summary>
    ...
    /// (bit <c>i</c> set ⇒ way <c>i</c> matches). Scans all 8 tags (16 bytes) with a single 128-bit
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static long ReadU48(scoped ReadOnlySpan<byte> src) =>
        src[0] | ((long)src[1] << 8) | ((long)src[2] << 16) | ((long)src[3] << 24) | ((long)src[4] << 32) | ((long)src[5] << 40);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static void WriteU48(Span<byte> dest, long value)
    {
        dest[0] = (byte)value;
        dest[1] = (byte)(value >> 8);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static int ReadU24(scoped ReadOnlySpan<byte> src) => src[0] | (src[1] << 8) | (src[2] << 16);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static void WriteU24(Span<byte> dest, int value)
    {
        dest[0] = (byte)value;
        dest[1] = (byte)(value >> 8);
        dest[2] = (byte)(value >> 16);
