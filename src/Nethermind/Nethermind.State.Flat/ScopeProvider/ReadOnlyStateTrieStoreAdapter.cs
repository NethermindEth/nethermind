...
internal class ReadOnlyStateTrieStoreAdapter(ReadOnlySnapshotBundle bundle) : AbstractMinimalTrieStore
{
    ...
    {
        if (bundle.TryFindStateNodes(path, hash, out TrieNode? node))
        {
            return node;
        }
    ...
    {
        throw new InvalidOperationException("Commit not supported");
    }
...
): AbstractMinimalTrieStore
{
    public override TrieNode FindCachedOrUnknown(in TreePath path, Hash256 hash)
    {
        if (bundle.TryFindStorageNodes(addressHash, path, hash, -1, out TrieNode? node))
        {
            return node;
        }
