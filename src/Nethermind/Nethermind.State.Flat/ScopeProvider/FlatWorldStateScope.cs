...
public sealed class FlatWorldStateScope : IWorldStateScopeProvider.IScope, ITrieWarmer.IAddressWarmer
{
    ...
    // Demand-gating: warming stays off until a scope proves it does enough work to be worth the
    // warmer's CPU/dispatch overhead, then latches on for the rest of the block. Light blocks never
    // cross the threshold so they pay no warmer tax; heavy blocks cross it quickly and warm normally.
