...
public sealed class TrieWarmer : ITrieWarmer, IAsyncDisposable
{
    ...
    private void RunPrimaryWorker(CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (true)
            {
                ...
                if (_isPaused) { _pauseGate.Wait(cancellationToken); continue; }
    ...
    private void RunSecondaryWorker(CancellationToken cancellationToken)
    {
        try
        {
            ...
            while (true)
            {
                ...
                if (_isPaused) { Interlocked.Decrement(ref _activeSecondaryWorker); _pauseGate.Wait(cancellationToken); Interlocked.Increment(ref _activeSecondaryWorker); continue; }
    ...
    public void Pause() { _isPaused = true; _pauseGate.Reset(); }
    public void Resume() { _isPaused = false; _pauseGate.Set(); }
