    ...
    ILogManager logManager) : ISnapshotCompactor
{
    ...
    public SnapshotPooledList GetSnapshotsToCompact(Snapshot snapshot)
    {
        ...
        if (isMidCompaction)
        {
            ...
            foreach (StateId id in snapshotRepository.GetStatesAtBlockNumber(blockNumber - _compactSize))
            {
        ...
        SnapshotPooledList snapshots = snapshotRepository.AssembleSnapshotsUntil(snapshot.To, startingBlockNumber, compactSize);
    ...
    public Snapshot CompactSnapshotBundle(SnapshotPooledList snapshots)
    {
        ...
        for (int i = 0; i < snapshots.Count; i++)
        {
            ...
            foreach (KeyValuePair<AddressAsKey, Account?> knownStateAccount in knownState.Accounts)
            {
            ...
            foreach (KeyValuePair<(AddressAsKey, UInt256), SlotValue?> knownStateStorage in knownState.Storages)
            {
                storages[knownStateStorage.Key] = knownStateStorage.Value;
            }

            foreach (KeyValuePair<TreePath, TrieNode> kv in knownState.StateNodes)
            {
                stateNodes[kv.Key] = kv.Value;
            }

            foreach (KeyValuePair<(Hash256AsKey, TreePath), TrieNode> kv in knownState.StorageNodes)
            {
