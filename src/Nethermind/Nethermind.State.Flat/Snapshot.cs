...
) : RefCountingDisposable
{
    ...
    public IEnumerable<(Hash256AsKey, TreePath)> StorageTrieNodeKeys => content.StorageNodes.Select(static kv => kv.Key);
    public IEnumerable<KeyValuePair<TreePath, RefCountingTrieNode>> StateNodes => content.StateNodes;
    public IEnumerable<TreePath> StateNodeKeys => content.StateNodes.Select(static kv => kv.Key);
