...
public static class PersistedSnapshotBuilder
{
    ...
    /// <summary>
    /// Partitioning/hashtable options for the per-address column and slot-prefix HSSTs, derived
    /// from <see cref="IFlatDbConfig"/>. Other B-tree levels keep <see cref="HsstBTreeOptions.Default"/>.
    /// </summary>
    public static HsstBTreeOptions SlotOptions(IFlatDbConfig config) => new()
    {
        PartitionThresholdBytes = config.PersistedSnapshotSlotPartitionThresholdBytes,
        HashtableMinKeys = (int)config.PersistedSnapshotSlotHashtableMinKeys,
    };
        ...
        HsstBTreeOptions? slotOptions) where TWriter : IByteBufferWriterWithReader<TReader, TPin> where TReader : IHsstByteReader<TPin>, allows ref struct where TPin : struct, IBufferPin, allows ref struct
    {
        ...
        // Address-level HSST keyed by raw 20-byte Address, partitioned so large columns gain a
        // per-partition hashtable; small ones degrade to a plain B-tree.
        ...
        HsstPartitionedBTreeBuilder<TWriter, TReader, TPin> addressLevel = new(ref addressWriter, ref addressLevelBuffers.Buffers, PersistedSnapshotTags.AddressKeyLength, slotOptions, keyFirst: false);
        ...
        for (int addrIdx = 0; addrIdx < uniqueAddresses.Count; addrIdx++)
        {
            ...
            // tags in strictly descending order.
            {
                ...
                HsstPartitionedBTreeBuilder<TWriter, TReader, TPin> prefixLevel = new(ref slotWriter, ref slotPrefixBuffers.Buffers, slotPrefixLength, slotOptions, keyFirst: true);
