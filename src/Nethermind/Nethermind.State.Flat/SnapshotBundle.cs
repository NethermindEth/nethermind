...
public sealed class SnapshotBundle : IDisposable
{
    ...
    private bool TryFindStateNodeInPersistence(in TreePath path, Hash256 hash, [NotNullWhen(true)] out TrieNode? node)
    {
    ...
    /// </para>
    /// <para>
    /// Costs, per warmer persistence read that publishes: one Keccak over the RLP, one RLP copy and decode, and one
    /// <see cref="TrieNode"/>. A warmer miss also now occupies a slot in both <see cref="TransientResource.MissNodes"/>
    /// and <see cref="TransientResource.Nodes"/>, so the latter's count - which drives the regrow decision in
    /// <see cref="TrieNodeCache.ChildCache.Reset"/> - climbs faster. Publishing uses <c>GetOrAdd</c>, which replaces a
    /// slot held by a different hash, so a publish racing a trie commit can evict the just-committed node from the
    /// transient; the evicted node stays authoritative in <c>_changedStateNodes</c>/<c>_snapshots</c>, so this costs a
    /// re-read rather than correctness.
    /// </para>
    /// </remarks>
    ...
    {
        // The persistence read is keyed by path alone - it never looks at `hash` - so `rlp` is whatever node
        // currently sits at `path`, which need not be the node the warmer asked for. The warmer can reach a
        // newer (path, hash) pair through the cross-block TrieNodeCache or a committed snapshot while the
        // pinned persistence view still holds the older node at that path. Publishing those bytes under the
        // requested hash would defeat every reader-side guard at once, because ChildCache.TryGet,
        // TrieNodeCache.TryGet and FindCachedOrUnknown all compare against the node's claimed Keccak - which
        // would be `hash` by construction. Verify the binding rather than assert it.
        ...
        // TrieNodeCache.Add, neither of which may see a node that is still being resolved. TryResolveNode
        // rather than ResolveNode so a publish can never change the outcome of the read it piggybacks on.
        TrieNode node = new(NodeType.Unknown, hash, (ReadOnlySpan<byte>)rlp);
