    ...
    public class EthereumStepsManager
    {
        ...
        private async Task ExecuteStep(IStep step, StepInfo stepInfo, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (_logger.IsDebug)
                    _logger.Debug(
                        $"Step {step.GetType().Name.PadRight(24)} executed in {Stopwatch.GetElapsedTime(startTime).TotalMilliseconds}ms");
            ...
            catch (Exception exception)
            {
                if (step.MustInitialize)
                {
                    if (_logger.IsError)
                        _logger.Error(
                            $"Step {step.GetType().Name.PadRight(24)} failed after {Stopwatch.GetElapsedTime(startTime).TotalMilliseconds}ms",
                            exception);
                ...
                {
                    _logger.Warn(
                        $"Step {step.GetType().Name.PadRight(24)} failed after {Stopwatch.GetElapsedTime(startTime).TotalMilliseconds}ms {exception}");
                }
                stepInfo.Stage = StepInitializationStage.Complete;
            }
            finally
