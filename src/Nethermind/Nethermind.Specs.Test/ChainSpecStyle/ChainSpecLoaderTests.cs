...
namespace Nethermind.Specs.Test.ChainSpecStyle
{
    ...
    public class ChainSpecLoaderTests
    {
        ...
        public void Can_load_chainspec_from_geth_genesis(string chainName)
        {
            ...
            var gethFilePath = Path.Combine(TestContext.CurrentContext.WorkDirectory, $"Specs/{chainName}_genesis.json");
            var parityFilePath = Path.Combine(TestContext.CurrentContext.WorkDirectory, "../../../../", $"Chains/{chainName}.json");
            ChainSpec gethChainSpec = LoadChainSpec(gethFilePath);
            ChainSpec parityChainSpec = LoadChainSpec(parityFilePath);

            gethChainSpec.SealEngineType.Should().Be(parityChainSpec.SealEngineType);
