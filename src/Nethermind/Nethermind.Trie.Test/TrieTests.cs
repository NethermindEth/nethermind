...
namespace Nethermind.Trie.Test
{
    ...
    public class TrieTests
    {
        ...
        public void Test_try_delete_and_read_missing_nodes(int i)
        {
            ...
            for (int j = 0; j < i; j++)
            {
                Commitment key = TestItem.Keccaks[j + 100];
            ...
            int lookupLimit)
        {
            ...
            Queue<Commitment> rootQueue = new();
            ...
            while (rootQueue.TryDequeue(out Commitment currentRoot))
            {
                try
                {
            ...
            int? seed)
        {
            ...
            Queue<Commitment> rootQueue = new();
            Stack<Commitment> rootStack = new();
            ...
            rootQueue.Clear();
            Stack<Commitment> stackCopy = new();
            while (rootStack.Any())
            {
            ...
            while (rootStack.TryPop(out Commitment currentRoot))
            {
                try
                {
                    patriciaTree.RootHash = currentRoot;
