...
namespace Nethermind.Trie.Test.Pruning
{
    ...
    public class RefsJournalTests
    {
        ...
        {
            IRefsJournal refsJournal = new RefsJournal(_trieNodeCache, _logManager);
        ...
        public void Fuzz_unwind_rewind()
        {
            ...
            for (int i = 0; i < numberOfBlocks; i++)
            {
                ...
                Reorganize(refsJournal, reorgDepth, shouldRewind);
                
                booksCount++;
                AddOneFuzzBook(
                    refsJournal, _trieNodeCache, expectedRefs, i, numberOfChangesInTheBlock, numberOfInitialNodes);
            }

            for (int i = 0; i < numberOfBlocks; i++)
            {
                int reorgDepth = _random.Next(Math.Min(booksCount, i));
                Reorganize(refsJournal, reorgDepth, true);
            }
            ...
            {
                _logger.Trace($"Checking {key} - expected {value}, is {_trieNodeCache.Get(key).Refs}");
                _trieNodeCache.Get(key).Refs.Should().Be(value);
            }
        }
