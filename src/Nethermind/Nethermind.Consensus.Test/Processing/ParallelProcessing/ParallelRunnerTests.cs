...
public class ParallelRunnerTests
{
    ...
    private async Task Run<T>(int blockSize, Operation[][] operationsPerTx, Dictionary<int, byte[]> expected) where T : struct, IFlag
    {
        ...
        ParallelBlockMetricsCollector parallelBlockMetricsCollector = new(blockSize);
        using ParallelRunner<int, byte[], T> runner = new(
            parallelScheduler,
            multiVersionMemory,
            parallelTrace,
            parallelTransactionProcessorMock,
            parallelBlockMetricsCollector,
            12);
