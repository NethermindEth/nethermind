...
public class BackgroundTaskSchedulerBurstTests
{
    ...
    /// <summary>
    /// Simulates normal operation: a heavy P2P serving task (e.g. GetReceipts that does
    /// expensive DB reads) is running when a lightweight task (e.g. tx processing) is
    /// scheduled. The lightweight task should not have to wait for the heavy task to
    /// finish — it should start within a reasonable time.
    ///
    /// With concurrency=2 and both workers blocked by heavy tasks, this fails.
    /// </summary>
    [Test]
    public async Task Heavy_tasks_should_not_block_lightweight_tasks_during_normal_operation()
    {
        int concurrency = 2;
        await using BackgroundTaskScheduler scheduler = new(_branchProcessor, _chainHeadInfo, concurrency, 2048, LimboLogs.Instance);

        // Simulate heavy tasks occupying all worker slots (like receipt/body serving doing DB reads)
        SemaphoreSlim heavyTaskBlocker = new(0);
        int heavyTasksStarted = 0;

        for (int i = 0; i < concurrency; i++)
        {
            scheduler.TryScheduleTask(i, async (_, token) =>
            {
                Interlocked.Increment(ref heavyTasksStarted);
                // Simulate a heavy DB read that takes 500ms (receipt serving)
                await heavyTaskBlocker.WaitAsync(token);
            });
        }

        // Wait for heavy tasks to occupy all workers
        Assert.That(() => Volatile.Read(ref heavyTasksStarted), Is.EqualTo(concurrency).After(2000, 10));

        // Now schedule a lightweight task (like tx processing) — it should start quickly
        Stopwatch sw = Stopwatch.StartNew();
        TaskCompletionSource lightweightStarted = new(TaskCreationOptions.RunContinuationsAsynchronously);

        scheduler.TryScheduleTask(999, (_, _) =>
        {
            lightweightStarted.TrySetResult();
            return Task.CompletedTask;
        });

        // The lightweight task should start within 50ms even though heavy tasks are running.
        // Currently it will block until one of the heavy tasks finishes.
        Task completed = await Task.WhenAny(lightweightStarted.Task, Task.Delay(200));
        sw.Stop();

        // Release heavy tasks for cleanup
        heavyTaskBlocker.Release(concurrency);

        completed.Should().BeSameAs(lightweightStarted.Task,
            "lightweight task should not be blocked waiting for heavy tasks to finish — " +
            $"waited {sw.ElapsedMilliseconds}ms but should have started within 50ms");
    }

    /// <summary>
    /// After block processing completes, all tasks that were held back during the
    /// ~100-300ms processing window burst through at once. This burst overwhelms
    /// the 2 workers and causes latency spikes.
    ///
    /// This test verifies that tasks scheduled during block processing don't all
    /// experience high latency when block processing ends — the p95 queue wait
    /// time should remain bounded. Tasks simulate realistic work (DB reads, txpool
    /// operations, trie pruning) not just instant completions.
    /// </summary>
    [Test]
    public async Task Post_block_processing_burst_should_not_cause_latency_spikes()
    {
        int capacity = 2048;
        int concurrency = 2;
        ...
        ConcurrentBag<long> startLatencies = new();
        int tasksQueued = 50;

        // Start block processing
        _branchProcessor.BlocksProcessing += Raise.EventWith(new BlocksProcessingEventArgs(null));

        // Queue tasks during block processing — these represent a mix of real work:
        // tx processing, receipt serving, body serving, trie pruning, etc.
        // Each task does 20ms of simulated work (conservative for receipt/body serving).
        for (int i = 0; i < tasksQueued; i++)
        {
            long queuedAt = Stopwatch.GetTimestamp();
            scheduler.TryScheduleTask(i, async (_, _) =>
            {
                long startedAt = Stopwatch.GetTimestamp();
                long latencyMs = (startedAt - queuedAt) * 1000 / Stopwatch.Frequency;
                startLatencies.Add(latencyMs);
                // Simulate real work — DB read for receipt/body serving, txpool validation, etc.
                await Task.Delay(20, CancellationToken.None);
            }).Should().BeTrue();
        }

        // Simulate block processing taking 200ms
        await Task.Delay(200);

        // End block processing — all tasks burst through
        _branchProcessor.BlockProcessed += Raise.EventWith(new BlockProcessedEventArgs(null, null));

        // Wait for all tasks to complete
        Assert.That(() => startLatencies.Count, Is.EqualTo(tasksQueued).After(10000, 10));

        // Calculate p95 latency
        long[] latencies = startLatencies.ToArray();
        Array.Sort(latencies);
        long p95 = latencies[(int)(latencies.Length * 0.95)];

        // With 50 tasks each taking 20ms and only 2 workers, the last tasks in the burst
        // wait: 200ms (block processing) + (50/2 * 20ms) = 700ms total.
        // This means the burst creates ~500ms of additional queueing delay on top of
        // the block processing time. For a well-designed scheduler, tasks should interleave
        // or have priority, keeping p95 under 400ms.
        p95.Should().BeLessThan(400,
            $"p95 queue-to-start latency was {p95}ms — the post-block-processing burst " +
            "is causing excessive queueing delays because all held-back tasks (tx processing, " +
            "receipt serving, pruning) burst through 2 workers simultaneously");
    }
    ...
    /// <summary>
    /// Tests task starvation: when heavy tasks (receipt/body serving) dominate the queue,
    /// lightweight tasks (tx processing) experience unacceptable delays.
    ///
    /// In normal operation (not syncing), every 12 seconds a block is processed. Peers
    /// continuously send GetReceipts/GetBlockBodies. Meanwhile, tx messages need to be
    /// processed promptly for the mempool. This test verifies that tx-like tasks don't
    /// get starved by heavy serving tasks.
    /// </summary>
    [Test]
    public async Task Lightweight_tasks_should_not_be_starved_by_heavy_serving_tasks()
    {
        int capacity = 2048;
        int concurrency = 2;
        await using BackgroundTaskScheduler scheduler = new(_branchProcessor, _chainHeadInfo, concurrency, capacity, LimboLogs.Instance);

        ConcurrentBag<long> lightweightLatencies = new();
        int heavyTasksScheduled = 0;
        int lightweightTasksScheduled = 0;

        // First, flood the queue with heavy tasks (simulates receipt/body serving)
        for (int i = 0; i < 30; i++)
        {
            scheduler.TryScheduleTask(i, async (_, token) =>
            {
                // Simulate heavy DB read (receipt serving)
                await Task.Delay(50, CancellationToken.None);
            });
            Interlocked.Increment(ref heavyTasksScheduled);
        }

        // Wait a bit for some heavy tasks to start processing
        await Task.Delay(50);

        // Now schedule lightweight tasks (simulates incoming tx messages)
        // These should not have to wait behind all the heavy tasks
        for (int i = 0; i < 10; i++)
        {
            long queuedAt = Stopwatch.GetTimestamp();
            scheduler.TryScheduleTask(1000 + i, (_, _) =>
            {
                long startedAt = Stopwatch.GetTimestamp();
                long latencyMs = (startedAt - queuedAt) * 1000 / Stopwatch.Frequency;
                lightweightLatencies.Add(latencyMs);
                return Task.CompletedTask;
            });
            Interlocked.Increment(ref lightweightTasksScheduled);
        }

        // Wait for lightweight tasks to complete
        Assert.That(() => lightweightLatencies.Count, Is.EqualTo(10).After(15000, 10));

        long[] latencies = lightweightLatencies.ToArray();
        Array.Sort(latencies);
        long maxLatency = latencies[^1];
        long medianLatency = latencies[latencies.Length / 2];

        // Lightweight tasks should start within 200ms even when heavy tasks are queued.
        // With the current design (FIFO, 2 workers, ~50ms/heavy task), 30 heavy tasks
        // take ~750ms to drain. Lightweight tasks queued at position 30+ will wait ~750ms+.
        maxLatency.Should().BeLessThan(200,
            $"max lightweight task latency was {maxLatency}ms (median {medianLatency}ms) — " +
            "lightweight tasks are being starved by heavy serving tasks in the shared queue");
    }

    /// <summary>
    /// During block processing, tasks that reach their deadline are executed serially by the
    /// 2 workers. With many tasks expiring simultaneously (e.g., 200 tasks all with 100ms
    /// deadline during a 200ms block processing period), the workers must drain them one by
    /// one. This creates head-of-line blocking: tasks at the back of the queue experience
    /// latency proportional to (task_count × work_per_task / worker_count).
    ///
    /// In production: receipt-serving tasks have 2s default timeout. During block processing
    /// (~200ms), many of these expire simultaneously. Each expired task is executed with a
    /// cancelled token, but its cleanup (DB cursor release, resource disposal) still takes
    /// time. Tasks at the back of the expired queue wait for ALL preceding cleanup to finish.
    ///
    /// This contributes to block processing degradation: the workers are busy draining expired
    /// tasks instead of being available for new work after block processing completes.
    /// </summary>
    [Test]
    public async Task Block_processing_expired_task_drain_creates_head_of_line_blocking()
    {
        int capacity = 2048;
        await using BackgroundTaskScheduler scheduler = new(_branchProcessor, _chainHeadInfo, 2, capacity, LimboLogs.Instance);

        // Start block processing — tasks get held back
        _branchProcessor.BlocksProcessing += Raise.EventWith(new BlocksProcessingEventArgs(null));

        // Schedule 200 tasks with short deadline (50ms). They will ALL expire during
        // block processing and need to be drained. Also add 100 long-deadline tasks to
        // simulate the realistic mix of expired + active tasks.
        int shortDeadlineCount = 200;
        ConcurrentBag<long> expiredTaskLatencies = new();

        for (int i = 0; i < shortDeadlineCount; i++)
        {
            long queuedAt = Stopwatch.GetTimestamp();
            scheduler.TryScheduleTask(i, async (_, token) =>
            {
                long startedAt = Stopwatch.GetTimestamp();
                long latencyMs = (startedAt - queuedAt) * 1000 / Stopwatch.Frequency;
                expiredTaskLatencies.Add(latencyMs);
                // Simulate cleanup work: releasing DB cursor, disposing resources
        ...
        for (int i = 0; i < 100; i++)
        {
            scheduler.TryScheduleTask(shortDeadlineCount + i, (_, _) => Task.CompletedTask,
                TimeSpan.FromSeconds(30));
        }

        // Wait for all short-deadline tasks to expire and be drained
        Assert.That(() => expiredTaskLatencies.Count,
            Is.EqualTo(shortDeadlineCount).After(10000, 10),
            "all expired tasks should be executed during block processing");

        _branchProcessor.BlockProcessed += Raise.EventWith(new BlockProcessedEventArgs(null, null));

        long[] latencies = expiredTaskLatencies.ToArray();
        Array.Sort(latencies);
        long p50 = latencies[latencies.Length / 2];
        long p95 = latencies[(int)(latencies.Length * 0.95)];
        long max = latencies[^1];

        // With 200 expired tasks each doing 5ms cleanup, and 2 workers:
        // Theoretical drain time: 200 * 5ms / 2 = 500ms
        // Tasks at the back experience: 50ms (deadline) + ~500ms (queue drain) = ~550ms
        // Tasks at the front: 50ms (deadline) + ~0ms = ~50ms
        //
        // An ideal scheduler would parallelize expired task cleanup more aggressively
        // or not batch all expirations together. p95 should be close to the deadline (50ms),
        // not 10x the deadline.
        p95.Should().BeLessThan(200,
            $"expired task p95 latency was {p95}ms (p50: {p50}ms, max: {max}ms) — " +
            "tasks at the back of the expiry queue experience head-of-line blocking. " +
            "With 200 expired tasks × 5ms cleanup / 2 workers, the last tasks wait ~500ms " +
            "past their deadline. In production with receipt cleanup (~50ms each), " +
            "this exceeds 5 seconds and directly delays post-block-processing recovery.");
    }
    ...
    /// <summary>
    /// Tests that when the scheduler is under heavy load from one type of task (receipts),
    /// other task types (tx processing) still get fair access. Currently there's no
    /// priority or fairness mechanism — tasks are processed in deadline order.
    ///
    /// This test schedules a mix of heavy and lightweight tasks and measures whether
    /// lightweight tasks get a fair share of execution time.
    /// </summary>
    [Test]
    public async Task Mixed_workload_should_provide_fair_execution_between_task_types()
    {
        int capacity = 2048;
        int concurrency = 2;
        await using BackgroundTaskScheduler scheduler = new(_branchProcessor, _chainHeadInfo, concurrency, capacity, LimboLogs.Instance);

        ConcurrentBag<long> heavyLatencies = new();
        ConcurrentBag<long> lightLatencies = new();

        // Schedule alternating heavy and light tasks
        for (int i = 0; i < 40; i++)
        {
            if (i % 2 == 0)
            {
                // Heavy task (receipt serving) — 50ms of work
                long queuedAt = Stopwatch.GetTimestamp();
                scheduler.TryScheduleTask(i, async (_, _) =>
                {
                    long started = Stopwatch.GetTimestamp();
                    heavyLatencies.Add((started - queuedAt) * 1000 / Stopwatch.Frequency);
                    await Task.Delay(50, CancellationToken.None);
                });
            }
            else
            {
                // Light task (tx processing) — near-instant
                long queuedAt = Stopwatch.GetTimestamp();
                scheduler.TryScheduleTask(i, (_, _) =>
                {
                    long started = Stopwatch.GetTimestamp();
                    lightLatencies.Add((started - queuedAt) * 1000 / Stopwatch.Frequency);
                    return Task.CompletedTask;
                });
            }
        }
        ...
        // Wait for all tasks to complete
        Assert.That(() => lightLatencies.Count, Is.EqualTo(20).After(15000, 10));
        Assert.That(() => heavyLatencies.Count, Is.EqualTo(20).After(15000, 10));

        long[] light = lightLatencies.ToArray();
        long[] heavy = heavyLatencies.ToArray();
        Array.Sort(light);
        Array.Sort(heavy);

        long lightP95 = light[(int)(light.Length * 0.95)];
        long heavyP95 = heavy[(int)(heavy.Length * 0.95)];

        // Light tasks should not have dramatically worse latency than heavy tasks.
        // With fair scheduling, light tasks interleave with heavy tasks.
        // Without fairness, light tasks queue behind heavy tasks and wait ~50ms * (queue_position / concurrency).
        lightP95.Should().BeLessThan(500,
            $"lightweight task p95 latency was {lightP95}ms (heavy p95: {heavyP95}ms) — " +
            "lightweight tasks are starved because the scheduler has no priority/fairness mechanism");
    }

    /// <summary>
    /// Edge case: what happens when the scheduler is IsSyncing=true and then transitions
    /// to IsSyncing=false while under load? During syncing, the throttle mechanism is
    /// disabled. When syncing completes (node reaches chain tip), the throttle kicks in.
    /// If there's already a backlog of heavy tasks from the sync period, the first
    /// block processing event will try to throttle them, causing a massive burst.
    /// </summary>
    [Test]
    public async Task Transition_from_syncing_to_synced_should_not_cause_massive_burst()
    {
        int capacity = 2048;
        int concurrency = 2;

        // Start as syncing — no throttling
        _chainHeadInfo.IsSyncing.Returns(true);
        await using BackgroundTaskScheduler scheduler = new(_branchProcessor, _chainHeadInfo, concurrency, capacity, LimboLogs.Instance);

        int executedCount = 0;
        int heavyTaskCount = 100;

        // Flood with heavy tasks while syncing (no throttle, they all queue/execute freely)
        ...
        // Let some tasks start draining
        await Task.Delay(200);
        int executedBeforeTransition = Volatile.Read(ref executedCount);

        // Transition to synced — next block processing will activate throttle
        _chainHeadInfo.IsSyncing.Returns(false);

        // Block processing starts — this cancels current tasks and throttles remaining
        _branchProcessor.BlocksProcessing += Raise.EventWith(new BlocksProcessingEventArgs(null));

        // Schedule more tasks — these will be held back
        int additionalTasks = 50;
        ConcurrentBag<long> burstLatencies = new();
        for (int i = 0; i < additionalTasks; i++)
        {
            long queuedAt = Stopwatch.GetTimestamp();
            scheduler.TryScheduleTask(heavyTaskCount + i, (_, token) =>
            {
                long started = Stopwatch.GetTimestamp();
                long latencyMs = (started - queuedAt) * 1000 / Stopwatch.Frequency;
                burstLatencies.Add(latencyMs);
                Interlocked.Increment(ref executedCount);
                return Task.CompletedTask;
            }, TimeSpan.FromSeconds(10));
        }

        // Block processing ends after 200ms
        await Task.Delay(200);
        _branchProcessor.BlockProcessed += Raise.EventWith(new BlockProcessedEventArgs(null, null));

        // Wait for additional tasks to complete
        Assert.That(() => burstLatencies.Count, Is.EqualTo(additionalTasks).After(10000, 10));

        long[] latencies = burstLatencies.ToArray();
        Array.Sort(latencies);
        long p95 = latencies[(int)(latencies.Length * 0.95)];

        // The remaining heavy tasks from the sync period plus the new tasks
        // should not create an excessive burst. p95 latency should be bounded.
        p95.Should().BeLessThan(1000,
            $"p95 latency after sync→synced transition was {p95}ms — the combined backlog " +
            "from syncing period + new tasks creates an excessive burst when throttling activates");
    }
