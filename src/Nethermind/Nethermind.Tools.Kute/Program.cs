...
static class Program
{
    ...
    public static async Task OldMain(string[] args)
    {
        ...
        Uri uri = new(config.Host);
        string secret = await File.ReadAllTextAsync(config.JwtSecretFile);
        string authToken = GenerateJwtToken(secret.Trim());
        ...
        IAsyncEnumerable<string> rawMessages = File.ReadLinesAsync(config.MessagesFile);
        await foreach (string rm in rawMessages)
        {
            lines++;

            JsonDocument? rpcPayload = JsonSerializer.Deserialize<JsonDocument>(rm);
            if (rpcPayload is not null)
            {
                if (rpcPayload.RootElement.TryGetProperty("response", out _))
                {
                    responses++;
                }
                else
                {
                    requests++;
                    if (rpcPayload.RootElement.TryGetProperty("method", out var methodProp))
                    {
                        var methodName = methodProp.GetString()!;
                        if (methodCounter.ContainsKey(methodName))
                        {
                            methodCounter[methodName]++;
                        }
                        else
                        {
                            methodCounter[methodName] = 0;
                        }
                    }
                    ...
                    var request = new HttpRequestMessage(HttpMethod.Post, uri)
                    {
                        ...
                        Content = new StringContent(rm, Encoding.UTF8, "application/json")
                    };
        ...
        Results:
            ...
            Methods:
                ...
                Requests:  {requests}
        """);
        foreach (var (method, count) in methodCounter)
        {
            Console.WriteLine($"""
                    {method}: {count}
        """);
    ...
    {
        SymmetricSecurityKey signingKey = new SymmetricSecurityKey(StringToByteArray(secretKey));
        SigningCredentials credentials = new SigningCredentials(signingKey, SecurityAlgorithms.HmacSha256);
        Claim[] claims = new[] { new Claim("iat", DateTimeOffset.UtcNow.ToUnixTimeSeconds().ToString()), };
        JwtSecurityToken token = new JwtSecurityToken(claims: claims, signingCredentials: credentials);
