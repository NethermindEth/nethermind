    ...
    public static class SnapProviderHelperPaprika
    {
        ...
        )
        {
            ...
            (AddRangeResult result, List<(TrieNode, TreePath)> sortedBoundaryList, bool moreChildrenToRight) =
                FillBoundaryTree(startingHash, lastHash, limitHash, expectedRootHash, proofs);
        ...
        )
        {
            ...
            (AddRangeResult result, List<(TrieNode, TreePath)> sortedBoundaryList, bool moreChildrenToRight) = FillBoundaryTree(startingHash, lastHash, ValueKeccak.MaxValue, expectedRootHash, proofs);
        ...
        private static Dictionary<ValueHash256, TrieNode> CreateProofDict(IReadOnlyList<byte[]> proofs)
        {
            ...
            var resolver = NullTrieNodeResolver.Instance;
            ...
            for (int i = 0; i < proofs.Count; i++)
            {
                ...
                TreePath emptyPath = TreePath.Empty;
                node.ResolveNode(resolver, emptyPath);
                node.ResolveKey(resolver, ref emptyPath, isRoot: i == 0);
