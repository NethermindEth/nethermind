...
public class PatriciaSnapTrieFactory(INodeStorage nodeStorage, ILogManager logManager) : ISnapTrieFactory
{
    ...
    {
        SnapUpperBoundAdapter adapter = new SnapUpperBoundAdapter(_stateTrieStore);
    ...
    public ISnapTree<PathWithStorageSlot> CreateStorageTree(in ValueHash256 accountPath)
    {
        ...
        RawScopedTrieStore storageTrieStore = new RawScopedTrieStore(nodeStorage, address);
        SnapUpperBoundAdapter adapter = new SnapUpperBoundAdapter(storageTrieStore);
