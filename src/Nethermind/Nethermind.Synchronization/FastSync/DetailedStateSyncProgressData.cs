...
{
    internal class DetailedStateSyncProgressData
    {
        ...
        public DetailedStateSyncProgressData(int chainId, byte[] serializedInitialState)
        {
        ...
        internal void DisplayProgressReport(int pendingRequestsCount, ILogger logger)
        {
            ...
            if (sinceLastReport > TimeSpan.FromSeconds(1))
            {
                ...
                {
                    dataSizeInfo = string.Concat(dataSizeInfo, $" / {(decimal) _chainSizeInfo.Value.Current / 1000 / 1000,6:F2}MB");
                }

                if (logger.IsInfo) logger.Info($"State Sync {TimeSpan.FromSeconds(SecondsInSync):dd\\.hh\\:mm\\:ss} | {dataSizeInfo} | kB/s: {savedKBytesPerSecond,5:F0} | P: {pendingRequestsCount} | acc {SavedAccounts} | nodes {SavedNodesCount} | db_delay {AverageTimeInHandler:f2}ms");
                if (logger.IsDebug && DateTime.UtcNow - LastReportTime.full > TimeSpan.FromSeconds(10))
                {
                    ...
                    {
                        if (logger.IsDebug) logger.Debug($"Consumed {(decimal) ConsumedNodesCount / RequestedNodesCount:p2} | Saved {(decimal) SavedNodesCount / RequestedNodesCount:p2} | DB Reads : {(decimal) DbChecks / RequestedNodesCount:p2} | DB checks {StateWasThere}/{StateWasNotThere + StateWasThere} | Cached {(decimal) CheckWasCached / allChecks:P2} + {(decimal) CheckWasInDependencies / allChecks:P2}");
                    }
