    ...
    IVerifyTrieStarter? verifyTrieStarter = null) : IStateSyncRunner
{
    ...
    internal async Task<bool> RunBalHealing(BlockHeader? firstPivot, CancellationToken token)
    {
        ...
        {
            if (_logger.IsInfo) _logger.Info("BAL healing skipped - the active state backend cannot heal BALs.");
            return false;
        }
        ...
        if (firstPivot.BlockAccessListHash is null)
        {
            if (_logger.IsInfo) _logger.Info($"BAL healing skipped - block access lists not active at block {firstPivot.Number}.");
            return false;
        }
