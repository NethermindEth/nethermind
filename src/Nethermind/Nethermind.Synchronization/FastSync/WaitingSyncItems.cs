...
{
    internal class PendingSyncItems
    {
        ...
        internal void PushToSelectedStream(StateSyncItem stateSyncItem, decimal progress)
        {
        ...
        public string Description => $"{CodeItems?.Count ?? 0:D4} + {StorageItemsPriority0?.Count ?? 0:D6} {StorageItemsPriority1?.Count ?? 0:D6} {StorageItemsPriority2?.Count ?? 0:D6} + {StateItemsPriority0?.Count ?? 0:D6} {StateItemsPriority0?.Count ?? 0:D6} {StateItemsPriority0?.Count ?? 0:D6}";
        public int Count => _allStacks.Sum(n => n?.Count ?? 0);

        internal StateSyncItem PeekState()
        {
        ...
        internal string RecalculatePriorities()
        {
