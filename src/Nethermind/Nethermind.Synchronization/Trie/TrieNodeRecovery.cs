...
public interface ITrieNodeRecovery<in TRequest>
{
    ...
    Task<byte[]?> Recover(ValueCommitment rlpHash, TRequest request);
}
...
public abstract class TrieNodeRecovery<TRequest> : ITrieNodeRecovery<TRequest>
{
    ...
    public async Task<byte[]?> Recover(ValueCommitment rlpHash, TRequest request)
    {
    ...
    protected ArrayPoolList<Recovery> GenerateKeyRecoveries(in ValueCommitment rlpHash, TRequest request, CancellationTokenSource cts)
    {
    ...
    private async Task<(Recovery, byte[]?)> RecoverRlpFromPeer(ValueCommitment rlpHash, Recovery recovery, TRequest request, CancellationTokenSource cts)
    {
    ...
    protected abstract Task<byte[]?> RecoverRlpFromPeerBase(ValueCommitment rlpHash, ISyncPeer peer, TRequest request, CancellationTokenSource cts);
