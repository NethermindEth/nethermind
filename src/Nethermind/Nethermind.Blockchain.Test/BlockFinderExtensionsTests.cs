...
public class BlockFinderExtensionsTests
{
    ...
    public void BlockParameter_ToString_ReturnsLowercaseTypeName(BlockParameterType type, string expected) => new BlockParameter(type).ToString().Should().Be(expected);

    [Test, MaxTime(Timeout.MaxTestTime)]
    public void BlockParameter_ToString_ReturnsBlockNumber() => new BlockParameter(12345L).ToString().Should().Be("12345");
