...
public class FrameTxFloodMeasurement
{
    ...
    /// <remarks><see cref="Eip8141MeasurementCeilings.SoispokeDeclaredBudget"/> exceeds
    /// <see cref="Eip8141Constants.MaxVerifyGas"/> (300,000), so of the methods this array feeds, only the
    /// signature-stuffed ones — refused before they ever reach that cap — produce a row at that point; every
    /// keccak-wide/production/ramp arm is gated by it and Assert.Ignores instead.</remarks>
    private static readonly ulong[] SweptCeilings = Eip8141MeasurementCeilings.Swept;
