...
public class BeaconBlockRootHandlerTests
{
        ...
        [ValueSource(nameof(BeaconRootSpecs))] IReleaseSpec spec)
    {
        ...
        UInt256 timestamp = new(state.Get(new StorageCell(Eip4788Constants.BeaconRootsAddress, 4154)), isBigEndian: true);
        UInt256 root = new(state.Get(new StorageCell(Eip4788Constants.BeaconRootsAddress, 12345)), isBigEndian: true);
        using (Assert.EnterMultipleScope())
        {
