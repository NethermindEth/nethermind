...
public class BlockchainProcessorTests
{
    ...
    [Test]
    public void Process_WhenPersistedStateAheadOfHead_SkipsGapBlocksAndFastForwardsHead()
    {
        PersistedStateRecoverySetup setup = new(persistedNumber: 8, persistedRoot: null);

        foreach (Block block in setup.GapBlocks)
        {
            Block? result = setup.Processor.Process(block, ProcessingOptions.None, NullBlockTracer.Instance);
        ...
        setup.BranchProcessor.DidNotReceiveWithAnyArgs().Process(default, default!, default, default!, default);
    }

    [Test]
    public void Process_WhenJunctionStateRootMismatches_FallsBackToBranchProcessing()
    {
        PersistedStateRecoverySetup setup = new(persistedNumber: 8, persistedRoot: TestItem.KeccakB);

        foreach (Block block in setup.GapBlocks)
        {
            setup.Processor.Process(block, ProcessingOptions.None, NullBlockTracer.Instance);
        }

        setup.BranchProcessor.ReceivedWithAnyArgs(1).Process(default, default!, default, default!, default);
    }

    [Test]
    public void Process_WhenHeadAtOrAbovePersistedState_ProcessesNormally()
    {
        PersistedStateRecoverySetup setup = new(persistedNumber: 3, persistedRoot: TestItem.KeccakA);

        setup.Processor.Process(setup.GapBlocks[0], ProcessingOptions.None, NullBlockTracer.Instance);
    ...
    private sealed class PersistedStateRecoverySetup
    {
        ...
        public IBranchProcessor BranchProcessor { get; } = Substitute.For<IBranchProcessor>();
