    ...
    private class ProcessingTestContext
    {
        ...
        public class AfterBlock(ILogManager logManager, ProcessingTestContext processingTestContext, Block block)
        {
            ...
            public ProcessingTestContext IsDeletedAsInvalid()
            {
                ...
                // Subscribe first, then back-fill in case deletion already fired before we subscribed.
                ...
                try
                {
                    ...
                    Assert.That(deletedEvent.Wait(ProcessingWait), Is.True,
                        $"Timed out waiting for block {block.ToString(Block.Format.Short)} to be deleted as invalid");
                }
                finally
                {
                    processingTestContext._blockTree.BlockInvalidated -= OnBlockInvalidated;
