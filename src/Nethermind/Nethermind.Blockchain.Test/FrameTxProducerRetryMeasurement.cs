...
public class FrameTxProducerRetryMeasurement
{
    ...
    /// <remarks><see cref="Eip8141MeasurementCeilings.SoispokeDeclaredBudget"/> is the declared privacy-pool
    /// budget this fixture's upper cases sweep.
    ...
    /// execution gas limit with no signatures, so at every ceiling this fixture sweeps (not only the declared
    /// one) the EVM burn is a uniform tight-loop shape, distinct from the signature/Groth16 shapes the mempool/flood
    ...
    [TestCase(true, Eip8141MeasurementCeilings.SoispokeDeclaredBudget, TestName = "control: a prefix that approves is included and paid for")]
    [TestCase(false, 300_000ul, TestName = "never approves, at the default MAX_VERIFY_GAS")]
    [TestCase(false, Eip8141MeasurementCeilings.SoispokeDeclaredBudget, TestName = "never approves, at soispoke's declared privacy-pool budget")]
    ...
    {
        foreach (ulong verifyGas in Eip8141MeasurementCeilings.Swept)
        {
