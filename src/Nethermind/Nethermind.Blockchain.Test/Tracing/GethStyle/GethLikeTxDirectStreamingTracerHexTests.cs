...
public class GethLikeTxDirectStreamingTracerHexTests
{
    ...
    public void FormatHexAscii_FormatsCorrectly(byte[] input, bool withPrefix, bool trimLeadingZeros, string expected)
    {
        ...
        actual.Should().Be(expected, "FormatHexAscii must produce Geth-compatible hex output");
    }
    ...
    public void FormatHexAscii_FullEvmWord_EmitsExactly64HexChars()
    {
        ...
        written.Should().Be(64, "a 32-byte word must produce exactly 64 hex chars when untrimmed");
        Encoding.ASCII.GetString(output[..written]).Should().Be("00070e151c232a31383f464d545b626970777e858c939aa1a8afb6bdc4cbd2d9", "byte-by-byte hex encoding must match the reference output");
    }
}
