...
public class TransactionProcessorTests
{
    ...
    [Test]
    public void Direct_transfer_basic_ether_transfer()
    {
        long gasLimit = 100000;
        Transaction tx = Build.A.Transaction
            .WithValue(HalfEther)
        ...
        _stateProvider.GetBalance(TestItem.AddressA).Should().Be(AccountBalance - HalfEther - (UInt256)GasCostOf.Transaction);
        _stateProvider.GetBalance(TestItem.AddressB).Should().Be(HalfEther);
        _stateProvider.GetNonce(TestItem.AddressA).Should().Be(1);
    }

    [Test]
    public void Direct_transfer_self_transfer()
    {
        long gasLimit = 100000;
        Transaction tx = Build.A.Transaction
        ...
        result.Should().Be(TransactionResult.Ok);
        // Self-transfer: value cancels out, only gas cost deducted
        _stateProvider.GetBalance(TestItem.AddressA).Should().Be(AccountBalance - (UInt256)GasCostOf.Transaction);
        _stateProvider.GetNonce(TestItem.AddressA).Should().Be(1);
    }

    [Test]
    public void Direct_transfer_to_non_existent_recipient()
    {
        long gasLimit = 100000;
        Transaction tx = Build.A.Transaction
            .WithValue(TenthEther)
        ...
        _stateProvider.GetBalance(TestItem.AddressD).Should().Be(TenthEther);
    }

    [Test]
    public void Direct_transfer_zero_value()
    {
        long gasLimit = 100000;
        Transaction tx = Build.A.Transaction
            .WithValue(0)
        ...
        result.Should().Be(TransactionResult.Ok);
        _stateProvider.GetBalance(TestItem.AddressA).Should().Be(AccountBalance - (UInt256)GasCostOf.Transaction);
        _stateProvider.GetBalance(TestItem.AddressB).Should().Be(UInt256.Zero);
        _stateProvider.GetNonce(TestItem.AddressA).Should().Be(1);
    }
    ...
    [Test]
    public void Direct_transfer_wrong_nonce_fails()
    {
        Transaction tx = Build.A.Transaction
            .WithValue(TenthEther)
            .WithGasPrice(1)
        ...
        // Balance and nonce unchanged
        _stateProvider.GetBalance(TestItem.AddressA).Should().Be(AccountBalance);
        _stateProvider.GetNonce(TestItem.AddressA).Should().Be(0);
    }

    [Test]
    public void Direct_transfer_insufficient_balance_fails()
    {
        Transaction tx = Build.A.Transaction
            .WithValue(2.Ether())
            .WithGasPrice(1)
    ...
    [Test]
    public void Direct_transfer_insufficient_balance_for_gas_fails()
    {
        // Set value so that balance - value < gas cost
        UInt256 transferValue = AccountBalance - (UInt256)(GasCostOf.Transaction - 1);
        Transaction tx = Build.A.Transaction
    ...
    [Test]
    public void Direct_transfer_exact_balance_succeeds()
    {
        // Spend exactly all balance: value + gas = 1 ETH
        UInt256 transferValue = AccountBalance - (UInt256)GasCostOf.Transaction;
        Transaction tx = Build.A.Transaction
            .WithValue(transferValue)
            .WithGasPrice(1)
            .WithGasLimit(GasCostOf.Transaction)
        ...
        result.Should().Be(TransactionResult.Ok);
        _stateProvider.GetBalance(TestItem.AddressA).Should().Be(UInt256.Zero);
    ...
    [Test]
    public void Direct_transfer_with_higher_gas_limit_refunds_unused()
    {
        long gasLimit = 100000;
        Transaction tx = Build.A.Transaction
            .WithValue(TenthEther)
        ...
        result.Should().Be(TransactionResult.Ok);
        // Only 21000 gas used, rest refunded
        _stateProvider.GetBalance(TestItem.AddressA).Should().Be(AccountBalance - TenthEther - (UInt256)GasCostOf.Transaction);
    }
