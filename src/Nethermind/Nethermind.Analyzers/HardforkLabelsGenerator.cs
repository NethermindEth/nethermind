...
public sealed class HardforkLabelsGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(forks.Collect(), static (spc, collected) =>
        {
            ...
            // Gather the per-EIP JSON property names referenced across all forks so the generated
            // IEipTransitionFields interface declares exactly the surface labels touch.
            // ChainSpecParamsJson and ChainParameters already declare these as public { get; set; },
            // so the interface lands as implicit-impl.
            SortedSet<string> fieldNames = new(StringComparer.Ordinal);
            foreach (ForkInfo fork in all)
            {
                if (fork.ParentName is null) continue;
                foreach (string field in ResolveJsonFields(fork)) fieldNames.Add(field);
            }

            spc.AddSource("IEipTransitionFields.g.cs", SourceText.From(EmitInterface(fieldNames), Encoding.UTF8));
            spc.AddSource("HardforkLabels.g.cs", SourceText.From(Emit(all), Encoding.UTF8));
        });
    }

    private static string EmitInterface(SortedSet<string> fieldNames)
    {
        StringBuilder sb = new();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("// Generated by Nethermind.Analyzers.HardforkLabelsGenerator.");
        sb.AppendLine("// The members listed here are exactly the EIP transition fields HardforkLabels");
        sb.AppendLine("// references; ChainSpecParamsJson and ChainParameters provide them.");
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        sb.AppendLine("namespace Nethermind.Specs.ChainSpecStyle;");
        sb.AppendLine();
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// Shared per-EIP transition surface used by <see cref=\"HardforkLabels\"/>. Implemented by both");
        sb.AppendLine("/// <c>ChainSpecParamsJson</c> (wire format) and <c>ChainParameters</c> (post-processed model);");
        sb.AppendLine("/// the property names are identical across the two by convention and enforced by");
        sb.AppendLine("/// <c>ChainParametersTests.ChainParameters_should_have_same_properties_as_chainSpecParamsJson</c>.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("public partial interface IEipTransitionFields");
        sb.AppendLine("{");
        foreach (string name in fieldNames)
        {
            // EipNNNTransitionTimestamp → ulong?; everything else (EipNNNTransition, EipNNNDisableTransition) → long?.
            string type = name.EndsWith("TransitionTimestamp", StringComparison.Ordinal) ? "ulong?" : "long?";
            sb.Append("    ").Append(type).Append(' ').Append(name).AppendLine(" { get; set; }");
        }
        sb.AppendLine("}");
        return sb.ToString();
    }
