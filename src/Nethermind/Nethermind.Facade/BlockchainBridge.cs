...
namespace Nethermind.Facade
{
        ...
        : IBlockchainBridge
    {
        ...
        private static string? ConstructError(TransactionResult txResult, string? tracerError)
        {
            ...
            {
                { TransactionExecuted: true } when txResult.EvmExceptionType is not (EvmExceptionType.None or EvmExceptionType.Revert) => txResult.ErrorErrorDescription is { Length: > 0 } d ? d : txResult.EvmExceptionType.GetEvmExceptionDescription(),
                { TransactionExecuted: true } when tracerError is not null => tracerError,
                { TransactionExecuted: false, Error: not TransactionResult.ErrorType.None } => txResult.ErrorErrorDescription,
                _ => null
            };
