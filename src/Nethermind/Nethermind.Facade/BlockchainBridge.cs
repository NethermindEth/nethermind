...
namespace Nethermind.Facade
{
        ...
        : IBlockchainBridge
    {
        ...
        private CallOutput RunEstimateGas(ITransactionProcessor txProcessor, IWorldState worldState, BlockHeader header, Transaction tx, int errorMargin, UInt256? blobBaseFeeOverride, CancellationToken cancellationToken)
        {
            ...
            error = err switch
            {
                // Probe failed only because gas hint was below standard intrinsic or above the
                // EIP-8037 total cap: if estimation succeeds, clear the probe error.
                ...
                // Probe's low-gas or over-cap failure is superseded by whatever the estimator found at full gas.
