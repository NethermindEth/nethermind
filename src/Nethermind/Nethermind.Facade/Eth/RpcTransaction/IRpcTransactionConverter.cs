...
{
    IRpcTransaction FromTransaction(Transaction tx, TxReceipt receipt);
}
...
public class ComposeTransactionConverter : IRpcTransactionConverter
{
    ...
    public IRpcTransaction FromTransaction(Transaction tx, TxReceipt receipt)
    {
        var converter = _converters[(byte)tx.Type] ?? throw new ArgumentException("No converter for transaction type");
        return converter.FromTransaction(tx, receipt);
    }
}
