...
namespace Nethermind.Facade
{
    public interface IBlockchainBridge : ILogFinder
    {
        ...
        TxReceipt GetReceipt(Commitment txHash);
        (TxReceipt? Receipt, TxGasInfo? GasInfo, int LogIndexStart) GetReceiptAndGasInfo(Commitment txHash);
        (TxReceipt? Receipt, Transaction Transaction, UInt256? baseFee) GetTransaction(Commitment txHash);
        ...
        Commitment[] GetBlockFilterChanges(int filterId);
        Commitment[] GetPendingTransactionFilterChanges(int filterId);
        ...
        void RunTreeVisitor(ITreeVisitor treeVisitor, Commitment stateRoot);
