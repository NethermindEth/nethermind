...
namespace Nethermind.Network.Portal.History.Rpc;
...
) : IPortalHistoryRpcModule
{
    ...
    public Task<ResultWrapper<FindContentResult>> portal_historyGetContent(byte[] contentKey)
    {
        return contentNetworkRpcHandler.LookupContent(contentKey);
    }

    public Task<ResultWrapper<byte[]>> portal_historyOffer(string enrStr, byte[][][] content)
    {
        return contentNetworkRpcHandler.Offer(enrStr, content);
    }
