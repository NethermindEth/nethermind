...
)
{
    ...
    public async Task<ResultWrapper<FindContentResult>> LookupContent(byte[] contentKey)
    {
        var token = new CancellationToken();

        (var payload, var utpTransfer) = await contentLookupService.LookupContent(contentKey, token);
    ...
    public async Task<ResultWrapper<byte[]>> Offer(string enrStr, byte[][][] content)
    {
        ...
        try
        {
            ...
            {
                if (kv is { Length: 2 })
                    await contentDistributor.OfferAndSendContent(enr, kv[0], kv[1], cts.Token);
            }
