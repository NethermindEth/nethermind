...
public class StateCompositionSnapshotStoreTests
{
    ...
    {
        // Regression: prior block-number-prefixed schema accumulated keys
        // forever (one set per snapshot) which on the bloatnet grew to 1.6 TB
        // / 4,300 SSTs / 769 stale generations of orphan data in RocksDB. The
        // gen-rotated schema must overwrite, so after N writes the key count
        // is bounded by 2 × (per-snapshot key count) — never N × it.
        MemDb db = new();
