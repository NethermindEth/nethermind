...
public class UTPPacketHeader : IEquatable<UTPPacketHeader>
{
    ...
    public static (UTPPacketHeader, int) DecodePacket(ReadOnlySpan<byte> payload)
    {
        ...
        {
            var nextNextExtension = payload[readByte];
            var extensionLength = payload[readByte + 1];
    ...
    {
        var headerPortion = EncodePacketHeader(utpPacket, buffer);
    ...
    public static int EncodePacketHeader(UTPPacketHeader utpPacket, Span<byte> buffer)
    {
        ...
        var writtenBytes = HEADER_LENGTH;
    ...
    public override string ToString()
    {
        ...
        {
            var bitSetString = string.Concat(SelectiveAck.Select(b => Convert.ToString(b, 2).PadLeft(8, '0')));
