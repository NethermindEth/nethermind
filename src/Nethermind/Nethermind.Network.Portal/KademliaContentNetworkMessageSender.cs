...
) : IContentMessageSender<IEnr, byte[], LookupContentResult>, IKademliaMessageSender<IEnr>
{
    ...
    public async Task Ping(IEnr receiver, CancellationToken token)
    {
        ...
        {
            var radius = new UInt256(pong.CustomPayload, false);
    ...
    public async Task<IEnr[]> FindNeighbours(IEnr receiver, ValueHash256 hash, CancellationToken token)
    {
        ...
        // nodeid.
        var theDistance = (ushort)Hash256XorUtils.CalculateDistance(_nodeHashProvider.GetHash(receiver), hash);
        ...
        var toAddExtra = 4;
        var queryDistance = new ushort[1 + toAddExtra];
        var nowUpper = true;
        var upper = theDistance;
        var lower = theDistance;
        queryDistance[0] = theDistance;
        for (var i = 0; i < toAddExtra; i++)
        {
