...
) : ITalkReqProtocolHandler
{
    ...
    private async Task<byte[]?> HandlePing(IEnr sender, Ping ping)
    {
        ...
        {
            var radius = new UInt256(ping.CustomPayload, isBigEndian: false);
    ...
    {
        var connectionId = (ushort)Random.Shared.Next();
        ...
        Task.Run(async () =>
        {
            ...
            var inputStream = new MemoryStream(valuePayload);
    ...
    private Task<byte[]?> HandleOffer(IEnr sender, Offer offer)
    {
        ...
        var toAccept = new BitArray(offer.ContentKeys.Length);
        var hasAccept = false;
