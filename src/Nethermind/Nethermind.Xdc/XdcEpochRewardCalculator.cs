...
/// <summary>
/// Calculates XDC epoch rewards on demand without persisting results or mutating state.
/// </summary>
/// <remarks>
/// Rewards are paid only at epoch checkpoints. Pre-upgrade: proportional split of
/// <see cref="IXdcReleaseSpec.Reward"/> across masternode signatures. TIP-upgrade: fixed
/// per-signer rewards for masternode/protector/observer. Holder split is always 90% owner,
/// 10% foundation.
/// </remarks>
public class XdcEpochRewardCalculator(
    IEpochSwitchManager epochSwitchManager,
    ISpecProvider specProvider,
    IBlockTree blockTree,
    IMasternodeVotingContract masternodeVotingContract,
    ISigningTxCache signingTxCache) : IXdcRewardCalculator
{
    private readonly EthereumEcdsa _ethereumEcdsa = new(specProvider.ChainId);

    public BlockReward[] CalculateEpochRewards(XdcBlockHeader epochHeader)
        => CalculateEpochRewardsCore(epochHeader).Rewards;

    /// <summary>
    /// Calculates epoch rewards and returns accounting values for the minted-record contract update.
    /// </summary>
    /// <remarks>
    ...
    internal (BlockReward[] Rewards, UInt256 BurnedInEpoch, bool IsTipUpgrade) CalculateEpochRewardsCore(XdcBlockHeader epochHeader)
    {
        ...
        if (!epochSwitchManager.IsEpochSwitchAtBlock(epochHeader))
            return ([], UInt256.Zero, false);

        ulong number = epochHeader.Number;
        IXdcReleaseSpec spec = specProvider.GetXdcSpec(epochHeader, epochHeader.ExtraConsensusData.BlockRound);
        ...
        {
            Dictionary<Address, UInt256> masternodeRewards = CalculateFixedRewardForSigners(spec.MasternodeReward, masternodeSigners);
        ...
        return ([.. rewards], burnedInOneEpoch, spec.IsTipUpgradeRewardEnabled);
    }
        ...
        UInt256 BurnedInOneEpoch) GetSigningTxCount(XdcBlockHeader epochHeader, IXdcReleaseSpec spec)
    {
        ...
        while (true)
        {
            ...
            h = (XdcBlockHeader)blockTree.FindHeader(parentHash!, blockIdx) ?? throw new InvalidOperationException($"Header with hash {parentHash} not found");
            ...
            if (epochSwitchManager.IsEpochSwitchAtBlock(h) && h.Number != spec.SwitchBlock + 1)
            {
                epochCount++;
            ...
            Transaction[] signingTxs = signingTxCache.GetSigningTransactions(h.Hash, blockIdx, spec);
    ...
    private static void IncrementSignerCount(Dictionary<Address, ulong> signers, Address addr)
    {
    ...
    private Dictionary<Address, UInt256> CalculateRewardForSigners(UInt256 totalReward,
        Dictionary<Address, ulong> signers)
    {
        Dictionary<Address, UInt256> rewardSigners = [];
        ulong totalSigningCount = 0;
        foreach (ulong signerCount in signers.Values)
            totalSigningCount += signerCount;

        foreach ((Address signer, ulong count) in signers)
        {
        ...
        Dictionary<Address, ulong> signers)
    {
        ...
        foreach ((Address signer, ulong _) in signers)
    ...
    /// <summary>
    /// Distributes a signer's reward using the 90/10 owner/foundation split.
    /// </summary>
    /// <remarks>
    /// If the masternode owner is the foundation wallet, the reference client stores only the
    /// foundation entry (overwrite semantics), so the holder reward is the 10% foundation amount
    /// and the separate foundation contribution is zero.
    /// </remarks>
    protected internal virtual (BlockReward HolderReward, UInt256 FoundationWalletReward) DistributeRewards(
