    ...
    IForensicsProcessor forensicsProcessor) : IVotesManager
{
    ...
    public bool VerifyVotingRules(BlockRoundInfo roundInfo, QuorumCertificate qc) =>
        VerifyVotingRules(roundInfo.Hash, roundInfo.BlockNumber, roundInfo.Round, qc, out _);
    ...
    public bool VerifyVotingRules(Hash256 blockHash, long blockNumber, ulong roundNumber, QuorumCertificate qc, out string? error)
    {
        ...
        {
            error = $"Block {blockHash} (number {blockNumber}, round {roundNumber}) does not extend from locked QC block {_ctx.LockQC.ProposedBlockInfo.Hash} (number {_ctx.LockQC.ProposedBlockInfo.BlockNumber}, round {_ctx.LockQC.ProposedBlockInfo.Round})";
            return false;
        }
