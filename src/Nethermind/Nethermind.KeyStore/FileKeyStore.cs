...
namespace Nethermind.KeyStore
{
    ...
    public class FileKeyStore : IKeyStore
    {
        ...
        public (byte[] Key, Result Result) GetKeyBytes(Address address, SecureString password)
        {
            ...
            var restoredMac = Keccak.Compute(derivedKey.Slice(kdfParams.DkLen - 16, 16).Concat(cipher).ToArray()).Span;
        ...
        public Result StoreKey(Address address, byte[] keyContent, SecureString password)
        {
            ...
            var mac = Keccak.Compute(derivedKey.Skip(_config.KdfparamsDklen - 16).Take(16).Concat(cipher).ToArray()).ValueKeccak;
