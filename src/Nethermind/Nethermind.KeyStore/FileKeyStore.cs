    ...
    public class FileKeyStore : IKeyStore
    {
        ...
        private string ReadKey(Address address)
        {
            ...
            try
            {
                ...
                if (files.Length == 0)
                {
                    if(_logger.IsError) _logger.Error($"A private key for address: {address} does not exists in directory {Path.GetFullPath(_keyStoreIOSettingsProvider.StoreDirectory)}.");
                    return null;
                }
