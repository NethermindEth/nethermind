...
{
    public Task<ResultWrapper<string>> admin_exportEraHistory(string destinationPath, ulong from, ulong to) =>
        Task.FromResult(eraService.ExportHistory(destinationPath, from, to));

    public Task<ResultWrapper<string>> admin_importEraHistory(string sourcePath, ulong from = 0, ulong to = 0, string? accumulatorFile = null) =>
