...
{
    private const int PairLength = Eip2537.LenG2 + Eip2537.LenFr;
    private const int TrimmedPairLength = Eip2537.LenG2Trimmed + Eip2537.LenFr;
    ...
    private static Result<byte[]> Msm(ReadOnlySpan<byte> inputData, int pairCount)
    {
        ...
        if (pairCount <= StackallocPairCountThreshold)
        {
            ...
            byte status = ZiskBindings.Crypto.bls12_381_g2_msm_c(output, input, (nuint)pairCount);
    ...
    {
        Span<byte> input = stackalloc byte[Eip2537.LenG2Trimmed + Eip2537.LenFr];

        if (!Eip2537.TryDecodeG2(inputData[..Eip2537.LenG2], input[..Eip2537.LenG2Trimmed]))
            return Errors.InvalidFieldElementTopBytes;

        inputData[Eip2537.LenG2..].CopyTo(input[Eip2537.LenG2Trimmed..]);

        Span<byte> output = stackalloc byte[Eip2537.LenG2Trimmed];

        byte status = ZiskBindings.Crypto.bls12_381_g2_msm_c(output, input, (nuint)1);
    ...
    private static bool TryDecodeInput(ReadOnlySpan<byte> inputData, Span<byte> packedInput, int pairCount)
    {
        ...
        {
            if (!Eip2537.TryDecodeG2(inputData.Slice(srcOffset, Eip2537.LenG2), packedInput.Slice(dstOffset, Eip2537.LenG2Trimmed)))
                return false;

            inputData.Slice(srcOffset + Eip2537.LenG2, Eip2537.LenFr)
                .CopyTo(packedInput.Slice(dstOffset + Eip2537.LenG2Trimmed, Eip2537.LenFr));
