...
namespace Nethermind.Abi
{
    public class AbiAddress : AbiUInt
    {
        ...
        public override (object, int) Decode(byte[] data, int position, bool packed) => (new Address(data.Slice(position + (packed ? 0 : 12), Address.LengthInBytes)), position + (packed ? Address.LengthInBytes : UInt256.LengthInBytes));
    }
}
