...
public class EthereumEthApi(string l1EthApiEndpoint, IJsonSerializer jsonSerializer, ILogManager logManager) : IEthApi
{
    ...
    public async Task<UInt256> GetBlobBaseFee()
    {
        return await _ethRpcClient.Post<UInt256?>("eth_blobBaseFee") ?? UInt256.Zero;
    }

    public async Task<L1FeeHistoryResults?> GetFeeHistory(int blockCount, BlockParameter newestBlock, double[]? rewardPercentiles)
    {
        return await _ethRpcClient.Post<L1FeeHistoryResults?>("eth_feeHistory", blockCount, newestBlock, rewardPercentiles ?? []);
    }
}
