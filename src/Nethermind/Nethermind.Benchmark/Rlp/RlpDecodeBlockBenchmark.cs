...
namespace Nethermind.Benchmarks.Rlp
{
    public class RlpDecodeBlockBenchmark
    {
        ...
        public RlpDecodeBlockBenchmark()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                transactions[i] = Build.A.Transaction.WithData(new byte[] { (byte)i }).WithNonce((UInt256)i).WithValue((UInt256)i).Signed(new EthereumEcdsa(TestChainIds.ChainId, LimboLogs.Instance), TestItem.PrivateKeyA).TestObject;
            }
