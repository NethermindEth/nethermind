...
namespace Nethermind.Benchmarks.Rlp
{
    public class RlpEncodeBlockBenchmark
    {
        ...
        public RlpEncodeBlockBenchmark()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                transactions[i] = Build.A.Transaction.WithData(new byte[] { (byte)i }).WithNonce((ulong)i).WithValue((UInt256)i).Signed(new EthereumEcdsa(TestBlockchainIds.ChainId), TestItem.PrivateKeyA).TestObject;
            }
