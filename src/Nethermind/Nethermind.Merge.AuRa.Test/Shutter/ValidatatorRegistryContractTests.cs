...
{
    private readonly Address _validatorAddress = new("0x000000000000000000000000000000000000beef");
    private readonly ulong _validatorIndex = 2;
...
#pragma warning disable format
        ...
        0, 0, 0, 0, 0, 0, 0, 2, // validator index = 99
...
#pragma warning restore format
    ...
    public void SetUp()
    {
        ...
        // validator index = 2
        _validatorStore.GetValidators().Returns(new Address[] { Address.Zero, Address.Zero, _validatorAddress, Address.Zero });
        ...
        _transactionProcessor.When(x => x.Execute(Arg.Any<Transaction>(), Arg.Any<Evm.BlockExecutionContext>(), Arg.Any<CallOutputTracer>())).Do(x =>
        {
            ...
            {
                // encode update
                byte[] message = new ValidatorRegistryContract.Message(_validatorAddress, 0, 1000).ComputeRegistrationMessage();
    ...
    {
        byte[] encodedRegistrationMessage = new ValidatorRegistryContract.Message(_validatorAddress, _validatorIndex, _nonce).ComputeRegistrationMessage();
    ...
    public void Can_decode_registration_message()
    {
        ...
        decodedRegistrationMessage.Sender.Should().Be(_validatorAddress);
    ...
    // {
    //     ValidatorRegistryContract contract = new(_transactionProcessor, AbiEncoder.Instance, _contractAddress, _signer, _txSender, _txSealer, _validatorStore, _blockHeader);
    //     ulong validatorIndex = contract.GetValidatorIndex(_validatorStore);
    ...
    {
        ValidatorRegistryContract contract = new(_transactionProcessor, AbiEncoder.Instance, _validatorAddress, _signer, _txSender, _txSealer, _validatorStore, _blockHeader);
        ulong nonce = contract.GetNextNonce(_blockHeader, 2);
