...
public class SSZGenerator : IIncrementalGenerator
{
        ...
        ImmutableArray<StructDeclarationSyntax> structs)
    {
        ...
        sb.Append(GenerateMainFunction(fields, structs));
    ...
    private static string GenerateMainFunction(ImmutableArray<FieldDeclarationSyntax> fields, ImmutableArray<StructDeclarationSyntax> structs)
    {
        var sb = new StringBuilder();
        ...
        */
        for(int i=0; i < fields.Length;i++){ //generate field parameters
            var variable = fields[i].Declaration.Variables.FirstOrDefault();
            if (variable != null)
            {
                var fieldType = fields[i].Declaration.Type.ToString();
                string fieldName = char.ToLower(variable.Identifier.Text[0]) + variable.Identifier.Text.Substring(1);
                sb.Append($"{fieldType} {fieldName}{(i+1==fields.Length && structs.Length == 0 ? "" : ",")} ");
            }
        }

        for(int i=0;i<structs.Length;i++){ //generate struct parameters
            string structVarName = structs[i].Identifier.Text[0]+ structs[i].Identifier.Text.Substring(1);
            sb.Append($"{structs[i].Identifier.Text} {structVarName}{(i+1==structs.Length ? "" : ",")} ");
        }
        ...
        2) Generate calling on CalculateByteSize function to calculate total bytes needed */
        sb.AppendLine("            int offset = 0;");
        sb.AppendLine("            int byteLength = 0;");
        foreach(var fieldDecl in fields){
            var variable = fieldDecl.Declaration.Variables.FirstOrDefault();
            if (variable != null)
            {
                string fieldName = char.ToLower(variable.Identifier.Text[0]) + variable.Identifier.Text.Substring(1);
                sb.AppendLine($"            byteLength += CalculateByteSize({fieldName});");
            }
        }
        foreach(var structDecl in structs){
            string structVarName = structDecl.Identifier.Text[0]+ structDecl.Identifier.Text.Substring(1);
            sb.AppendLine($"            byteLength += CalculateByteSize({structVarName});");
        }

        sb.AppendLine("            var buffer = new byte[byteLength];");
        sb.AppendLine();


        /*** 
        Generate code on calling encoding functions
        */
        ...
        for(int i=0; i < fields.Length;i++){
            ...
            if (variable != null)
            {
                var fieldType = fields[i].Declaration.Type.ToString();
                string fieldName = char.ToLower(variable.Identifier.Text[0]) + variable.Identifier.Text.Substring(1);
                sb.Append($" {fieldName} {(i+1 == fields.Length ? ");" : ",")}");
            }
        }
        sb.AppendLine();
        ...
        sb.AppendLine(@"
            var slicedBuffer = new ArraySegment<byte>(buffer, 0, offset).ToArray();
            return slicedBuffer;
        }
        ");
