...
namespace Nethermind.Trie
{
    ...
    public class PatriciaTree
    {
        ...
        public virtual ReadOnlySpan<byte> Get(ReadOnlySpan<byte> rawKey, Hash256? rootHash = null)
        {
            try
            {
                var nibbles = new NibblePath.Ref(rawKey);
        ...
        [DebuggerStepThrough]
        public byte[]? GetNodeByPath(NibblePath.Ref nibbles, Hash256? rootHash = null)
        {
            try
            {
                TreePath updatePathTreePath = TreePath.Empty; // Only used on update.
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static void EnhanceExceptionNibble(NibblePath.Ref nibble, ValueHash256 rootHash, TrieException baseException)
        {
            static TrieNodeException? GetTrieNodeException(TrieException? exception) =>
                exception switch
                {
            ...
            else
            {
                ...
                if (trieIsEmpty)
                {
                    if (traverseContext.UpdateValue.IsNotNull)
                    {
                        ...
                        NibblePath key = updatePath.ToLeafPath();
        ...
        private ref readonly CappedArray<byte> TraverseLeaf(TrieNode node, scoped in TraverseContext traverseContext, scoped ref TreePath path)
        {
            ...
            NibblePath.Ref shorterPath;
            NibblePath.Ref longerPath;
            ...
            {
                NibblePath extensionPath = longerPath.ToExtensionPath(extensionLength);
            ...
            {
                NibblePath shortLeafPath = shorterPath.ToLeafPath(extensionLength + 1);
            ...
            NibblePath leafPath = longerPath.ToLeafPath(extensionLength + 1);
        ...
        private ref readonly CappedArray<byte> TraverseExtension(TrieNode node, scoped in TraverseContext traverseContext, scoped ref TreePath path)
        {
            ...
            if (extensionLength != 0)
            {
                NibblePath extensionPath = node.Key[..extensionLength];
            ...
            else
            {
                ...
                TrieNode shortLeaf = TrieNodeFactory.CreateLeaf(remainingPath.ToLeafPath(), in traverseContext.UpdateValue);
            ...
            {
                NibblePath extensionPath = pathBeforeUpdate.Slice(extensionLength + 1, pathBeforeUpdate.Length - extensionLength - 1);
        ...
        private ref readonly CappedArray<byte> ResolveCurrent(scoped in TraverseContext traverseContext)
        {
            ...
            TrieNode leaf = TrieNodeFactory.CreateLeaf(leafPath.ToLeafPath(), in traverseContext.UpdateValue);
        ...
        private readonly ref struct TraverseContext
        {
            ...
            public readonly NibblePath.Ref UpdatePath;
            ...
            public NibblePath.Ref GetRemainingUpdatePath() => UpdatePath.Slice(CurrentIndex, RemainingUpdatePathLength);
