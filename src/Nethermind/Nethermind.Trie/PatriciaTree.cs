...
namespace Nethermind.Trie
{
    ...
    public partial class PatriciaTree
    {
        ...
        /// <summary>
        /// Gets a value from the trie using partial key matching.
        /// This is useful when the key stored in flat storage is only a prefix (e.g., 20 bytes)
        /// of the full trie path (32 bytes).
        /// </summary>
        /// <param name="partialKey">The partial key to match (will match if it's a prefix of a leaf's full path)</param>
        /// <param name="rootHash">Optional root hash to use instead of current root</param>
        /// <returns>The value if a matching leaf is found, empty span otherwise</returns>
        [SkipLocalsInit]
        [DebuggerStepThrough]
        public virtual ReadOnlySpan<byte> GetWithPartialKey(ReadOnlySpan<byte> partialKey, Hash256? rootHash = null)
        {
            byte[]? array = null;
            try
            {
                int nibblesCount = 2 * partialKey.Length;
                Span<byte> nibbles = (partialKey.Length <= MaxKeyStackAlloc
                        ? stackalloc byte[MaxKeyStackAlloc]
                        : array = ArrayPool<byte>.Shared.Rent(nibblesCount))
                    [..nibblesCount];
                ...
                SpanSource result = GetWithPartialKeyNew(nibbles, ref emptyPath, root);

                return result.IsNull ? ReadOnlySpan<byte>.Empty : result.Span;
            }
            catch (TrieException e)
            {
                EnhanceException(partialKey, rootHash ?? RootHash, e);
                throw;
            }
            finally
            {
        ...
        /// <summary>
        /// Variant of GetNew that supports partial key matching.
        /// When reaching a leaf, it checks if the remaining partial key matches the beginning
        /// of the leaf's key rather than requiring an exact match.
        /// </summary>
        private SpanSource GetWithPartialKeyNew(Span<byte> remainingKey, ref TreePath path, TrieNode? node)
        {
            int originalPathLength = path.Length;
            ...
            try
            {
                while (true)
                {
                    ...
                    if (node.IsLeaf || node.IsExtension)
                    {
                        ...
                        if (node.IsLeaf)
                        {
                            // For partial key matching: if the remaining key is fully consumed,
                            // we have a match (the partial key is a prefix of the full path)
                            if (commonPrefixLength == remainingKey.Length)
                            {
                                return node.Value;
                            }
                            // No match - the partial key doesn't match this leaf's path
                            return default;
                        }
                        ...
                        else if (commonPrefixLength == remainingKey.Length)
                        {
                            // Partial key is fully consumed within extension - need to find first leaf
                            path.AppendMut(node.Key);
                            TrieNode? extensionChild = node.GetChildWithChildPath(TrieStore, ref path, 0);
                            return GetFirstLeafValue(ref path, extensionChild);
                        }
                    ...
                    {
                        // Partial key fully consumed at branch - get first leaf from any child
                        return GetFirstLeafValue(ref path, node);
                    }
        ...
        /// <summary>
        /// Gets the value of the first leaf found in the subtree rooted at the given node.
        /// Used for partial key matching when we need to find any matching leaf.
        /// </summary>
        private SpanSource GetFirstLeafValue(ref TreePath path, TrieNode? node)
        {
            while (node is not null)
            {
                ...
                {
                    path.AppendMut(node.Key!);
                    node = node.GetChildWithChildPath(TrieStore, ref path, 0);
                    continue;
                }

                // Branch node - find first non-null child
                for (int i = 0; i < 16; i++)
                {
                    path.AppendMut(i);
                    TrieNode? child = node.GetChildWithChildPath(TrieStore, ref path, i);
