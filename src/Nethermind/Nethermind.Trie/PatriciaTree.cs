    ...
    public class PatriciaTree
    {
        ...
        public virtual byte[]? Get(ReadOnlySpan<byte> rawKey, Hash256? rootHash = null)
        {
            try
            {
                ...
                ref readonly CappedArray<byte> result = ref Run(nibbles,
                                                                nibblesCount,
                                                                in CappedArray<byte>.Empty,
                                                                isUpdate: false,
                                                                startRootHash: rootHash);
                ...
                if (node.IsBranch)
                {
                    ...
                    else
                    {
                        if (node.Value!.Length != 0)
                        {
                            ...
                            TrieNode leafFromBranch = TrieNodeFactory.CreateLeaf(Array.Empty<byte>(), in node.ValueRef);
