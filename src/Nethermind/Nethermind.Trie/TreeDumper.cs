...
namespace Nethermind.Trie
{
    public class TreeDumper : ITreeVisitor<OldStyleTrieVisitContext>
    {
        ...
        {
            _builder.AppendLine($"{GetPrefix(context)}EXTENSION {node.Key.ToHexString(false)} -> {KeccakOrRlpStringOfNode(node)}");
        }
        ...
        public void VisitAccount(in OldStyleTrieVisitContext context, TrieNode node, in AccountStruct account)
        {
            ...
            _builder.AppendLine($"{GetPrefix(context)}{leafDescription} {node.Key.ToHexString(false)} -> {KeccakOrRlpStringOfNode(node)}");
