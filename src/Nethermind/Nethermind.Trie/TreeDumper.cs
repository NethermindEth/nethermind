...
namespace Nethermind.Trie
{
    public class TreeDumper : ITreeVisitor
    {
        ...
        {
            _builder.AppendLine($"{GetPrefix(trieVisitContext)}BRANCH | -> {(node.Keccak?.ToByteArray() ?? node.FullRlp)?.ToHexString()}");
        }

        public void VisitExtension(TrieNode node, TrieVisitContext trieVisitContext)
        {
            _builder.AppendLine($"{GetPrefix(trieVisitContext)}EXTENSION {Nibbles.FromBytes(node.Key).ToPackedByteArray().ToHexString(false)} -> {(node.Keccak?.ToByteArray() ?? node.FullRlp)?.ToHexString()}");
        }
        ...
        public void VisitLeaf(TrieNode node, TrieVisitContext trieVisitContext, byte[] value = null)
        {
            ...
            _builder.AppendLine($"{GetPrefix(trieVisitContext)}{leafDescription} {Nibbles.FromBytes(node.Key).ToPackedByteArray().ToHexString(false)} -> {(node.Keccak?.ToByteArray() ?? node.FullRlp)?.ToHexString()}");
