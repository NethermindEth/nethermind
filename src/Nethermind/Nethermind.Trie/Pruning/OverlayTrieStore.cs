...
{
    public override bool IsPersisted(in ValueHash256 address, in TreePath path, in ValueHash256 keccak) =>
        base.IsPersisted(address, in path, in keccak) || store.IsPersisted(address, in path, in keccak);

    public override TrieNode FindCachedOrUnknown(in ValueHash256 address, in TreePath path, in ValueHash256 hash)
    {
    ...
    public override byte[]? LoadRlp(in ValueHash256 address, in TreePath path, in ValueHash256 hash, ReadFlags flags = ReadFlags.None) =>
        base.TryLoadRlp(address, in path, hash, flags) ?? store.LoadRlp(address, in path, hash, flags);

    public override byte[]? TryLoadRlp(in ValueHash256 address, in TreePath path, in ValueHash256 hash, ReadFlags flags = ReadFlags.None) =>
