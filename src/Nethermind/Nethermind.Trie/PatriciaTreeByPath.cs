    ...
    public class PatriciaTreeByPath : IPatriciaTree
    {
        ...
        private byte[]? TraverseBranch(TrieNode node, TraverseContext traverseContext)
        {
            ...
            if (childNode is null)
            {
                ...
                TrieNode leaf = TrieNodeFactory.CreateLeaf(leafPath, traverseContext.UpdateValue, traverseContext.GetCurrentPath());
                ConnectNodes(leaf);
        ...
        private byte[]? TraverseLeaf(TrieNode node, TraverseContext traverseContext)
        {
            ...
            if (extensionLength != 0)
            {
                ...
                TrieNode extension = TrieNodeFactory.CreateExtension(extensionPath.ToArray(), traverseContext.GetCurrentPath());
            ...
            TrieNode withUpdatedKeyAndValue = node.CloneWithChangedKeyAndValue(leafPath.ToArray(), longerPathValue, pathToLeaf.ToArray());
        ...
        private byte[]? TraverseExtension(TrieNode node, TraverseContext traverseContext)
        {
            ...
            else
            {
                ...
                TrieNode shortLeaf = TrieNodeFactory.CreateLeaf(path, traverseContext.UpdateValue, traverseContext.UpdatePath.Slice(0, traverseContext.CurrentIndex + extensionLength + 1));
