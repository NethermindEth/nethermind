...
public static class Out
{
    ...
    private static List<StackFrame> GetCallStack(int skipFrames = 2, int maxDepth = 20)
    {
        ...
        List<StackFrame> frames = new();

        int frameCount = Math.Min(stackTrace.FrameCount, maxDepth);
        ...
        for (int i = 0; i < frameCount; i++)
        {
            ...
            if (method?.DeclaringType != null)
            {
                ...
                if (methodName.Contains("<") && methodName.Contains(">"))
                {
                    ...
                    int startIdx = methodName.IndexOf('<') + 1;
                    int endIdx = methodName.IndexOf('>');
                    if (startIdx < endIdx)
    ...
    private static string GetCallStackString(int skipFrames = 3)
    {
        ...
        StringBuilder sb = new(frames.Count * 30);
