...
public class AddressConverter : JsonConverter<Address>
{
    ...
    internal static Address? ReadAddress(ref Utf8JsonReader reader)
    {
        ...
        if (ByteArrayConverter.TryConvertToExactLength(ref reader, bytes, strictHexFormat: true, requireEvenLength: true))
        {
            return new Address(bytes);
        }

        byte[]? addressBytes = ByteArrayConverter.Convert(ref reader, strictHexFormat: true, requireEvenLength: true);
    ...
    internal static Address ReadAddressPropertyName(ref Utf8JsonReader reader)
    {
        ...
        return new Address(ByteArrayConverter.Convert(ref reader, strictHexFormat: true, requireEvenLength: true) ?? throw new JsonException("Invalid address property name"));
    }
