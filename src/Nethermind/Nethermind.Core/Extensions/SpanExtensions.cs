...
namespace Nethermind.Core.Extensions
{
    public static class SpanExtensions
    {
        ...
        /// <remarks>
        /// This is an inlinable fast path for the common case of hashing 32 bytes (Hash256/UInt256).
        /// Uses AES-NI for high-throughput hashing with excellent diffusion.
        /// </remarks>
        ...
        internal static int FastHash(ReadOnlySpan<byte> input, uint seed)
        {
            ...
            ref byte p = ref MemoryMarshal.GetReference(input);
            int remaining = input.Length;
            uint hash = seed + (uint)input.Length;

            while (remaining >= 8)
            {
                hash = BitOperations.Crc32C(hash, Unsafe.ReadUnaligned<ulong>(ref p));
                p = ref Unsafe.Add(ref p, 8);
                remaining -= 8;
            }
