...
public static class Int64Extensions
{
    ...
    public static ulong ToLongFromBigEndianByteArrayWithoutLeadingZeros(this ReadOnlySpan<byte> bytes)
    {
        if (bytes.IsEmpty)
        {
            return 0UL;
        ...
        return UInt64Extensions.ToULongFromBigEndianByteArrayWithoutLeadingZeros(bytes);
    }

    public static ulong ToLongFromBigEndianByteArrayWithoutLeadingZeros(this Span<byte> bytes)
        => ToLongFromBigEndianByteArrayWithoutLeadingZeros((ReadOnlySpan<byte>)bytes);

    public static ulong ToLongFromBigEndianByteArrayWithoutLeadingZeros(this byte[]? bytes)
    {
        if (bytes is null)
        {
            return 0UL;
