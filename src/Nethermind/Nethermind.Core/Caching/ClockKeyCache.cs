...
{
    public sealed class ClockKeyCache<TKey>
        where TKey : struct, IEquatable<TKey>
    {
        private const int BitShiftPerInt64 = 6;
        private readonly int _maxCapacity;
        private readonly ConcurrentDictionary<TKey, int> _cacheMap;
        ...
        public ClockKeyCache(int maxCapacity, string name)
        {
            ArgumentOutOfRangeException.ThrowIfLessThan(maxCapacity, 1);

            _name = name;
            _maxCapacity = maxCapacity;
            _cacheMap = new ConcurrentDictionary<TKey, int>(); // do not initialize it at the full capacity
