...
public static class BlockExtensions
{
    ...
    public static bool IsPostTTD(this BlockHeader header, ISpecProvider specProvider, UInt256 totalDifficulty) => specProvider.TerminalTotalDifficulty is not null && totalDifficulty >= specProvider.TerminalTotalDifficulty;
    ...
    /// </remarks>
    public static bool IsTerminalBlock(this BlockHeader header, ISpecProvider specProvider, UInt256 totalDifficulty)
    {
        bool ParentBeforeTTD()
        {
            UInt256 parentTotalDifficulty = totalDifficulty >= header.Difficulty ? totalDifficulty - header.Difficulty : 0;
            return parentTotalDifficulty < specProvider.TerminalTotalDifficulty;
        }

        return header.IsPostTTD(specProvider, totalDifficulty) && (header.IsGenesis || header.Difficulty != 0 && ParentBeforeTTD());
    }
