...
namespace Nethermind.Core
{
    public static class TransactionExtensions
    {
        ...
        public static bool TryCalculatePremiumPerGas(this Transaction tx, in UInt256? baseFeePerGas, out UInt256 premiumPerGas)
        {
            ...
            premiumPerGas = UInt256.Min(tx.MaxPriorityFeePerGas, feeCap - (baseFeePerGas ?? 0));
            return true;
        }
        ...
        public static UInt256 CalculateTransactionPotentialCost(this Transaction tx, bool eip1559Enabled, in UInt256? baseFee)
        {
            if (eip1559Enabled)
            {
                UInt256 effectiveGasPrice = tx.CalculateEffectiveGasPrice(eip1559Enabled, baseFee!.Value);
        ...
        public static UInt256 CalculateEffectiveGasPrice(this Transaction tx, bool eip1559Enabled, in UInt256? baseFee)
        {
            ...
            if (eip1559Enabled)
            {
                if (UInt256.AddOverflow(tx.MaxPriorityFeePerGas, baseFee!.Value, out UInt256 effectiveFee))
                {
        ...
        {
            return eip1559Enabled ? UInt256.Min(tx.MaxPriorityFeePerGas, tx.MaxFeePerGas > baseFee!.Value ? tx.MaxFeePerGas - baseFee.Value : 0) : tx.MaxPriorityFeePerGas;
        }
