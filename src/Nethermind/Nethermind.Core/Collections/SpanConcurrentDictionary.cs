...
namespace Nethermind.Core.Collections
{
    ...
    /// <remarks>
    /// All public and protected members of <see cref="SpanConcurrentDictionary{TKey,TValue}"/> are thread-safe and may be used
    /// concurrently from multiple threads.
    /// </remarks>
    [DebuggerTypeProxy(typeof(IDictionaryDebugView<,>))]
    [DebuggerDisplay("Count = {Count}")]
    public class SpanConcurrentDictionary<TKey, TValue> : IDictionary<TKey[], TValue>, IDictionary, IReadOnlyDictionary<TKey[], TValue> where TKey : notnull
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="SpanConcurrentDictionary{TKey,TValue}"/>
        /// class that is empty, has the specified concurrency level and capacity, and uses the specified
        /// <see cref="ISpanEqualityComparer{TKey}"/>.
        /// </summary>
        /// <param name="comparer">The <see cref="IEqualityComparer{TKey}"/> implementation to use when comparing keys.</param>
        public SpanConcurrentDictionary(ISpanEqualityComparer<TKey> comparer)
        ...
        /// level, has the default initial capacity, and uses the specified <see cref="ISpanEqualityComparer{TKey}"/>.
        /// </summary>
        /// <param name="collection">The <see cref="IEnumerable{T}"/> whose elements are copied to the new <see cref="SpanConcurrentDictionary{TKey,TValue}"/>.</param>
        /// <param name="comparer">The <see cref="IEqualityComparer{TKey}"/> implementation to use when comparing keys.</param>
        /// <exception cref="ArgumentNullException"><paramref name="collection"/> is a null reference (Nothing in Visual Basic).</exception>
        public SpanConcurrentDictionary(IEnumerable<KeyValuePair<TKey[], TValue>> collection, ISpanEqualityComparer<TKey> comparer)
            : this(comparer)
        {
        ...
        /// The estimated number of threads that will update the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> concurrently.
        /// </param>
        /// <param name="collection">The <see cref="IEnumerable{T}"/> whose elements are copied to the new
        /// <see cref="SpanConcurrentDictionary{TKey,TValue}"/>.</param>
        /// <param name="comparer">The <see cref="ISpanEqualityComparer{TKey}"/> implementation to use when comparing keys.</param>
        /// <exception cref="ArgumentNullException"><paramref name="collection"/> is a null reference (Nothing in Visual Basic).</exception>
        ...
        public SpanConcurrentDictionary(int concurrencyLevel, IEnumerable<KeyValuePair<TKey[], TValue>> collection, ISpanEqualityComparer<TKey> comparer)
        ...
        private void InitializeFromCollection(IEnumerable<KeyValuePair<TKey[], TValue>> collection)
        {
            foreach (KeyValuePair<TKey[], TValue> pair in collection)
            {
                ...
                if (!TryAddInternal(pair.Key, pair.Key, null, pair.Value, updateIfExists: false, acquireLock: false, out _))
                {
        ...
        /// </summary>
        /// <param name="concurrencyLevel">The estimated number of threads that will update the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> concurrently.</param>
        /// <param name="capacity">The initial number of elements that the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> can contain.</param>
        /// <param name="comparer">The <see cref="IEqualityComparer{TKey}"/> implementation to use when comparing keys.</param>
        ...
        public SpanConcurrentDictionary(int concurrencyLevel, int capacity, ISpanEqualityComparer<TKey> comparer)
            : this(concurrencyLevel, capacity, growLockArray: false, comparer)
        {
        }

        internal SpanConcurrentDictionary(int concurrencyLevel, int capacity, bool growLockArray, ISpanEqualityComparer<TKey> comparer)
        {
            if (concurrencyLevel < 1)
            {
                throw new ArgumentOutOfRangeException(nameof(concurrencyLevel), "The concurrencyLevel argument must be positive.");
            }
            if (capacity < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(capacity), "The capacity argument must be greater than or equal to zero.");
            }
        ...
        /// <summary>
        /// Attempts to add the specified key and value to the <see cref="SpanConcurrentDictionary{TKey,TValue}"/>.
        /// </summary>
        /// <param name="key">The key of the element to add.</param>
        /// <param name="value">The value of the element to add. The value can be a null reference (Nothing
        /// in Visual Basic) for reference types.</param>
        /// <returns>
        /// true if the key/value pair was added to the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> successfully; otherwise, false.
        /// </returns>
        /// <exception cref="ArgumentNullException"><paramref name="key"/> is null reference (Nothing in Visual Basic).</exception>
        /// <exception cref="OverflowException">The <see cref="SpanConcurrentDictionary{TKey,TValue}"/> contains too many elements.</exception>
        public bool TryAdd(TKey[] key, TValue value)
        {
            ArgumentNullException.ThrowIfNull(key);
            return TryAddInternal(key, key, null, value, updateIfExists: false, acquireLock: true, out _);
        }
        ...
        {
            return TryAddInternal(key, null, null, value, updateIfExists: false, acquireLock: true, out _);
        }

        /// <summary>
        /// Determines whether the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> contains the specified key.
        /// </summary>
        /// <param name="key">The key to locate in the <see cref="SpanConcurrentDictionary{TKey,TValue}"/>.</param>
        /// <returns>true if the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> contains an element with the specified key; otherwise, false.</returns>
        ...
        /// <summary>
        /// Attempts to remove and return the value with the specified key from the <see cref="SpanConcurrentDictionary{TKey,TValue}"/>.
        /// </summary>
        /// <param name="key">The key of the element to remove and return.</param>
        /// <param name="value">
        /// When this method returns, <paramref name="value"/> contains the object removed from the
        /// <see cref="SpanConcurrentDictionary{TKey,TValue}"/> or the default value of <typeparamref
        /// name="TValue"/> if the operation failed.
        /// </param>
        /// <returns>true if an object was removed successfully; otherwise, false.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="key"/> is a null reference (Nothing in Visual Basic).</exception>
        public bool TryRemove(ReadOnlySpan<TKey> key, [MaybeNullWhen(false)] out TValue value)
        {
        ...
        private bool TryRemoveInternal(ReadOnlySpan<TKey> key, [MaybeNullWhen(false)] out TValue value, bool matchValue, TValue? oldValue)
        {
            ...
            while (true)
            {
                ...
                lock (locks[lockNo])
                {
                    ...
                    for (Node? curr = bucket; curr != null; curr = curr._next)
                    {
                        ...
                        if (hashcode == curr._hashcode && comparer.Equals(curr._key, key))
                        {
                            if (matchValue)
                            {
        ...
        /// <summary>
        /// Attempts to get the value associated with the specified key from the <see cref="SpanConcurrentDictionary{TKey,TValue}"/>.
        /// </summary>
        /// <param name="key">The key of the value to get.</param>
        /// <param name="value">
        /// When this method returns, <paramref name="value"/> contains the object from
        /// the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> with the specified key or the default value of
        /// <typeparamref name="TValue"/>, if the operation failed.
        /// </param>
        /// <returns>true if the key was found in the <see cref="SpanConcurrentDictionary{TKey,TValue}"/>; otherwise, false.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="key"/> is a null reference (Nothing in Visual Basic).</exception>
        ...
        private bool TryGetValueInternal(ReadOnlySpan<TKey> key, int hashcode, [MaybeNullWhen(false)] out TValue value)
        {
            // We must capture the volatile _tables field into a local variable: it is set to a new table on each table resize.
            // The Volatile.Read on the array element then ensures that we have a copy of the reference to tables._buckets[bucketNo]:
            // this protects us from reading fields ('_hashcode', '_key', '_value' and '_next') of different instances.
        ...
        public bool TryUpdate(ReadOnlySpan<TKey> key, TValue newValue, TValue comparisonValue)
        {
        ...
        /// </returns>
        /// <exception cref="ArgumentNullException"><paramref name="key"/> is a null reference.</exception>
        private bool TryUpdateInternal(ReadOnlySpan<TKey> key, int? nullableHashcode, TValue newValue, TValue comparisonValue)
        {
            ISpanEqualityComparer<TKey> comparer = _comparer;
        ...
        /// <summary>
        /// Removes all keys and values from the <see cref="SpanConcurrentDictionary{TKey,TValue}"/>.
        /// </summary>
        public void Clear()
        {
            int locksAcquired = 0;
        ...
        void ICollection<KeyValuePair<TKey[], TValue>>.CopyTo(KeyValuePair<TKey[], TValue>[] array, int index)
        {
            ...
            if (index < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(index), "The index argument is less than zero.");
            }
        ...
        /// <summary>
        /// Copies the key and value pairs stored in the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> to a
        /// new array.
        /// </summary>
        /// <returns>A new array containing a snapshot of key and value pairs copied from the <see cref="SpanConcurrentDictionary{TKey,TValue}"/>.
        /// </returns>
        ...
        /// cref="SpanConcurrentDictionary{TKey,TValue}"/>.</summary>
        /// <returns>An enumerator for the <see cref="SpanConcurrentDictionary{TKey,TValue}"/>.</returns>
        /// <remarks>
        ...
        private sealed class Enumerator : IEnumerator<KeyValuePair<TKey[], TValue>>
        {
            ...
            private SpanConcurrentDictionary<TKey, TValue>.Node?[]? _buckets;
            private Node? _node;
            private int _i;
            private int _state;
            ...
            public Enumerator(SpanConcurrentDictionary<TKey, TValue> dictionary)
            {
        ...
        /// </summary>
        private bool TryAddInternal(ReadOnlySpan<TKey> key, TKey[]? keyArray, int? nullableHashcode, TValue value, bool updateIfExists, bool acquireLock, out TValue resultingValue)
        {
            ISpanEqualityComparer<TKey> comparer = _comparer;
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    var resultNode = new Node(keyArray ?? key.ToArray(), value, hashcode, bucket);
        ...
        public TValue this[TKey[] key]
        {
            ...
            set
            {
                ...
                TryAddInternal(key, key, null, value, updateIfExists: true, acquireLock: true, out _);
            }
        }
        ...
        public TValue this[ReadOnlySpan<TKey> key]
        {
            ...
            set
            {
                TryAddInternal(key, null, null, value, updateIfExists: true, acquireLock: true, out _);
            }
        }
        ...
        /// <remarks>Separate from ThrowHelper to avoid boxing at call site while reusing this generic instantiation.</remarks>
        [DoesNotReturn]
        private static void ThrowKeyNotFoundException(ReadOnlySpan<TKey> key) =>
        ...
        /// <see cref="SpanConcurrentDictionary{TKey,TValue}" /> and to provide hash values for the keys.
        /// </value>
        /// <remarks>
        /// <see cref="SpanConcurrentDictionary{TKey,TValue}" /> requires an equality implementation to determine
        ...
        /// cref="SpanConcurrentDictionary{TKey,TValue}"/>.
        /// </summary>
        ...
        /// cref="SpanConcurrentDictionary{TKey,TValue}"/>.</value>
        ...
        /// <summary>
        /// Gets the number of key/value pairs contained in the <see
        /// cref="SpanConcurrentDictionary{TKey,TValue}"/>. Should only be used after all locks
        /// have been acquired.
        /// </summary>
        /// <exception cref="OverflowException">The dictionary contains too many
        /// elements.</exception>
        ...
        /// <summary>
        /// Adds a key/value pair to the <see cref="SpanConcurrentDictionary{TKey,TValue}"/>
        ...
        /// key is already in the dictionary, or the new value for the key as returned by valueFactory
        /// if the key was not in the dictionary.</returns>
        public TValue GetOrAdd(ReadOnlySpan<TKey> key, Func<TValue> valueFactory)
        {
            return GetOrAddInternal(key, null, valueFactory());
        }
        ...
        {
            ArgumentNullException.ThrowIfNull(key);
            return GetOrAddInternal(key, key, valueFactory(key, factoryArgument));
        }
        ...
        /// key is already in the dictionary, or the new value for the key as returned by valueFactory
        /// if the key was not in the dictionary.</returns>
        public TValue GetOrAdd<TArg>(ReadOnlySpan<TKey> key, Func<TArg, TValue> valueFactory, TArg factoryArgument)
        {
            return GetOrAddInternal(key, null, valueFactory(factoryArgument));
        }
        ...
        {
            ArgumentNullException.ThrowIfNull(key);
            return GetOrAddInternal(key, key, value);
        }
        ...
        {
            return GetOrAddInternal(key, null, value);
        }

        private TValue GetOrAddInternal(ReadOnlySpan<TKey> key, TKey[]? keyArray, TValue value)
        {
            ISpanEqualityComparer<TKey> comparer = _comparer;
            int hashcode = comparer.GetHashCode(key);
            ...
            {
                TryAddInternal(key, keyArray, hashcode, value, updateIfExists: false, acquireLock: true, out resultingValue);
            }
        ...
        /// <summary>
        /// Adds a key/value pair to the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> if the key does not already
        /// exist, or updates a key/value pair in the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> if the key
        /// already exists.
        /// </summary>
        ...
        public TValue AddOrUpdate<TArg>(TKey[] key, Func<TKey[], TArg, TValue> addValueFactory, Func<TKey[], TValue, TArg, TValue> updateValueFactory, TArg factoryArgument)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    if (TryAddInternal(key, key, hashcode, addValueFactory(key, factoryArgument), updateIfExists: false, acquireLock: true, out TValue resultingValue))
                    {
                        return resultingValue;
                    }
                }
        ...
        public TValue AddOrUpdate(TKey[] key, Func<TKey[], TValue> addValueFactory, Func<TKey[], TValue, TValue> updateValueFactory)
        {
            ...
            while (true)
            {
                ...
                {
                    // key doesn't exist, try to add
                    if (TryAddInternal(key, key, hashcode, addValueFactory(key), updateIfExists: false, acquireLock: true, out TValue resultingValue))
                    {
                        return resultingValue;
                    }
                }
        ...
        public TValue AddOrUpdate(TKey[] key, TValue addValue, Func<TKey[], TValue, TValue> updateValueFactory)
        {
            ...
            while (true)
            {
                ...
                {
                    // key doesn't exist, try to add
                    if (TryAddInternal(key, key, hashcode, addValue, updateIfExists: false, acquireLock: true, out TValue resultingValue))
                    {
                        return resultingValue;
                    }
                }
        ...
        /// <summary>
        /// Gets a value that indicates whether the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> is empty.
        /// </summary>
        /// <value>true if the <see cref="SpanConcurrentDictionary{TKey,TValue}"/> is empty; otherwise,
        /// false.</value>
        public bool IsEmpty
        {
            get
        ...
        /// cref="SpanConcurrentDictionary{TKey,TValue}"/>.</exception>
        ...
        /// made to the dictionary after <see cref="GetEnumerator"/> was called.
        /// </remarks>
        IEnumerator IEnumerable.GetEnumerator() => ((SpanConcurrentDictionary<TKey, TValue>)this).GetEnumerator();
        ...
        /// cref="SpanConcurrentDictionary{TKey,TValue}"/>, this property always
        /// returns false.</value>
        ...
        /// cref="SpanConcurrentDictionary{TKey,TValue}"/>. -or- A value is being
        ...
        object? IDictionary.this[object key]
        {
            ...
            set
            {
                ...
                ((SpanConcurrentDictionary<TKey, TValue>)this)[(TKey[])key] = (TValue)value!;
            }
        }
        ...
        void ICollection.CopyTo(Array array, int index)
        {
            ...
            try
            {
                ...
                throw new ArgumentException("The array is multidimensional, or the type parameter for the set cannot be cast automatically to the type of the destination array.", nameof(array));
            }
            finally
            {
                ReleaseLocks(0, locksAcquired);
        ...
        private sealed class DictionaryEnumerator : IDictionaryEnumerator
        {
            ...
            internal DictionaryEnumerator(SpanConcurrentDictionary<TKey, TValue> dictionary) => _enumerator = dictionary.GetEnumerator();
