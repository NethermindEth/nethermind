    ...
    public static partial class Ssz
    {
        ...
        private static SignedBeaconBlockHeader DecodeSignedBeaconBlockHeader(ReadOnlySpan<byte> span, ref int offset)
        {
            SignedBeaconBlockHeader container =
                DecodeSignedBeaconBlockHeader(span.Slice(offset, SignedBeaconBlockHeaderLength));
            offset += SignedBeaconBlockHeaderLength;
            return container;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void Encode(Span<byte> span, SignedBeaconBlockHeader value, ref int offset)
        {
            Encode(span.Slice(offset, SignedBeaconBlockHeaderLength), value);
            offset += SignedBeaconBlockHeaderLength;
        }
    }
}