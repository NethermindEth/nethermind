...
namespace Nethermind.AccountAbstraction.Source
{
    public class UserOperationPool : IUserOperationPool, IDisposable
    {
        ...
        private readonly Commitment _userOperationEventTopic;
        ...
        private readonly ConcurrentDictionary<long, HashSet<Commitment>> _userOperationsToDelete = new();
            ...
            )
        {
            ...
            _userOperationEventTopic = new Commitment("0x33fd4d1f25a5461bea901784a6571de6debc16cd0831932c22c6969cd73ba994");
        ...
        private void ReAddReorganizedUserOperations(Block? previousBlock)
        {
            if (previousBlock is not null && _removedUserOperations.TryGetValue(previousBlock.Number, out HashSet<UserOperation>? value))
            {
                ...
                {
                    ResultWrapper<Commitment> result = AddUserOperation(op);
        ...
        public ResultWrapper<Commitment> AddUserOperation(UserOperation userOperation)
        {
            ...
            UpdateCurrentBaseFee();
            ResultWrapper<Commitment> result = ValidateUserOperation(userOperation);
            if (result.Result == Result.Success)
            {
                ...
                return ResultWrapper<Commitment>.Fail("failed to insert userOp into pool");
            }
        ...
        public bool RemoveUserOperation(Commitment? userOperationHash)
        {
        ...
        private void RemoveProcessedUserOperations(Block block)
        {
            ...
            if (_userOperationsToDelete.TryGetValue(block.Number, out HashSet<Commitment>? value))
            {
            ...
            foreach (FilterLog log in foundLogs)
            {
                ...
                {
                    Commitment requestId = log.Topics[1];
        ...
        private ResultWrapper<Commitment> ValidateUserOperation(UserOperation userOperation)
        {
            // make sure op not already in pool
            if (_userOperationSortedPool.TryGetValue(userOperation.RequestId!, out _))
                return ResultWrapper<Commitment>.Fail("userOp is already present in the pool");

            if (userOperation.MaxFeePerGas < _currentBaseFee * 70 / 100)
            {
                return ResultWrapper<Commitment>.Fail($"maxFeePerGas must be at least 70% of baseFee to be accepted into pool");
            }
            ...
            switch (paymasterStatus)
            {
                ...
                case PaymasterStatus.Banned: return ResultWrapper<Commitment>.Fail("paymaster banned");
                case PaymasterStatus.Throttled:
                    {
                        ...
                        if (poolUserOperations.Any(poolOp => poolOp.Paymaster == userOperation.Paymaster))
                            return ResultWrapper<Commitment>.Fail(
            ...
            {
                return ResultWrapper<Commitment>.Fail($"the pool already contains the maximum {_accountAbstractionConfig.MaximumUserOperationPerSender} user operations from the {userOperation.Sender} sender");
            }

            if (userOperation.MaxFeePerGas < _accountAbstractionConfig.MinimumGasPrice)
                return ResultWrapper<Commitment>.Fail($"maxFeePerGas below minimum gas price {_accountAbstractionConfig.MinimumGasPrice} wei");

            if (userOperation.CallGas < Transaction.BaseTxGasCost)
                return ResultWrapper<Commitment>.Fail($"callGas too low, must be at least {Transaction.BaseTxGasCost}");
            ...
            if (
                ...
                return ResultWrapper<Commitment>.Fail("sender doesn't exist");
            ...
            if (userOperation.Paymaster != Address.Zero)
            {
                if (!_stateProvider.AccountExists(userOperation.Paymaster)
                    ...
                    return ResultWrapper<Commitment>.Fail("paymaster is used but is not a contract or is banned");
            }

            ResultWrapper<Commitment> successfulSimulation = Simulate(userOperation, _blockTree.Head!.Header);
            ...
            if (paymasterStatus == PaymasterStatus.Throttled && successfulSimulation.Result == Result.Success)
            {
                ...
                _userOperationsToDelete.AddOrUpdate(blockNumberToDelete,
                    k => new HashSet<Commitment>() { userOperation.RequestId! },
                    (k, v) =>
                    {
        ...
        private ResultWrapper<Commitment> Simulate(UserOperation userOperation, BlockHeader parent)
        {
            Metrics.UserOperationsSimulated++;
            ResultWrapper<Commitment> success = _userOperationSimulator.Simulate(
                userOperation,
                parent,
