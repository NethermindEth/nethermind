...
namespace Nethermind.AccountAbstraction
{
    public class AccountAbstractionRpcModule : IAccountAbstractionRpcModule
    {
        ...
        public ResultWrapper<Hash256> eth_sendUserOperation(UserOperationRpc userOperationRpc, Address entryPointAddress)
        {
            if (!_supportedEntryPoints.Contains(entryPointAddress))
            {
                return ResultWrapper<Hash256>.Fail($"entryPoint {entryPointAddress} not supported, supported entryPoints: {string.Join(", ", _supportedEntryPoints.ToList())}");
            }
            ...
            if (!allow)
            {
                return ResultWrapper<Hash256>.Fail("op with same nonce and sender already present in a pool but op fee increase is not large enough to replace it");
            }
