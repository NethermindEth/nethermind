...
namespace Nethermind.Serialization.Rlp
{
    ...
    public class AccountStructDecoder : IRlpStreamDecoder<AccountStruct?>
    {
        private readonly bool _slimFormat = false;

        public int GetLength(AccountStruct? item, RlpBehaviors rlpBehaviors)
        {
            if (item is null)
            {
                return 1;
        ...
        public void Encode(RlpStream rlpStream, AccountStruct? item, RlpBehaviors rlpBehaviors = RlpBehaviors.None)
        {
            if (item is null)
            {
        ...
        public int GetContentLength(AccountStruct? itemN)
        {
            if (itemN is null)
            {
                return 0;
