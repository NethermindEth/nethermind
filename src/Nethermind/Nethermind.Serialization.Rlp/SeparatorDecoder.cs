...
{
    public class SeparatorDecoder : IRlpValueDecoder<Block>, IRlpStreamDecoder<Block>
    {
        private readonly HeaderDecoder _headerDecoder = new();
        private readonly TxDecoder _txDecoder = new();
        private readonly WithdrawalDecoder _withdrawalDecoder = new();

        public (ArraySegment<byte> header, ArraySegment<byte> body) SeparateHeaderAndBody(RlpStream blockRlpStream)
        {
            if (blockRlpStream.Length == 0)
                throw new RlpException($"Received a 0 length stream when decoding a {nameof(Block)}");

            if (blockRlpStream.IsNextItemNull())
            {
                blockRlpStream.ReadByte();
                return (null, null);
            }
            int sequenceLength = blockRlpStream.ReadSequenceLength();
            var headerStart = blockRlpStream.Position;
            int blockCheck = blockRlpStream.Position + sequenceLength;

            var before = blockRlpStream.Position;

            int headerSequenceLength = blockRlpStream.ReadSequenceLength();
            var blockBodyLength = blockCheck - headerStart - headerSequenceLength;
            return (new ArraySegment<byte>(blockRlpStream.Data, headerStart, headerSequenceLength), new ArraySegment<byte>(blockRlpStream.Data, headerSequenceLength + headerStart, blockBodyLength));
        }
