...
namespace Nethermind.Serialization.Rlp
{
    ...
    public class Rlp
    {
        ...
        {
            return GetObjectDecoder<Transaction>() is ISigningHashEncoder<Transaction, TxSignatureHashParams> rlpDecoder ?
                rlpDecoder.Encode(item, new TxSignatureHashParams { ChainId = chainId, IsEip155Enabled = isEip155Enabled }) :
                throw new RlpException($"{nameof(Rlp)} does not support encoding {nameof(Transaction)}");
        }
