...
public sealed class TxFrameSignatureDecoder : RlpDecoder<TxFrameSignature>
{
    ...
    private static RlpLimit SignatureRlpLimit => RlpLimit.For<TxFrameSignature>(
        (int)Math.Min(RlpLimit.MaxBlockGas / GasCostOf.TxDataZero + 1, int.MaxValue),
        nameof(TxFrameSignature.Signature));
