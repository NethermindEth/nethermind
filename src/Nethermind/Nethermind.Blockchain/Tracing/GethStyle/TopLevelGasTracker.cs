...
public struct TopLevelGasTracker(ulong? standardIntrinsicGas)
{
    ...
    /// <summary>Calculates standard intrinsic gas when the transaction lists are valid for the active specification.</summary>
    ...
    /// <returns>
    /// Standard intrinsic gas, or <see langword="null"/> when the transaction or specification is absent or a
    /// transaction list is not enabled by the specification.
    /// </returns>
        ...
        ulong blockGasLimit = 0)
    {
        if (transaction is null || spec is null ||
            transaction.AccessList is not null && !spec.UseTxAccessLists ||
            transaction.AuthorizationList is not null && !spec.IsAuthorizationListEnabled)
        {
            return null;
        }
    ...
    public ulong? GetReceiptFallback(in GasConsumed gasSpent) =>
