    ...
    public abstract partial class Contract
    {
        ...
        protected byte[] CallCore(ITransactionProcessor transactionProcessor, BlockHeader header, string functionName, Transaction transaction, bool callAndRestore = false)
        {
            ...
            try
            {
                if (callAndRestore)
                {
                    transactionProcessor.CallAndRestore(transaction, new BlockExecutionContext(header, forceBlobBaseFee: 0), tracer);
                }
                else
                {
                    transactionProcessor.Execute(transaction, new BlockExecutionContext(header, forceBlobBaseFee: 0), tracer);
                }
