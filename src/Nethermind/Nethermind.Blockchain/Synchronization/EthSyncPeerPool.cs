...
//            _refreshLoopTask = Task.Run(RunRefreshPeerLoop, _refreshLoopCancellation.Token)
        ...
        public int UsefulPeerCount => (_peers.Count(p => p.Value.IsInitialized && p.Value.TotalDifficulty >= (_blockTree.BestSuggested?.TotalDifficulty ?? 0)) - _sleepingPeers.Count);
