...
public interface ISyncConfig : IConfig
{
    ...
    [ConfigItem(Description = "The number of account range partitions to create. Increases the Snap sync request concurrency. Allowed values are between 1 and 256.", DefaultValue = "1")]
    int SnapSyncAccountRangePartitionCount { get; set; }

    [ConfigItem(Description = "_Technical._ Maximum requested Snap account-range response size, in bytes. Caps local account trie commit bursts while preserving the adaptive Snap range request sizer. Values less than or equal to 0 use the protocol maximum.", DefaultValue = "125000", HiddenFromDocs = true)]
    int SnapSyncAccountRangeMaxResponseBytes { get; set; }

    [ConfigItem(Description = "_Technical._ Maximum requested Snap storage-range response size, in bytes. Caps local storage trie commit bursts while preserving the adaptive Snap range request sizer. Values less than or equal to 0 use the protocol maximum.", DefaultValue = "125000", HiddenFromDocs = true)]
    int SnapSyncStorageRangeMaxResponseBytes { get; set; }
    ...
    [ConfigItem(Description = "_Technical._ Maximum active Snap storage-range batches. Lower values reduce single-writer storage backend contention. Values less than or equal to 0 disable this cap.", DefaultValue = "2", HiddenFromDocs = true)]
    ...
    [ConfigItem(Description = "The max number of threads used for syncing. `0` to use the number of logical processors, except snap sync, which uses a bounded higher auto-concurrency to keep storage requests active.", DefaultValue = "0")]
