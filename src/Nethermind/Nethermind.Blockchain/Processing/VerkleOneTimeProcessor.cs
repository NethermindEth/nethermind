...
namespace Nethermind.Blockchain.Processing
{
    public class VerkleOneTimeChainProcessor : IBlockchainProcessor
    {
        ...
        private readonly IVerkleReadOnlyVerkleTrieStore _readOnlyTrieStore;

        private object _lock = new();

        public VerkleOneTimeChainProcessor(IVerkleReadOnlyVerkleTrieStore readOnlyTrieStore, IBlockchainProcessor processor)
        {
        ...
        public Block? Process(Block block, ProcessingOptions options, IBlockTracer tracer)
        {
            lock (_lock)
            {
                ...
                {
                    _readOnlyTrieStore.ClearTempChanges();
                }
