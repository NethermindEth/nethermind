...
/// writes, suggested header and chain-level writes, and block-access-list writes) on a single background consumer so the engine API paths
...
public sealed class DeferredBlockDataWriter : IDeferredBlockDataWriter
{
    ...
    /// counts individual writes, not blocks: a BAL-enabled block can enqueue up to seven items, although
