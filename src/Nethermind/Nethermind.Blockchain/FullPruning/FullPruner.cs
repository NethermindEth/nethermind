...
namespace Nethermind.Blockchain.FullPruning
{
    ...
    public class FullPruner : IDisposable
    {
        ...
        private bool HaveEnoughDiskSpaceToRun()
        {
            ...
            if (available < currentChainSize * ChainSizeThresholdFactor / 100)
            {
                if (_logger.IsWarn)
                    _logger.Warn(
                        $"Not enough disk space to run full pruning. Required {(currentChainSize * ChainSizeThresholdFactor / 100) / 1.GB()} GB. Have {available / 1.GB()} GB");
                return false;
            }
            return true;
        }
