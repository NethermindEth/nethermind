...
public interface IBlocksConfig : IConfig
{
    ...
    [ConfigItem(Description = "Specify pre-warm state concurrency. Default is the logical processors of the larger kind of core the node may run on - 1 (the performance cores on an unpinned hybrid CPU), which on a CPU without efficiency cores is all logical processors - 1.", DefaultValue = "0", HiddenFromDocs = true)]
