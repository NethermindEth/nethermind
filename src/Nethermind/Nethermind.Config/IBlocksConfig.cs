...
public interface IBlocksConfig : IConfig
{
    ...
    [ConfigItem(Description = "On a supported hybrid CPU, pin block processing to: `All` logical processors, both hyperthreads of every `Performance` core, one hyperthread of every performance core (`PerformancePhysical`), or the fastest performance core alone, kept clear of prewarm (`Fastest`). No effect on other CPUs. Background workers prefer efficiency cores independently of DedicatedProcessingThread; All disables their affinity too.", DefaultValue = "Performance", HiddenFromDocs = true)]
