...
public class ResourcePoolTests
{
    ...
    {
        var content = _resourcePool.GetSnapshotContent(ResourcePool.Usage.MainBlockProcessing);
    ...
    public void Test_SnapshotContentPool_RespectsCapacity()
    {
        ...
        var items = new List<SnapshotContent>();
        ...
        {
            var content = _resourcePool.GetSnapshotContent(usage);
        ...
        var newContent = _resourcePool.GetSnapshotContent(usage);
    ...
    {
        var resource = _resourcePool.GetCachedResource(ResourcePool.Usage.MainBlockProcessing);
    ...
    public void Test_CachedResourcePool_RespectsCapacity()
    {
        ...
        var p1 = _resourcePool.GetCachedResource(usage);
        var p2 = _resourcePool.GetCachedResource(usage);
        var p3 = _resourcePool.GetCachedResource(usage);
    ...
    {
        var from = new StateId(1, Keccak.Zero);
        var to = new StateId(2, Keccak.Zero);
        var usage = ResourcePool.Usage.MainBlockProcessing;

        SnapshotContent content;
        using (var snapshot = _resourcePool.CreateSnapshot(from, to, usage))
        {
        ...
        var recycledContent = _resourcePool.GetSnapshotContent(usage);
    ...
    {
        var contentMain = _resourcePool.GetSnapshotContent(ResourcePool.Usage.MainBlockProcessing);
        var contentCompactor = _resourcePool.GetSnapshotContent(ResourcePool.Usage.Compactor);

        _resourcePool.ReturnSnapshotContent(ResourcePool.Usage.MainBlockProcessing, contentMain);

        var contentCompactor2 = _resourcePool.GetSnapshotContent(ResourcePool.Usage.Compactor);
        Assert.That(contentCompactor2, Is.Not.SameAs(contentMain));

        var contentMain2 = _resourcePool.GetSnapshotContent(ResourcePool.Usage.MainBlockProcessing);
