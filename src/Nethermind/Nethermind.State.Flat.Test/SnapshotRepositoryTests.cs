...
public class SnapshotRepositoryTests
{
    ...
    {
        var from = CreateStateId(fromBlock);
        var to = CreateStateId(toBlock);
        var snapshot = CreateSnapshot(from, to, withData);
    ...
    public void TryAddSnapshot_NewAndDuplicate_BehavesCorrectly()
    {
        ...
        var snapshot1 = CreateSnapshot(from, to);
        var snapshot2 = CreateSnapshot(from, to);
    ...
    public void AddAndRemoveSnapshot_CannotLeaseAfterRemoval()
    {
        StateId from = CreateStateId(0);
        StateId to = CreateStateId(1);
        var snapshot = CreateSnapshot(from, to);
        _repository.AddStateId(to);

        _repository.TryAddSnapshot(snapshot);
        bool leasedBefore = _repository.TryLeaseState(to, out var leasedSnapshot);
    ...
    public void RemoveSnapshot_WithActiveLeases_DisposesWhenAllReleased()
    {
        ...
        bool leased1 = _repository.TryLeaseState(to, out var snapshot1);
        bool leased2 = _repository.TryLeaseState(to, out var snapshot2);
    ...
    public void TryLeaseState_ExistingAndNonExistent()
    {
        ...
        bool leasedExisting = _repository.TryLeaseState(existing, out var snapshot);
        Assert.That(leasedExisting, Is.True);
        Assert.That(snapshot, Is.Not.Null);
        snapshot!.Dispose();

        StateId nonExistent = CreateStateId(999);
        bool leasedNonExistent = _repository.TryLeaseState(nonExistent, out var nonExistentSnapshot);
    ...
    public void TryLeaseState_MultipleLeases_AllSucceed()
    {
        ...
        bool leased1 = _repository.TryLeaseState(to, out var snapshot1);
        bool leased2 = _repository.TryLeaseState(to, out var snapshot2);
        bool leased3 = _repository.TryLeaseState(to, out var snapshot3);
    ...
    public void TryLeaseCompactedState_ExistingAndNonExistent()
    {
        ...
        StateId existing = CreateStateId(1);
        bool leasedExisting = _repository.TryLeaseCompactedState(existing, out var snapshot);
        Assert.That(leasedExisting, Is.True);
        Assert.That(snapshot, Is.Not.Null);
        snapshot!.Dispose();

        StateId nonExistent = CreateStateId(999);
        bool leasedNonExistent = _repository.TryLeaseCompactedState(nonExistent, out var nonExistentSnapshot);
        Assert.That(leasedNonExistent, Is.False);
        Assert.That(nonExistentSnapshot, Is.Null);
    }
    ...
    public void TryLeaseCompactedState_MultipleLeases_AllSucceed()
    {
        ...
        StateId to = CreateStateId(1);
        bool leased1 = _repository.TryLeaseCompactedState(to, out var snapshot1);
        bool leased2 = _repository.TryLeaseCompactedState(to, out var snapshot2);
    ...
    public void GetSnapshotBeforeStateId_EmptyRepository()
    {
        ...
        var states = _repository.GetSnapshotBeforeStateId(target);
    ...
    public void AssembleSnapshotsUntil_EmptyRepository()
    {
        ...
        using var assembled = _repository.AssembleSnapshotsUntil(target, 0, 10);
    ...
    public void AssembleSnapshotsUntil_StopsAtStartingBlock()
    {
        ...
        StateId target = CreateStateId(4);
        using var assembled = _repository.AssembleSnapshotsUntil(target, 2, 10);
    ...
    public void AssembleSnapshotsUntil_PrefersCompacted()
    {
        ...
        var compacted = CreateSnapshot(from, to);
        _repository.TryAddCompactedSnapshot(compacted);

        using var assembled = _repository.AssembleSnapshotsUntil(to, 0, 10);
