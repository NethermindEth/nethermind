...
public class LongFinalityIntegrationTests
{
    ...
    public void MergeSnapshotData_AllEntryTypes()
    {
        ...
        Assert.That(mergedSnap.TryGetAccount(TestItem.AddressA.ToAccountPath, out _), Is.True);
        Assert.That(mergedSnap.TryGetAccount(TestItem.AddressB.ToAccountPath, out _), Is.True);
    }
    ...
    public void EmptySnapshot_PersistsAndLoads()
    {
        ...
        Assert.That(repo.TryLeaseSnapshotTo(s1, out PersistedSnapshot? persisted), Is.True);
        Assert.That(persisted!.TryGetAccount(TestItem.AddressA.ToAccountPath, out _), Is.False);
