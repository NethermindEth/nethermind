...
public class TrieNodeCacheTests
{
    ...
    public void SetUp()
    {
        _config = new FlatDbConfig { TrieCacheMemoryTarget = 1024 * 1024 };
    ...
    {
        var config = new FlatDbConfig { TrieCacheMemoryTarget = 0 };
    ...
    {
        var config = new FlatDbConfig { TrieCacheMemoryTarget = 1 };
        Assert.DoesNotThrow(() => new TrieNodeCache(config, LimboLogs.Instance));
    }
    ...
    {
        var zeroConfig = new FlatDbConfig { TrieCacheMemoryTarget = 0 };
        var zeroCache = new TrieNodeCache(zeroConfig, LimboLogs.Instance);
        var zeroResourcePool = new ResourcePool(zeroConfig);

        TreePath path = TreePath.FromHexString("abcd");
        Hash256 hash = Keccak.Compute([1, 2, 3]);
        TrieNode trieNode = new TrieNode(NodeType.Leaf, hash);

        var transientResource = zeroResourcePool.GetCachedResource(ResourcePool.Usage.MainBlockProcessing);
        transientResource.Nodes.Set(null, in path, trieNode);
    ...
    public void Add_OverwritesExistingNode_OnCollision()
    {
        ...
        var transientResource1 = _resourcePool.GetCachedResource(ResourcePool.Usage.MainBlockProcessing);
        transientResource1.Nodes.Set(null, in path, new TrieNode(NodeType.Leaf, hash1));

        _cache.Add(transientResource1);

        var transientResource2 = _resourcePool.GetCachedResource(ResourcePool.Usage.MainBlockProcessing);
...
public class ChildCacheTests
{
    ...
    {
        var smallCache = new TrieNodeCache.ChildCache(16);
    ...
    [Test]
    public void DifferentAddresses_StoreSeparateNodes()
    {
        Hash256 address1 = Keccak.Compute([0x01]);
        Hash256 address2 = Keccak.Compute([0x02]);
        TreePath path = TreePath.FromHexString("abcd");
        Hash256 hash1 = Keccak.Compute([1, 2, 3]);
        Hash256 hash2 = Keccak.Compute([4, 5, 6]);
        TrieNode node1 = new TrieNode(NodeType.Leaf, hash1);
        TrieNode node2 = new TrieNode(NodeType.Branch, hash2);

        _cache.Set(address1, in path, node1);
        _cache.Set(address2, in path, node2);

        bool found1 = _cache.TryGet(address1, in path, hash1, out TrieNode? retrieved1);
        bool found2 = _cache.TryGet(address2, in path, hash2, out TrieNode? retrieved2);

        Assert.That(found1, Is.True);
        Assert.That(found2, Is.True);
        Assert.That(retrieved1, Is.SameAs(node1));
        Assert.That(retrieved2, Is.SameAs(node2));
    }
