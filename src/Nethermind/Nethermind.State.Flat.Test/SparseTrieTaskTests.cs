...
public class SparseTrieTaskTests
{
    ...
    public async Task SnapshotStorageProofs_AreBoundedAndFinishDrainsWhileOwnerRemainsResponsive()
    {
        ...
        Assert.That(
            ...
            Is.True,
            "The owner did not process account work while storage proof reads were blocked.");
    ...
    public async Task SnapshotStorageProofs_SerializeSameAccountAndQueuedFinishWins()
    {
        ...
        Assert.That(
            ...
            Is.False,
            "Two proof jobs read the same storage trie concurrently.");
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(result.StateRoot, Is.EqualTo(state.StateRoot));
            Assert.That(reader.MaxActiveReadsPerAccount, Is.EqualTo(1));
            Assert.That(block.StorageProofReadsStarted, Is.EqualTo(1));
        }
    }

    [Test]
    public async Task SnapshotStorageProofs_ChangedStorageMatchesPatricia()
    {
        MultiContractTestState state = BuildMultiContractState(contractCount: 1, slotsPerContract: 4);
        MultiContractState contract = state.Contracts[0];
        UInt256 slot = UInt256.Zero;
        byte[] finalValue = [0x55];
        using BlockingSnapshotPersistenceReader reader = new(state.NodeStorage);
        using SnapshotBundle bundle = CreateSnapshotBundle(reader);
        await using SparseTrieWorker worker = CreateWorker(storageProofWorkerCount: 2);
        ...
        EnqueueStorageTouch(block, contract, slot: 0);
        Assert.That(
            SpinWait.SpinUntil(() => block.StorageProofReadsStarted == 1, TimeSpan.FromSeconds(5)),
            Is.True);
        SparseTrieBlockResult result = await block.FinishAsync(new SparseTrieFinalState(
            [new SparseTrieFinalStorageBatch(
                contract.Address,
            ...
            [new SparseTrieFinalAccount(contract.Address, contract.Account)]));
        await block.PrepareCommitAsync();
        await block.AcceptAsync();
        ...
        Hash256 expectedStateRoot = ApplyAccount(
            state.StateTree,
            contract.Address,
            contract.Account.WithChangedStorageRoot(expectedStorageRoot));

        using (Assert.EnterMultipleScope())
        {
            Assert.That(result.StorageRoots[contract.Address], Is.EqualTo(expectedStorageRoot));
            Assert.That(result.StateRoot, Is.EqualTo(expectedStateRoot));
            Assert.That(block.StorageProofReadsStarted, Is.EqualTo(1));
        }
    }
    ...
    [Test]
    public async Task SnapshotStorageProofs_StopDrainsBeforeRejectedTerminalCommandsComplete()
    {
        MultiContractTestState state = BuildMultiContractState(contractCount: 1, slotsPerContract: 4);
        using BlockingSnapshotPersistenceReader reader = new(state.NodeStorage);
        using SnapshotBundle bundle = CreateSnapshotBundle(reader);
        reader.BlockStorageReads();
        SparseTrieWorker worker = CreateWorker(storageProofWorkerCount: 2);
        SparseTrieBlockHandle block = worker.BeginBlock(state.StateRoot, bundle);

        EnqueueStorageTouch(block, state.Contracts[0], slot: 0);
        Assert.That(reader.WaitForActiveStorageReads(1, TimeSpan.FromSeconds(5)), Is.True);

        Task stopTask = worker.DisposeAsync().AsTask();
        Task<SparseTrieBlockResult> finishTask = block.FinishAsync(new SparseTrieFinalState([], []));
        Task abortTask = block.AbortAsync();
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(finishTask.IsCompleted, Is.False);
            Assert.That(abortTask.IsCompleted, Is.False);
        }

        reader.ReleaseStorageReads();
        await stopTask;
        Assert.ThrowsAsync<ObjectDisposedException>(async () => await finishTask);
        Assert.ThrowsAsync<ObjectDisposedException>(async () => await abortTask);
        bundle.Dispose();
        Assert.That(reader.DisposedDuringActiveRead, Is.False);
    }
    ...
    [Test]
    public async Task SnapshotStorageProofFailure_DoesNotCompleteFinishBeforeOtherProofsDrain()
    {
        MultiContractTestState state = BuildMultiContractState(contractCount: 2, slotsPerContract: 4);
        using BlockingSnapshotPersistenceReader reader = new(
            state.NodeStorage,
        ...
        EnqueueStorageTouch(block, state.Contracts[1], slot: 0);
        Assert.That(
            SpinWait.SpinUntil(() => reader.StorageReads >= 2, TimeSpan.FromSeconds(5)),
            Is.True);

        Task<SparseTrieBlockResult> finishTask = block.FinishAsync(new SparseTrieFinalState([], []));
        Task first = await Task.WhenAny(finishTask, Task.Delay(TimeSpan.FromMilliseconds(100)));
        Assert.That(first, Is.Not.SameAs(finishTask), "Proof failure bypassed the in-flight proof drain.");

        reader.ReleaseStorageReads();
        Func<Task> finish = async () => await finishTask;
        Assert.ThrowsAsync<InvalidOperationException>(finish);
    ...
    private sealed class BlockingSnapshotPersistenceReader(
        RecordingNodeStorage nodeStorage,
        bool failStorageReads = false,
        bool failFirstStorageRead = false) : IPersistence.IPersistenceReader
    {
        ...
        {
            int readNumber = Interlocked.Increment(ref _storageReads);
            ...
            try
            {
                if (failStorageReads || failFirstStorageRead && readNumber == 1)
