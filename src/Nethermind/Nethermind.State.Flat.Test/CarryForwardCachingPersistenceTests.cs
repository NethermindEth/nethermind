...
public class CarryForwardCachingPersistenceTests
{
    ...
    [TestCaseSource(nameof(CacheKinds))]
    public async Task OnCommitted_PublishesCacheCount(CacheKind kind)
    {
        FakePersistence inner = new();
        await using IContainer container = CreateCacheContainer();
        CarryForwardCachingPersistence cache = ResolveCache(container, inner);
    ...
    public void AccountRefresh_AtCapacityKeepsResidentUntilColdFillWipes()
    {
        ...
        try
        {
            ...
            long slotWipesBefore = Metrics.CarryForwardSlotWipes;
            ReadAccount(cache, Address);
            ...
            long slotWipesDelta = Metrics.CarryForwardSlotWipes - slotWipesBefore;
            ...
            using (Assert.EnterMultipleScope())
            {
                ...
                Assert.That(slotWipesDelta, Is.Zero, "an account wipe is not reported as a slot wipe");
