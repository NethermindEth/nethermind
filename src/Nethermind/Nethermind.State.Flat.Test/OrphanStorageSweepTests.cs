...
public class OrphanStorageSweepTests
{
    ...
    private IPersistenceManager ManagerLikeTheReal(Action? beforeTheBatch = null, Action? afterTheBatch = null, Func<bool>? stateSyncWriting = null, bool announcesSync = false)
    {
        IPersistenceManager manager = Substitute.For<IPersistenceManager>();
        manager.StateSyncWriting.Returns(announcesSync);
    ...
    public void Slots_under_an_absent_or_storage_less_account_are_deleted_and_everything_else_stays()
    {
        using (IPersistence.IWriteBatch batch = _persistence.CreateWriteBatch(StateId.Sync, StateId.Sync))
        {
            ...
            batch.SetStorage(WithStorage, 1, BaseFlatPersistence.DecodeSlotValue([0x0A]));
            batch.SetStorageTrieNode(WithStorage.ToAccountPath.ToCommitment(), TreePath.Empty, Node);

            batch.SetStorage(Absent, 1, BaseFlatPersistence.DecodeSlotValue([0x0B]));

            batch.SetAccount(EmptyRoot, new Account(0, 0));
            batch.SetStorage(EmptyRoot, 1, BaseFlatPersistence.DecodeSlotValue([0x0C]));
            batch.SetStorage(EmptyRoot, 2, BaseFlatPersistence.DecodeSlotValue([0x0D]));
    ...
    [Test]
    public void A_writer_that_never_announced_itself_and_cleared_the_base_is_still_caught_inside_the_slice()
    {
        ValueHash256 slot = TestItem.KeccakB.ValueHash256;
        ValueHash256 orphan = PathWithPrefix(0x10000000, tail: 0x01);
        using (IPersistence.IWriteBatch batch = _persistence.CreateWriteBatch(StateId.Sync, StateId.Sync))
        ...
        Assert.That(() => sweep.RunToCompletion(repair: true, CancellationToken.None), Throws.InstanceOf<OperationCanceledException>(),
            "not every rebuilder announces itself; the slice re-reads the pointer and refuses to judge a base without one");
    ...
    [Test]
    public void A_check_running_while_a_state_sync_that_keeps_the_pointer_writes_is_inconclusive()
    {
        ValueHash256 slot = TestItem.KeccakB.ValueHash256;
        using (IPersistence.IWriteBatch batch = _persistence.CreateWriteBatch(StateId.Sync, StateId.Sync))
        {
        ...
        using OrphanStorageSweep sweep = new(_db, ManagerLikeTheReal(announcesSync: true), new SweepPacer(), LimboLogs.Instance);

        sweep.RunToCompletion(repair: false, CancellationToken.None);

        Assert.That(sweep.CheckInconclusive, Is.True, "a tree sync leaves the pointer in place while it rewrites accounts, so the check must ask the manager, not only the pointer");
    }
    ...
    public void An_account_that_gained_storage_between_the_scan_and_the_delete_keeps_its_slots()
    {
        ...
        using (IPersistence.IWriteBatch batch = _persistence.CreateWriteBatch(StateId.Sync, StateId.Sync))
        {
            batch.SetStorage(revived, 1, BaseFlatPersistence.DecodeSlotValue([0x0E]));
        }
