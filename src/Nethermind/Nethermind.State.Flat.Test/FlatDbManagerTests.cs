...
public class FlatDbManagerTests
{
    ...
    public async Task HasStateForBlock_FoundInRepository_ReturnsTrue()
    {
        ...
        var result = manager.HasStateForBlock(stateId);
    ...
    [Test]
    public async Task CreateReader_DelegatesToPersistenceManager()
    {
        var mockReader = Substitute.For<IPersistence.IPersistenceReader>();
        _persistenceManager.LeaseReader().Returns(mockReader);

        await using var manager = CreateManager();
        var reader = manager.CreateReader();

        Assert.That(reader, Is.SameAs(mockReader));
    ...
    {
        var persistedStateId = CreateStateId(100);
        _persistenceManager.GetCurrentPersistedStateId().Returns(persistedStateId);

        var realResourcePool = new ResourcePool(_config);
        var snapshotFrom = CreateStateId(50);
        var snapshotTo = CreateStateId(51);
        var snapshot = realResourcePool.CreateSnapshot(snapshotFrom, snapshotTo, ResourcePool.Usage.MainBlockProcessing);
        var transientResource = realResourcePool.GetCachedResource(ResourcePool.Usage.MainBlockProcessing);
    ...
    {
        var persistedStateId = CreateStateId(5);
        _persistenceManager.GetCurrentPersistedStateId().Returns(persistedStateId);
        _snapshotRepository.TryAddSnapshot(Arg.Any<Snapshot>()).Returns(true);

        var realResourcePool = new ResourcePool(_config);
        var snapshotFrom = CreateStateId(10);
        var snapshotTo = CreateStateId(11);
        var snapshot = realResourcePool.CreateSnapshot(snapshotFrom, snapshotTo, ResourcePool.Usage.MainBlockProcessing);
        var transientResource = realResourcePool.GetCachedResource(ResourcePool.Usage.MainBlockProcessing);
