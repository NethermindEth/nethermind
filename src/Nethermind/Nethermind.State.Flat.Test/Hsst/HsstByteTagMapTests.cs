...
public class HsstByteTagMapTests
{
    ...
    public void RoundTrip_HitsMissesAndIteration(int n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            // Bounded so that even at n=256 the cumulative values total stays under u16
            // (the format's hard ceiling). With (i+1) max=256 and 256 entries:
            // sum ≤ 256·257/2 ≈ 33 K, comfortably below 65 535.
            int len = (i % 5 == 0) ? 0 : (i + 1);
            vals[i] = new byte[len];
    ...
    public void TrailerLayout_MatchesSpec_3EntryFixture()
    {
        ...
        // Layout: [Value_0=2][Value_1=0][Value_2=1][Ends: 3·u16 LE][Tags: 3][Count:1][IndexType:1].
        // Ends: [2, 2, 3] (cumulative end offsets from byte 0 of HSST). Count stores N-1 = 2.
        Assert.That(data.Length, Is.EqualTo(2 + 0 + 1 + 6 + 3 + 1 + 1));
        Assert.That(data[^1], Is.EqualTo((byte)IndexType.ByteTagMap));
        Assert.That(data[^2], Is.EqualTo((byte)2)); // Count = N - 1
        // Tags adjacent to count.
        Assert.That(data[^5..^2], Is.EqualTo(new byte[] { 0x01, 0x02, 0x03 }));
        // Ends right before tags: 3 u16 LE values.
        ReadOnlySpan<byte> endsSpan = data.AsSpan(data.Length - 5 - 6, 6);
        Assert.That(BinaryPrimitives.ReadUInt16LittleEndian(endsSpan[0..]), Is.EqualTo(2));
        Assert.That(BinaryPrimitives.ReadUInt16LittleEndian(endsSpan[2..]), Is.EqualTo(2));
        Assert.That(BinaryPrimitives.ReadUInt16LittleEndian(endsSpan[4..]), Is.EqualTo(3));
        // Values up front.
    ...
    [Test]
    public void Build_RejectsValuesRegionExceedingU16()
    {
        // ByteTagMap end offsets are fixed u16; valuesTotal > 65535 must throw at Build time.
        bool threw = false;
        using (PooledByteBufferWriter p = new(256 * 1024))
        {
            using HsstByteTagMapBuilder<PooledByteBufferWriter.Writer> b = new(ref p.GetWriter());
            // 4 × 20 000 = 80 000 > ushort.MaxValue (65 535).
        ...
        Assert.That(threw, Is.True, "valuesTotal > u16 must throw at Build");
    }
}
