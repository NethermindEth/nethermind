...
public class TrieReassemblerTests
{
    ...
    public void Reassembles_state_root_from_single_account()
    {
        ...
        Hash256? reassembledRoot = _reassembler.TryReassemble([], CancellationToken.None);
    ...
    public void Reassembles_storage_root_from_single_slot()
    {
        ...
        Hash256? reassembledRoot = _reassembler.TryReassemble(new[] { TestItem.KeccakA }, CancellationToken.None);
        Assert.That(reassembledRoot, Is.EqualTo(stateTreeRoot));
    }
    ...
    public void Reassembles_storage_root_for_updated_account()
    {
        ...
        Hash256? reassembledRoot = _reassembler.TryReassemble([TestItem.KeccakA, TestItem.KeccakC], CancellationToken.None);
    ...
    [Test]
    public void Reuses_persisted_storage_root_when_storage_nodes_are_intact()
    {
        Hash256 storageRoot = BuildStorageTree(TestItem.KeccakA, new[]
        {
            (TestItem.KeccakB, new byte[] { 0x01, 0x02, 0x03 }),
            (TestItem.KeccakC, new byte[] { 0x04, 0x05, 0x06 }),
        });
        Hash256 expectedStateRoot = BuildStateTree(new[]
        {
            (TestItem.KeccakA, new Account(0, 100, storageRoot, Keccak.OfAnEmptyString)),
            (TestItem.KeccakD, new Account(0, 200, Keccak.EmptyTreeHash, Keccak.OfAnEmptyString)),
        });

        // Storage nodes survive: the shape left by an account refreshed through ProgressTracker,
        // whose storage trie was re-downloaded in full while the state trie above it went stale.
        DeleteStateNodes();
    ...
    public void Returns_no_root_when_existing_trie_node_is_corrupt()
    {
        ...
        Hash256? result = _reassembler.TryReassemble([TestItem.KeccakA, TestItem.KeccakB], CancellationToken.None);
    ...
    {
        Hash256? result = _reassembler.TryReassemble([], CancellationToken.None);
        Assert.That(result, Is.Null);
    }

    [Test]
    public void Throws_when_cancelled()
    {
        BuildStateTree(new[]
        {
            (TestItem.KeccakA, new Account(0, 100, Keccak.EmptyTreeHash, Keccak.OfAnEmptyString)),
        ...
        Assert.That(() => _reassembler.TryReassemble([], cts.Token), Throws.InstanceOf<OperationCanceledException>());
    }
    ...
    private Hash256 BuildStorageTree(Hash256 addressHash, IEnumerable<(Hash256 path, byte[] value)> slots)
    {
        ...
        foreach ((Hash256 path, byte[] value) in slots)
        {
            ...
            bulkSetEntries.Add(new BulkSetEntry(path, rlpValue));
            writeBatch.SetStorageRawEncoded(addressHash, path, rlpValue);
        }
