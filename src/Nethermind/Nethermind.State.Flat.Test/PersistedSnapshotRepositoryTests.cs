...
public class PersistedSnapshotRepositoryTests
{
    ...
    public void ConvertSnapshot_RoundTrip_AllDataCategories()
    {
        ...
        Assert.That(readSlot.Value.ToBigEndian(), Is.EqualTo(slotBytes));
    ...
    public void ManyBaseSnapshots_ShareUnderlyingFiles([Values(100, 1000)] int count)
    {
        ...
        int blobFileCount = Directory.GetFiles(Path.Combine(tier.BaseDbPath, "persistedSnapshot", "blob"), "blob_*.bin").Length;
    ...
    public void Stack_RealBloom_AdmitsPresentSkipsAbsentAddresses()
    {
        ...
        Assert.That(sv!.Value.Value.ToBigEndian().AsSpan()[^1], Is.EqualTo((byte)0x55));
