...
public class PersistedSnapshotCompactorTests
{
    ...
    public void Compact_PartitionedColumns_MergeBuiltBloom_HasNoFalseNegatives()
    {
        ...
        try
        {
            ...
            // Addresses 1..32 are SHARED across every snapshot ⇒ matchCount == n. Addresses >32
            // are unique to one snapshot ⇒ matchCount == 1. Both go through MergeValues (a single
            // matching source is the degenerate case of the same merge) and must end up in the
            // merge-built bloom — the symptom account (repeatedly updated, high nonce) is a
            // multi-source entry, which the previous disjoint-address version never exercised.
            const int shared = 32;
            ...
            for (int s = 1; s <= n; s++)
            {
                ...
                for (int k = 0; k < uniquePerSnap; k++) Put(shared + (s - 1) * uniquePerSnap + k + 1); // unique ⇒ single-source merge
                repo.ConvertSnapshotToPersistedSnapshot(new Snapshot(prev, next, c, _pool, ResourcePool.Usage.MainBlockProcessing)).Dispose();
                prev = next;
            }
