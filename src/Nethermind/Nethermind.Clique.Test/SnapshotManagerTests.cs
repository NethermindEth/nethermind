...
namespace Nethermind.Clique.Test
{
    ...
    public class SnapshotManagerTests
    {
        ...
        public void Recognises_signer_turn()
        {
            ...
            Block genesis = CliqueTests.GetGenesis();
            Snapshot snapshot = snapshotManager.GetOrCreateSnapshot(0, genesis.Hash);
            const ulong chainId = 0x85; // random chain id, should be independent of test.
            SnapshotManager manager = new(CliqueConfig.Default, _snapshotDb, _blockTree, new EthereumEcdsa(chainId, LimboLogs.Instance), LimboLogs.Instance);
            // Block 1
