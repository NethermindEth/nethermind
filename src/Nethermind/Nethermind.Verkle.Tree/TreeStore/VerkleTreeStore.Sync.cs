...
public partial class VerkleTreeStore<TPersistence>
{
    ...
    public List<PathWithSubTree>? GetLeafRangeIterator(byte[] fromRange, byte[] toRange, Hash256 stateRoot, long bytes)
    {
        ...
        Dictionary<byte[], List<LeafInSubTree>> rangesToReturn = new(Bytes.EqualityComparer);
        var rangesToReturnSpan = rangesToReturn.GetAlternateLookup<ReadOnlySpan<byte>>();

        if (!ranges.MoveNext()) return null;

        // handle the first element
        Span<byte> stem = ranges.Current.Key.AsSpan()[..31];
        rangesToReturnSpan.TryAdd(stem, new List<LeafInSubTree>());
        rangesToReturnSpan[stem].Add(new LeafInSubTree(ranges.Current.Key[31], ranges.Current.Value!));
        currentBytes += 64;
        ...
        if (bytesConsumed)
            ...
            while (ranges.MoveNext())
            {
                ...
                if (rangesToReturnSpan.TryGetValue(stem, out List<LeafInSubTree>? listOfLeafs))
                {
