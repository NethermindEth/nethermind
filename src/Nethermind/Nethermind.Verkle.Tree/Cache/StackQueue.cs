...
public class StackQueue<T>
{
    ...
    {
        if (Count == _array.Length) throw new ArgumentException($"QueueIsFull: {Count}");
    ...
    public bool Contains(T item)
    {
        ...
        if (_head < _tail) return Array.IndexOf(_array, item, _head, Count) >= 0;
    ...
    public void CopyTo(T[] array, int arrayIndex)
    {
        ...
        if (array.Length - arrayIndex < Count) throw new ArgumentException("InvalidOffLen");
    ...
    private void CopyTo(Array array, int index)
    {
        ...
        if (array.Rank != 1) throw new ArgumentException("RankMultiDimNotSupported", nameof(array));

        if (array.GetLowerBound(0) != 0) throw new ArgumentException("NonZeroLowerBound", nameof(array));

        var arrayLen = array.Length;
        if (index < 0 || index > arrayLen)
            throw new ArgumentOutOfRangeException(nameof(index), index, "IndexMustBeLessOrEqual");

        if (arrayLen - index < Count) throw new ArgumentException("InvalidOffLen");
    ...
    {
        private readonly int Started { get; }
        private readonly int Ended { get; }
