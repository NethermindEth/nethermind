...
public class BlockBranchCache
{
    private readonly Dictionary<byte[], BlockBranchNode> _stateRootToNodeMapping;
    private readonly Dictionary<byte[], BlockBranchNode>.AlternateLookup<ReadOnlySpan<byte>> _stateRootToNodeMappingSpans;
    private readonly int _cacheSize;
    public BlockBranchCache(int cacheSize)
    {
        _cacheSize = cacheSize;
        _stateRootToNodeMapping = new(Bytes.EqualityComparer);
        _stateRootToNodeMappingSpans = _stateRootToNodeMapping.GetAlternateLookup<ReadOnlySpan<byte>>();
    }
    ...
    public void InitCache(long blockNumber, Hash256 stateRoot)
    {
        ...
        _stateRootToNodeMappingSpans[stateRoot.Bytes] = _lastNode = node;
        _version++;
    }
        ...
        [MaybeNullWhen(false)] out BlockBranchNode outNode)
    {
        ...
        BlockBranchNode? parentNode = _stateRootToNodeMappingSpans[parentRoot.Bytes];
        node.ParentNode = parentNode;
        outNode = _stateRootToNodeMappingSpans[stateRoot.Bytes] = node;
        _version++;
        return true;
    }
        ...
        Hash256 parentRoot, [MaybeNullWhen(false)] out StateInfo node)
    {
        ...
        if (_lastNode is not null && blockNumber - _lastNode.Data.BlockNumber > _cacheSize)
        {
            ...
            _stateRootToNodeMappingSpans.Remove(currentNode.ParentNode!.Data.StateRoot.Bytes);
    ...
    public bool GetStateRootNode(Hash256 stateRoot, [MaybeNullWhen(false)] out BlockBranchNode node)
    {
        ...
        return _stateRootToNodeMappingSpans.TryGetValue(stateRoot.Bytes, out node);
    }
