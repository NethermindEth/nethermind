...
public partial class VerkleTree
{
    ...
    public static bool VerifyVerkleProofSer(
        ...
        [NotNullWhen(true)] out UpdateHint? updateHint)
    {
        ...
        var isTrue = VerifyVerkleProofStructSer(new VerkleProofStructSerialized(ipaProof, d), allPathsAndZs, leafValuesByPathAndZ,
            commByPath);
        updateHint = new UpdateHint
        {
            DepthAndExtByStem = depthsAndExtByStem,
    ...
    public static bool VerifyVerkleProofSer(VerkleProofSerialized proof, List<byte[]> keys, List<byte[]?> values, Banderwagon root,
        [NotNullWhen(true)] out UpdateHint? updateHint)
    {
        return VerifyVerkleProofSer(proof.Proof.IpaProofSerialized, proof.Proof.D, proof.CommsSorted, proof.VerifyHint.Depths,
            proof.VerifyHint.ExtensionPresent, proof.VerifyHint.DifferentStemNoProof.Select(x => new Stem(x)).ToArray(),
            keys, values, root,
            out updateHint);
    }
    ...
    private static bool VerifyVerkleProofStructSer(VerkleProofStructSerialized proof, SortedSet<(byte[], byte)> allPathsAndZs,
        Dictionary<(byte[], byte), FrE> leafValuesByPathAndZ, SpanDictionary<byte, Banderwagon> commByPath)
    {
        var comms = new Banderwagon[allPathsAndZs.Count];
        var index = 0;
