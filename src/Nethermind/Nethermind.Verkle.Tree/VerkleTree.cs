...
public partial class VerkleTree : IVerkleTree
{
    ...
    public VerkleTree(IVerkleTreeStore verkleStateStore, ILogManager logManager)
    {
        ...
        _leafUpdateCache = new(Bytes.EqualityComparer);
        _leafUpdateCacheSpan = _leafUpdateCache.GetAlternateLookup<ReadOnlySpan<byte>>();
    }
    /// <summary>
    ...
    private readonly ConcurrentDictionary<byte[], LeafUpdateDelta> _leafUpdateCache;
    private readonly ConcurrentDictionary<byte[], LeafUpdateDelta>.AlternateLookup<ReadOnlySpan<byte>> _leafUpdateCacheSpan;

    private readonly ILogger _logger;

    // the store that is responsible to store the tree in a key-value store
    public readonly IVerkleTreeStore VerkleStateStore;
    ...
    public void Insert(Hash256 key, in ReadOnlySpan<byte> value)
    {
        ...
        var present = _leafUpdateCacheSpan.TryGetValue(stem, out LeafUpdateDelta leafUpdateDelta);
        ...
        _leafUpdateCacheSpan[stem] = leafUpdateDelta;
    }
    ...
    {
        if (replace || !TreeCache.InternalTableSpan.TryGetValue(nodeKey, out InternalNode? prevNode))
        {
