...
public sealed class StackPushBytesGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        IncrementalValuesProvider<Candidate> candidates = context.SyntaxProvider.CreateSyntaxProvider(
    ...
    private static void EmitOneMethod(SourceProductionContext spc, StringBuilder sb, int indent, Candidate c)
    {
        ...
        {
            sb.Indent(indent + 1).Append("if (").Append(tracingTpName).AppendLine(".IsActive)");

            if (isByteValue)
            {
                sb.Indent(indent + 2).Append("_tracer.ReportStackPush(").Append(valueName).AppendLine(");");
            }
            else
            {
                sb.Indent(indent + 2).Append("_tracer.TraceBytes(in ").Append(valueName).Append(", ").Append(size).AppendLine(");");
            }

            sb.AppendLine();
        }

        sb.Indent(indent + 1).AppendLine("ref Vector256<byte> head = ref Unsafe.As<byte, Vector256<byte>>(ref PushBytesNullableRef());");
        sb.Indent(indent + 1).AppendLine("if (Unsafe.IsNullRef(ref head)) goto StackOverflow;");
        ...
        if (size == 32)
        {
            ...
            sb.Indent(indent + 1)
              .Append("head = Unsafe.ReadUnaligned<Vector256<byte>>(")
        ...
        sb.Indent(indent + 1).AppendLine("return EvmExceptionType.None;");
    ...
    private static void EmitPadRightBody(StringBuilder sb, int indent, int size, bool isByteValue, string valueName)
    {
        ...
        if (size == 16)
        {
            sb.Indent(indent).Append("Vector128<byte> src = ")
              .Append("Unsafe.ReadUnaligned<Vector128<byte>>(")
              .Append("ref ").Append(valueName).AppendLine(");");
            sb.Indent(indent).AppendLine("head = Vector256.Create(src, default);");
            return;
        }
