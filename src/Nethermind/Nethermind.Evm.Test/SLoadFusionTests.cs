...
public class SLoadFusionPreBerlinTests : SLoadFusionTestsBase
{
    ...
    [TestCase(false, 100000UL, GasCostOf.Transaction + 3 + 6 * GasCostOf.SLoadEip1884, StatusCode.Success)]
    [TestCase(true, 100000UL, GasCostOf.Transaction + 3 + 6 * GasCostOf.SLoadEip1884, StatusCode.Success)]
    ...
    public void Chained_reads_price_flat_per_new_cell(bool traced, ulong gasLimit, ulong expectedGasSpent, byte expectedStatus)
    {
        ...
        (ulong gasSpent, byte statusCode, _) = Run(traced, gasLimit, SLoadRun(6));
