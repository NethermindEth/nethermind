...
namespace Nethermind.Evm.Test
{
    public class Eip3860Tests : VirtualMachineTestsBase
    {
        ...
        public void Test_EIP_3860_GasCost_Create(string createCode, bool eip3860Enabled, long expectedGasUsage)
        {
            ...
            var tracer = Execute(MainnetSpecProvider.GrayGlacierBlockNumber, 100000, callCode, timestamp: eip3860Enabled ? Timestamp : Timestamp - 1);
        ...
        {
            var tracer = PrepExecuteCreateTransaction(MainnetSpecProvider.GrayGlacierBlockNumber, Spec.MaxInitCodeSize + 1, Timestamp - 1);
        ...
        {
            var tracer = PrepExecuteCreateTransaction(MainnetSpecProvider.GrayGlacierBlockNumber, Spec.MaxInitCodeSize + 1, Timestamp);
        ...
        public void Test_EIP_3860_Enabled_InitCode_TxCreation_Within_Limit_Succeeds()
        {
            ...
            var tracer = PrepExecuteCreateTransaction(MainnetSpecProvider.GrayGlacierBlockNumber, Spec.MaxInitCodeSize - 7680, Timestamp);

            Assert.AreEqual(StatusCode.Success, tracer.StatusCode);
        }

        protected TestAllTracerWithOutput PrepExecuteCreateTransaction(long blockNumber, long byteCodeSize, ulong timestamp)
        {
