...
internal class TransactionProcessorEip7702Tests
{
    ...
    public void Execute_TxHasDifferentAmountOfAuthorizedCode_UsedGasIsExpected(int count)
    {
        ...
        _transactionProcessor.Execute(tx, new BlockExecutionContext(block.Header), tracer);
    ...
    public void Execute_FirstTxHasAuthorizedCodeThatIncrementsAndSecondDoesNot_StorageSlotIsOnlyIncrementedOnce()
    {
        ...
        _transactionProcessor.Execute(tx1, new BlockExecutionContext(block.Header), NullTxTracer.Instance);
        _transactionProcessor.Execute(tx2, new BlockExecutionContext(block.Header), NullTxTracer.Instance);
    ...
    public void Execute_DelegatedCodeUsesEXTOPCODES_StoresExpectedValue(byte[] code, byte[] expectedValue)
    {
        ...
        _ = _transactionProcessor.Execute(tx, new BlockExecutionContext(block.Header), callOutputTracer);
    ...
    public void Execute_CodeSavesEXTCODEHASHWhenAccountIsDelegatedOrNot_SavesExpectedValue(Action<IWorldState, Address, Address> setupAccount, byte[] expected)
    {
        ...
        Block block = Build.A.Block.WithNumber(long.MaxValue)
            ...
            .WithTransactions(tx)
            .WithGasLimit(10000000).TestObject;
        _ = _transactionProcessor.Execute(tx, new BlockExecutionContext(block.Header), NullTxTracer.Instance);
    ...
    public void Execute_DiffentAccountAccessOpcodes_ChargesCorrectAccountAccessGas(byte[] code, long expectedGas, bool isDelegated, long gasLimit, bool shouldRunOutOfGas)
    {
        ...
        _ = _transactionProcessor.Execute(tx, new BlockExecutionContext(block.Header), estimateGasTracer);
    ...
    public void Execute_CombinationOfValidAndInvalidTuples_AddsTheCorrectAddressesToAccessedAddresses(AuthorizationTuple[] tuples, Address[] shouldCountAsAccessed)
    {
        ...
        TransactionResult result = _transactionProcessor.Execute(tx, new BlockExecutionContext(block.Header), txTracer);
    ...
    public void Execute_EXTCODESIZEOnDelegatedThatTriggersOptimization_ReturnsZeroIfDelegated(bool isDelegated)
    {
        ...
        Block block = Build.A.Block.WithNumber(long.MaxValue)
            ...
            .WithGasLimit(10000000).TestObject;
        CallOutputTracer tracer = new();
        _ = _transactionProcessor.Execute(tx, new BlockExecutionContext(block.Header), tracer);
