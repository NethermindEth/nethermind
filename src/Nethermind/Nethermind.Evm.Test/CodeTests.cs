...
public class CodeTests
{
    ...
    public void TestByteCodeIsSameAsReconstructedVerkleCodeWithIncompleteCode()
    {
...

        {
            ...
            while (index < code.Length)
            {
                ...
                {
                    skip = false;
                    Assert.That(Bytes.BytesComparer.Compare(verkleCodeSlice, emptyCodeSlice[..verkleCodeSlice.Length]), Is.EqualTo(0));
                }
                else
                {
                    skip = true;
                    ReadOnlySpan<byte> byteCodeSlice = byteCodeSpan[index..endIndex];
                    Assert.That(Bytes.BytesComparer.Compare(verkleCodeSlice, byteCodeSlice), Is.EqualTo(0));
                }
                index += 31;
            }
        }
    ...
    private void InsertAlternateChunks(byte[] code, VerkleStateTree tree)
    {
        ...
        tree.Set(TestItem.AddressA, new Account(0, 0, 200000, 2000, Keccak.EmptyTreeHash, Keccak.MaxValue));
        tree.Commit();
    }
