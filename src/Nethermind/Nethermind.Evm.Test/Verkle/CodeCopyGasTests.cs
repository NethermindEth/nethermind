...
public class CodeCopyGasTests : VerkleVirtualMachineTestsBase
{
    ...
    public void TestNoChargeForCallDataCodeCopy()
    {
        ...
        Transaction createTx = Build.A.Transaction.WithCode(code).WithValue(0.Ether()).WithGasLimit(1000000).SignedAndResolved(ecdsa, TestItem.PrivateKeyA).TestObject;
    ...
    public void TestNoChargeForCallDataCodeCopy2()
    {
        ...
        TestAllTracerWithOutput tracer = CreateTracer();
        _processor.Execute(createTx, new BlockExecutionContext(block.Header, SpecProvider.GetSpec(block.Header)),
            tracer);
