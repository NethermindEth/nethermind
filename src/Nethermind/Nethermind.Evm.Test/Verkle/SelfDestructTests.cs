...
public class SelfDestructTests : VerkleVirtualMachineTestsBase
{
    ...
    public void TestSelfDestructNotInSameTransaction()
    {
        ...
        Transaction initTx = Build.A.Transaction.WithCode(initCode).WithValue(99.Ether()).WithGasLimit(1000000).SignedAndResolved(ecdsa, TestItem.PrivateKeyA).TestObject;

        byte[] contractCall = Prepare.EvmCode
            .Call(_contractAddress, 100000)
            .Op(Instruction.STOP).Done;
        Transaction callTxn = Build.A.Transaction.WithCode(contractCall).WithGasLimit(1000000).WithNonce(1).SignedAndResolved(ecdsa, TestItem.PrivateKeyA).TestObject;

        Block block = Build.A.Block.WithNumber(BlockNumber)
            .WithTimestamp(Timestamp)
            .WithTransactions(initTx, callTxn).WithGasLimit(2 * 1000000).TestObject;

        _processor.Execute(initTx, new BlockExecutionContext(block.Header, SpecProvider.GetSpec(block.Header)), NullTxTracer.Instance);
        ...
        _processor.Execute(callTxn, new BlockExecutionContext(block.Header, SpecProvider.GetSpec(block.Header)), tracer);
    ...
    public void TestSelfDestructNotInSameTransactionShouldNotBurn()
    {
        ...
        byte[] contractCall = Prepare.EvmCode
            .Call(_contractAddress, 100000)
            .Op(Instruction.STOP).Done;
        Transaction tx1 = Build.A.Transaction.WithCode(contractCall).WithGasLimit(1000000).WithNonce(1).SignedAndResolved(ecdsa, TestItem.PrivateKeyA).TestObject;
    ...
    public void TestSelfDestructInSameTransaction()
    {
        ...
        EthereumEcdsa ecdsa = new(1);
        Transaction createTx = Build.A.Transaction.WithCode(tx1).WithValue(100.Ether()).WithGasLimit(1000000).SignedAndResolved(ecdsa, TestItem.PrivateKeyA).TestObject;
        Block block = Build.A.Block.WithNumber(BlockNumber)
            .WithTimestamp(Timestamp)
            .WithTransactions(createTx).WithGasLimit(2 * 1000000).TestObject;

        TestAllTracerWithOutput tracer = CreateTracer();
        _processor.Execute(createTx, new BlockExecutionContext(block.Header, SpecProvider.GetSpec(block.Header)), tracer);
    ...
    public void TestSelfDestructInInitCodesOfCreateTxn()
    {
        var selfDestructCode = Prepare.EvmCode
            ...
            .Done;
        EthereumEcdsa ecdsa = new(1);
        Transaction createTx = Build.A.Transaction.WithCode(selfDestructCode).WithValue(99.Ether()).WithGasLimit(1000000).SignedAndResolved(ecdsa, TestItem.PrivateKeyA).TestObject;
        Block block = Build.A.Block.WithNumber(BlockNumber)
            .WithTimestamp(Timestamp)
            .WithTransactions(createTx).WithGasLimit(2 * 1000000).TestObject;
