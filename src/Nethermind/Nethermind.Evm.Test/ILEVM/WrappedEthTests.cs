...
public class WrappedEthTests(bool useIlEvm) : RealContractTestsBase(useIlEvm)
{
    ...
    public void Deposit()
    {
        // https://etherscan.io/tx/0x88505c3d1a4e5e33028498a082f16d46950ecff39e6427b2b914a3db201b3921
        UInt256 deposit = 1000;

        (Block block, Transaction transaction) = PrepareTx(Activation, 100000, parsed, DepositSelector, deposit);

        ReadOnlySpan<byte> before = TestState.Get(SenderBalanceCell);
    ...
    private static readonly byte[] parsed = Bytes.FromHexString(
    ...
    protected override byte[] ByteCode => parsed;
}
