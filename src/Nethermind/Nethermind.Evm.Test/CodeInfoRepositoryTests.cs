...
public class CodeInfoRepositoryTests
{
    ...
    public void HasDelegation_CodeIsNotDelegation_ReturnsFalse(byte[] code)
    {
        ...
        sut.HasDelegation(TestItem.AddressA).Should().Be(false);
    }
    ...
    public void HasDelegation_CodeIsDelegation_ReturnsTrue(byte[] code)
    {
        IWorldState stateProvider = TestWorldStateFactory.CreateForTest();
        using IDisposable _scope = stateProvider.BeginScope(IWorldState.PreGenesis);
        stateProvider.CreateAccount(TestItem.AddressA, 0);
        ...
        sut.HasDelegation(TestItem.AddressA).Should().Be(true);
    }

    [TestCaseSource(nameof(DelegationCodeCases))]
    public void GetCachedCodeInfo_CodeIsDelegation_CorrectDelegationAddressIsSet(byte[] code)
    {
        IWorldState stateProvider = TestWorldStateFactory.CreateForTest();
        ...
        sut.GetCachedCodeInfo(TestItem.AddressA, followDelegation: false, _releaseSpec, out Address? result);
