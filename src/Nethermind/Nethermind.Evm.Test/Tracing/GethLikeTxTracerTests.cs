...
public class GethLikeTxTracerTests : VirtualMachineTestsBase
{
    ...
    public void Memory_is_cleared_and_restored_when_moving_between_call_levels()
    {
        ...
        Assert.AreEqual(0, trace.Entries[0].Memory?.Count ?? 0, "BEGIN 1");
        Assert.AreEqual(3, trace.Entries[10].Memory?.Count ?? 0, "CALL FROM 1");
        Assert.AreEqual(0, trace.Entries[11].Memory?.Count ?? 0, "BEGIN 2");
        Assert.AreEqual(2, trace.Entries[23].Memory?.Count ?? 0, "CREATE FROM 2");
        Assert.AreEqual(0, trace.Entries[24].Memory?.Count ?? 0, "BEGIN 3");
        Assert.AreEqual(1, trace.Entries[29].Memory?.Count ?? 0, "END 3");
        Assert.AreEqual(2, trace.Entries[30].Memory?.Count ?? 0, "END 2");
        Assert.AreEqual(3, trace.Entries[31].Memory?.Count ?? 0, "END 1");
    }
    ...
    public void Can_trace_memory()
    {
        ...
        Assert.AreEqual(0, trace.Entries[0].Memory?.Count ?? 0, "entry[0] length");

        Assert.AreEqual(0, trace.Entries[1].Memory?.Count ?? 0, "entry[1] length");

        Assert.AreEqual(1, trace.Entries[2].Memory?.Count ?? 0, "entry[2] length");
        Assert.AreEqual(HexZero.PadLeft(64, '0'), trace.Entries[2].Memory[0], "entry[2][0]");

        Assert.AreEqual(1, trace.Entries[3].Memory?.Count ?? 0, "entry[3] length");
        Assert.AreEqual(SampleHexData1.PadLeft(64, '0'), trace.Entries[3].Memory[0], "entry[3][0]");

        Assert.AreEqual(1, trace.Entries[4].Memory?.Count ?? 0, "entry[4] length");
        Assert.AreEqual(SampleHexData1.PadLeft(64, '0'), trace.Entries[4].Memory[0], "entry[4][0]");

        Assert.AreEqual(2, trace.Entries[5].Memory?.Count ?? 0, "entry[5] length");
        ...
        Assert.AreEqual(2, trace.Entries[6].Memory?.Count ?? 0, "entry[2] length");
