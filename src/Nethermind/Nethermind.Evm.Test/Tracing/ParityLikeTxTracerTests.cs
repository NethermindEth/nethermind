...
public class ParityLikeTxTracerTests : VirtualMachineTestsBase
{
    ...
    public void On_failure_block_and_tx_fields_are_set()
    {
        ...
        Assert.That(trace.Action.TraceAddress.AsSpan().ToArray(), Is.EqualTo(Array.Empty<int>()), "trace address");
    }
    ...
    public void Trace_address_is_valid_in_simple_cases()
    {
        byte[] code = Prepare.EvmCode
            ...
            .Done;
        (ParityLikeTxTrace trace, _, _) = ExecuteAndTraceParityCall(code);
        Assert.That(trace.Action.TraceAddress.AsSpan().ToArray(), Is.EqualTo(Array.Empty<int>()));
    }
    ...
    public void Can_trace_nested_calls()
    {
        ...
        Assert.That(trace.Action.Subtraces[0].TraceAddress.AsSpan().ToArray(), Is.EqualTo(new[] { 0 }), "[0] address");
        Assert.That(trace.Action.Subtraces[0].Subtraces[0].Subtraces.Count, Is.EqualTo(0), "[0, 0] subtraces");
        Assert.That(trace.Action.Subtraces[0].Subtraces[0].TraceAddress.AsSpan().ToArray(), Is.EqualTo(new[] { 0, 0 }), "[0, 0] address");
    }
    ...
    public void Can_trace_same_level_calls()
    {
        ...
        Assert.That(trace.Action.Subtraces[1].TraceAddress.AsSpan().ToArray(), Is.EqualTo(new[] { 1 }), "[1] address");
        ...
        Assert.That(trace.Action.Subtraces[1].Subtraces[0].TraceAddress.AsSpan().ToArray(), Is.EqualTo(new[] { 1, 0 }), "[1, 0] address");
    ...
    public void Is_tracing_rewards_only_when_rewards_trace_type_selected()
    {
        ...
        Assert.That(tracer.IsTracingRewards, Is.False);

        ParityLikeBlockTracer tracer2 = new(ParityTraceTypes.Rewards);
        Assert.That(tracer2.IsTracingRewards, Is.True);
    }
