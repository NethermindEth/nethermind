...
public class Eip7981Tests
{
    ...
    private static IEnumerable<TestCaseData> AccessListOnlyCases()
    {
        ...
        // Address + non-zero storage key: 208 tokens
        yield return new TestCaseData(
    ...
    public void Access_list_token_pricing(AccessList accessList, ulong expectedTokens)
    {
        ...
        ulong expectedStandard = GasCostOf.Transaction
            + (ulong)addressCount * GasCostOf.AccessAccountListEntry
            + (ulong)storageKeyCount * GasCostOf.AccessStorageListEntry
            + FloorPerToken * expectedTokens;
        ulong expectedFloor = GasCostOf.Transaction + FloorPerToken * expectedTokens;
    ...
    private static IEnumerable<TestCaseData> CalldataWithAccessListCases()
    {
        ...
        yield return new TestCaseData(new byte[] { 0 }, 1, 0, 24684UL, 22344UL)
        ...
        yield return new TestCaseData(new byte[40], 1, 0, 24840UL, 24840UL)
        ...
        yield return new TestCaseData(new byte[41], 1, 0, 24844UL, 24904UL)
        ...
        yield return new TestCaseData(new byte[100], 1, 1, 29028UL, 30728UL)
            .SetName("100 zero bytes + 1 address + 1 key: floor dominates");
    }

    [TestCaseSource(nameof(CalldataWithAccessListCases))]
    public void Calldata_with_access_list_floor_pricing(byte[] data, int addressCount, int storageKeyCount, ulong expectedStandard, ulong expectedFloor)
    {
