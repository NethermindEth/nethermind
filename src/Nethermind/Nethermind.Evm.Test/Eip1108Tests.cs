...
public class Eip1108Tests : VirtualMachineTestsBase
{
    ...
    public void Test_add_before_istanbul()
    {
        ...
        byte[] code = Prepare.EvmCode
            .CallWithInput(ECAddPrecompile.Address, 1000L, new byte[128])
            .Done;
    ...
    public void Test_mul_before_istanbul()
    {
        _blockNumberAdjustment = -1;
        byte[] code = Prepare.EvmCode
            .CallWithInput(ECMulPrecompile.Address, 50000L, new byte[128])
            .Done;
        TestAllTracerWithOutput result = Execute(code);
        Assert.That(result.StatusCode, Is.EqualTo(StatusCode.Success));
    ...
    {
        byte[] code = Prepare.EvmCode
            .CallWithInput(ECMulPrecompile.Address, 10000L, new byte[128])
            .Done;
        TestAllTracerWithOutput result = Execute(code);
        Assert.That(result.StatusCode, Is.EqualTo(StatusCode.Success));
    ...
    public void Test_pairing_before_istanbul()
    {
        _blockNumberAdjustment = -1;
        byte[] code = Prepare.EvmCode
            .CallWithInput(ECPairingPrecompile.Address, 200000L, new byte[192])
            .Done;
