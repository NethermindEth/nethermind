...
public class Eip8037Tests : VirtualMachineTestsBase
{
    ...
    [Test]
    public void Exceptional_halt_discards_refunded_child_spill_history()
    {
    ...
    [Test]
    public void Revert_with_partial_spill_refund_discards_child_spill_history()
    {
        EthereumGasPolicy parent = new() { Value = 1_000, StateReservoir = 0, StateGasUsed = 0 };
        EthereumGasPolicy child = EthereumGasPolicy.CreateChildFrameGas(ref parent, 500);
        Assert.That(EthereumGasPolicy.ConsumeStateGas(ref child, 200), Is.True);
    ...
    [Test]
    public void Code_deposit_halt_discards_refunded_child_spill_history()
    {
        EthereumGasPolicy parent = new() { Value = 1_000, StateReservoir = 0, StateGasUsed = 0 };
        EthereumGasPolicy child = EthereumGasPolicy.CreateChildFrameGas(ref parent, 1_000);
        EthereumGasPolicy.ConsumeStateGas(ref child, 200);
    ...
    [TestCase(false, TestName = "Reverted_child_does_not_propagate_frame_local_spill")]
    [TestCase(true, TestName = "Halted_child_does_not_propagate_frame_local_spill")]
    public void Failed_child_does_not_propagate_frame_local_spill(bool exceptionalHalt)
    {
        EthereumGasPolicy parent = new()
        {
            Value = 1_000,
        ...
        EthereumGasPolicy child = EthereumGasPolicy.CreateChildFrameGas(ref parent, 500);
        Assert.That(EthereumGasPolicy.ConsumeStateGas(ref child, 150), Is.True);
        EthereumGasPolicy.RefundStateGas(ref child, 20, stateGasFloor: 0);
        ...
        Assert.That((parent.StateReservoir, parent.StateGasSpill, parent.StateGasSpillRefunded),
            Is.EqualTo((100L, 17L, 5L)));
    }

    [Test]
    public void Code_deposit_halt_removes_merged_child_spill()
    {
        EthereumGasPolicy parent = new()
        {
            Value = 1_000,
    ...
    [Test]
    public void Inner_revert_refills_spill_without_propagating_history()
    {
    ...
    [Test]
    public void Reverted_state_gas_spill_does_not_propagate_through_success_chain()
    {
        ...
        Assert.That(child.StateGasSpill, Is.Zero);
