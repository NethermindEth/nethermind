...
namespace Nethermind.Evm.Test
{
    public class CallTests : VirtualMachineTestsBase
    {
        ...
        [Test]
        public void Nested_returns_above_the_retained_limit_keep_their_contents_across_pool_growth()
        {
            // 40 KiB rents, 100 KiB outgrows that rental, 50 KiB reuses the larger one.
            int[] sizes = [40 * 1024, 100 * 1024, 50 * 1024];
            ...
            {
                byte[] output = DeploySelfReturningCode(targets[i], sizes[i], (byte)(0x10 * (i + 1)));
                parent.CALL(200_000, targets[i], 0, 0, 0, 0, 0).Op(Instruction.POP)
                    .RETURNDATACOPY((UInt256)offset, 0, (UInt256)sizes[i]);
                expected.AddRange(output);
                offset += sizes[i];
            }

            // The last output's logical length, which must not be the larger rental's length.
            byte[] parentCode = parent
                .Op(Instruction.RETURNDATASIZE).MSTORE((UInt256)offset)
                .RETURN(0, (UInt256)(offset + EvmPooledMemory.WordSize))
                .Done;
            expected.AddRange(((UInt256)sizes[^1]).ToBigEndian());
            ...
            {
                Assert.That(result.EvmExceptionType, Is.EqualTo(EvmExceptionType.None));
                Assert.That(result.Output.ToArray(), Is.EqualTo(expected.ToArray()));
                Assert.That(Machine.RetainedReturnDataScratchLength, Is.Zero);
        ...
        [Test]
        public void Pooled_return_scratch_is_released_when_the_transaction_halts()
        {
            DeploySelfReturningCode(TestItem.AddressC, 40 * 1024, 0x21);
            byte[] parentCode = Prepare.EvmCode
                .CALL(200_000, TestItem.AddressC, 0, 0, 0, 0, 0).Op(Instruction.POP)
            ...
            using (Assert.EnterMultipleScope())
            {
                Assert.That(result.EvmExceptionType, Is.EqualTo(EvmExceptionType.BadInstruction));
                Assert.That(Machine.HoldsPooledReturnDataScratch, Is.False);
            }
        }

        [Test]
        public void Top_level_large_return_output_survives_a_later_nested_large_return()
        {
            byte[] firstCode = SelfReturningCode(40 * 1024, 0x31);
            ReadOnlyMemory<byte> retainedOutput = ExecuteDirect(firstCode).Output;

            DeploySelfReturningCode(TestItem.AddressC, 40 * 1024, 0x41);
            ExecuteDirect(Prepare.EvmCode
                .CALL(200_000, TestItem.AddressC, 0, 0, 0, 0, 0).Op(Instruction.POP)
                .Op(Instruction.STOP)
                .Done);

            Assert.That(retainedOutput.ToArray(), Is.EqualTo(firstCode));
        }

        [Test]
        public void Tracer_can_retain_large_nested_return_output_after_later_sibling_return()
        {
            byte[] firstOutput = DeploySelfReturningCode(TestItem.AddressC, 40 * 1024, 0x51);
            byte[] secondOutput = DeploySelfReturningCode(TestItem.AddressD, 40 * 1024, 0x61);
            RetainingOutputTracer tracer = new();

            ExecuteDirect(Prepare.EvmCode
                .CALL(200_000, TestItem.AddressC, 0, 0, 0, 0, 0).Op(Instruction.POP)
                .CALL(200_000, TestItem.AddressD, 0, 0, 0, 0, 0).Op(Instruction.POP)
                .Op(Instruction.STOP)
                .Done, tracer);
            ...
            {
                Assert.That(tracer.Outputs.Count, Is.GreaterThanOrEqualTo(2));
                Assert.That(tracer.Outputs[0].ToArray(), Is.EqualTo(firstOutput));
                Assert.That(tracer.Outputs[1].ToArray(), Is.EqualTo(secondOutput));
                Assert.That(Machine.HoldsPooledReturnDataScratch, Is.False);
            }
        }

        /// <summary>Code that returns itself, padded with a position-dependent pattern to <paramref name="size"/> bytes.</summary>
        private static byte[] SelfReturningCode(int size, byte seed)
        {
            byte[] prefix = Prepare.EvmCode.CODECOPY(0, 0, (UInt256)size).RETURN(0, (UInt256)size).Done;
            byte[] code = new byte[size];
            prefix.CopyTo(code, 0);
            for (int i = prefix.Length; i < size; i++) code[i] = (byte)(seed + i);
            return code;
        }

        private byte[] DeploySelfReturningCode(Address target, int size, byte seed)
        {
            byte[] code = SelfReturningCode(size, seed);
            TestState.CreateAccount(target, UInt256.Zero);
            TestState.InsertCode(target, code, SpecProvider.GenesisSpec);
            return code;
        }
