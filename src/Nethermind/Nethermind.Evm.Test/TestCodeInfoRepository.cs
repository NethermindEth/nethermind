...
{
    private readonly FrozenDictionary<AddressAsKey, PrecompileInfo> _precompiles = InitializePrecompiledContracts();
    ...
    private ICodeInfo InternalGetCachedCode(IWorldState worldState, Address codeSource, IReleaseSpec vmSpec)
    {
    ...
    private ICodeInfo InternalGetCachedCode(IReadOnlyStateProvider worldState, Address codeSource, IReleaseSpec vmSpec)
    {
        ValueHash256 codeHash = worldState.GetCodeHash(codeSource);
        return InternalGetCachedCode(worldState, in codeHash, vmSpec);
    }

    private ICodeInfo InternalGetCachedCode(IReadOnlyStateProvider worldState, in ValueHash256 codeHash, IReleaseSpec vmSpec)
    {
