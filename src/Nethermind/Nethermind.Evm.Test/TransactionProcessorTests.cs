...
public class TransactionProcessorTests
{
    ...
    public void Can_estimate_with_refund()
    {
        ...
        _transactionProcessor.CallAndRestore(tx, new BlockExecutionContext(block.Header, _specProvider.GetSpec(block.Header)), gethTracer);
    ...
    public void Can_estimate_with_destroy_refund_and_below_intrinsic_pre_berlin()
    {
        ...
        _transactionProcessor.Execute(initTx, new BlockExecutionContext(block.Header, _specProvider.GetSpec(block.Header)), NullTxTracer.Instance);
    ...
    private void ConfirmEnoughEstimate(Transaction tx, Block block, long estimate)
    {
        ...
        _transactionProcessor.CallAndRestore(tx, new BlockExecutionContext(block.Header, _specProvider.GetSpec(block.Header)), outputTracer);
    ...
    public void State_changed_twice_in_buildup_should_have_correct_gas_cost()
    {
        ...
        Snapshot state = _stateProvider.TakeSnapshot();
        _transactionProcessor.BuildUp(tx1, new BlockExecutionContext(block.Header, _specProvider.GetSpec(block.Header)), NullTxTracer.Instance);
        _stateProvider.GetBalance(TestItem.PrivateKeyA.Address).Should().Be(AccountBalance - GasCostOf.Transaction);

        _transactionProcessor.BuildUp(tx2, new BlockExecutionContext(block.Header, _specProvider.GetSpec(block.Header)), NullTxTracer.Instance);
