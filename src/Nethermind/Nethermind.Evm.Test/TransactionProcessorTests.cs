...
public class TransactionProcessorTests
{
    ...
    public void Balance_is_not_changed_on_call_and_restore()
    {
        ...
        _transactionProcessor.CallAndRestore(tx, new BlockExecutionContext(block.Header), NullTxTracer.Instance);
    ...
    public void Can_estimate_with_value(bool systemUser)
    {
        ...
        Action action = () => _transactionProcessor.CallAndRestore(tx, new BlockExecutionContext(block.Header), tracer);
        if (!systemUser)
        {
    ...
    public void Can_estimate_simple()
    {
        ...
        BlocksConfig blocksConfig = new();
        GasEstimator estimator = new(_transactionProcessor, _stateProvider, _specProvider, blocksConfig);
        _transactionProcessor.CallAndRestore(tx, new BlockExecutionContext(block.Header), tracer);
    ...
    public void Can_estimate_with_refund()
    {
        ...
        _transactionProcessor.CallAndRestore(tx, new BlockExecutionContext(block.Header), gethTracer);
    ...
    public void Can_estimate_with_destroy_refund_and_below_intrinsic_pre_berlin()
    {
        ...
        _transactionProcessor.Execute(initTx, new BlockExecutionContext(block.Header), NullTxTracer.Instance);
    ...
    private void ConfirmEnoughEstimate(Transaction tx, Block block, long estimate)
    {
        ...
        _transactionProcessor.CallAndRestore(tx, new BlockExecutionContext(block.Header), outputTracer);
    ...
    public void Balance_is_changed_on_buildup_and_restored()
    {
        ...
        _transactionProcessor.BuildUp(tx, new BlockExecutionContext(block.Header), NullTxTracer.Instance);
    ...
    public void State_changed_twice_in_buildup_should_have_correct_gas_cost()
    {
        ...
        Snapshot state = _stateProvider.TakeSnapshot();
        _transactionProcessor.BuildUp(tx1, new BlockExecutionContext(block.Header), NullTxTracer.Instance);
        _stateProvider.GetBalance(TestItem.PrivateKeyA.Address).Should().Be(AccountBalance - GasCostOf.Transaction);

        _transactionProcessor.BuildUp(tx2, new BlockExecutionContext(block.Header), NullTxTracer.Instance);
    ...
    private TransactionResult Execute(Transaction tx, Block block, BlockReceiptsTracer? tracer = null)
    {
        ...
        TransactionResult result = _transactionProcessor.Execute(tx, new BlockExecutionContext(block.Header), tracer ?? NullTxTracer.Instance);
        if (result)
        {
            tracer?.EndTxTrace();
    ...
    private TransactionResult CallAndRestore(Transaction tx, Block block, BlockReceiptsTracer? tracer = null)
    {
        tracer?.StartNewBlockTrace(block);
        tracer?.StartNewTxTrace(tx);
        TransactionResult result = _transactionProcessor.CallAndRestore(tx, new BlockExecutionContext(block.Header), tracer ?? NullTxTracer.Instance);
        if (result)
        {
            tracer?.EndTxTrace();
            tracer?.EndBlockTrace();
