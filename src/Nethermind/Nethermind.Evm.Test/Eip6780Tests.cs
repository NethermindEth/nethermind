...
namespace Nethermind.Evm.Test
{
    ...
    public class Eip6780Tests : VirtualMachineTestsBase
    {
        ...
        public void self_destruct_not_in_same_transaction(ulong timestamp, bool onlyOnSameTransaction)
        {
            ...
            _processor.Execute(initTx, new(block.Header, SpecProvider.GetSpec(block.Header)), NullTxTracer.Instance);
            UInt256 contractBalanceAfterInit = TestState.GetBalance(_contractAddress);
            _processor.Execute(tx1, new(block.Header, SpecProvider.GetSpec(block.Header)), NullTxTracer.Instance);
        ...
        public void self_destruct_in_same_transaction()
        {
            ...
            _processor.Execute(createTx, new(block.Header, SpecProvider.GetSpec(block.Header)), NullTxTracer.Instance);
