...
{Whitespace}
{Shift(2, decl.SpanSize is not null ? $"MemoryExtensions.CopyTo(container, data);" : "")}
{Whitespace}
...
{Whitespace}
    ...
    {{
        container = new();
{Whitespace}
{Shift(2, decl.SpanSize is not null ? $"data.Slice(0, {decl.StaticLength}).CopyTo(container);" : "")}
{Whitespace}
{Shift(2, decl.Members.Select(m =>
{
