...
public class SszGenerator : IIncrementalGenerator
{
    ...
    private sealed class SszDeclaration
    {
        ...
        public List<SszType> FoundTypes { get; init; } = [];
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(classDeclarations, (spc, decl) =>
        {
            ...
            string? generatedCode = GenerateClassCode(spc, decl.Type, decl.FoundTypes, decl.Location);
...
internal static class SszCodecHelpers
{
    ...
    internal static void MerkleizeRefTypeVector<T>(ReadOnlySpan<T> items, ulong length, FeedItem<T> feed, out UInt256 root)
    {
        Debug.Assert((ulong)items.Length == length, "Vector items count must equal declared length");
        MerkleizeRefTypeCore(items, feed, RefTypeMerkleKind.Vector, limit: 0, out root);
    }

    internal static void MerkleizeRefTypeList<T>(ReadOnlySpan<T> items, ulong limit, FeedItem<T> feed, out UInt256 root)
        => MerkleizeRefTypeCore(items, feed, RefTypeMerkleKind.List, limit, out root);

    internal static void MerkleizeRefTypeProgressiveList<T>(ReadOnlySpan<T> items, FeedItem<T> feed, out UInt256 root)
        => MerkleizeRefTypeCore(items, feed, RefTypeMerkleKind.ProgressiveList, limit: 0, out root);
...
""";
    ...
    private static string StaticFieldEncodeStatement(SszProperty property, int staticOffset, string containerExpr)
    {
        ...
        // slice is already zero-initialized, matching the "absent field encodes as zeros"
        // semantic and aligning class and struct (default-valued) container behaviour.
        return property.IsNullable ? $"if ({valueExpr} is not null) {statement}" : statement;
    }
    ...
    private static string DecodeAndAssign(SszType decl, SszProperty property, string sliceExpression)
    {
        ...
        if ((property.Kind is Kind.Vector or Kind.List) && property.Type.Kind == Kind.Basic && property.Type.IsRefType)
        {
            ...
            string decodeBody = property.Type.CustomDecodeTemplate!
                ...
                .Replace("{1}", $"{property.Type.TypeReferenceName} __item");
            ...
            string remainderGuard = property.Kind == Kind.List
                ? $"if ({sliceExpression}.Length % {itemSize} != 0) throw new System.IO.InvalidDataException($\"{decl.TypeReferenceName}.{property.Name}: expected a multiple of {itemSize} bytes, got {{{sliceExpression}.Length}}\");"
                : string.Empty;
    ...
    private static string MerkleizeRootStatement(SszProperty property, string expression, string rootName) =>
        property.Kind switch
        {
            ...
            Kind.Vector when property.Type.Kind == Kind.Basic && property.Type.IsRefType => $"MerkleizeRefTypeVector({SpanExpression(property, expression)}, {property.Length}, {property.Type.CustomFeedMethod}, out {rootName});",
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.IsRefType => $"MerkleizeRefTypeList({SpanExpression(property, expression)}, {property.Limit}, {property.Type.CustomFeedMethod}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.IsRefType => $"MerkleizeRefTypeProgressiveList({SpanExpression(property, expression)}, {property.Type.CustomFeedMethod}, out {rootName});",
    ...
    private static string MerkleizeEmptyCollectionRootStatement(SszProperty property, string rootName) =>
        property.Kind switch
        {
            Kind.List when property.Type.Kind == Kind.Basic && !property.Type.IsRefType => $"MerkleizeBasicList(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, {property.Type.StaticLength}, {property.Limit}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && !property.Type.IsRefType => $"MerkleizeProgressiveBasicList(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, out {rootName});",
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.IsRefType => $"MerkleizeRefTypeList(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, {property.Limit}, {property.Type.CustomFeedMethod}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.IsRefType => $"MerkleizeRefTypeProgressiveList(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, {property.Type.CustomFeedMethod}, out {rootName});",
    ...
    private static string MerkleizeFeedStatement(SszProperty property, string expression)
    {
        ...
        // Nullable reference-typed members feed the zero root when absent, matching the
        // "encode as zeros" semantic on the encode/length paths.
        if (!property.IsNullable || property.IsSpanLikeProperty || property.Kind is Kind.BitList or Kind.ProgressiveBitList)
        {
            return body;
        }

        if (property.Kind is Kind.List or Kind.ProgressiveList)
        {
            string rootName = VarName(property.Name) + "Root";
            return $"if ({expression} is null) {{ UInt256 {rootName}; {MerkleizeEmptyCollectionRootStatement(property, rootName)} merkleizer.Feed({rootName}); }} else {{ {body} }}";
        }

        return $"if ({expression} is null) {{ merkleizer.Feed(default(UInt256)); }} else {{ {body} }}";
    }
...
{Whitespace}
        switch(container.Selector) {{
{Shift(3, decl.CompatibleUnionMembers!.Select(m => $"case {decl.Selector!.Type.StaticMemberAccess}.{m.Name}: {EncodeStatement("data.Slice(1)", m, $"container.{m.Name}")} break;"))}
            default:
