...
public class SszGenerator : IIncrementalGenerator
{
    ...
    private static readonly ConditionalWeakTable<Compilation, Lazy<SszVectorConverterInfo[]>> ConverterCache = new();
        ...
        foreach (AttributeListSyntax attributeList in classDeclaration.AttributeLists)
        {
            foreach (AttributeSyntax attribute in attributeList.Attributes)
            {
                ...
                if (methodSymbol is not null && IsSszRootAttribute(methodSymbol.ContainingType))
                {
                    ...
                    try
                    {
                        List<SszType> foundTypes = SszType.CreateBasicTypes(FindConverters(context.SemanticModel.Compilation));
    ...
    private static IEnumerable<SszVectorConverterInfo> FindConverters(Compilation compilation) =>
        ConverterCache.GetValue(
            compilation,
            static currentCompilation => new Lazy<SszVectorConverterInfo[]>(() => SszVectorConverterInfo.Find(currentCompilation).ToArray()))
            .Value;
...
""";
    ...
    private static string MerkleizeRootStatement(SszProperty property, string expression, string rootName) =>
        property.Kind switch
        {
            ...
            Kind.Vector when property.Type.Kind == Kind.Basic && property.Type.EnumType?.CustomMerkleizeVectorMethod is not null => $"{property.Type.EnumType.CustomMerkleizeVectorMethod}({EnumSpanExpression(property, expression)}, {property.Length}, out {rootName});",
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.EnumType?.CustomMerkleizeListMethod is not null => $"{property.Type.EnumType.CustomMerkleizeListMethod}({EnumSpanExpression(property, expression)}, {property.Limit}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.EnumType?.CustomMerkleizeProgressiveListMethod is not null => $"{property.Type.EnumType.CustomMerkleizeProgressiveListMethod}({EnumSpanExpression(property, expression)}, out {rootName});",
            Kind.Vector when property.Type.Kind == Kind.Basic && property.Type.EnumType is { HasCustomInlineCodec: true, PacksItems: true } enumType => $"MerkleizeBasicVectorWithConverter<{enumType.TypeReferenceName}>({EnumSpanExpression(property, expression)}, {enumType.StaticLength}, {property.Length}, {enumType.CustomEncodeMethod}, out {rootName});",
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.EnumType is { HasCustomInlineCodec: true, PacksItems: true } enumType => $"MerkleizeBasicListWithConverter<{enumType.TypeReferenceName}>({EnumSpanExpression(property, expression)}, {enumType.StaticLength}, {property.Limit}, {enumType.CustomEncodeMethod}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.EnumType is { HasCustomInlineCodec: true, PacksItems: true } enumType => $"MerkleizeProgressiveBasicListWithConverter<{enumType.TypeReferenceName}>({EnumSpanExpression(property, expression)}, {enumType.StaticLength}, {enumType.CustomEncodeMethod}, out {rootName});",
            Kind.Vector when property.Type.Kind == Kind.Basic && property.Type.CustomMerkleizeVectorMethod is not null => $"{property.Type.CustomMerkleizeVectorMethod}({SpanExpression(property, expression)}, {property.Length}, out {rootName});",
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.CustomMerkleizeListMethod is not null => $"{property.Type.CustomMerkleizeListMethod}({SpanExpression(property, expression)}, {property.Limit}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.CustomMerkleizeProgressiveListMethod is not null => $"{property.Type.CustomMerkleizeProgressiveListMethod}({SpanExpression(property, expression)}, out {rootName});",
            Kind.Vector when property.Type.Kind == Kind.Basic && property.Type.HasCustomInlineCodec && property.Type.PacksItems => $"MerkleizeBasicVectorWithConverter<{property.Type.TypeReferenceName}>({SpanExpression(property, expression)}, {property.Type.StaticLength}, {property.Length}, {property.Type.CustomEncodeMethod}, out {rootName});",
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.HasCustomInlineCodec && property.Type.PacksItems => $"MerkleizeBasicListWithConverter<{property.Type.TypeReferenceName}>({SpanExpression(property, expression)}, {property.Type.StaticLength}, {property.Limit}, {property.Type.CustomEncodeMethod}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.HasCustomInlineCodec && property.Type.PacksItems => $"MerkleizeProgressiveBasicListWithConverter<{property.Type.TypeReferenceName}>({SpanExpression(property, expression)}, {property.Type.StaticLength}, {property.Type.CustomEncodeMethod}, out {rootName});",
            Kind.Vector when property.Type.Kind == Kind.Basic && property.Type.PacksItems => $"MerkleizeBasicVector({SpanExpression(property, expression)}, {property.Type.StaticLength}, {property.Length}, out {rootName});",
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.PacksItems => $"MerkleizeBasicList({SpanExpression(property, expression)}, {property.Type.StaticLength}, {property.Limit}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.PacksItems => $"MerkleizeProgressiveBasicList({SpanExpression(property, expression)}, out {rootName});",
    ...
    private static string MerkleizeEmptyCollectionRootStatement(SszProperty property, string rootName) =>
        property.Kind switch
        {
            ...
            Kind.Vector when property.Type.Kind == Kind.Basic && property.Type.EnumType?.CustomMerkleizeVectorMethod is not null => $"{property.Type.EnumType.CustomMerkleizeVectorMethod}(System.Runtime.InteropServices.MemoryMarshal.Cast<{property.Type.TypeReferenceName}, {property.Type.EnumType.TypeReferenceName}>(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty), {property.Length}, out {rootName});",
            ...
            Kind.Vector when property.Type.Kind == Kind.Basic && property.Type.CustomMerkleizeVectorMethod is not null => $"{property.Type.CustomMerkleizeVectorMethod}(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, {property.Length}, out {rootName});",
            Kind.Vector when property.Type.Kind == Kind.Basic && property.Type.HasCustomInlineCodec && property.Type.PacksItems => $"MerkleizeBasicVectorWithConverter<{property.Type.TypeReferenceName}>(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, {property.Type.StaticLength}, {property.Length}, {property.Type.CustomEncodeMethod}, out {rootName});",
            Kind.Vector when property.Type.Kind == Kind.Basic && property.Type.PacksItems => $"MerkleizeBasicVector(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, {property.Type.StaticLength}, {property.Length}, out {rootName});",
            ...
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.EnumType?.CustomMerkleizeListMethod is not null => $"{property.Type.EnumType.CustomMerkleizeListMethod}(System.Runtime.InteropServices.MemoryMarshal.Cast<{property.Type.TypeReferenceName}, {property.Type.EnumType.TypeReferenceName}>(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty), {property.Limit}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.EnumType?.CustomMerkleizeProgressiveListMethod is not null => $"{property.Type.EnumType.CustomMerkleizeProgressiveListMethod}(System.Runtime.InteropServices.MemoryMarshal.Cast<{property.Type.TypeReferenceName}, {property.Type.EnumType.TypeReferenceName}>(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty), out {rootName});",
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.EnumType is { HasCustomInlineCodec: true, PacksItems: true } enumType => $"MerkleizeBasicListWithConverter<{enumType.TypeReferenceName}>(System.Runtime.InteropServices.MemoryMarshal.Cast<{property.Type.TypeReferenceName}, {enumType.TypeReferenceName}>(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty), {enumType.StaticLength}, {property.Limit}, {enumType.CustomEncodeMethod}, out {rootName});",
            ...
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.CustomMerkleizeListMethod is not null => $"{property.Type.CustomMerkleizeListMethod}(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, {property.Limit}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.CustomMerkleizeProgressiveListMethod is not null => $"{property.Type.CustomMerkleizeProgressiveListMethod}(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, out {rootName});",
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.HasCustomInlineCodec && property.Type.PacksItems => $"MerkleizeBasicListWithConverter<{property.Type.TypeReferenceName}>(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, {property.Type.StaticLength}, {property.Limit}, {property.Type.CustomEncodeMethod}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.HasCustomInlineCodec && property.Type.PacksItems => $"MerkleizeProgressiveBasicListWithConverter<{property.Type.TypeReferenceName}>(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, {property.Type.StaticLength}, {property.Type.CustomEncodeMethod}, out {rootName});",
            Kind.List when property.Type.Kind == Kind.Basic && property.Type.PacksItems => $"MerkleizeBasicList(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, {property.Type.StaticLength}, {property.Limit}, out {rootName});",
            Kind.ProgressiveList when property.Type.Kind == Kind.Basic && property.Type.PacksItems => $"MerkleizeProgressiveBasicList(ReadOnlySpan<{property.Type.TypeReferenceName}>.Empty, out {rootName});",
