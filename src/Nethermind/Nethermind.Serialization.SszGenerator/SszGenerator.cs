    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(classDeclarations, (spc, decl) =>
        {
            ...
            if (generatedCode is not null)
            {
                ...
                spc.AddSource($"Serialization.SszEncoding.{decl.Value.Type.Name}.Codec.cs", SourceText.From(GenerateCodecPartial(decl.Value.Type), Encoding.UTF8));
            }
        });
    }

    private static string GenerateCodecPartial(SszType decl)
    {
        string keyword = decl.IsStruct ? "struct" : "class";
        string namespaceLine = string.IsNullOrEmpty(decl.Namespace) ? string.Empty : $"namespace {decl.Namespace};\n\n";
        return
$@"// <auto-generated/>
#nullable enable
using System;
...
{namespaceLine}partial {keyword} {decl.Name} : ISszCodec<{decl.Name}>
{{
    static int ISszCodec<{decl.Name}>.GetLength({decl.Name} value) =>
        SszEncoding.GetLength(value);

    static int ISszCodec<{decl.Name}>.GetLength(ReadOnlySpan<{decl.Name}> items) =>
        SszEncoding.GetLength(items);

    static void ISszCodec<{decl.Name}>.Decode(ReadOnlySpan<byte> data, out {decl.Name} value) =>
        SszEncoding.Decode(data, out value);

    static void ISszCodec<{decl.Name}>.Decode(ReadOnlySpan<byte> data, out {decl.Name}[] values) =>
        SszEncoding.Decode(data, out values);
    ...
    static byte[] ISszCodec<{decl.Name}>.Encode(ReadOnlySpan<{decl.Name}> items) =>
        SszEncoding.Encode(items);

    static void ISszCodec<{decl.Name}>.Encode(Span<byte> data, ReadOnlySpan<{decl.Name}> items) =>
        SszEncoding.Encode(data, items);

    static void ISszCodec<{decl.Name}>.Merkleize({decl.Name} value, out UInt256 root) =>
        SszEncoding.Merkleize(value, out root);

    static void ISszCodec<{decl.Name}>.MerkleizeVector(ReadOnlySpan<{decl.Name}> items, out UInt256 root) =>
        SszEncoding.MerkleizeVector(items, out root);

    static void ISszCodec<{decl.Name}>.MerkleizeList(ReadOnlySpan<{decl.Name}> items, ulong limit, out UInt256 root) =>
        SszEncoding.MerkleizeList(items, limit, out root);

    static void ISszCodec<{decl.Name}>.MerkleizeProgressiveList(ReadOnlySpan<{decl.Name}> items, out UInt256 root) =>
        SszEncoding.MerkleizeProgressiveList(items, out root);
}}
";
    }
...
public partial class SszEncoding
{
    ...
    private static void ValidateSszVectorLength<T>(ReadOnlySpan<T> items, int expectedLength, string typeName, string fieldName)
    {
    ...
    private static void ValidateSszListLimit<T>(ReadOnlySpan<T> items, int limit, string typeName, string fieldName)
    {
    ...
    private static void MerkleizeBasicVector<T>(ReadOnlySpan<T> values, int itemSize, ulong length, out UInt256 root)
        where T : struct
    {
    ...
    private static void MerkleizeBasicList<T>(ReadOnlySpan<T> values, int itemSize, ulong limit, out UInt256 root)
        where T : struct
    {
    ...
    private static void MerkleizeProgressiveBasicList<T>(ReadOnlySpan<T> values, out UInt256 root)
        where T : struct
    {
...
""";
    ...
    private static string MerkleizeRootStatement(SszProperty property, string expression, string rootName)
    {
        return property.Kind switch
        {
            ...
            Kind.Vector => $"MerkleizeVector({SpanExpression(property, expression)}, out {rootName});",
            Kind.List => $"MerkleizeList({SpanExpression(property, expression)}, {property.Limit}, out {rootName});",
            Kind.ProgressiveList => $"MerkleizeProgressiveList({SpanExpression(property, expression)}, out {rootName});",
    ...
    private static string DynamicLength(SszType container, SszProperty m)
    {
        ...
        return m.IsCollection
            ? $"GetLength({SpanExpression(m, $"container.{m.Name}")})"
            : $"GetLength(container.{m.Name})";
    }
