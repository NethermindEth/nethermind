...
{{
    ...
    static int ISszCodec<{decl.Name}>.GetLength(IReadOnlyCollection<{decl.Name}>? items) =>
    ...
    static byte[] ISszCodec<{decl.Name}>.Encode(IReadOnlyCollection<{decl.Name}>? items) =>
        SszEncoding.Encode(items);

    static void ISszCodec<{decl.Name}>.Encode(Span<byte> data, IReadOnlyCollection<{decl.Name}>? items) =>
    ...
    static void ISszCodec<{decl.Name}>.MerkleizeVector(IReadOnlyList<{decl.Name}>? items, out UInt256 root) =>
        SszEncoding.MerkleizeVector(items, out root);

    static void ISszCodec<{decl.Name}>.MerkleizeList(IReadOnlyList<{decl.Name}>? items, ulong limit, out UInt256 root) =>
        SszEncoding.MerkleizeList(items, limit, out root);

    static void ISszCodec<{decl.Name}>.MerkleizeProgressiveList(IReadOnlyList<{decl.Name}>? items, out UInt256 root) =>
...
public partial class SszEncoding
{
    ...
    private static void ValidateSszVectorLength<T>(IReadOnlyCollection<T>? items, int expectedLength, string typeName, string fieldName)
    {
    ...
    private static void ValidateSszListLimit<T>(IReadOnlyCollection<T>? items, int limit, string typeName, string fieldName)
    {
    ...
    private static string DynamicLength(SszType container, SszProperty m)
    {
        if ((m.Kind & (Kind.Collection | Kind.BitList | Kind.ProgressiveBitList)) != Kind.None && m.Type.Kind == Kind.Basic)
        {
            return m.Kind is Kind.BitList or Kind.ProgressiveBitList
                ...
                : $"(container.{m.Name} is not null ? {m.Type.StaticLength} * ((IReadOnlyCollection<{m.Type.Name}>)container.{m.Name}).Count : 0)";
        }
...
{Whitespace}
    public static int GetLength(IReadOnlyCollection<{decl.Name}>? container)
    {{
...
{Whitespace}
    public static byte[] Encode(IReadOnlyCollection<{decl.Name}>? items)
    {{
        if (items is null)
        {{
            return [];
...
{Whitespace}
    public static void Encode(Span<byte> data, IReadOnlyCollection<{decl.Name}>? items)
    {{
...
{Whitespace}
    public static void MerkleizeVector(IReadOnlyList<{decl.Name}>? container, out UInt256 root)
    {{
        if(container is null)
        {{
            root = 0;
...
{Whitespace}
    public static void MerkleizeList(IReadOnlyList<{decl.Name}>? container, ulong limit, out UInt256 root)
    {{
...
{Whitespace}
    public static void MerkleizeProgressiveList(IReadOnlyList<{decl.Name}>? container, out UInt256 root)
    {{
        int count = container?.Count ?? 0;
        UInt256[] subRoots = new UInt256[count];
        for(int i = 0; i < count; i++)
