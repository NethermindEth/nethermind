...
namespace Nethermind.Consensus.Clique
{
        ...
        : ICliqueRpcModule
    {
        ...
        public Snapshot GetSnapshot(long? number = null)
        {
            ...
            if (number is not null && (long)head.Number != number)
            {
                head = _blockTree.FindBlock(number.Value);
            }
            return _snapshotManager.GetOrCreateSnapshot((long)head.Number, head.Hash);
        }
        ...
        {
            Block head = _blockTree.Head;
            return _snapshotManager.GetOrCreateSnapshot((long)head.Number, head.Hash).Signers.Select(static s => s.Key).ToArray();
        }
        ...
        public Address[] GetSigners(long number)
        {
            ...
            return _snapshotManager.GetOrCreateSnapshot((long)header.Number, header.Hash).Signers
