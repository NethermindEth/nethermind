...
namespace Nethermind.Consensus.Clique
{
    public class CliqueRpcModule : ICliqueRpcModule
    {
        ...
        public bool ProduceBlock(Commitment parentHash)
        {
        ...
        public Snapshot GetSnapshot(Commitment hash)
        {
        ...
        public Address[] GetSigners(Commitment hash)
        {
        ...
        public string[] GetSignersAnnotated(Commitment hash)
        {
            BlockHeader header = _blockTree.FindHeader(hash, BlockTreeLookupOptions.TotalDifficultyNotNeeded);
            return _snapshotManager.GetOrCreateSnapshot(header.Number, header.Hash).Signers
                .Select(s => string.Concat(s.Key, $" ({KnownAddresses.GetDescription(s.Key)})")).ToArray();
        ...
        public ResultWrapper<bool> clique_produceBlock(Commitment parentHash)
        {
        ...
        public ResultWrapper<Snapshot> clique_getSnapshotAtHash(Commitment hash)
        {
        ...
        public ResultWrapper<Address[]> clique_getSignersAtHash(Commitment hash)
        {
        ...
        public ResultWrapper<string[]> clique_getSignersAtHashAnnotated(Commitment hash)
        {
            return ResultWrapper<string[]>.Success(GetSignersAnnotated(hash).ToArray());
        }

        public ResultWrapper<Address?> clique_getBlockSigner(Commitment? hash)
        {
            if (hash is null)
            {
