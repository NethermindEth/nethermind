// SPDX-FileCopyrightText: 2022 Demerzel Solutions Limited
// SPDX-License-Identifier: LGPL-3.0-only

using Nethermind.Core;
using Nethermind.Crypto;

namespace Nethermind.Consensus.Clique
{
    /// <summary>
    /// This small thing caused so much trouble that it deserves its own class
    /// </summary>
    internal class WiggleRandomizer
    {
        private readonly ICryptoRandom _cryptoRandom;
        private readonly ISnapshotManager _snapshotManager;
        private readonly int _minimumWiggle;
        private long _lastWiggleAtNumber;

        private int _lastWiggle;

        public WiggleRandomizer(ICryptoRandom cryptoRandom, ISnapshotManager snapshotManager, int minWiggle = 0)
        {
            _cryptoRandom = cryptoRandom;
            _snapshotManager = snapshotManager;
            _minimumWiggle = minWiggle;
        }

        public int WiggleFor(BlockHeader header)
        {
            if (header.Difficulty == Clique.DifficultyInTurn)
            {
                return 0;
            }

            long headerNumber = checked((long)header.Number);
            if (headerNumber != _lastWiggleAtNumber)
            {
                int multiplier = _snapshotManager.GetOrCreateSnapshot(headerNumber - 1, header.ParentHash!).Signers.Count / 2 + 1;
                int randomPart = _cryptoRandom.NextInt(multiplier * Clique.WiggleTime);
                _lastWiggle = randomPart < _minimumWiggle ? _minimumWiggle : randomPart;
                _lastWiggleAtNumber = headerNumber;
            }

            return _lastWiggle;
        }
    }
}
