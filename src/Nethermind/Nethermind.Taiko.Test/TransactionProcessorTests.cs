...
public class TransactionProcessorTests
{
    ...
    [Test]
    public void Execute_WithMissingSender_ShouldRecoverSender()
    {
        Transaction tx = Build.A.Transaction
            .WithValue(1)
            .WithGasPrice(1)
        ...
        // Clear the sender address
        tx.SenderAddress = null;

        Block block = Build.A.Block.WithNumber(1).WithTransactions(tx)
            .WithBaseFeePerGas(1)
            .WithGasLimit(21000).TestObject;

        _transactionProcessor!.SetBlockExecutionContext(new BlockExecutionContext(block.Header, _specProvider.GetSpec(block.Header)));
        _transactionProcessor!.Execute(tx, NullTxTracer.Instance);

        // Verify that the sender was recovered
        Assert.That(tx.SenderAddress, Is.EqualTo(TestItem.PrivateKeyA.Address));
    }

    [Test]
    public void Execute_WithExistingSender_ShouldNotOverrideSender()
    {
        Transaction tx = Build.A.Transaction
            .WithValue(1)
            .WithGasPrice(1)
        ...
        // Set a different sender address
        var existingSender = TestItem.AddressC;
        tx.SenderAddress = existingSender;

        // Create the account in the world state so RecoverSenderIfNeeded doesn't override it
        _stateProvider!.CreateAccount(existingSender, AccountBalance);
        ...
        // Verify that the existing sender was not overridden
        Assert.That(tx.SenderAddress, Is.EqualTo(existingSender));
    }
}
