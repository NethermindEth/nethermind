...
public abstract partial class TransactionProcessorBase<TGasPolicy>
{
    ...
    private TransactionResult ExecuteFrameTx(Transaction tx, ITxTracer tracer, ExecutionOptions opts, BlockHeader header, IReleaseSpec spec)
    {
        ...
        for (int i = 0; i < frames.Length; i++)
        {
            ...
            {
                // An APPROVE in an inner call leaves a signal even if the enclosing frame reverted.
            ...
            if (inBatch)
            {
                if (!frameSucceeded)
                {
                    ...
                    // The unrolled writes are gone, so their state charges are not owed; the counter only grows.
                    totalFrameStateGasUsed = batchStartStateGas;
                    // No payer/sender_approved rollback: EIP-8141 forbids approval scope on batch frames.
        ...
        // per-frame receipts stay gross, and the EIP-7623 floor bounds the net charge from below.
        ...
        // reservoir), which is what makes the SaturatingSub in CalculateBlockExecutionGas sound.
        ulong blockRegularGas = Eip8037BlockGasInclusionCheck.CalculateBlockExecutionGas(grossGas, blockStateGas, floorGas);
        // Set explicitly like the regular path: the BlockGasUsed getter otherwise falls back to
        ...
        if (tracer.IsTracingReceipt)
        {
            ...
            // Derived from the frame receipts rather than a parallel union, so the two cannot diverge.
    ...
    /// <summary>The transaction log set: the frame-order concatenation of the per-frame receipt logs.</summary>
    ...
    /// <summary>Simulates a frame transaction's validation prefix against head state for mempool admission,
    /// resolving the payer under the <c>MAX_VERIFY_GAS</c> bound.</summary>
    ...
    /// <remarks>Same helper the main path escrows on, but the blob leg prices at <c>max_fee_per_blob_gas</c>:
    /// the fee at inclusion is unknowable here, and over-reserving only makes admission stricter.</remarks>
    ...
    /// <remarks>Declared gas_limits are not structurally bounded, so this cap is what holds the budget.</remarks>
    ...
    private TransactionSubstate ExecuteFrame(TxFrame frame, Address resolvedTarget, Address caller, bool isStatic, FrameTxContext frameContext, in StackAccessTracker accessTracker, IReleaseSpec spec, ITxTracer tracer, out ulong gasUsed, out long stateGasUsed)
    {
        ...
        // Only a VERIFY frame's codeless target runs default code; every other frame runs a top-level
        // call, which dispatches a precompile by address.
        ...
        // The frame pays its target's access out of its own gas limit. Precompiles are checked explicitly
        ...
        ulong entryExecution = spec.UseHotAndColdStorage
            ...
            : 0;
        // Before the deadness query, itself a recorded read: an unaffordable frame must leave the target untouched.
        ...
        // Clamped rather than asserted: unlike the standard path this also runs for reverted or errored
    ...
    /// <summary>EIP-8141 default code of a codeless <c>VERIFY</c> target: require a canonical-hash SECP256K1
    /// signature by the target, then APPROVE. Cryptographic validity was already checked in pre-flight.</summary>
    ...
    {
        // Approval validity is enforced by the APPROVE handler; the outer loop only forwards signals
        // of successfully completed frames.
        ...
        {
            // Re-enforced here because the default-code sponsor path signals approval directly, bypassing
            // APPROVE's own guards; without it a second approval charges MaxCost and the nonce twice.
            if (frameContext.Payer is not null || !frameContext.SenderApproved) return;

            // Re-checked at charge time: an inner-call APPROVE may be followed by balance movement, and
            // the debit must never throw mid-block.
            if (WorldState.GetBalance(resolvedTarget) < frameContext.MaxCost) return;

            // EIP-8250: an approval that cannot consume its nonce performs no approval effects at all.
            ...
            // Charged up front; unused gas is refunded to the payer at the end of the transaction.
