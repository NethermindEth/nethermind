...
#pragma warning restore 618
        ...
        private static long DataGasCostInternal(ReadOnlySpan<byte> extendedInput, ReadOnlyMemory<byte> inputData, IReleaseSpec releaseSpec)
        {
            ...
            bool isModexpRepricingEnabled = true;

            if (ExceedsMaxInputSize(isModexpRepricingEnabled, baseLength, expLength, modulusLength))
            {
                return long.MaxValue;
            }

            UInt256 complexity = MultComplexity(baseLength, modulusLength, isModexpRepricingEnabled);
        ...
        private static bool ExceedsMaxInputSize(bool isEip7823Enabled, UInt256 baseLength, UInt256 expLength, UInt256 modulusLength)
            => isEip7823Enabled &&
        ...
        public unsafe (byte[], bool) Run(ReadOnlyMemory<byte> inputData, IReleaseSpec releaseSpec)
        {
            ...
            if (ExceedsMaxInputSize(isModexpRepricingEnabled, (uint)baseLength, (uint)expLength, (uint)modulusLength))
            {
