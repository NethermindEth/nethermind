...
public struct EthereumGasPolicy : IGasPolicy<EthereumGasPolicy>
{
    ...
    /// </summary>
    /// <remarks>EIP-8141 (ethereum/EIPs#12062): the EIP-8037 reservoir spill does not apply within a frame transaction.</remarks>
    ...
    public static bool ConsumeStateGas(ref EthereumGasPolicy gas, long stateGasCost)
    {
        ...
        {
            // EIP-8141 (ethereum/EIPs#12062): within a frame the pools are independent, so a charge exceeding state_gas_left halts and is never paid from execution gas.
            gas.OutOfGas = true;
            return false;
        }
        ...
        where TIsSlotCreation : struct, IFlag
    {
        ...
        ulong executionWriteCost = spec.IsEip8038Enabled ? Eip8038Constants.StorageWrite : GasCostOf.SSetExecution;
