...
public interface ITxTracer : IWorldStateTracer
{
    ...
    void MarkAsSuccess(Address recipient, long gasSpent, byte[] output, LogEntry[] logs, Commitment? stateRoot = null);
    ...
    /// <param name="stateRoot">State root after transaction, depends on EIP-658</param>
    /// <remarks>Depends on <see cref="IsTracingReceipt"/></remarks>
    void MarkAsFailed(Address recipient, long gasSpent, byte[] output, string error, Commitment? stateRoot = null);
    ...
    void ReportBlockHash(Commitment blockHash);
