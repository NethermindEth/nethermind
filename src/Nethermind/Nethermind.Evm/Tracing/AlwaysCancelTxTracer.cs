...
public class AlwaysCancelTxTracer : ITxTracer
{
    ...
    public void MarkAsSuccess(Address recipient, long gasSpent, byte[] output, LogEntry[] logs, Commitment? stateRoot = null) => throw new OperationCanceledException(ErrorMessage);

    public void MarkAsFailed(Address recipient, long gasSpent, byte[] output, string error, Commitment? stateRoot = null) => throw new OperationCanceledException(ErrorMessage);
    ...
    public void ReportBlockHash(Commitment blockHash) => throw new OperationCanceledException(ErrorMessage);
