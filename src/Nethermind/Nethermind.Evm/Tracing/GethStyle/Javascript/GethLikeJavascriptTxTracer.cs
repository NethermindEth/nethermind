...
public class GethLikeJavascriptTxTracer : GethLikeTxTracer<GethTxTraceEntry>
{
    ...
    public GethLikeJavascriptTxTracer(IWorldState worldState, GethTraceOptions options) : base(options)
    {
        ...
        _engine.AddHostObject("toHex", new Func<IList, string>(list => list.ToHexString()));
        _engine.AddHostObject("toAddress", new Func<IList, string>(list => list.ToHexAddress()));
        _engine.AddHostObject("isPrecompiled", new Func<IList, bool>(list => list.IsPrecompile()));
        _engine.AddHostObject("slice", new Func<string, int, int, string>(JavascriptConverter.SliceAndConvertToString));
    ...
    private void Step(GethJavascriptStyleLog log, GethJavascriptStyleDb db)
    {
        try
        {
            _tracer.step(log, db);
    ...
    public override void ReportMemoryChange(long offset, in ReadOnlySpan<byte> data)
    {
        ...
        _customTraceEntry.memory = new GethJavascriptStyleLog.JSMemory(_engine,_memory);
