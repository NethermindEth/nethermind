...
internal static class StreamInterpreter
{
    ...
    // NETHERMIND_EVM_STREAM_NO_FUSION=1 retains stream predecode and block charging while disabling
    // superinstructions. Select it before process startup so cached streams cannot mix fused and unfused layouts.
    internal static readonly bool FusionsEnabled = !string.Equals(
        Environment.GetEnvironmentVariable("NETHERMIND_EVM_STREAM_NO_FUSION"),
        "1",
        StringComparison.Ordinal);
