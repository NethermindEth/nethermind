...
internal sealed partial class EvmInstructions
{
    ...
    [SkipLocalsInit]
    public static EvmExceptionType InstructionRevert(IEvm vm, ref EvmStack stack, ref long gasAvailable, out object returnData)
    {
        ...
        EvmState state = vm.State;
        if (!UpdateMemoryCost(state, ref gasAvailable, in position, in length))
        {
            return EvmExceptionType.OutOfGas;
        }

        returnData = state.Memory.Load(in position, in length).ToArray();
        return EvmExceptionType.None;
    }

    [SkipLocalsInit]
    public static EvmExceptionType InstructionReturn(IEvm vm, ref EvmStack stack, ref long gasAvailable, out object returnData)
    {
        SkipInit(out returnData);
