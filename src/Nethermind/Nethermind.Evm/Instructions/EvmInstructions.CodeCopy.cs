...
internal static partial class EvmInstructions
{
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        gasAvailable -= GasCostOf.VeryLow + GasCostOf.Memory * EvmInstructionsUtils.Div32Ceiling(in result, out bool outOfGas);
        ...
        if (!result.IsZero)
        {
            ...
            if (!EvmInstructionsUtils.UpdateMemoryCost(vm.EvmState, ref gasAvailable, in a, result))
                goto OutOfGas;
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        gasAvailable -= spec.GetExtCodeCost() + GasCostOf.Memory * EvmInstructionsUtils.Div32Ceiling(in result, out bool outOfGas);
        if (outOfGas) goto OutOfGas;

        // Charge gas for account access (considering hot/cold storage costs).
        if (!EvmInstructionsUtils.ChargeAccountAccessGas(ref gasAvailable, vm, address))
            goto OutOfGas;
