...
internal static partial class EvmInstructions
{
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        TGasPolicy.ConsumeGas(ref gasState, GasCostOf.VeryLow + GasCostOf.Memory * EvmCalculations.Div32Ceiling(in result, out bool outOfGas));
        ...
        if (!result.IsZero)
        {
            ...
            if (!TGasPolicy.UpdateMemoryCost(ref gasState, in a, result, vm.EvmState))
                goto OutOfGas;
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        TGasPolicy.ConsumeGas(ref gasState, spec.GetExtCodeCost() + GasCostOf.Memory * EvmCalculations.Div32Ceiling(in result, out bool outOfGas));
        if (outOfGas) goto OutOfGas;

        // Charge gas for account access (considering hot/cold storage costs).
        if (!TGasPolicy.ChargeAccountAccessGas(ref gasState, vm, address))
            goto OutOfGas;
        ...
        if (!result.IsZero)
        {
            ...
            if (spec.IsEip7907Enabled)
            {
                ...
                if (excessContractSize > 0 && !ChargeForLargeContractAccess(excessContractSize, address, in vm.EvmState.AccessTracker, ref gasState))
                    goto OutOfGas;
            }
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        TGasPolicy.ConsumeGas(ref gasState, spec.GetExtCodeCost());
