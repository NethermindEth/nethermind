    ...
    internal static partial class EvmInstructions
    {
        ...
        public struct Op1 : IOpCount
        {
                ...
                where TTracingInst : struct, IFlag
            {
                ...
                if (usedFromCode == Size)
                {
                    ...
                    stack.PushByte<TTracingInst>(Unsafe.Add(ref bytes, programCounter));
                }
                else
                {
            ...
            where TTracingInst : struct, IFlag
        {
            ...
            else if (remainingCode >= Size)
            {
                ...
                stack.Push2Bytes<TTracingInst>(ref Unsafe.Add(ref bytes, programCounter));
            }
        ...
        public struct Op4 : IOpCount
        {
                ...
                where TTracingInst : struct, IFlag
            {
                ...
                if (usedFromCode == Size)
                {
                    ...
                    stack.Push4Bytes<TTracingInst>(ref Unsafe.Add(ref bytes, programCounter));
                }
                else
                {
                    stack.PushLeftPaddedBytes<TTracingInst>(code.Slice(programCounter, usedFromCode), length);
        ...
        public struct Op8 : IOpCount
        {
                ...
                where TTracingInst : struct, IFlag
            {
                ...
                {
                    ref byte bytes = ref MemoryMarshal.GetReference(code);
                    stack.Push8Bytes<TTracingInst>(ref Unsafe.Add(ref bytes, programCounter));
                }
                else
                {
                    stack.PushLeftPaddedBytes<TTracingInst>(code.Slice(programCounter, usedFromCode), length);
        ...
        public struct Op16 : IOpCount
        {
                ...
                where TTracingInst : struct, IFlag
            {
                ...
                {
                    ref byte bytes = ref MemoryMarshal.GetReference(code);
                    stack.Push16Bytes<TTracingInst>(ref Unsafe.Add(ref bytes, programCounter));
                }
                else
                {
                    stack.PushLeftPaddedBytes<TTracingInst>(code.Slice(programCounter, usedFromCode), length);
        ...
        public struct Op20 : IOpCount
        {
                ...
                where TTracingInst : struct, IFlag
            {
                ...
                if (usedFromCode == Size)
                {
                    ...
                    ref byte bytes = ref MemoryMarshal.GetReference(code);
                    stack.Push20Bytes<TTracingInst>(ref Unsafe.Add(ref bytes, programCounter));
                }
                else
                {
                    stack.PushLeftPaddedBytes<TTracingInst>(code.Slice(programCounter, usedFromCode), length);
        ...
        public struct Op32 : IOpCount
        {
                ...
                where TTracingInst : struct, IFlag
            {
                ...
                if (usedFromCode == Size)
                {
                    ...
                    stack.Push32Bytes<TTracingInst>(in Unsafe.As<byte, Word>(ref Unsafe.Add(ref MemoryMarshal.GetReference(code), programCounter)));
                }
                else
                {
                    stack.PushLeftPaddedBytes<TTracingInst>(code.Slice(programCounter, usedFromCode), length);
