...
internal static partial class EvmInstructions
{
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        // Single state lookup - GetAccount returns TotallyEmpty for non-existent accounts.
        Account account = vm.WorldState.GetAccount(address);
        // For dead accounts (empty or non-existent), push zero per EIP-1052.
        // Otherwise, push the account's code hash.
        EvmExceptionType result = account.IsEmpty
            ? stack.PushZero<TTracingInst>()
            : stack.Push32Bytes<TTracingInst>(in account.CodeHash.ValueHash256);
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        return new(programCounter, stack.PushUInt64<TTracingInst>((ulong)TGasPolicy.GetRemainingGas(in gas)));
    // Jump forward to be unpredicted by the branch predictor.
    OutOfGas:
        return new(programCounter, EvmExceptionType.OutOfGas);
    }
