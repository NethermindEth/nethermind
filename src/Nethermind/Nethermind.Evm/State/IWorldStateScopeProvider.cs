...
public interface IWorldStateScopeProvider
{
    ...
    /// <param name="trackWitness">When <c>true</c>, the scope records every account/slot it reads and every
    /// code it serves, and exposes the resulting storage witness through <see cref="IScope.Witness"/>. Default
    /// <c>false</c> keeps the scope on its fast read path with no witness bookkeeping.</param>
