...
public class JsonRpcServiceTests
{
    ...
    {
        RpcModuleProvider moduleProvider = new(new FileSystem(), _configurationProvider.GetConfig<IJsonRpcConfig>(),
            LimboLogs.Instance);
        moduleProvider.Register(new SingletonModulePool<T>(new SingletonFactory<T>(module)));
        _jsonRpcService =
            new JsonRpcService(moduleProvider, _logManager, _configurationProvider.GetConfig<IJsonRpcConfig>());
    ...
    public void GetBlockByNumberTest()
    {
        ...
        ethRpcModule.eth_getBlockByNumber(Arg.Any<BlockParameter>(), true).ReturnsForAnyArgs(x =>
            ResultWrapper<BlockForRpc>.Success(new BlockForRpc(Build.A.Block.WithNumber(2).TestObject, true,
                specProvider)));
        JsonRpcSuccessResponse? response =
            TestRequest(ethRpcModule, "eth_getBlockByNumber", "0x1b4", "true") as JsonRpcSuccessResponse;
    ...
    public void CanHandleOptionalArguments()
    {
        ...
        IEthRpcModule ethRpcModule = Substitute.For<IEthRpcModule>();
        ethRpcModule.eth_call(Arg.Any<TransactionForRpc>())
            .ReturnsForAnyArgs(x => ResultWrapper<string>.Success("0x1"));
    ...
    public void GetNewFilterTest()
    {
        ...
        JsonRpcSuccessResponse? response =
            TestRequest(ethRpcModule, "eth_newFilter", JsonConvert.SerializeObject(parameters)) as
    ...
    public void Eth_call_is_working_with_implicit_null_as_the_last_argument()
    {
        EthereumJsonSerializer serializer = new();
        IEthRpcModule ethRpcModule = Substitute.For<IEthRpcModule>();
        ethRpcModule.eth_call(Arg.Any<TransactionForRpc>(), Arg.Any<BlockParameter?>())
            .ReturnsForAnyArgs(x => ResultWrapper<string>.Success("0x"));
    ...
    public void Eth_call_is_working_with_explicit_null_as_the_last_argument(string nullValue)
    {
        ...
        JsonRpcSuccessResponse? response =
            TestRequest(ethRpcModule, "eth_call", serialized, nullValue) as JsonRpcSuccessResponse;
        Assert.That(response?.Result, Is.EqualTo("0x"));
    }
    ...
    {
        IEthRpcModule ethRpcModule = Substitute.For<IEthRpcModule>();
        ethRpcModule.eth_getWork().ReturnsForAnyArgs(x =>
            ResultWrapper<IEnumerable<byte[]>>.Success(new[] { Bytes.FromHexString("aa"), Bytes.FromHexString("01") }));
    ...
    public void IncorrectMethodNameTest()
    {
        JsonRpcErrorResponse? response =
            TestRequest(Substitute.For<IEthRpcModule>(), "incorrect_method") as JsonRpcErrorResponse;
    ...
    public void Web3ShaTest()
    {
        ...
        web3RpcModule.web3_sha3(Arg.Any<byte[]>())
            .ReturnsForAnyArgs(x => ResultWrapper<Keccak>.Success(TestItem.KeccakA));
        JsonRpcSuccessResponse? response =
            TestRequest(web3RpcModule, "web3_sha3", "0x68656c6c6f20776f726c64") as JsonRpcSuccessResponse;
        Assert.That(response?.Result, Is.EqualTo(TestItem.KeccakA));
    }

    [TestCaseSource(nameof(BlockForRpcTestSource))]
    public void BlockForRpc_should_expose_withdrawals_if_any((bool Expected, Core.Block Block) item)
    {
        ISpecProvider? specProvider = Substitute.For<ISpecProvider>();
        BlockForRpc rpcBlock = new BlockForRpc(item.Block, false, specProvider);
