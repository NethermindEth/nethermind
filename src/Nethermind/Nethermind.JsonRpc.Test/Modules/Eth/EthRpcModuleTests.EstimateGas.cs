...
public partial class EthRpcModuleTests
{
    ...
    public async Task Eth_create_access_list_calculates_proper_gas(bool optimize, long loads)
    {
        ...
        transaction.AccessList = test.JsonSerializer.Deserialize<RpcAccessList>(JToken.Parse(serializedCreateAccessList).SelectToken("result.accessList")!.ToString());
        ...
        long gasPriceWithAccessList)
    {
        ...
        (byte[] code, RpcAccessList accessList) = GetTestAccessList(2, senderAccessList);
    ...
    public async Task Eth_estimate_gas_is_lower_with_optimized_access_list()
    {
        ...
        (byte[] code, RpcAccessList accessList) = GetTestAccessList(2, true);
        (byte[] _, RpcAccessList optimizedAccessList) = GetTestAccessList(2, false);
    ...
    public async Task Estimate_gas_with_base_fee_opcode()
    {
        ...
        var parameters = ctx.Test.JsonSerializer.Serialize(transaction);
        string serialized = await ctx.Test.TestEthRpc("eth_estimateGas", parameters);
        Assert.That(
    ...
    public async Task should_estimate_transaction_with_deployed_code_when_eip3607_enabled()
    {
        ...
        RpcLegacyTransaction transaction = new RpcLegacyTransaction(tx, 1, Keccak.Zero, 1L)
        {
