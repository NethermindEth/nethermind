...
public partial class EthRpcModuleTests
{
    ...
    public async Task Estimate_gas_with_base_fee_opcode()
    {
        ...
        string serialized = await ctx.Test.TestEthRpc("eth_estimateGas", transaction);
        Assert.That(
            serialized, Is.EqualTo("{\"jsonrpc\":\"2.0\",\"result\":\"0xe8a9\",\"id\":67}"));
    }
    ...
    [TestCase(
        ...
        """{"jsonrpc":"2.0","result":"0xac0d","id":67}""" // Store uint256 (cold access) + few other light instructions + intrinsic transaction cost
    )]
    [TestCase(
        ...
        """{"jsonrpc":"2.0","result":"0x6608","id":67}""" // ECRecover call + intrinsic transaction cost
    )]
    ...
    private static IEnumerable<TestCaseData> EstimateGasFloorCostCases()
    {
        ...
        const ulong belowFloor = GasCostOf.Transaction + GasCostOf.TxDataZero;
