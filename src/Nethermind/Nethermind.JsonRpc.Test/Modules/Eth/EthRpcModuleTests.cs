...
public partial class EthRpcModuleTests
{
    ...
    [Test]
    public async Task Eth_get_proof_explicit_null_block_argument_returns_invalid_params()
    {
        using Context ctx = await Context.Create();
        string serialized = await ctx.Test.TestEthRpc("eth_getProof", TestBlockchain.AccountA.ToString(), "[]", null);
        Assert.That(serialized, Is.EqualTo("{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"missing value for required argument 2\"},\"id\":67}"));
    }
