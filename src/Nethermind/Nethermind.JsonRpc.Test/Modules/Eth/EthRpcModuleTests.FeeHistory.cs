...
namespace Nethermind.JsonRpc.Test.Modules.Eth
{
    public partial class EthRpcModuleTests
    {
        ...
        public async Task Eth_feeHistory(string blockCount, string blockParameter, string rewardPercentiles, string expectedResult)
        {
            ...
            Transaction tx1FirstBlock = Build.A.Transaction.WithGasPrice(3).SignedAndResolved().TestObject; //Reward: Min (3, 3-2) => 1 
            Transaction tx2FirstBlock = Build.A.Transaction.WithGasPrice(4).SignedAndResolved().TestObject;
            Transaction tx1SecondBlock = Build.A.Transaction.WithGasPrice(2).SignedAndResolved().TestObject; //Reward: BaseFee > FeeCap => 0
            Transaction tx2SecondBlock = Build.A.Transaction.WithGasPrice(5).SignedAndResolved().TestObject;
