...
public class AdminModuleTests
{
    ...
    public void AdminPeers_WithoutEthHandler_UsesHighestAdvertisedEthCapabilityForProtocolInfo()
    {
        ...
        PeerInfo peerInfo = result.Data[0];
        peerInfo.Protocols.Should().ContainKeys(new[] { "eth", "snap" }, because: "both protocols are advertised");
        GetProtocolVersion(peerInfo.Protocols["eth"]).Should().Be(68, because: "fallback protocol info should use the highest advertised eth capability");
    }
    ...
    public void AdminPeers_WithNegotiatedEthHandler_UsesNegotiatedEthVersionForProtocolInfo()
    {
        ...
        PeerInfo peerInfo = result.Data[0];
        peerInfo.Protocols.Should().ContainKeys(new[] { "eth", "snap" }, because: "both protocols are advertised");
        GetProtocolVersion(peerInfo.Protocols["eth"]).Should().Be(72, because: "an active eth handler exposes the negotiated protocol version");
    }
