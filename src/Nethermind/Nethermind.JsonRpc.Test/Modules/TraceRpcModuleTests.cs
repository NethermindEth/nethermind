...
namespace Nethermind.JsonRpc.Test.Modules
{
    ...
    public class TraceRpcModuleTests
    {
        ...
        private async Task Initialize(bool auRa = false)
        {
            ...
            VirtualMachine virtualMachine = new(_stateProvider, storageProvider, new BlockhashProvider(blockTree, LimboLogs.Instance), specProvider, LimboLogs.Instance);
            TransactionProcessor txProcessor = new(specProvider, _stateProvider, storageProvider, virtualMachine, LimboLogs.Instance);
            ...
            BlockchainProcessor blockchainProcessor = new(blockTree, blockProcessor, signatureRecovery, LimboLogs.Instance, BlockchainProcessor.Options.Default);
            ...
            BlockBuilder genesisBlockBuilder = Build.A.Block.Genesis.WithStateRoot(new Keccak("0x1ef7300d8961797263939a3d29bbba4ccf1702fabf02d8ad7a20b454edb6fd2f"));
            if (auRa)
            {
        ...
        [Test]
        public void Trace_filter()
        {
            var traceFilterRequest = new TraceFilterForRpc();
            string serialized = RpcTest.TestSerializedRequest(EthModuleFactory.Converters.Union(TraceModuleFactory.Converters).ToList(), _traceRpcModule, "trace_filter", new EthereumJsonSerializer().Serialize(traceFilterRequest));
