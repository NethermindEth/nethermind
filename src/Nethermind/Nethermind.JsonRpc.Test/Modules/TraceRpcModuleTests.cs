    ...
    private class Context
    {
        public async Task Build(ISpecProvider? specProvider = null, bool isAura = false)
        {
            ...
            for (int i = 1; i < 10; i++)
            {
                ...
                for (int j = 0; j < i; j++)
                {
                    transactions.Add(Core.Test.Builders.Build.A.Transaction.WithNonce(Blockchain.State.GetNonce(TestItem.AddressB) + (UInt256)j).SignedAndResolved(Blockchain.EthereumEcdsa, TestItem.PrivateKeyB).TestObject);
                }
