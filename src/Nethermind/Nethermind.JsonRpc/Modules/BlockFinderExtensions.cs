    ...
    public static class BlockFinderExtensions
    {
        ...
        public static SearchResult<Block> SearchForBlock(this IBlockFinder blockFinder, BlockParameter? blockParameter, bool allowNulls = false)
        {
            ...
            if (block is null)
            {
                ...
                {
                    return new SearchResult<Block>($"Block {blockParameter.BlockHash?.ToString() ?? blockParameter.BlockNumber?.ToString() ?? blockParameter.Type.ToString()} pruned history unavailable", ErrorCodes.PrunedHistoryUnavailable);
                }
