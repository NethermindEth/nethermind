...
public class DebugBridge : IDebugBridge
{
    ...
    {
        var options = (gethTraceOptions ?? GethTraceOptions.Default) with { TxHash = transactionHash };

        return _tracer.Trace(blockRlp, transactionHash, options, cancellationToken);
    }
