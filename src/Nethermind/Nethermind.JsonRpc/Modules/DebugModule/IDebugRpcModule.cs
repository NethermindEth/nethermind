...
public interface IDebugRpcModule : IRpcModule
{
    ...
    [JsonRpcMethod(Description = "Deletes chain levels on all branches (Nethermind specific). Requires paused, drained processing. When replacing the head, the preceding block must be canonical with its body and processing state available. Returns a resource-unavailable error when deletion is refused.", IsImplemented = true, IsSharable = true)]
