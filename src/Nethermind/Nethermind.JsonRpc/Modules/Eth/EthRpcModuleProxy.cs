...
public class EthRpcModuleProxy : IEthRpcModule
{
    ...
    public ResultWrapper<FeeHistoryResults> eth_feeHistory(long blockCount, BlockParameter newestBlock,
    ...
    {
        return ResultWrapper<long?>.From(await _proxy.eth_blockNumber());
    }

    public async Task<ResultWrapper<UInt256?>> eth_getBalance(Address address, BlockParameter blockParameter)
    {
        return ResultWrapper<UInt256?>.From(await _proxy.eth_getBalance(address,
    ...
    {
        return ResultWrapper<UInt256>.From(
            await _proxy.eth_getTransactionCount(address, MapBlockParameter(blockParameter)));
    }
    ...
    public ResultWrapper<AccessListForRpc> eth_createAccessList(TransactionForRpc transactionCall,
        BlockParameter? blockParameter = null, bool optimize = true)
    {
        throw new NotSupportedException();
    }
    ...
    {
        if (blockParameter is null) return null;
        ...
        return new BlockParameterModel { Type = blockParameter.Type.ToString() };
    }
}
