...
public class SimulateTxExecutor<TTracer, TTrace>(IBlockchainBridge blockchainBridge, IBlockFinder blockFinder, IJsonRpcConfig rpcConfig, ulong? secondsPerSlot = null)
    : ExecutorBase<IReadOnlyList<SimulateBlockResult<TTrace>>, SimulatePayload<TransactionForRpc>,
    SimulatePayload<TransactionWithSourceDetails>>(blockchainBridge, blockFinder, rpcConfig) where TTracer : class, IBlockTracer<SimulateBlockResult<TTrace>>
{
    ...
    {
        SimulateOutput<TTrace>? results = _blockchainBridge.Simulate<TTracer, TTrace>(header, tx, token);
