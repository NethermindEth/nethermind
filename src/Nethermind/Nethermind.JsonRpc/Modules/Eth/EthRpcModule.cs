...
public partial class EthRpcModule : IEthRpcModule
{
    ...
    public ResultWrapper<UInt256?> eth_getBlockTransactionCountByHash(Commitment blockHash)
    {
    ...
    public ResultWrapper<UInt256?> eth_getUncleCountByBlockHash(Commitment blockHash)
    {
        SearchResult<Block> searchResult = _blockFinder.SearchForBlock(new BlockParameter(blockHash));
        return searchResult.IsError
            ? GetFailureResult<UInt256?, Block>(searchResult, _ethSyncingInfo.SyncMode.HaveNotSyncedBodiesYet())
    ...
    public Task<ResultWrapper<Commitment>> eth_sendTransaction(TransactionForRpc rpcTx)
    {
    ...
    public async Task<ResultWrapper<Commitment>> eth_sendRawTransaction(byte[] transaction)
    {
        try
        {
        ...
        {
            return ResultWrapper<Commitment>.Fail("Invalid RLP.", ErrorCodes.TransactionRejected);
        }
    }

    private async Task<ResultWrapper<Commitment>> SendTx(Transaction tx,
        TxHandlingOptions txHandlingOptions = TxHandlingOptions.None)
    {
        try
        {
            (Commitment txHash, AcceptTxResult? acceptTxResult) =
            ...
            return acceptTxResult.Equals(AcceptTxResult.Accepted)
                ? ResultWrapper<Commitment>.Success(txHash)
                : ResultWrapper<Commitment>.Fail(acceptTxResult?.ToString() ?? string.Empty, ErrorCodes.TransactionRejected);
        }
        catch (SecurityException e)
        {
            return ResultWrapper<Commitment>.Fail(e.Message, ErrorCodes.AccountLocked);
        }
        catch (Exception e)
        {
            if (_logger.IsError) _logger.Error("Failed to send transaction.", e);
            return ResultWrapper<Commitment>.Fail(e.Message, ErrorCodes.TransactionRejected);
        }
    }
    ...
    public ResultWrapper<BlockForRpc> eth_getBlockByHash(Commitment blockHash, bool returnFullTransactionObjects)
    {
    ...
    public Task<ResultWrapper<TransactionForRpc>> eth_getTransactionByHash(Commitment transactionHash)
    {
    ...
    public ResultWrapper<TransactionForRpc> eth_getTransactionByBlockHashAndIndex(Commitment blockHash,
    ...
    public Task<ResultWrapper<ReceiptForRpc>> eth_getTransactionReceipt(Commitment txHash)
    {
    ...
    public ResultWrapper<BlockForRpc> eth_getUncleByBlockHashAndIndex(Commitment blockHash, UInt256 positionIndex)
    {
