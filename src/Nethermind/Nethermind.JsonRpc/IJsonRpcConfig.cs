...
public interface IJsonRpcConfig : IConfig
{
    [ConfigItem(
        Description =
            "Defines whether the JSON RPC service is enabled on node startup. Configure host and port if default values do not work for you.",
        DefaultValue = "false")]
    bool Enabled { get; set; }

    [ConfigItem(
        Description =
            "Host for JSON RPC calls. Ensure the firewall is configured when enabling JSON RPC. If it does not work with 127.0.0.1 try something like 10.0.0.4 or 192.168.0.1",
    ...
    [ConfigItem(
        Description =
            "Defines which RPC modules should be enabled. Built in modules are: Admin, Clique, Consensus, Db, Debug, Deposit, Erc20, Eth, Evm, Health Mev, NdmConsumer, NdmProvider, Net, Nft, Parity, Personal, Proof, Subscribe, Trace, TxPool, Vault, Web3.",
        DefaultValue = "[Eth, Subscribe, Trace, TxPool, Web3, Personal, Proof, Net, Parity, Health, Rpc]")]
    string[] EnabledModules { get; set; }

    [ConfigItem(
        Description =
            "Defines additional RPC urls to listen on. Example url format: http://localhost:8550|http;wss|engine;eth;net;subscribe",
        DefaultValue = "[]")]
    ...
    [ConfigItem(
        Description = "Gas limit multiplier for eth_multicall. uint256 = GasCap * value",
        DefaultValue = "2")]
    ulong? GasCapMultiplier { get; set; }
    ...
    [ConfigItem(
        Description =
            "Buffer responses before sending them to client. This allows to set Content-Length in response instead of using Transfer-Encoding: chunked. This may degrade performance on big responses. Max buffered response size is 2GB, chunked responses can be bigger.",
        DefaultValue = "false")]
    ...
    [ConfigItem(Description = "Path to file with hex encoded secret for jwt authentication",
        DefaultValue = "keystore/jwt-secret")]
    public string JwtSecretFile { get; set; }

    [ConfigItem(
        Description =
            "It shouldn't be set to true for production nodes. If set to true all modules can work without RPC authentication.",
    ...
    [ConfigItem(
        Description =
            "Defines method names of Json RPC service requests to NOT log. Example: {\"eth_blockNumber\"} will not log \"eth_blockNumber\" requests.",
        DefaultValue =
    ...
    [ConfigItem(
        Description =
            "Host for Execution Engine calls. Ensure the firewall is configured when enabling JSON RPC. If it does not work with 127.0.0.1 try something like 10.0.0.4 or 192.168.0.1",
        DefaultValue = "\"127.0.0.1\"")]
    ...
    [ConfigItem(
        Description =
            "Defines which RPC modules should be enabled Execution Engine port. Built in modules are: Admin, Clique, Consensus, Db, Debug, Deposit, Erc20, Eth, Evm, Health Mev, NdmConsumer, NdmProvider, Net, Nft, Parity, Personal, Proof, Subscribe, Trace, TxPool, Vault, Web3.",
