...
public sealed class JsonRpcService(IRpcModuleProvider rpcModuleProvider, ILogManager logManager, IJsonRpcConfig jsonRpcConfig) : IJsonRpcService
{
    ...
    private JsonRpcErrorResponse HandleInvocationException(Exception ex, string methodName, JsonRpcRequest request, Action? returnAction)
    {
        return ex switch
        {
            ...
            InvalidTransactionException { Reason.ErrorDescription: var description } =>
                GetErrorResponse(methodName, ErrorCodes.Default, description, null, request.Id, returnAction),

            { InnerException: InvalidTransactionException { Reason.ErrorDescription: var description } } =>
                GetErrorResponse(methodName, ErrorCodes.Default, description, null, request.Id, returnAction),
            ...
            MissingTrieNodeException { Message: var message } =>
                GetErrorResponse(methodName, ErrorCodes.ResourceUnavailable, message, ex.ToString(), request.Id, returnAction),

            TargetInvocationException { InnerException: MissingTrieNodeException { Message: var message } } =>
                GetErrorResponse(methodName, ErrorCodes.ResourceUnavailable, message, ex.ToString(), request.Id, returnAction),
