...
{
    public class VerkleTxPermissionFilterTest
    {
        ...
        public async Task<(bool IsAllowed, bool Cache)> V1(Func<Task<VerkleTestTxPermissionsBlockchain>> chainFactory, Transaction tx) => await ChainTest(chainFactory, tx, 1);
        ...
        public async Task<(bool IsAllowed, bool Cache)> V2(Func<Task<VerkleTestTxPermissionsBlockchain>> chainFactory, Transaction tx) => await ChainTest(chainFactory, tx, 2);
        ...
        public async Task<(bool IsAllowed, bool Cache)> V3(Func<Task<VerkleTestTxPermissionsBlockchain>> chainFactory, Transaction tx) => await ChainTest(chainFactory, tx, 3);
        ...
        public async Task<(bool IsAllowed, bool Cache)> V4(Func<Task<VerkleTestTxPermissionsBlockchain>> chainFactory, Transaction tx) => await ChainTest(chainFactory, tx, 4);
        private static async Task<(bool IsAllowed, bool Cache)> ChainTest(Func<Task<VerkleTestTxPermissionsBlockchain>> chainFactory, Transaction tx, UInt256 version)
        {
            using VerkleTestTxPermissionsBlockchain chain = await chainFactory();
        ...
        private static IEnumerable<TestCaseData> GetTestCases(IEnumerable<Test> tests, string testsName, Func<Test, ITransactionPermissionContract.TxPermissions, TransactionBuilder<Transaction>> transactionBuilder)
        {
            TestCaseData GetTestCase(
                Func<Task<VerkleTestTxPermissionsBlockchain>> chainFactory,
                Test test,
            ...
            foreach (Test test in tests)
            {
                ...
                {
                    Task<VerkleTestTxPermissionsBlockchain> chainTask = VerkleTestContractBlockchain.ForTest<VerkleTestTxPermissionsBlockchain, VerkleTxPermissionFilterTest>(testsName);
                    Func<Task<VerkleTestTxPermissionsBlockchain>> testFactory = async () =>
                    {
                        VerkleTestTxPermissionsBlockchain chain = await chainTask;
        ...
        public class VerkleTestTxPermissionsBlockchain : VerkleTestContractBlockchain
        {
            ...
            protected override BlockProcessor CreateBlockProcessor()
            {
                ...
                IReadOnlyTxProcessorSource txProcessorSource = new VerkleReadOnlyTxProcessingEnv(
                    DbProvider,
                ...
                VersionedTransactionPermissionContract transactionPermissionContract = new(AbiEncoder.Instance, _contractAddress, 1,
                    new VerkleReadOnlyTxProcessingEnv(DbProvider, TrieStore.AsReadOnly(), BlockTree, SpecProvider, LimboLogs.Instance), TransactionPermissionContractVersions, LimboLogs.Instance, SpecProvider);
