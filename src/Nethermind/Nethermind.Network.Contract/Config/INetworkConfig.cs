...
public interface INetworkConfig : IConfig
{
    ...
    /// supplies an override.
    /// </remarks>
    [ConfigItem(Description = "The local IP for inbound listeners. When unset, listeners try a dual-stack wildcard and fall back to IPv4 if it cannot bind. Set to `0.0.0.0` for IPv4-only, `::` for explicit dual-stack, or a specific address to restrict listeners to that address and family.", DefaultValue = "null")]
