...
namespace Nethermind.Facade.Test
{
    public class BlockchainBridgeTests
    {
        ...
        [Test]
        public void Sum_of_previous_log_indexes()
        {
            ReadOnlyTxProcessingEnv processingEnv = new ReadOnlyTxProcessingEnv(
                new ReadOnlyDbProvider(_dbProvider, false),
                new TrieStore(_dbProvider.StateDb, LimboLogs.Instance).AsReadOnly(), 
            ...
            EthereumEcdsa ethereumEcdsa = new(_specProvider.ChainId, LimboLogs.Instance);
            ...
            Transaction transaction2 = Build.A.Transaction.Signed(ethereumEcdsa, TestItem.PrivateKeyD, false)
                .WithSenderAddress(Address.FromNumber((UInt256)blockNumber))
            ...
            processingEnv.BlockTree.FindBlock(Arg.Any<Keccak>(), Arg.Any<BlockTreeLookupOptions>()).Returns(block);

            TxReceipt[] receipts = {receipt1, receipt2};
            _receiptStorage.FindBlockHash(Arg.Any<Keccak>()).Returns(TestItem.KeccakA);
            _receiptStorage.Get(Arg.Any<Keccak>()).Returns(receipts);
            _receiptStorage.Get(Arg.Any<Block>()).Returns(receipts);

            _blockchainBridge.GetReceiptAndEffectiveGasPrice(transaction2.Hash).SumOfPreviousLogIndexes.Should().Be(2);
        }
