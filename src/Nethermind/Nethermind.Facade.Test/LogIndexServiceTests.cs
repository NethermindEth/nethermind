...
public class LogIndexServiceTests
{
    ...
    private LogIndexService GetService(ILogIndexStorage logIndexStorage)
    {
        return new LogIndexService(
            logIndexStorage, _config, _blockTree, _syncConfig, _receiptFinder, _receiptStorage, _logManager
    ...
    )
    {
        ...
        LogIndexService service = GetService(storage);
    ...
    )
    {
        ...
        LogIndexService service = GetService(new FailingLogIndexStorage(failAfter, exception));

        await service.StartAsync();

        using (Assert.EnterMultipleScope())
        {
            Exception thrown = Assert.ThrowsAsync<Exception>(() => service.BackwardSyncCompletion.WaitAsync(TimeSpan.FromSeconds(999)));
            Assert.That(thrown, Is.EqualTo(exception));
            Assert.That(service.LastError, Is.EqualTo(exception));
        }
    }
    ...
    )
    {
        ...
        _syncConfig.AncientReceiptsBarrier = minBarrier;
        LogIndexService service = GetService(new FailingLogIndexStorage(0, new("Should not set new receipts."))
        {
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(service.BackwardSyncCompletion.IsCompleted);
            Assert.That(service.LastError, Is.Null);
            Assert.That(service.LastUpdate, Is.Null);
        }
    }
