...
public interface ITxPoolConfig : IConfig
{
    ...
    [ConfigItem(DefaultValue = "2", Description = "EIP-8141: the number of *consecutive* chain heads a pending frame transaction whose revalidation spent this node's own simulation bounds may be carried across before the pool stops re-queuing it. The count lives only on the previous head, so any head that revalidates the transaction without deferring it again clears the count: one that exhausts the budget and is later revalidated because a block names its dependencies is granted a fresh budget. That still bounds what was unbounded, the carry feeding itself, because each re-arming costs a block that genuinely touched the transaction's dependencies and would have triggered a revalidation anyway. Each carried transaction costs one validation-prefix simulation per head, run while the pool holds its head write lock, so an unbounded carry turns a backlog larger than `TxPool.FrameTxSimulationBudgetPerHeadMs` can clear into a permanent per-head stall. Exhausting the budget neither evicts nor approves: the transaction stays pending and unjudged until a later block names its dependencies, which is the standing a one-off dependency change already leaves it in. `0` stops re-queuing entirely.")]
