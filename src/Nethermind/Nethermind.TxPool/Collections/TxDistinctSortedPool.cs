...
namespace Nethermind.TxPool.Collections
{
    public class TxDistinctSortedPool : DistinctValueSortedPool<Keccak, Transaction, Address>
    {
        public TxDistinctSortedPool(int capacity, IComparer<Transaction> comparer = null) 
            : base(capacity, comparer ?? GasBasedTxComparer.Instance, CompetingTransactionEqualityComparer.Instance)
        {
        }

        protected override IComparer<Transaction> GetComparerWithIdentity(IComparer<Transaction> comparer) =>
            TxSortedPool.GetTxComparerWithIdentity(comparer);
