...
internal sealed class NotSupportedTxFilter(ITxPoolConfig txPoolConfig, IChainHeadSpecProvider specProvider, ILogger logger) : IIncomingTxFilter
{
    ...
    public AcceptTxResult Accept(Transaction tx, ref TxFilteringState state, TxHandlingOptions txHandlingOptions)
    {
        ...
        // max_cost helper) and for blob-pool records restored from disk, whose payer and paymaster are not
        // persisted so they hold neither a reservation nor a cap slot, re-counting the cap when a pay
