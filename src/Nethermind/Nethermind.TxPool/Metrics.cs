...
namespace Nethermind.TxPool
{
    public static class Metrics
    {
        ...
        [CounterMetric]
        [Description("Number of pending transaction retry handlers rejected because a configured retry admission limit was reached.")]
        ...
        [KeyIsLabel("client")]
        public static NonBlocking.ConcurrentDictionary<string, long> NewPooledTransactionsAnnouncedByClient { get; } = new();

        [CounterMetric]
        [Description("Number of pending transactions requested from peers, grouped by peer client.")]
        [KeyIsLabel("client")]
        public static NonBlocking.ConcurrentDictionary<string, long> NewPooledTransactionsRequestedByClient { get; } = new();
        ...
        public static NonBlocking.ConcurrentDictionary<(string Client, string Reason), long> NewPooledTransactionsRequestedByClientAndReason { get; } = new();

        [CounterMetric]
        [Description("Number of pooled transaction request messages sent to peers, grouped by peer client and request reason.")]
        [KeyIsLabel("client", "reason")]
        public static NonBlocking.ConcurrentDictionary<(string Client, string Reason), long> NewPooledTransactionRequestMessagesByClientAndReason { get; } = new();

        [CounterMetric]
        [Description("Raw number of transaction payloads returned by peers, including duplicate or unsolicited transactions, grouped by peer client.")]
        [KeyIsLabel("client")]
        public static NonBlocking.ConcurrentDictionary<string, long> NewPooledTransactionsReturnedByClient { get; } = new();

        [CounterMetric]
        [Description("Raw number of pooled transaction response messages received from peers, grouped by peer client.")]
        [KeyIsLabel("client")]
        public static NonBlocking.ConcurrentDictionary<string, long> NewPooledTransactionResponseMessagesByClient { get; } = new();

        [CounterMetric]
        [Description("Number of empty pooled transaction response messages received from peers, grouped by peer client.")]
        [KeyIsLabel("client")]
        public static NonBlocking.ConcurrentDictionary<string, long> NewPooledTransactionEmptyResponseMessagesByClient { get; } = new();
        ...
        private static void AddBy<TKey>(NonBlocking.ConcurrentDictionary<TKey, long> metric, TKey key, long count)
            where TKey : notnull
        {
            if (count > 0)
            {
                metric.AddBy(key, count);
            }
        }
    }
