...
namespace Nethermind.TxPool
{
    ...
    internal class HashCache
    {
        ...
        private readonly LruKeyCache<ValueCommitment> _longTermCache = new(
            MemoryAllowance.TxHashCacheSize,
            Math.Min(SafeCapacity, MemoryAllowance.TxHashCacheSize),
            "long term hash cache");

        private readonly LruKeyCache<ValueCommitment> _currentBlockCache = new(
            SafeCapacity,
            Math.Min(SafeCapacity, MemoryAllowance.TxHashCacheSize),
            "current block hash cache");

        public bool Get(Commitment hash)
        {
            return _currentBlockCache.Get(hash) || _longTermCache.Get(hash);
        }

        public void SetLongTerm(Commitment hash)
        {
            _longTermCache.Set(hash);
        }

        public void SetForCurrentBlock(Commitment hash)
        {
            _currentBlockCache.Set(hash);
        }

        public void DeleteFromLongTerm(Commitment hash)
        {
            _longTermCache.Delete(hash);
        }

        public void Delete(Commitment hash)
        {
            _longTermCache.Delete(hash);
