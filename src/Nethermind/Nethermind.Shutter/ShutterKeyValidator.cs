    ...
    ILogManager logManager) : IShutterKeyValidator
{
    ...
    public IShutterKeyValidator.ValidatedKeys? ValidateKeys(Dto.DecryptionKeys decryptionKeys)
    {
        lock (_lockObject)
        {
            ...
            {
                if (_logger.IsInfo) _logger.Info($"Skipping Shutter decryption keys from slot {decryptionKeys.Gnosis.Slot}, keys currently stored for slot {_highestValidatedSlot}.");
                return null;
            }
            ...
            if (eonInfo is null)
            {
                if (_logger.IsInfo) _logger.Info("Cannot check Shutter decryption keys, eon info was not found.");
                return null;
            }
    ...
    private bool CheckDecryptionKeys(in Dto.DecryptionKeys decryptionKeys, in IShutterEon.Info eonInfo)
    {
        ...
        {
            if (_logger.IsInfo) _logger.Info($"Invalid Shutter decryption keys received: instanceID {decryptionKeys.InstanceID} did not match expected value {_instanceId}.");
            return false;
        }

        if (decryptionKeys.Eon != eonInfo.Eon)
        {
            if (_logger.IsInfo) _logger.Info($"Invalid Shutter decryption keys received: eon {decryptionKeys.Eon} did not match expected value {eonInfo.Eon}.");
            return false;
        }

        if (decryptionKeys.Keys.Count == 0)
        {
            if (_logger.IsInfo) _logger.Error("Invalid Shutter decryption keys received: expected placeholder key.");
            return false;
        }
        ...
        foreach (Dto.Key key in decryptionKeys.Keys.AsEnumerable().Skip(1))
        {
            ...
            {
                if (_logger.IsInfo) _logger.Error("Invalid Shutter decryption keys received.", e);
                return false;
            }

            if (!ShutterCrypto.CheckDecryptionKey(dk, new(eonInfo.Key), identity))
            {
                if (_logger.IsInfo) _logger.Info("Invalid Shutter decryption keys received: decryption key did not match eon key.");
                return false;
            }
        }
        ...
        {
            if (_logger.IsInfo) _logger.Info("Invalid Shutter decryption keys received: incorrect number of signer indices.");
            return false;
        }

        if (decryptionKeys.Gnosis.Signatures.Count != signerIndicesCount)
        {
            if (_logger.IsInfo) _logger.Info("Invalid Shutter decryption keys received: incorrect number of signatures.");
            return false;
        }

        if (signerIndicesCount != (int)eonInfo.Threshold)
        {
            if (_logger.IsInfo) _logger.Info($"Invalid Shutter decryption keys received: signer indices did not match threshold.");
            return false;
        }
        ...
        foreach ((ulong signerIndex, ByteString signature) in decryptionKeys.Gnosis.SignerIndices.Zip(decryptionKeys.Gnosis.Signatures))
        {
            ...
            {
                if (_logger.IsInfo) _logger.Info($"Invalid Shutter decryption keys received: bad signature.");
                return false;
            }
        }
