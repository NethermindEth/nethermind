...
namespace Nethermind.Store.Test
{
    ...
    public class StateReaderTests
    {
        ...
        public async Task Can_ask_about_balance_in_parallel()
        {
            ...
            Commitment stateRoot0 = provider.StateRoot;

            provider.AddToBalance(_address1, 1, spec);
            provider.Commit(spec);
            provider.CommitTree(0);
            Commitment stateRoot1 = provider.StateRoot;

            provider.AddToBalance(_address1, 1, spec);
            provider.Commit(spec);
            provider.CommitTree(0);
            Commitment stateRoot2 = provider.StateRoot;

            provider.AddToBalance(_address1, 1, spec);
            provider.Commit(spec);
            provider.CommitTree(0);
            Commitment stateRoot3 = provider.StateRoot;
        ...
        private Task StartStorageTask(StateReader reader, Commitment stateRoot, StorageCell storageCell, byte[] value)
        {
            return Task.Run(
                () =>
                {
                    ...
                    {
                        Commitment storageRoot = reader.GetStorageRoot(stateRoot, storageCell.Address);
