...
namespace Nethermind.Store.Test
{
    ...
    public class StateReaderTests
    {
        ...
        public async Task Can_ask_about_balance_in_parallel()
        {
            ...
            provider.Commit(spec);
            provider.CommitTree(0);
            BlockHeader baseBlock1 = Build.A.BlockHeader.WithParent(baseBlock0).WithStateRoot(provider.StateRoot).TestObject;

            provider.AddToBalance(_address1, 1, spec);
            provider.Commit(spec);
            provider.CommitTree(0);
            BlockHeader baseBlock2 = Build.A.BlockHeader.WithParent(baseBlock1).WithStateRoot(provider.StateRoot).TestObject;

            provider.AddToBalance(_address1, 1, spec);
            provider.Commit(spec);
            provider.CommitTree(0);
            BlockHeader baseBlock3 = Build.A.BlockHeader.WithParent(baseBlock2).WithStateRoot(provider.StateRoot).TestObject;
