...
public class StorageProviderTests(bool useFlat)
{
    ...
    public void Cancelled_storage_reporting_does_not_leak_clears_into_the_next_commit([Values] bool cancelOnRead)
    {
        ...
        ReadCollectingStorageTracer next = new();
        provider.Commit(Frontier.Instance, next);

        Assert.That(next.Clears, Is.Empty);
    }
    ...
    public void StorageClearedMetric_CountsOnlyCommittedClearOfNonEmptyStorage(bool markStorageDestroyed)
    {
        ...
        provider.Set(persistedCell, Value(1));
        provider.Commit(Frontier.Instance);
        long storageClearedAfterEmptyStorageReadAndWrite = Db.Metrics.StorageCleared;

        provider.Set(new StorageCell(ctx.Address2, 1), Value(1));
        ...
        provider.Set(freshCell, Value(1));
        provider.Commit(Frontier.Instance);
    ...
    public void StorageClearedMetric_CountsEachContractInParallelFlush()
    {
        ...
        foreach (Address address in addresses)
            provider.Set(new StorageCell(address, 1), Value(1));
