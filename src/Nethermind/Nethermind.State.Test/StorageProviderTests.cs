...
namespace Nethermind.Store.Test
{
    ...
    public class StorageProviderTests
    {
        ...
        public void Same_address_same_index_different_values_restore(int snapshot)
        {
            using Context ctx = new();
            WorldState provider = BuildStorageProvider(ctx);
            provider.Set(new StorageCell(ctx.Address1, 1), _values[1].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address1, 1), _values[2].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address1, 1), _values[3].ToEvmWord());
        ...
        public void Same_address_different_index(int snapshot)
        {
            ...
            WorldState provider = BuildStorageProvider(ctx);
            provider.Set(new StorageCell(ctx.Address1, 1), _values[1].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address1, 2), _values[2].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address1, 3), _values[3].ToEvmWord());
            provider.Restore(Snapshot.EmptyPosition, snapshot, Snapshot.EmptyPosition);
        ...
        public void Commit_restore()
        {
            ...
            provider.Set(new StorageCell(ctx.Address1, 3), _values[3].ToEvmWord());
            provider.Commit(Frontier.Instance);
            provider.Set(new StorageCell(ctx.Address2, 1), _values[4].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address2, 2), _values[5].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address2, 3), _values[6].ToEvmWord());
            provider.Commit(Frontier.Instance);
            provider.Set(new StorageCell(ctx.Address1, 1), _values[7].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address1, 2), _values[8].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address1, 3), _values[9].ToEvmWord());
            provider.Commit(Frontier.Instance);
            provider.Set(new StorageCell(ctx.Address2, 1), _values[10].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address2, 2), _values[11].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address2, 3), _values[12].ToEvmWord());
            provider.Commit(Frontier.Instance);
            provider.Restore(Snapshot.Empty);
        ...
        public void Commit_trees_clear_caches_get_previous_root()
        {
            ...
            storageProvider.Set(new StorageCell(ctx.Address1, 1), _values[1].ToEvmWord());
            ...
            storageProvider.Set(new StorageCell(ctx.Address1, 1), _values[2].ToEvmWord());
            storageProvider.Commit(Frontier.Instance);
            ctx.StateProvider.Commit(Frontier.Instance);
        ...
        public void Can_commit_when_exactly_at_capacity_regression()
        {
            ...
            {
                storageProvider.Set(new StorageCell(ctx.Address1, 1), _values[i % 2].ToEvmWord());
            }
        ...
        public void Can_tload_uninitialized_locations()
        {
            ...
            provider.SetTransientState(new StorageCell(ctx.Address1, 2), _values[1].ToEvmWord());
            Assert.True(provider.GetTransientState(new StorageCell(ctx.Address1, 1)).IsZero());
        ...
        public void Tload_same_address_same_index_different_values_restore(int snapshot)
        {
            ...
            provider.SetTransientState(new StorageCell(ctx.Address1, 1), _values[1].ToEvmWord());
            snapshots[1] = provider.TakeSnapshot();
            provider.SetTransientState(new StorageCell(ctx.Address1, 1), _values[2].ToEvmWord());
            snapshots[2] = provider.TakeSnapshot();
            provider.SetTransientState(new StorageCell(ctx.Address1, 1), _values[3].ToEvmWord());
        ...
        public void Transient_state_restores_independent_of_persistent_state(int snapshot)
        {
            ...
            // Update both
            provider.SetTransientState(new StorageCell(ctx.Address1, 1), _values[2].ToEvmWord());
            provider.Set(new StorageCell(ctx.Address1, 1), _values[9].ToEvmWord());
            snapshots[2] = provider.TakeSnapshot();

            // Only update persistent
            provider.Set(new StorageCell(ctx.Address1, 1), _values[8].ToEvmWord());
            snapshots[3] = provider.TakeSnapshot();
        ...
        public void Persistent_state_restores_independent_of_transient_state(int snapshot)
        {
            ...
            // Update both
            provider.Set(new StorageCell(ctx.Address1, 1), _values[2].ToEvmWord());
            provider.SetTransientState(new StorageCell(ctx.Address1, 1), _values[9].ToEvmWord());
            snapshots[2] = (provider).TakeSnapshot();

            // Only update transient
            provider.SetTransientState(new StorageCell(ctx.Address1, 1), _values[8].ToEvmWord());
            snapshots[3] = (provider).TakeSnapshot();
        ...
        private class Context : IDisposable
        {
            ...
            public Context()
            {
                StateProvider = new WorldState(_stateDb, Substitute.For<IDb>(), LogManager);
