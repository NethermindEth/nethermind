...
public class StateProviderTests(bool useFlat)
{
    ...
    private class Context : IDisposable
    {
        ...
        public Context(bool useFlat, IParentHeaderProvider parentHeaderProvider, ILogManager? logManager = null)
        {
            logManager ??= Logger;
            if (useFlat)
            {
                (IWorldStateScopeProvider scopeProvider, IContainer container) = TestWorldStateFactory.CreateFlatScopeProvider(parentHeaderProvider);
    ...
    {
        using Context ctx = new(useFlat, UnavailableParentHeaderProvider.Instance);
    ...
    public void Updating_code_hash_is_not_logged_at_debug()
    {
        ...
        using Context ctx = new(useFlat, UnavailableParentHeaderProvider.Instance, new OneLoggerLogManager(new ILogger(logger)));
        IWorldState provider = ctx.WorldState;
        using IDisposable _ = provider.BeginScope(IWorldState.PreGenesis);
    ...
    public void Same_code_can_be_redeployed_across_overlay_resets()
    {
        ...
        if (useFlat)
        {
            (_, IContainer container) = TestWorldStateFactory.CreateFlatScopeProvider(UnavailableParentHeaderProvider.Instance);
