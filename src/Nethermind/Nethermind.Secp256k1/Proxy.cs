...
namespace Nethermind.Secp256k1
{
    ...
    public static class Proxy
    {
        ...
        static Proxy()
        {
            Context = secp256k1_context_create(Secp256K1ContextSign | Secp256K1ContextVerify);
        }
        ...
        public static byte[] GetPublicKey(byte[] privateKey, bool compressed)
        {
            ...
            if (!secp256k1_ec_pubkey_serialize(Context, serializedPublicKey, ref outputSize, publicKey, flags))
            {
                return null;
            }
        ...
        public static byte[] SignCompact(byte[] messageHash, byte[] privateKey, out int recoveryId)
        {
            ...
            recoveryId = 0;
            if (!secp256k1_ecdsa_sign_recoverable(Context, recoverableSignature, messageHash, privateKey, IntPtr.Zero, IntPtr.Zero))
            {
                return null;
            }

            if (!secp256k1_ecdsa_recoverable_signature_serialize_compact(Context, compactSignature, out recoveryId, recoverableSignature))
            {
                return null;
            }
        ...
        public static byte[] RecoverKeyFromCompact(byte[] messageHash, byte[] compactSignature, int recoveryId, bool compressed)
        {
            ...
            byte[] recoverableSignature = new byte[65];
            if (!secp256k1_ecdsa_recoverable_signature_parse_compact(Context, recoverableSignature, compactSignature, recoveryId))
            {
                return null;
            }

            if (!secp256k1_ecdsa_recover(Context, publicKey, recoverableSignature, messageHash))
            {
                return null;
            }
