...
public class Always : IBlockValidator, ISealValidator, IUnclesValidator, ITxValidator
{
    ...
    public bool ValidateWithdrawals(Block block, [NotNullWhen(false)] out string? error) => Validate(out error);
    ...
    private bool Validate([NotNullWhen(false)] out string? error)
    {
