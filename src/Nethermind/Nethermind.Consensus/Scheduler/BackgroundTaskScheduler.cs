...
public class BackgroundTaskScheduler : IBackgroundTaskScheduler, IAsyncDisposable
{
    ...
    public bool TryScheduleTask<TReq>(in TReq request, Func<TReq, CancellationToken, Task> fulfillFunc, TimeSpan? timeout = null, string? source = null) where TReq : notnull
    {
        ...
        if (_logger.IsWarn && now - lastLog > 10_000 && Interlocked.CompareExchange(ref _lastDropLogTicks, now, lastLog) == lastLog)
        {
            _logger.Warn(
                ...
                $"Stats: {string.Join(", ", _stats.Where(static kv => kv.Value > 0).Select(static kv => $"({kv.Key}: {kv.Value})"))}");
        }
        Interlocked.Decrement(ref _queueCount);
        request.TryDispose();
        activity.Return();
    ...
    private void IncrementStats<TReq>(TReq request) where TReq : notnull =>
        _stats.AddOrUpdate(request.ToString() ?? typeof(TReq).Name, 1, static (_, value) => value + 1);

    private void DecrementStats(IActivity activity) =>
        _stats.AddOrUpdate(activity.ToString() ?? string.Empty, 0, static (_, value) => value - 1);
