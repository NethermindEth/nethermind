...
public abstract class GethStyleTracerBase(
    ...
    IOverridableEnv<GethStyleTracerBase.BlockProcessingComponents> blockProcessingEnv
) : IGethStyleTracer
{
    ...
    public GethLikeTxTrace? Trace(long blockNumber, Transaction tx, GethTraceOptions options, CancellationToken cancellationToken)
    {
        ...
        IBlockTracer<GethLikeTxTrace> blockTracer = CreateOptionsTracerInternal(block.Header, options with { TxHash = tx.Hash }, scope.Component.WorldState, specProvider);
        try
        {
        ...
        ProcessingOptions processingOptions = ProcessingOptions.Trace)
    {
        ...
        IBlockTracer<GethLikeTxTrace> tracer = CreateOptionsTracerInternal(block.Header, options with { TxHash = txHash }, scope.Component.WorldState, specProvider);
    ...
    protected abstract IBlockTracer<GethLikeTxTrace> CreateOptionsTracerInternal(BlockHeader block, GethTraceOptions options, IWorldState worldState, ISpecProvider specProvider);
    ...
    private IReadOnlyCollection<GethLikeTxTrace> TraceBlockImpl(Block? block, GethTraceOptions options, CancellationToken cancellationToken)
    {
        ...
        IBlockTracer<GethLikeTxTrace> tracer = CreateOptionsTracerInternal(block.Header, options, scope.Component.WorldState, specProvider);
        try
        {
            scope.Component.BlockchainProcessor.Process(block, ProcessingOptions.Trace, tracer.WithCancellation(cancellationToken), cancellationToken);
