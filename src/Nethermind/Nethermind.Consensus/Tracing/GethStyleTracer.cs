...
) : IGethStyleTracer
{
    ...
    public GethLikeTxTrace? Trace(ulong blockNumber, Transaction tx, GethTraceOptions options, CancellationToken cancellationToken)
    {
        ...
        try
        {
            scope.Component.BlockchainProcessor.Process(block, Tracer.ReadOnlyReplay, blockTracer.WithCancellation(cancellationToken), cancellationToken);
    ...
    public IReadOnlyCollection<Hash256> TraceBlockIntermediateRoots(Hash256 blockHash, GethTraceOptions options, CancellationToken cancellationToken)
    {
        ...
        scope.Component.BlockchainProcessor.Process(block, Tracer.ReadOnlyReplay, tracer.WithCancellation(cancellationToken), cancellationToken);
        ...
        bool useBlockAsBase = false, Utf8JsonWriter? writer = null, PipeWriter? pipeWriter = null)
    {
        ...
        try
        {
            ...
            scope.Component.BlockchainProcessor.Process(block, Tracer.ReadOnlyReplay, executionTracer, cancellationToken);
