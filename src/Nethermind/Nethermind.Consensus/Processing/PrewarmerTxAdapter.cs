...
/// the main thread has already started.
/// </summary>
/// <remarks>
/// Without this, the prewarmer redundantly re-executes the in-flight transaction, which is wasteful
/// and contends with the main thread — pathologically so for blocks dominated by a single heavy
/// transaction at a low index. Only the main (warm) world state reports progress; the prewarmer's own
/// speculative scope is filtered out by the <see cref="IPreBlockCaches.IsWarmWorldState"/> guard.
/// </remarks>
