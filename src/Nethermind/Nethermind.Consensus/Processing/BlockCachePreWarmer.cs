...
public sealed class BlockCachePreWarmer : IBlockCachePreWarmer
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!addressWarmer.HasBal)
            {
                ...
                WarmupWithdrawals(parallelOptions, spec, suggestedBlock);
            }
    ...
    private void WarmupWithdrawals(ParallelOptions parallelOptions, IReleaseSpec spec, Block block)
    {
        ...
        try
        {
            ...
            {
                ParallelUnbalancedWork.For(0, block.Withdrawals.Length, parallelOptions, (EnvPool: _envPool, Block: block),
                    static (i, state) =>
                    {
                        IReadOnlyTxProcessorSource env = state.EnvPool.Get();
                        try
                        {
                            using IReadOnlyTxProcessingScope scope = env.BuildAtTarget(state.Block.Header);
