    ...
    : IBranchProcessor
{
    ...
    public Block[] Process(BlockHeader? baseBlock, IReadOnlyList<Block> suggestedBlocks, ProcessingOptions options, IBlockTracer blockTracer, CancellationToken token = default)
    {
        ...
        void CancelBackgroundWork()
        {
            ...
            // Wait for prewarmer threads to fully stop before merkle computation starts.
            // Concurrent trie reads during merkle cause 5ms overhead (7.7ms vs 2.8ms).
            if (preWarmer is BlockCachePreWarmer pw)
                pw.ActiveTask?.Wait();
        }
