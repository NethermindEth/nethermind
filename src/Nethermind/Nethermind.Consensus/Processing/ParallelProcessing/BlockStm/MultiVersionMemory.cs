...
public class MultiVersionMemory<TLocation, TData, TLogger>(int txCount, ParallelTrace<TLogger> parallelTrace) where TLogger : struct, IFlag where TLocation : notnull
{
    ...
    /// </summary>
    /// <param name="version">TxVersion of transaction execution</param>
    ...
    public bool Record(TxVersion version, HashSet<Read<TLocation>> readSet, Dictionary<TLocation, TData> writeSet)
    {
        if (typeof(TLogger) == typeof(OnFlag)) parallelTrace.Add($"{version} Record read-set: {{{string.Join(",", readSet.Select(r => $"{r.Location}:{r.TxVersion}"))}}}, write-set: {{{string.Join(",", writeSet.Select(r => $"{r.Key}:{parallelTrace.Format(r.Value)}"))}}}.");
    ...
    /// </returns>
    public Status TryRead(TLocation location, int txIndex, out TxVersion version, out TData value)
    {
        ...
        for (int prevTx = txIndex - 1; prevTx >= 0; prevTx--)
        {
            ...
            try
            {
                ...
                {
                    version = new TxVersion(prevTx, v.Incarnation); // return version info
                    if (v.IsEstimate)
                    {
...
/// <param name="TxVersion">TxVersion of transaction that written that Location earlier. <see cref="ParallelProcessing.TxVersion.Empty"/> if Location was read from database</param>
/// <typeparam name="TLocation">Location id type</typeparam>
public readonly record struct Read<TLocation>(TLocation Location, TxVersion TxVersion); // TODO: version->incarnation?
