...
internal static partial class WitnessNodeStorage
{
    ...
    public static INodeStorage Create(IOwnedReadOnlyList<byte[]> state) => new HashKeyedNodeStorage(state);

    /// <summary>A node storage that maps a node's keccak straight to its bytes.</summary>
    /// <remarks>
    /// The host reaches the same nodes through a <c>MemDb</c> behind <see cref="NodeStorage"/>
    /// (see <c>WitnessNodeStorage.std.cs</c>), which keys a dictionary by <c>byte[]</c>: every witness
    /// node pays a key-array allocation on load, and every read builds a key span, hashes its bytes and
    /// compares them against the stored array. Here the keccak is the key, so a read is one word-wise
    /// probe. Not thread-safe, which the single-threaded guest does not need.
    /// </remarks>
    private sealed class HashKeyedNodeStorage : INodeStorage, INodeStorage.IWriteBatch
    {
        ...
        {
            ReadOnlySpan<byte[]> stateElements = state.AsSpan();
            Dictionary<NodeKey, byte[]> nodes = new(stateElements.Length + 1);

            foreach (byte[] stateElement in stateElements)
            {
    ...
    /// this target. The hash code is the keccak's own leading bytes: they are already uniformly
