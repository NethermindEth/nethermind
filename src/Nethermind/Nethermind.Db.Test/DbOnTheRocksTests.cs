...
namespace Nethermind.Db.Test
{
    ...
    public class DbOnTheRocksTests
    {
        ...
        [Test]
        public void RocksDbFeatureCollectiveFour_ResolvesOptionsAndRoundTripsDeletesAndIteration()
        {
            DbConfig config = new();
            RocksDbConfigFactory configFactory = new(config, new PruningConfig(), new TestHardwareInfo(1.GiB), LimboLogs.Instance, validateConfig: false);

            IDictionary<string, string> globalOptions = DbOnTheRocks.ExtractOptions(
                configFactory.GetForDatabase(DbNames.Blocks, null).RocksDbOptions);
            IDictionary<string, string> flatOptions = DbOnTheRocks.ExtractOptions(
                configFactory.GetForDatabase(DbNames.Flat, null).RocksDbOptions);
            IDictionary<string, string> accountOptions = DbOnTheRocks.ExtractOptions(
                configFactory.GetForDatabase(DbNames.Flat, nameof(FlatDbColumns.Account)).RocksDbOptions);

            using (Assert.EnterMultipleScope())
            {
                Assert.That(globalOptions["use_direct_io_for_compaction_reads"], Is.EqualTo("true"));
