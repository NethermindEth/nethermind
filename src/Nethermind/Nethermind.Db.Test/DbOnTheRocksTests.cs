    ...
    public class DbOnTheRocksTests
    {
        ...
        public void GlobalRocksDbOption_AllowsFlushAndReopen()
        {
            ...
            using (DbOnTheRocks db = new(DbPath, GetRocksDbSettings(DbPath, "Blocks"), config, configFactory, LimboLogs.Instance))
            {
                db.Set(key, value);
                db.Flush();
                Assert.That(ReadOptionsFile(DbPath), Does.Contain("reuse_manifest_on_open=true"));
            }
            ...
            config.AdditionalRocksDbOptions = "reuse_manifest_on_open=false;";
            ...
            Assert.That(ReadOptionsFile(DbPath), Does.Contain("reuse_manifest_on_open=false"));
        }
