...
public class DiscoveryManager : IDiscoveryManager
{
    ...
    private readonly ConcurrentDictionary<Commitment, INodeLifecycleManager> _nodeLifecycleManagers = new();
    ...
    public async Task<bool> WasMessageReceived(Commitment senderIdHash, MsgType msgType, int timeout)
    {
    ...
    private TaskCompletionSource<DiscoveryMsg> GetCompletionSource(Commitment senderAddressHash, int messageType)
    {
    ...
    private TaskCompletionSource<DiscoveryMsg>? RemoveCompletionSource(Commitment senderAddressHash, int messageType)
    {
        MessageTypeKey key = new(senderAddressHash, messageType);
    ...
    private void CleanUpLifecycleManagers()
    {
        ...
        foreach ((Commitment key, INodeLifecycleManager value) in _nodeLifecycleManagers)
        {
        ...
        foreach ((Commitment key, INodeLifecycleManager value) in _nodeLifecycleManagers.ToArray()
    ...
    private bool RemoveManager((Commitment Hash, PublicKey Key) item)
    {
    ...
    {
        public Commitment SenderAddressHash { get; }

        public int MessageType { get; }

        public MessageTypeKey(Commitment senderAddressHash, int messageType)
        {
