...
public class NodesLocator : INodesLocator
{
    ...
    public async Task LocateNodesAsync(byte[]? searchedNodeId, CancellationToken cancellationToken)
    {
        ...
        if(_logger.IsInfo) _logger.Info($"Starting discovery process for node: {(searchedNodeId != null ? $"randomNode: {new PublicKey(searchedNodeId).ToShortString()}" : $"masterNode: {_masterNode.Id}")}");
    ...
    private async Task<Result> SendFindNode(Node destinationNode, byte[]? searchedNodeId)
    {
        try
        {
            _logger.Info($"Sending find nodes for {destinationNode}");
