...
{
    public class DictionaryContractDataStore<T> : IDictionaryContractDataStore<T>, IDisposable where T : notnull
    {
        ...
        private static ContractDataStore<T> CreateContractDataStore(
            ...
            IReceiptFinder receiptFinder,
            ILogManager logManager) =>
            new(collection, dataContract ?? throw new ArgumentNullException(nameof(dataContract)), blockTree, receiptFinder, logManager);
        ...
        public bool TryGetValue(BlockHeader header, T key, [MaybeNullWhen(false)] out T value)
        {
