...
namespace Nethermind.Consensus.AuRa.Contracts
{
    ...
    public sealed class RandomContract : Blockchain.Contracts.Contract, IRandomContract
    {
        ...
        /// The mining address of validator is last contract parameter.
        /// </remarks>
        private bool SentReveal(BlockHeader parentHeader, UInt256 collectRound) => Constant.Call<bool>(parentHeader, nameof(SentReveal), _signer.SigningAddress, collectRound, _signer);
        ...
        /// The mining address of validator is last contract parameter.
        /// </remarks>
        private bool IsCommitted(BlockHeader parentHeader, UInt256 collectRound) => Constant.Call<bool>(parentHeader, nameof(IsCommitted), _signer.SigningAddress, collectRound, _signer);
        ...
        private UInt256 CurrentCollectRound(BlockHeader parentHeader) => Constant.Call<UInt256>(parentHeader, nameof(CurrentCollectRound), _signer.SigningAddress);
        ...
        private bool IsCommitPhase(BlockHeader parentHeader) => Constant.Call<bool>(parentHeader, nameof(IsCommitPhase), _signer.SigningAddress);
        ...
        {
            var (hash, cipher) = Constant.Call<byte[], byte[]>(parentHeader, nameof(GetCommitAndCipher), _signer.SigningAddress, collectRound, _signer);
        ...
        /// <param name="cipher">The cipher of the validator's secret. Can be used by the node to restore the lost secret after the node is restarted (see the `getCipher` getter).</param>
        /// <returns>Transaction to be included in block.</returns>
        public Transaction CommitHash(in Keccak secretHash, byte[] cipher) => GenerateTransaction<GeneratedTransaction>(nameof(CommitHash), _signer.SigningAddress, secretHash.Bytes, cipher);
        ...
        /// <param name="number">The validator's number.</param>
        /// <returns>Transaction to be included in block.</returns>
        public Transaction RevealNumber(UInt256 number) => GenerateTransaction<GeneratedTransaction>(nameof(RevealNumber), _signer.SigningAddress, number);
    }
}
