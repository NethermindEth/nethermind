...
namespace Nethermind.Consensus.AuRa
{
    public class AuRaContractGasLimitOverride : IGasLimitCalculator
    {
        ...
        private long? GetGasLimitFromContract(BlockHeader parentHeader)
        {
            ...
            long parentNumberPlusOne = (long)parentHeader.Number + 1;
            if (_contracts.TryGetForBlock(parentNumberPlusOne, out IBlockGasLimitContract contract))
            {
                ...
                {
                    if ((ulong)gasLimit.Value != parentHeader.GasLimit)
                    {
                        if (_logger.IsInfo)
