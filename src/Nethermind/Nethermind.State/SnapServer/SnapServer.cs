        ...
        for (int i = 0; i < pathLength && !abort && !cancellationToken.IsCancellationRequested; i++)
        {
            ...
            switch (requestedPath.Length)
            {
                ...
                case 1:
                    try
                    {
                        byte[]? rlp = tree.GetNodeByPath(NibblePath.Ref.FromCompact(requestedPath[0]), rootHash);
                        response.Add(rlp);
                    }
                ...
                default:
                    try
                    {
                        ...
                        if (account is not null)
                        {
                            ...
                            {
                                byte[]? sRlp = sTree.GetNodeByPath(NibblePath.Ref.FromCompact(requestedPath[reqStorage]));
                                response.Add(sRlp);
                            }
                        }
                    }
