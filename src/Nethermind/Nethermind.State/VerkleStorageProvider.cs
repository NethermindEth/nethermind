...
public class VerkleStorageProvider: IStorageProvider
{
    ...
    private readonly VerkleStateProvider _stateProvider;
    ...
    private static readonly byte[] _zeroValue = new byte[32];
        ...
        for (int i = 0; i <= _currentPosition; i++)
        {
            ...
            switch (change.ChangeType)
            {
                ...
                case ChangeType.Update:
                    ...
                    _stateProvider.SetStorageValue(change.StorageCell, change.Value);
                    if (isTracing)
                    {
    ...
    private byte[] LoadFromTree(StorageCell storageCell)
    {
        ...
        byte[] value = _stateProvider.GetStorageValue(storageCell);
