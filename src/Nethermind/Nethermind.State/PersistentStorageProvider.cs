...
internal sealed class PersistentStorageProvider : PartialStorageProviderBase
{
    ...
    private readonly Dictionary<AddressAsKey, DefaultableDictionary> _blockCache = new(4_096);
    ...
    /// </summary>
    public override void Reset(bool resetBlockCache = true)
    {
        base.Reset();
    ...
    private void SaveToTree(HashSet<AddressAsKey> toUpdateRoots, Change change)
    {
        ...
        ref DefaultableDictionary? dict = ref CollectionsMarshal.GetValueRefOrAddDefault(_blockCache, change.StorageCell.Address, out bool exists);
        if (!exists)
        {
    ...
    {
        ref DefaultableDictionary? dict = ref CollectionsMarshal.GetValueRefOrAddDefault(_blockCache, storageCell.Address, out bool exists);
        if (!exists)
        {
            dict = new DefaultableDictionary();
        }
    ...
    public override void ClearStorage(Address address)
    {
        ...
        ref DefaultableDictionary? dict = ref CollectionsMarshal.GetValueRefOrAddDefault(_blockCache, address, out bool exists);
        if (!exists)
        {
            dict = new DefaultableDictionary();
        }
