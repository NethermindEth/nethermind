...
) : IWorldStateScopeProvider, IPreBlockCaches
{
        ...
        : IWorldStateScopeProvider.IScope
    {
        ...
        public void Dispose()
        {
            if (ReferenceEquals(_metricObserver, NoopMetricObserver.Instance))
            {
                baseScope.Dispose();
                return;
            }
        ...
        public Account? Get(Address address)
        {
            ...
            bool measureMetric = !ReferenceEquals(_metricObserver, NoopMetricObserver.Instance);
            ...
            if (populatePreBlockCache)
            {
                ...
                {
                    if (measureMetric) _metricObserver.Observe(Stopwatch.GetTimestamp() - sw, new PrewarmerGetTimeLabel("address_hit", populatePreBlockCache));
                    Metrics.IncrementStateTreeCacheHits();
                }
                else
                {
                    if (measureMetric) _metricObserver.Observe(Stopwatch.GetTimestamp() - sw, new PrewarmerGetTimeLabel("address_miss", populatePreBlockCache));
                }
                return account;
            }
            else
    ...
    ) : IWorldStateScopeProvider.IStorageTree
    {
        ...
        public byte[] Get(in UInt256 index)
        {
            ...
            if (populatePreBlockCache)
            {
                ...
                {
                    if (measureMetric) _metricObserver.Observe(Stopwatch.GetTimestamp() - sw, new PrewarmerGetTimeLabel("slot_get_hit", populatePreBlockCache));
                    // Read from Concurrent Cache
                    Db.Metrics.IncrementStorageTreeCache();
                }
                else
                {
                    if (measureMetric) _metricObserver.Observe(Stopwatch.GetTimestamp() - sw, new PrewarmerGetTimeLabel("slot_get_miss", populatePreBlockCache));
                }
                return value;
            }
            else
