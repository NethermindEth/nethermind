...
public sealed class ValidatorModeTransitionLogger(QbftForksSchedule forksSchedule, ILogManager logManager)
{
    ...
    {
        QbftConfigSnapshot current = forksSchedule.GetFork((long)parentHeader.Number, parentHeader.Timestamp);
        QbftConfigSnapshot next = forksSchedule.GetFork((long)parentHeader.Number + 1, parentHeader.Timestamp);
    ...
    public static string Describe(QbftConfigSnapshot config) =>
