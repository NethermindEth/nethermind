...
namespace Nethermind.Synchronization.Test
{
    ...
    public class SyncProgressResolverTests
    {
        ...
        public void Header_block_is_0_when_no_header_was_suggested()
        {
            ...
            IStateFactory stateFactory = Substitute.For<IStateFactory>();
            SyncConfig syncConfig = new()
            {
                PivotNumber = "1",
            };

            SyncProgressResolver syncProgressResolver = CreateProgressResolver(blockTree, stateReader, stateFactory, false, syncConfig, LimboLogs.Instance);
        ...
        public void Best_state_is_head_if_there_is_suggested_block_without_state()
        {
            ...
            stateFactory.HasRoot(head.StateRoot!).Returns(true);
            stateFactory.HasRoot(suggested.StateRoot!).Returns(false);
        ...
        public void Is_fast_block_receipts_finished_returns_true_when_receipts_not_downloaded_and_we_do_not_want_to_download_receipts()
        {
            ...
            SyncProgressResolver syncProgressResolver = CreateProgressResolver(blockTree, stateReader, stateFactory, true, syncConfig, LimboLogs.Instance);
            Assert.True(syncProgressResolver.IsFastBlocksReceiptsFinished());
        }


        private SyncProgressResolver CreateProgressResolver(IBlockTree blockTree, IStateReader stateReader, IStateFactory stateFactory, bool isReceiptFinished, SyncConfig syncConfig, LimboLogs limboLogs)
        {
            ...
            return new SyncProgressResolver(
                blockTree,
                new FullStateFinder(blockTree, stateReader, stateFactory),
                syncConfig,
