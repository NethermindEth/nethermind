...
public partial class ForwardHeaderProviderTests
{
    ...
    [TestCase(1ul, 0, 64, 0ul, 1ul)]
    [TestCase(1ul, 32, 64, 0ul, 0ul)]
    [TestCase(2ul, 0, 64, 0ul, 2ul)]
    [TestCase(2ul, 32, 64, 0ul, 0ul)]
    [TestCase(32ul, 0, 64, 0ul, 32ul)]
    [TestCase(32ul, 32, 64, 0ul, 0ul)]
    [TestCase(32ul, 16, 64, 0ul, 16ul)]
    [TestCase(32ul, 0, 16, 0ul, 15ul)]
    [TestCase(3ul, 0, 64, 0ul, 3ul)]
    [TestCase(3ul, 32, 64, 0ul, 0ul)]
    [TestCase((ulong)(SyncBatchSizeMax * 8), 0, 64, 0ul, 63ul)]
    [TestCase((ulong)(SyncBatchSizeMax * 8), 0, 64, 0ul, 63ul)]
    [TestCase((ulong)(SyncBatchSizeMax * 8), 32, 64, 0ul, 63ul)]
    [TestCase((ulong)(SyncBatchSizeMax * 8), 32, 64, 0ul, 63ul)]
    public async Task Happy_path(ulong headNumber, int skipLastN, int maxHeader, ulong expectedStartNumber, ulong expectedEndNumber)
    {
        long chainLength = checked((long)(headNumber + 1));
    ...
    public async Task Ancestor_lookup_simple()
    {
        ...
        for (int i = 0; i < 1023; i++)
        {
            ...
            Assert.That(syncPeer.BlockTree.FindBlock(number, BlockTreeLookupOptions.None)!.Hash, Is.EqualTo(ctx.BlockTree.FindBlock(number, BlockTreeLookupOptions.None)!.Hash), i.ToString());
        }

        using IOwnedReadOnlyList<BlockHeader?>? headers = await forwardHeader.GetBlockHeaders(0, 128, CancellationToken.None);
        headers?[0]?.Number.Should().Be(1019ul);
        headers?[^1]?.Number.Should().Be(1146ul);
    }
    ...
    public async Task Peer_only_advertise_some_header(ulong headNumber)
    {
        ...
        syncPeer.GetBlockHeaders(Arg.Any<ulong>(), Arg.Any<int>(), Arg.Any<int>(), Arg.Any<CancellationToken>())
            .Returns(_ => ctx.ResponseBuilder.BuildHeaderResponse(0ul, checked((int)(headNumber + 1)), Response.AllCorrect));
        ...
        using IOwnedReadOnlyList<BlockHeader?>? headers = await forwardHeader.GetBlockHeaders(0, 128, CancellationToken.None);
        headers?[0]?.Number.Should().Be(0ul);
    ...
    public async Task Cache_block_headers_unless_peer_changed()
    {
        ...
        (await forwardHeader.GetBlockHeaders(0, 128, CancellationToken.None)).Should().NotBeNull();
        await syncPeer.Received(1).GetBlockHeaders(Arg.Any<ulong>(), Arg.Any<int>(), Arg.Any<int>(), Arg.Any<CancellationToken>());
        await newSyncPeer.Received(1).GetBlockHeaders(Arg.Any<ulong>(), Arg.Any<int>(), Arg.Any<int>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task Validate_always_the_last_seal_and_random_seal_in_the_package()
    {
        ...
        PeerInfo peerInfo = new(syncPeer);
        syncPeer.HeadNumber.Returns(511ul);
        ctx.ConfigureBestPeer(peerInfo);
    ...
    private class ThrowingPeer(ulong number, UInt256? totalDiff, Hash256? headHash = null) : ISyncPeer
    {
            ...
            if (!justFirst)
            {
                for (int i = 0; i < blockHashes.Count; i++)
                {
                    blockHeaders[i] = consistent
                        ...
                        : Build.A.BlockHeader.WithNumber(blockHeaders[i - 1].Number + 1ul).WithHash(blockHashes[i]).TestObject;
