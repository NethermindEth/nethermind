...
public class RecoveryTests
{
    ...
    {
        byte[]? rlp = await Recover(_nodeDataRecovery, new List<Commitment> { _key }, _peerEth66);
    ...
    {
        byte[]? rlp = await Recover(_nodeDataRecovery, new List<Commitment> { _key }, _peerEth67);
        rlp.Should().BeNull();
    }
