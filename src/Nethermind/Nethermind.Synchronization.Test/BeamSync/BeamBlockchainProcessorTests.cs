...
namespace Nethermind.Synchronization.Test.BeamSync
{
    ...
    public class BeamBlockchainProcessorTests
    {
        ...
        public async Task Valid_block_with_transactions_makes_it_is_processed_normally_if_beam_syncing_finished()
        {
            ...
            syncModeSelector.Preparing += Raise.EventWith(new SyncModeChangedEventArgs(SyncMode.Beam, SyncMode.Idle));
            syncModeSelector.Changing += Raise.EventWith(new SyncModeChangedEventArgs(SyncMode.Beam, SyncMode.Idle));
            syncModeSelector.Changed += Raise.EventWith(new SyncModeChangedEventArgs(SyncMode.Beam, SyncMode.Idle));
        ...
        [TestCase(SyncMode.Idle, true)]
        [TestCase(SyncMode.Full, true)]
        [TestCase(SyncMode.FastBodies, true)]
        [TestCase(SyncMode.FastReceipts, true)]
        [TestCase(SyncMode.FastHeaders, true)]
