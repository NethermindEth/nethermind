...
public class ProgressTrackerTests
{
    ...
    public void Will_create_multiple_get_address_range_request()
    {
        ...
        request.AccountRangeRequest.LimitHash.GetValueOrDefault().Bytes[0].Should().Be(64);
        ...
        request.AccountRangeRequest.LimitHash.GetValueOrDefault().Bytes[0].Should().Be(128);
        finished.Should().BeFalse();

        (request, finished) = progressTracker.GetNextRequest();
        request.AccountRangeRequest.Should().NotBeNull();
        request.AccountRangeRequest!.StartingHash.Bytes[0].Should().Be(128);
        request.AccountRangeRequest.LimitHash.GetValueOrDefault().Bytes[0].Should().Be(192);
        finished.Should().BeFalse();

        (request, finished) = progressTracker.GetNextRequest();
        request.AccountRangeRequest.Should().NotBeNull();
        request.AccountRangeRequest!.StartingHash.Bytes[0].Should().Be(192);
        request.AccountRangeRequest.LimitHash.GetValueOrDefault().Bytes[0].Should().Be(255);
        finished.Should().BeFalse();
