    ...
    public void TestGetStorageRange()
    {
        ...
        try
        {
            var storageRangeRequest = new StorageRange()
            {
                ...
                Accounts = new ArrayPoolList<PathWithAccount>(1) { new(TestItem.Tree.AccountsWithPaths[0].Path, new Account(UInt256.Zero, 0).WithChangedStorageRoot(inputStorageTree.RootHash)) }
            };
