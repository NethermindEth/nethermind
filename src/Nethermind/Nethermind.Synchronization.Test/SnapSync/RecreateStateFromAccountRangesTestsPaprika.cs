...
{
    [TestFixture]
    public class RecreateStateFromAccountRangesTestsPaprika
    {
        ...
        public void RecreateAccountStateFromOneRangeWithExistenceProof()
        {
            ...
            SimpleHeal(_inputTree, _inputTree.TrieStore, rawState);
        ...
        public void RecreateAccountStateFromOneRangeWithoutProof()
        {
            ...
            var hasRoot = stateFactory.HasRoot(rootHash);
        ...
        [Test]
        public void RecreateAccountStateFromOneRangeWithoutProof_2()
        {
            ...
            PathWithAccount[] accounts = new PathWithAccount[]
            {
                new PathWithAccount(new Hash256("0aaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(0).TestObject),
                new PathWithAccount(new Hash256("1aaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(1).TestObject),
                new PathWithAccount(new Hash256("2aaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(2).TestObject),
                new PathWithAccount(new Hash256("3aaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(3).TestObject),
                new PathWithAccount(new Hash256("4aaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(4).TestObject),
                new PathWithAccount(new Hash256("5aaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(5).TestObject),
                new PathWithAccount(new Hash256("6aaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(6).TestObject),
                new PathWithAccount(new Hash256("7aaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(7).TestObject),
                new PathWithAccount(new Hash256("8aaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(8).TestObject),
                new PathWithAccount(new Hash256("9aaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(9).TestObject),
                new PathWithAccount(new Hash256("aaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(10).TestObject),
                new PathWithAccount(new Hash256("baaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(11).TestObject),
                new PathWithAccount(new Hash256("caaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(12).TestObject),
                new PathWithAccount(new Hash256("daaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(13).TestObject),
                new PathWithAccount(new Hash256("eaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(14).TestObject),
                new PathWithAccount(new Hash256("faaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccddddddd"), Build.An.Account.WithBalance(15).TestObject)
            };
            ...
            Hash256 rootHash = trie.RootHash;
            ...
            var result = snapProvider.AddAccountRange(1, rootHash, accounts[0].Path, accounts);
        ...
        [Test]
        public void RecreateAccountStateFromMultipleRange_a()
        {
            // output state
            MemDb db = new();
            IDbProvider dbProvider = new DbProvider();
            ...
            MemDb srcDb = new();
            TrieStore fullStore = new(srcDb, LimboLogs.Instance);
            IScopedTrieStore store = fullStore.GetTrieStore(null);
            StateTree tree = new(store, LimboLogs.Instance);

            Hash256[] paths = new Hash256[16];
            Hash256 somePath = new Hash256("0x0cccccccccccccccccccddddddddddddddddeeeeeeeeeeeeeeeeb12233445566");

            PathWithAccount[] accountsWithPaths = new PathWithAccount[16];

            for (byte i = 0; i < 16; i++)
            {
                byte[] rawBytes = somePath.BytesToArray();
                rawBytes[0] |= (byte)(i << 4);
                paths[i] = new Hash256(rawBytes);

                accountsWithPaths[i] = new PathWithAccount(paths[i],
                    Build.An.Account.WithBalance(new UInt256((ulong)i + 1)).TestObject);
                tree.Set(paths[i], accountsWithPaths[i].Account);
            }

            tree.Commit(10);
            Hash256 rootHash = tree.RootHash;

            byte[][] firstProof = CreateProofForPath(Keccak.Zero.Bytes, tree);
            byte[][] lastProof = CreateProofForPath(paths[8].Bytes);

            var result1 = snapProvider.AddAccountRange(1, rootHash, Keccak.Zero, accountsWithPaths[0..9], firstProof!.Concat(lastProof!).ToArray());
            Assert.That(result1, Is.EqualTo(AddRangeResult.OK));

            firstProof = CreateProofForPath(paths[9].Bytes, tree);
            lastProof = CreateProofForPath(paths[15].Bytes);
            var result2 = snapProvider.AddAccountRange(1, rootHash, Keccak.Zero, accountsWithPaths[9..16], firstProof!.Concat(lastProof!).ToArray());
            Assert.That(result2, Is.EqualTo(AddRangeResult.OK));

            IRawState rawState = progressTracker.GetNewRawState();
            SimpleHeal(tree, tree.TrieStore, rawState);
            rawState.Finalize(1);

            var state = stateFactory.Get(rootHash);
            foreach (var item in accountsWithPaths)
            {
                Account a = state.Get(item.Path);
                Assert.That((item.Account.IsTotallyEmpty && a is null) || (!item.Account.IsTotallyEmpty && a is not null), Is.True);
                Assert.That(a?.Balance ?? 0, Is.EqualTo(item.Account.Balance));
        ...
        public void RecreateAccountStateFromMultipleRange_Single()
        {
            ...
            IRawState rawState = progressTracker.GetNewRawState();
            SimpleHeal(_inputTree_9, _inputTree_9.TrieStore, rawState);
            rawState.Finalize(1);
        ...
        public void RecreateAccountStateFromMultipleRange_PivotChange_High()
        {
            ...
            IRawState rawState = progressTracker.GetSyncState();
            SimpleHeal(_inputTree_7, _inputTree_7.TrieStore, rawState);
            rawState.Finalize(200);
        ...
        public void RecreateAccountStateFromMultipleRange_PivotChange_Low()
        {
            ...
            IRawState rawState = progressTracker.GetSyncState();
            SimpleHeal(newPivotTree, newPivotTree.TrieStore, rawState);
            rawState.Finalize(200);
        ...
        public void CorrectlyDetermineHasMoreChildren()
        {
            ...
            bool HasMoreChildren(ValueHash256 limitHash)
            {
                (AddRangeResult _, bool moreChildrenToRight, IList<PathWithAccount> _, IList<ValueHash256> _) =
                    SnapProviderHelperPaprika.AddAccountRange(progressTracker.GetSyncState(), 1, rootHash, Keccak.Zero, limitHash.ToCommitment(), receiptAccounts, proofs);
        ...
        private void SimpleHeal(StateTree stateTree, ITrieNodeResolver nodeResolver, IRawState paprikaRawState)
        {
            SimpleHeal(stateTree.Root!, TreePath.Empty, nodeResolver, paprikaRawState);
        }

        private void SimpleHeal(TrieNode node, TreePath path, ITrieNodeResolver nodeResolver, IRawState paprikaRawState)
        {
            var existingHash = paprikaRawState.GetHash(path.Span, path.Length, false);
            if (existingHash == node.Keccak)
                return;
            ...
            if (node.IsExtension)
            {
                ...
                SimpleHeal(node.GetChild(nodeResolver, ref emptyPath, 0), childPath, nodeResolver, paprikaRawState);
                paprikaRawState.CreateProofExtension(Keccak.Zero, childPath.Span, path.Length, node.Key.Length);
            }
            else if (node.IsBranch)
            {
                List<byte> children = new List<byte>(12);
                List<Hash256> childHashes = new List<Hash256>(12);

                for (int i = 0; i < 16; i++)
                {
                    TrieNode childNode = node.GetChild(nodeResolver, ref emptyPath, i);
                    if (childNode is not null)
                    {
                        ...
                        path.AppendMut(i);
                        SimpleHeal(childNode, path, nodeResolver, paprikaRawState);
                        path.TruncateOne();
                    }
                }

                paprikaRawState.CreateProofBranch(Keccak.Zero, path.Span, path.Length, children.ToArray(), childHashes.ToArray());
            }
        }
    }
}
