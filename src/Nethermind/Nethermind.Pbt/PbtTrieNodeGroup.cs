...
public readonly ref struct PbtTrieNodeGroup<TLayout> where TLayout : IPbtTileLayout
{
    ...
    /// </summary>
    public static int MaxEncodedLength => OverheadLength + TLayout.PositionCount * HashLength + TLayout.BoundarySlots * ChainExtraLength;
    ...
    {
        UInt128 bit = UInt128.One << position;
        return (_masks.Presence & bit) == 0 ? PbtTrieNodeGroup.NodeKind.Absent
            : (_masks.Stems & bit) != 0 ? PbtTrieNodeGroup.NodeKind.Stem
            : PbtLayout.TrieNodeGroupIsBoundaryPosition(position) && (_masks.Chains >> PbtLayout.TrieNodeGroupBoundarySlot(position) & 1) != 0 ? PbtTrieNodeGroup.NodeKind.Chain
    ...
    {
        UInt128 below = (UInt128.One << position) - 1;
        return EntriesOffset
            + PbtLayout.PopCount(_masks.Presence & below) * HashLength
            + PbtLayout.PopCount(_masks.Stems & below) * Stem.Length
            + BitOperations.PopCount(_masks.Chains & PbtLayout.SlotsBelow(PbtLayout.TrieNodeGroupBoundarySlot(position))) * ChainExtraLength;
    }
    ...
    {
        UInt128 mask = PbtLayout.TrieNodeGroupSubtreeBitmask(position, width);
        return new NodeGroupBitmasks(_masks.Presence & mask, _masks.Stems & mask, _masks.Chains & TLayout.BoundaryBitmask(mask));
    }
    ...
    internal BoundarySlotMasks BoundaryShape() =>
        new(TLayout.BoundaryBitmask(_masks.Presence), TLayout.BoundaryBitmask(_masks.Stems), _masks.Chains);
    ...
    public static PbtTrieNodeGroup<TLayout> Decode(ReadOnlySpan<byte> data)
    {
        ...
        NodeGroupBitmasks masks = TLayout.ReadMasks(trailer);
        (UInt128 presence, UInt128 stems, ulong chains) = masks;
        if (presence >> TLayout.PositionCount != 0 || (stems & ~presence) != 0)
        {
        ...
        if ((chains & ~(TLayout.BoundaryBitmask(presence) & ~TLayout.BoundaryBitmask(stems))) != 0)
        {
        ...
        if ((presence & (UInt128.One << TLayout.RootPosition) & ~stems) != 0)
        {
        ...
        return new PbtTrieNodeGroup<TLayout>(data, format, masks);
    }
}
