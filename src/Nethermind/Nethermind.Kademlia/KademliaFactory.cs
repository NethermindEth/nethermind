...
public static class KademliaFactory
{
    ...
    /// <param name="loggerFactory">Optional logger factory. When omitted, logging is disabled.</param>
        ...
        where TKadKey : notnull
    {
        ...
        KBucketTree<TNode, TKadKey> routingTable = new(config, nodeHashProvider, distance, loggerFactory);
        NodeHealthTracker<TKey, TNode, TKadKey> nodeHealthTracker = new(config, routingTable, nodeHashProvider, sender, loggerFactory);
        LookupKNearestNeighbour<TKey, TNode, TKadKey> lookup = new(routingTable, nodeHashProvider, distance, nodeHealthTracker, config, loggerFactory);
