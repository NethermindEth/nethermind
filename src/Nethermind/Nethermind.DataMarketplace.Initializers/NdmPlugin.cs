...
namespace Nethermind.DataMarketplace.Initializers
{
    public class NdmPlugin : IPlugin
    {
        ...
        public async Task InitNetworkProtocol(INethermindApi api)
        {
            ...
            IHttpClient httpClient = new DefaultHttpClient(new HttpClient(), api.EthereumJsonSerializer, api.LogManager);
            INdmConfig ndmConfig = api.Config<INdmConfig>();
            if (ndmConfig.ProxyEnabled)
            {
                api.JsonRpcClientProxy = new JsonRpcClientProxy(httpClient, ndmConfig.JsonRpcUrlProxies,
                    api.LogManager);
                api.EthJsonRpcClientProxy = new EthJsonRpcClientProxy(api.JsonRpcClientProxy);
            }
            
            INdmCapabilityConnector capabilityConnector = await _ndmInitializer.InitAsync(
                api,
                // api.NdmConsumerChannelManager,
                // api.NdmDataPublisher,
                httpClient);
