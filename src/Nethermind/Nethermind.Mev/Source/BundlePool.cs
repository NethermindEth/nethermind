...
namespace Nethermind.Mev.Source
{
    ...
    public class BundlePool : IBundlePool, ISimulatedBundleSource, IDisposable
    {
        ...
        private readonly ConcurrentDictionary<long, ConcurrentDictionary<(MevBundle Bundle, Commitment BlockHash), SimulatedMevBundleContext>> _simulatedBundles = new();
        ...
        protected virtual SimulatedMevBundleContext? SimulateBundle(MevBundle bundle, BlockHeader parent)
        {
            ...
            (MevBundle, Commitment) key = (bundle, parent.Hash!);
            ...
            ConcurrentDictionary<(MevBundle, Commitment), SimulatedMevBundleContext> AddContext(ConcurrentDictionary<(MevBundle, Commitment), SimulatedMevBundleContext> d)
            {
                d.AddOrUpdate(key,
                    _ => CreateContext(),
                    (_, c) => c);
            ...
            _simulatedBundles.AddOrUpdate(parent.Number,
                    _ =>
                    {
                        ConcurrentDictionary<(MevBundle, Commitment), SimulatedMevBundleContext> d = new();
                        return AddContext(d);
                    },
                    (_, d) => AddContext(d));
        ...
        private void RemoveBundlesUpToBlock(long blockNumber)
        {
            ...
            foreach (KeyValuePair<long, ConcurrentDictionary<(MevBundle, Commitment), SimulatedMevBundleContext>> simulatedBundleBucket in simulatedBundlesToRemove)
            {
        ...
        {
            if (_simulatedBundles.TryGetValue(bundle.BlockNumber, out ConcurrentDictionary<(MevBundle Bundle, Commitment BlockHash), SimulatedMevBundleContext>? simulations))
            {
                IEnumerable<(MevBundle Bundle, Commitment BlockHash)> keys = simulations.Keys.Where(k => Equals(k.Bundle, bundle));

                foreach ((MevBundle Bundle, Commitment BlockHash) key in keys)
                {
        ...
        {
            if (_simulatedBundles.TryGetValue(parent.Number, out ConcurrentDictionary<(MevBundle Bundle, Commitment BlockHash), SimulatedMevBundleContext>? simulatedBundlesForBlock))
            {
