...
namespace Nethermind.Overseer.Test.Framework
{
    public class ProcessBuilder
    {
        public NethermindProcessWrapper Create(string name, string workingDirectory, string config, string dbPath, int httpPort, int p2pPort, string nodeKey, string bootnode)
        {
            ...
            var arguments = $"Nethermind.Runner.dll --config {config} --InitConfig.HttpPort {httpPort} --InitConfig.P2PPort {p2pPort} --InitConfig.DiscoveryPort {p2pPort} --KeyStoreConfig.TestNodeKey {nodeKey}";
            ...
            {
                arguments = $"{arguments} --DiscoveryConfig.Bootnodes {bootnode}";
            }
