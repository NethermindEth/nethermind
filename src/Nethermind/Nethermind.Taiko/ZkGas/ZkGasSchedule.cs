...
/// </summary>
/// <remarks>
/// <see cref="OpcodeMultipliers"/> / <see cref="PrecompileMultipliers"/> hold the recalibrated
/// default multipliers from taiko-mono#21720 (alethia-reth#187), used by every network unless its
/// chainspec overrides them via <c>unzenOpcodeZkGasMultipliers</c> / <c>unzenPrecompileZkGasMultipliers</c>.
/// A network that already finalized Unzen blocks under a different schedule (e.g. Masaya) pins the
/// prior table in its chainspec to preserve consensus on that history; see
/// <see cref="BuildOverriddenTable"/> for how an override resolves into a full table.
/// </remarks>
public static class ZkGasSchedule
{
    ...
    // The public surface returns ReadOnlyMemory<ushort> so callers can stash a stable handle
    // (indexed via .Span[..]) without exposing the underlying array.
    ...
    /// <summary>
    /// Recalibrated default per-opcode proving-cost multipliers indexed by opcode byte. Used by
    /// every network unless its chainspec overrides them (see <see cref="BuildOverriddenTable"/>).
    /// </summary>
    public static ReadOnlyMemory<ushort> OpcodeMultipliers => _opcodeMultipliers;

    /// <summary>
    /// Recalibrated default per-precompile proving-cost multipliers indexed by low-byte address.
    /// Used by every network unless its chainspec overrides them.
    /// </summary>
    public static ReadOnlyMemory<ushort> PrecompileMultipliers => _precompileMultipliers;

    /// <summary>
    /// Resolves a 256-entry multiplier table from an optional chainspec override map. When
    /// <paramref name="overrides"/> is null or empty the recalibrated <paramref name="defaults"/>
    /// table is returned unchanged; otherwise each listed index takes its multiplier and every
    /// other index falls back to <see cref="FailsafeMultiplier"/> — matching the fill semantics of
    /// the built-in tables, so a chainspec fully restates the schedule it pins (e.g. Masaya).
    /// </summary>
    /// <param name="overrides">Index (0–255) → multiplier (0–<see cref="ushort.MaxValue"/>) map from the chainspec, or null.</param>
    /// <param name="defaults">Recalibrated default table returned when no override is present.</param>
    /// <exception cref="InvalidConfigurationException">An index or multiplier is out of range.</exception>
    public static ReadOnlyMemory<ushort> BuildOverriddenTable(IReadOnlyDictionary<long, long>? overrides, ReadOnlyMemory<ushort> defaults)
    {
        ...
        foreach ((long index, long multiplier) in overrides)
        {
            if (index is < 0 or > byte.MaxValue)
            {
                throw new InvalidConfigurationException(
                    $"Unzen ZK gas multiplier index {index} is outside the valid opcode/precompile range [0, {byte.MaxValue}].",
            ...
            {
                throw new InvalidConfigurationException(
                    $"Unzen ZK gas multiplier {multiplier} for index 0x{index:x2} is outside the valid range [0, {ushort.MaxValue}].",
                    ExitCodes.ForbiddenOptionValue);
            }
