...
/// width. What does carry over is here, run under both, with <see cref="EipReferenceTree"/> as the
/// oracle throughout: a tiling changes where the bytes live and nothing else.
/// </remarks>
[TestFixture(PbtTiling.ClusteredFourLevel, PbtGroupFormat.EveryLevel)]
[TestFixture(PbtTiling.ClusteredFourLevel, PbtGroupFormat.Interleaved)]
[TestFixture(PbtTiling.ClusteredFourLevel, PbtGroupFormat.BoundaryOnly)]
[TestFixture(PbtTiling.ClusteredFourLevel, PbtGroupFormat.Every4Depth)]
[TestFixture(PbtTiling.SixLevel, PbtGroupFormat.EveryLevel)]
[TestFixture(PbtTiling.SixLevel, PbtGroupFormat.Interleaved)]
[TestFixture(PbtTiling.SixLevel, PbtGroupFormat.BoundaryOnly)]
[TestFixture(PbtTiling.SixLevel, PbtGroupFormat.Every4Depth)]
public class PbtTilingTests(PbtTiling tiling, PbtGroupFormat groupFormat)
{
    ...
    private PbtTrieFormat Format => new(tiling, groupFormat);

    private int LevelsPerGroup => tiling == PbtTiling.SixLevel ? PbtSixLevelTileLayout.LevelsPerGroup : PbtClusteredTileLayout.LevelsPerGroup;

    private int MaxGroupDepth => tiling == PbtTiling.SixLevel ? PbtSixLevelTileLayout.MaxGroupDepth : PbtClusteredTileLayout.MaxGroupDepth;

    private PbtTreeHarness NewHarness() => new(PooledRefCountingMemoryProvider.Instance, Format);
    ...
    public void StemsPartingAtAnyBit_FoldToTheReferenceRootAndCollapseBack(int divergenceBit)
    {
        ...
        int branchDepth = divergenceBit - divergenceBit % LevelsPerGroup;
    ...
    public void StemsPartingAtTheLastBit_BuildNoNodeBelowTheStemLevel()
    {
        ...
        Assert.That(
            () => TrieUpdater.UpdateRoot(NewHarness(), default, duplicate, PooledRefCountingMemoryProvider.Instance, Format, concurrency: 1, out _),
    ...
    /// since 60 is a tile boundary of both.
    /// </summary>
    [Test]
    public void ContractStorageSharingItsStemPrefix_CollapsesToOneChain()
    {
        ...
        for (int depth = chains[0].Depth; depth < 60; depth += LevelsPerGroup)
        {
        ...
        Assert.That(harness.Nodes.Keys.Max(key => key.Depth), Is.EqualTo(60));
    }
    ...
    public void RandomRounds_MatchTheOracleAndAFreshFold(bool drained)
    {
        ...
        for (int round = 0; round < 12; round++)
        {
            ...
            Assert.That(root, Is.EqualTo(ReferenceRoot(LiveEntries(live))), $"root mismatch after round {round} under {tiling}/{groupFormat}");
        }
    ...
    /// <summary>
    /// The tilings describe the same trie, so the same writes fold to the same root under both — which
    /// is the whole claim a second tiling rests on.
    /// </summary>
    [Test]
    public void BothTilings_FoldTheSameWritesToTheSameRoot()
    {
        ...
        PbtTreeHarness other = new(PooledRefCountingMemoryProvider.Instance, Format with { Tiling = tiling == PbtTiling.SixLevel ? PbtTiling.ClusteredFourLevel : PbtTiling.SixLevel });
