...
public class StemLeafBlobTests
{
    ...
    public void ApplyReadBackClearAndDeletionSignal()
    {
        ...
        byte[] blob = StemLeafBlob.Apply([], new Dictionary<byte, byte[]?> { [5] = value5, [200] = value200 });
        ...
        blob = StemLeafBlob.Apply(blob, new Dictionary<byte, byte[]?> { [5] = null });
        ...
        // an all-zero value clears too, and an empty blob signals stem deletion
        blob = StemLeafBlob.Apply(blob, new Dictionary<byte, byte[]?> { [200] = new byte[32] });
        Assert.That(blob, Is.Empty);
        Assert.That(StemLeafBlob.ComputeSubtreeRoot([]), Is.EqualTo(default(ValueHash256)));
    }
    ...
    public void SubtreeRootAndStemNodeHashMatchEipReference(int seed, int leafCount)
    {
        ...
        byte[] blob = StemLeafBlob.Apply([], changes);
        ValueHash256 subtreeRoot = StemLeafBlob.ComputeSubtreeRoot(blob);
