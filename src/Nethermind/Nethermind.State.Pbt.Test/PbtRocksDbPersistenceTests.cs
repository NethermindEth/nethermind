...
public class PbtRocksDbPersistenceTests
{
    ...
    [TestCase(null, PbtNodeGroupKeyLayout.Padded, PbtNodeGroupKeyLayout.Variable, TestName = "Unstamped_epoch_14_store_is_padded")]
    ...
    public void Node_group_key_layout_stamp_gates_the_configured_layout(byte[]? stamp, PbtNodeGroupKeyLayout? accepted, PbtNodeGroupKeyLayout rejected)
    {
        ...
        metadata[SchemaEpochKey] = Epoch(14);
    ...
    [Test]
    public void Completed_epoch_14_store_reopens_and_serves_canonical_records()
    {
        SnapshotableMemColumnsDb<PbtColumns> db = new("pbt");
    ...
    public void Old_epoch_rejection_does_not_mutate_any_column(bool importEnabled)
    {
        ...
        db.GetColumnDb(PbtColumns.Metadata)[SchemaEpochKey] = Epoch(13);
    ...
    public void Clear_storage_respects_staged_order_and_preserves_other_addresses(int slotNumber)
    {
        ...
        PbtTreeKey persistedKey = PbtStateKey.Storage(TestItem.AddressA, (UInt256)(uint)slotNumber);
        PbtTreeKey stagedKey = PbtStateKey.Storage(TestItem.AddressA, (UInt256)(uint)(slotNumber + 1));
        PbtTreeKey otherAddressKey = PbtStateKey.Storage(TestItem.AddressB, (UInt256)(uint)slotNumber);
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(reader.EnumerateStorage(new PbtTreeKey(persistedKey.Bytes[..33])).Drain().Single().Value, Is.EqualTo(replacement));
        }
    }
    ...
    public void Incremental_flush_reopens_with_partial_tail_and_latest_values()
    {
        ...
        try
        {
            ...
            using PbtWriteBatchBuilder<PbtTreeKey> mutations = new(0);
            byte[] deletedKey = Bytes.FromHexString("00");
            byte[] replacedKey = Bytes.FromHexString("08");
            mutations.Delete(new PbtTreeKey(deletedKey));
            mutations.Set(new PbtTreeKey(replacedKey), new ValueHash256(Value(99)));
    ...
    public void Fresh_store_is_versioned_but_remains_unpublished_until_the_first_commit()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(metadata.Get(SchemaEpochKey), Is.EqualTo(Epoch(14)));
    ...
    private static IEnumerable<TestCaseData> InvalidMetadataCases()
    {
        ...
        yield return new TestCaseData(Epoch(14), new byte[] { 0 }, null, false).SetName("Rejects_malformed_current_state");
        yield return new TestCaseData(Epoch(14), null, Array.Empty<byte>(), false).SetName("Rejects_empty_validity");
        yield return new TestCaseData(Epoch(14), null, new byte[] { 2 }, false).SetName("Rejects_unknown_validity");
        yield return new TestCaseData(Epoch(14), null, new byte[] { 1 }, false).SetName("Rejects_validity_without_current_state");
        yield return new TestCaseData(Epoch(14), CurrentState(), null, false).SetName("Rejects_current_state_without_validity");
        ...
        where TPath : struct, IPbtNodePath<TPath>
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(ReadNode(olderReader, path), Is.EqualTo(BranchNode(1)));
            Assert.That(db.GetColumnDb(PbtColumns.Metadata).Get(SchemaEpochKey), Is.EqualTo(Epoch(14)));
        }
    }
        ...
        [Values] bool stamped)
    {
        ...
        if (stamped) db.GetColumnDb(PbtColumns.Metadata).Set(SchemaEpochKey, Epoch(14));
