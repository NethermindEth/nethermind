...
public class PbtNodeGroupTests
{
    ...
    public void Borrowed_subtree_materializes_before_its_using_scope_releases_memory([Values] bool leaf)
    {
        ...
        using (new GroupFrameReader<PbtStorageTreeKey, PbtStorageNodePath>.Scope(ref reader))
        {
            TrieUpdater<PbtStorageTreeKey, PbtStorageNodePath>.TraversalSubtree borrowed = leaf
                ? reader.TakeRoot(groupPath).ToFoldResult(groupPath, 0).Borrow(groupPath)
        ...
        where TPath : struct, IPbtNodePath<TPath>
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(owned.Path.Length, Is.EqualTo(anchoredLength));
            Assert.That(owned.Encoding.IsEmpty, Is.EqualTo(splitDepth - groupDepth == anchoredLength));
        ...
        [Values] bool compressedSiblings)
    {
        ...
        using (new GroupFrameReader<PbtStorageTreeKey, PbtStorageNodePath>.Scope(ref reader))
        {
            ...
            result = TrieUpdater<PbtStorageTreeKey, PbtStorageNodePath>.Compose(ref reader, writer, groupPath, 0, null, ref frontier).Borrow(groupPath);
    ...
    public void Compact_frontier_boundary_results_track_insertions_and_deletions([Values(0, 15)] int slot)
    {
        ...
        using (new GroupFrameReader<PbtStorageTreeKey, PbtStorageNodePath>.Scope(ref reader))
        {
            ...
            subtree = TrieUpdater<PbtStorageTreeKey, PbtStorageNodePath>.Compose(ref reader, writer, groupPath, 0, null, ref frontier).Borrow(groupPath);
