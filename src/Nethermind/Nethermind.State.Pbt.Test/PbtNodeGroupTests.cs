...
public class PbtNodeGroupTests
{
        ...
        [Values(0, 1, 4, 7, 8, 9, 272, 528)] int depth)
    {
        ...
        PbtStorageNodePath expected = PbtNodePathOperations.Prefix<PbtStorageNodePath>(key, key.Length * 8, depth);
        ...
        Assert.That(cursor.ToPath<PbtStorageNodePath>().ToEncodedArray(),
            Is.EqualTo(PbtNodePathOperations.Prefix<PbtStorageNodePath>(new byte[key.Length], key.Length * 8, depth).ToEncodedArray()));
    }
    ...
    public void Store_identity_and_reader_payload_survive_cursor_mutation([Values] bool storage, [Values(0, 4, -1)] int groupDepth)
    {
        ...
        ValueHash256 hash = PbtNodeCodec.Hash(PbtNodeReader.FromValidated(encoding));
    ...
    private static void AssertAppendNib<TPath>(int depth) where TPath : struct, IPbtNodePath<TPath>
    {
        ...
        if (depth + 4 > TPath.MaxBitDepth)
        {
            ...
            using (Assert.EnterMultipleScope())
            {
                ...
                Assert.That(appended.Prefix(depth).Equals(path), Is.True);
        ...
        for (int nibble = 0; nibble < 16; nibble++)
        {
            ...
            using (Assert.EnterMultipleScope())
            {
                ...
                Assert.That(path.ToPathArray(), Is.EqualTo(bytes));
            }
        }
    }
    ...
    public void Small_and_storage_paths_share_identity_and_accept_wide_leaf_payloads(int depth)
    {
        ...
        store.SetNodeGroup(smallPath, PbtNodeCodec.Hash(PbtNodeReader.FromValidated(encoding)), publishedPayload);
        ...
        store.SetNodeGroup(storagePath, PbtNodeCodec.Hash(PbtNodeReader.FromValidated(encoding)), publishedPayload);
    ...
    public void Key_families_enforce_capacity_without_changing_storage_bytes(int length, int depth)
    {
        ...
        if (length == PbtPath.KeyLength)
        {
            ...
            PbtTreeKey treeKey = new(keyBytes);
    ...
    public void Borrowed_subtree_materializes_before_its_using_scope_releases_memory([Values] bool leaf)
    {
        ...
        ValueHash256 rootHash = leaf ? new(Value(9)) : PbtNodeCodec.Hash(PbtNodeReader.FromValidated(encoding));
        ...
        [Values(0, 1, 2)] int scenario, [Values] bool consume)
    {
        ...
        store.SetNode(rootPath, PbtTreeHarness.EncodeBranch(Bytes.FromHexString("A0"), 4, new ValueHash256(Value(1)), new ValueHash256(Value(2))), provider);
        ...
        where TKey : unmanaged, IPbtKey<TKey>
    {
        ...
        PbtTrieWarmer.WarmUpPath(reader, root, query, null);
        using (Assert.EnterMultipleScope())
        {
        ...
        where TKey : unmanaged, IPbtKey<TKey>
    {
        ...
        PbtTrieWarmer.WarmUpPath(unpinned, root, query, null);

        PbtPinnedGroups pinnedGroups = new();
        WarmReadStore pinning = new(store);
        PbtTrieWarmer.WarmUpPath(pinning, root, query, pinnedGroups);
        ...
        where TKey : unmanaged, IPbtKey<TKey>
    {
        ...
        PbtTrieWarmer.WarmUpPath(reader, default, key, null);
        ...
        if (invalidPayload) Assert.Catch(() => PbtTrieWarmer.WarmUpPath(reader, default, key, null));
        else PbtTrieWarmer.WarmUpPath(reader, default, key, null);
        ((IDisposable)payload).Dispose();
        Assert.That(TrackingMemoryProvider.CountUnreleased(memory.Rented), Is.Zero);
    }
