        ...
        using (PbtWriteBatchBuilder<TKey> batch = new(0))
        {
            ...
            using (Assert.EnterMultipleScope())
            {
                foreach (TKey key in keys)
                {
                    ...
                    Assert.That(PbtNodeTraverser.GetLeafHash(store, root, key), Is.EqualTo(expected), $"present key {Convert.ToHexString(key.Bytes)}");
                }
                for (int index = 0; index < 64; index++)
                {
                    TKey absent = RandomKey<TKey>(random);
                    Assert.That(PbtNodeTraverser.GetLeafHash(store, root, absent), Is.EqualTo(default(ValueHash256)), $"absent key {Convert.ToHexString(absent.Bytes)}");
                }
            }
        }
        store.Dispose();
