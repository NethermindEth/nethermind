...
public class OrderedComponentsTests
{
    ...
    public void TestNestedModuleConsistency()
    {
        using IContainer ctx = new ContainerBuilder()
            ...
            .AddLast(static _ => new Item("4"))
            .Build();

        ctx.Resolve<Item[]>().Select(static item => item.Name).Should().BeEquivalentTo(["1", "2", "3", "4"]);
        ctx.Resolve<IEnumerable<Item>>().Select(static item => item.Name).Should().BeEquivalentTo(["1", "2", "3", "4"]);
        ctx.Resolve<IReadOnlyList<Item>>().Select(static item => item.Name).Should().BeEquivalentTo(["1", "2", "3", "4"]);
    }
    ...
    {
        using IContainer ctx = new ContainerBuilder()
            .AddLast(static _ => new Item("2"))
            .AddLast(static _ => new Item("3"))
            .AddFirst(static _ => new Item("1"))
            .Build();

        ctx.Resolve<Item[]>().Select(static item => item.Name).Should().BeEquivalentTo(["1", "2", "3"]);
    }

    [Test]
    public void TestDisallowIndividualRegistration()
    {
        Action act = static () => new ContainerBuilder()
            .AddLast(static _ => new Item("1"))
            .AddSingleton<Item>(static _ => new Item("2"))
            .Build();
    ...
    public void TestCompositeResolved_EvenWhenNotLastRegistration()
    {
        ...
        using IContainer ctx = new ContainerBuilder()
            .AddLast<IItem>(static _ => new Item("1"))
            .AddCompositeOrderedComponents<IItem, CompositeItem>()
            .AddLast<IItem>(static _ => new Item("2"))
            .Build();

        IItem resolved = ctx.Resolve<IItem>();
        resolved.Should().BeOfType<CompositeItem>();
        ((CompositeItem)resolved).Items.Select(static i => i.Name).Should().BeEquivalentTo(["1", "2"]);
    }
    ...
    private class CompositeItem(IItem[] items) : IItem
    {
        ...
        public string Name => string.Join(",", Items.Select(static i => i.Name));
    }
}
