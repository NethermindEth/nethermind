...
namespace Nethermind.Core.Test.Crypto
{
    ...
    public class EthereumEcdsaTests
    {
        ...
        {
            EthereumEcdsa ecdsa = new(NetworkId.Ropsten, LimboLogs.Instance);
        ...
        {
            EthereumEcdsa ecdsa = new(NetworkId.Goerli, LimboLogs.Instance);
            PrivateKey key = Build.A.PrivateKey.TestObject;
            Transaction tx = Build.A.Transaction.TestObject;
        ...
        {
            EthereumEcdsa singEcdsa = new(NetworkId.Mainnet, LimboLogs.Instance);
            PrivateKey key = Build.A.PrivateKey.TestObject;
            Transaction tx = Build.A.Transaction.TestObject;
            singEcdsa.Sign(key, tx, eip155);

            EthereumEcdsa recoverEcdsa = new(NetworkId.Kovan, LimboLogs.Instance);
