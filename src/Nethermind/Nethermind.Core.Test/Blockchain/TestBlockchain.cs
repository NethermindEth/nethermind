...
public class TestBlockchain : IDisposable
{
    ...
    protected virtual async Task<TestBlockchain> Build(Action<ContainerBuilder>? configurer = null)
    {
        ...
        if (specProvider?.GetFinalSpec().WithdrawalsEnabled is true)
        {
            ...
            state.CreateAccount(Eip7002Constants.WithdrawalRequestPredeployAddress, 0, 1);
            state.InsertCode(Eip7002Constants.WithdrawalRequestPredeployAddress, code, SpecProvider.GenesisSpec);
        }
        ...
        if (specProvider?.GetFinalSpec().ConsolidationRequestsEnabled is true)
        {
            ...
            state.CreateAccount(Eip7251Constants.ConsolidationRequestPredeployAddress, 0, 1);
            state.InsertCode(Eip7251Constants.ConsolidationRequestPredeployAddress, code, SpecProvider.GenesisSpec);
        }
