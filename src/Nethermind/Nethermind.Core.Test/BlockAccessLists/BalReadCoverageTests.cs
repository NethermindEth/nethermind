...
/// <summary>
/// Pins the per-worker read-coverage lanes: the structural bitmap reports the first uncovered
/// declared read (the implied all-bits-set expectation), the chargeable count is per-slice-distinct,
/// and <see cref="BalReadCoverage.Absorb"/> OR-reduces bitmaps while summing chargeable counts.
/// </summary>
[TestFixture]
public class BalReadCoverageTests
{
    ...
    public void All_marked_reports_fully_covered()
    {
        ...
        for (int i = 0; i < 70; i++) coverage.MarkRead(i, sliceStamp: 1, chargeable: false);
    ...
    public void Single_unmarked_ordinal_is_found(int missing)
    {
        ...
        for (int i = 0; i < 70; i++)
        {
            if (i != missing) coverage.MarkRead(i, sliceStamp: 1, chargeable: false);
        }
    ...
    {
        using BalReadCoverage coverage = new(70);
        coverage.MarkRead(0, 1, false); // leave 1..69 uncovered
    ...
    public void Repeat_in_same_slice_counts_chargeable_once()
    {
        ...
        coverage.MarkRead(2, sliceStamp: 7, chargeable: true);
        coverage.MarkRead(2, sliceStamp: 7, chargeable: true);
    ...
    public void Same_ordinal_across_slices_counts_chargeable_per_slice()
    {
        using BalReadCoverage coverage = new(4);
        coverage.MarkRead(2, sliceStamp: 7, chargeable: true);
        coverage.MarkRead(2, sliceStamp: 8, chargeable: true);

        Assert.That(coverage.ChargeableCount, Is.EqualTo(2));
    }
    ...
    {
        using BalReadCoverage coverage = new(4);
        coverage.MarkRead(1, sliceStamp: 1, chargeable: false);

        Assert.That(coverage.ChargeableCount, Is.EqualTo(0));
        coverage.MarkRead(0, 1, true);
        coverage.MarkRead(2, 1, true);
        coverage.MarkRead(3, 1, true);
    ...
    public void Absorb_ors_coverage_and_sums_chargeable()
    {
        ...
        // Disjoint coverage across two workers; together they cover everything.
        for (int i = 0; i < 70; i++)
        {
            if ((i & 1) == 0) a.MarkRead(i, sliceStamp: 1, chargeable: true);
            else b.MarkRead(i, sliceStamp: 2, chargeable: true);
        }
        ...
        Assert.That(a.ChargeableCount, Is.EqualTo(70)); // 35 from each worker
    }
}
