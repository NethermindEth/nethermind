...
public class XdcExtendedEthModuleTests
{
    ...
    public async Task eth_getRewardByHash_returns_stored_epoch_rewards()
    {
        ...
        rewardsStore.TryGetEpochRewardsRpc(100UL, out Arg.Any<Dictionary<string, Dictionary<string, Dictionary<string, string>>>?>())
            .Returns(x =>
            {
                x[1] = payload;
                return true;
        ...
        result.Data.Should().ContainKey("rewards");
        result.Data!["rewards"].Should().ContainKey(TestItem.AddressA.ToString());
    }
    ...
    private static IXdcExtendedEthRpcModule CreateOwnerModule(Address owner)
    {
        ...
        IBlockFinder blockFinder = Substitute.For<IBlockFinder>();
        blockTreeHeadSetup(blockFinder, header);
    ...
    private static void blockTreeHeadSetup(IBlockFinder blockFinder, XdcBlockHeader header)
    {
