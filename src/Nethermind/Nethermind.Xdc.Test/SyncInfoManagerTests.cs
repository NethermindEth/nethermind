...
public class SyncInfoManagerTests
{
    ...
    {
        BlockRoundInfo blockRoundInfo = new BlockRoundInfo(Keccak.Zero, 1, 1);
        QuorumCertificate qc = new QuorumCertificate(blockRoundInfo, Array.Empty<Signature>(), 0);
        TimeoutCertificate tc = new TimeoutCertificate(1, Array.Empty<Signature>(), 0);
        SyncInfo syncInfo = new SyncInfo(qc, tc);
        ...
        SyncInfoManager manager = new SyncInfoManager(xdcContext, qcManager, timeoutManager, logManager);
    ...
    public void VerifySyncInfo_WhenQuorumCertificateRoundIsEqualToPrevious_ReturnsFalseAndError()
    {
        ...
        IXdcConsensusContext xdcContext = Substitute.For<IXdcConsensusContext>();
        QuorumCertificate contextQc = new QuorumCertificate(
    ...
    {
        BlockRoundInfo blockRoundInfo = new BlockRoundInfo(Keccak.Zero, 5, 1);
        QuorumCertificate qc = new QuorumCertificate(blockRoundInfo, Array.Empty<Signature>(), 0);
        TimeoutCertificate tc = new TimeoutCertificate(2, Array.Empty<Signature>(), 0);
        SyncInfo syncInfo = new SyncInfo(qc, tc);
        ...
        QuorumCertificate contextQc = new QuorumCertificate(
            ...
            Array.Empty<Signature>(),
            0);
        TimeoutCertificate contextTc = new TimeoutCertificate(2, Array.Empty<Signature>(), 0);
        xdcContext.HighestQC.Returns(contextQc);
    ...
    public void VerifySyncInfo_WhenAllVerificationsPass_ReturnsTrue()
    {
        BlockRoundInfo blockRoundInfo = new BlockRoundInfo(Keccak.Zero, 5, 1);
        QuorumCertificate qc = new QuorumCertificate(blockRoundInfo, Array.Empty<Signature>(), 0);
        TimeoutCertificate tc = new TimeoutCertificate(5, Array.Empty<Signature>(), 0);
        SyncInfo syncInfo = new SyncInfo(qc, tc);
    ...
    {
        BlockRoundInfo blockRoundInfo = new BlockRoundInfo(Keccak.Zero, 10, 1);
        QuorumCertificate qc = new QuorumCertificate(blockRoundInfo, Array.Empty<Signature>(), 0);
        TimeoutCertificate tc = new TimeoutCertificate(10, Array.Empty<Signature>(), 0);
        SyncInfo syncInfo = new SyncInfo(qc, tc);
        ...
        QuorumCertificate contextQc = new QuorumCertificate(
            ...
            Array.Empty<Signature>(),
            0);
        TimeoutCertificate contextTc = new TimeoutCertificate(5, Array.Empty<Signature>(), 0);
        xdcContext.HighestQC.Returns(contextQc);
        xdcContext.HighestTC.Returns(contextTc);
