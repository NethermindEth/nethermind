...
internal class QuorumCertificateDecoderTests
{
    ...
    public static IEnumerable QuorumCertificateCases
    {
        get
        {
            yield return new TestCaseData(new QuorumCertificate(new BlockInfo(Hash256.Zero, 1, 1), [new Signature(new byte[64], 0), new Signature(new byte[64], 0), new Signature(new byte[64], 0)], 0));
            yield return new TestCaseData(new QuorumCertificate(new BlockInfo(Hash256.Zero, 1, 1), [], 0));
            yield return new TestCaseData(new QuorumCertificate(new BlockInfo(Hash256.Zero, ulong.MaxValue, long.MaxValue), [], int.MaxValue));
        }
    }
    ...
    {
        QuorumCertificate quorumCert = new(new BlockInfo(Hash256.Zero, 1, 1), [new Signature(new byte[64], 0), new Signature(new byte[64], 0), new Signature(new byte[64], 0)], 0);
        QuorumCertificateDecoder decoder = new();
        RlpStream stream = new RlpStream(decoder.GetLength(quorumCert));
        decoder.Encode(stream, quorumCert);
        stream.Position = 0;
