...
internal class BlobProofsManagerV1 : IBlobProofsManager
{
    ...
    {
        ShardBlobNetworkWrapper result = new(new byte[blobs.Length * Ckzg.Ckzg.BytesPerBlob], new byte[blobs.Length * Ckzg.Ckzg.BytesPerCommitment], new byte[blobs.Length * Ckzg.Ckzg.BytesPerProof * Ckzg.Ckzg.CellsPerExtBlob], ProofVersion.V1);
    ...
    {
        int length = wrapper.Count * Ckzg.Ckzg.BytesPerCell * Ckzg.Ckzg.CellsPerExtBlob;
        byte[] cellsArray = ArrayPool<byte>.Shared.Rent(length);
        Span<byte> cells = new(cellsArray, 0, length);

        length = wrapper.Count * Ckzg.Ckzg.CellsPerExtBlob;
        ulong[] indicesArray = ArrayPool<ulong>.Shared.Rent(length);
        Span<ulong> indices = new(indicesArray, 0, length);

        length = wrapper.Count * Ckzg.Ckzg.BytesPerCommitment * Ckzg.Ckzg.CellsPerExtBlob;
        ...
        try
        {
            for (int i = 0; i < wrapper.Count; i++)
            {
                ...
                Span<byte> commitment = wrapper.CommitmentAt(i).Span;

                for (int j = 0; j < Ckzg.Ckzg.CellsPerExtBlob; j++)
                {
                    int cellNumber = i * Ckzg.Ckzg.CellsPerExtBlob + j;
                    indices[cellNumber] = (ulong)j;
                    commitment.CopyTo(flatCommitments.Slice(cellNumber * Ckzg.Ckzg.BytesPerCommitment, Ckzg.Ckzg.BytesPerCommitment));
                }
            }

            return Ckzg.Ckzg.VerifyCellKzgProofBatch(flatCommitments, indices, cells, wrapper.Proofs, wrapper.Count * Ckzg.Ckzg.CellsPerExtBlob, KzgPolynomialCommitments.CkzgSetup);
        }
