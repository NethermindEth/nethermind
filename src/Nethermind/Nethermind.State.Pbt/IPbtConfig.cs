...
public interface IPbtConfig : IConfig
{
    ...
    [ConfigItem(Description = "How the state is laid out on disk, as a tiling of the stem trie and the levels of it that store a node. The tiling comes first in the name: `FourLevel`, `FiveLevel`, `SixLevel` and `EightLevel` store 4-, 5-, 6- and 8-level tiles each in a blob of its own. The levels come second, and say which levels of a tile and of a stem's 256-leaf subtree store a node, the rest being folded on demand: `EveryLevel` stores all of them, `Interleaved` every other one, `BoundaryOnly` none at all - just the tile's boundary entries and the stem's leaves - and `Every4Depth` keeps the boundary-only tile but stores one stem node every four depth. `SixLevelEvery3Depth` keeps every third level in six-level trie groups while retaining interleaved stem leaf blobs; it shares on-disk keys with `SixLevelInterleaved`. Each in turn trades size in the trie node and leaf columns for work on rebuild. The state root is identical whichever layout is chosen, but the keys are not: a database is stamped with the tiling that wrote it and cannot be read under another, while the levels may change over a database that already holds state.", DefaultValue = "FourLevelInterleaved")]
