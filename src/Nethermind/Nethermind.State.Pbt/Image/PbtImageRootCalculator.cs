...
internal static class PbtImageRootCalculator
{
    internal static ValueHash256 Calculate(IEnumerable<RebuildEntry> entries, CancellationToken cancellationToken)
    {
        ...
        Frame[] frames = new Frame[PbtStorageTreeKey.MaxLength * 8];
        Span<byte> preimage = stackalloc byte[3 + PbtStorageTreeKey.MaxLength + 64];
        int frameCount = 0;
    ...
    private static void Fold(int nextSplit, in PbtStorageTreeKey key, ReadOnlySpan<Frame> frames,
