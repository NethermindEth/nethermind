    ...
    private ValueHash256 FlushAndCommit(IPbtPersistence.IWriteBatch writeBatch, ValueHash256 currentRoot, PbtWriteBatch changes, out PbtSubtreeStats stemDelta)
    {
        ...
        using (changes)
        {
            if (changes.Count > 0)
            {
                ...
                currentRoot = TrieUpdater.UpdateRoot(store, currentRoot, changes, PooledRefCountingMemoryProvider.Instance, config.TrieNodeLevels, config.RootFoldConcurrency, out stemDelta);
            }
        }
