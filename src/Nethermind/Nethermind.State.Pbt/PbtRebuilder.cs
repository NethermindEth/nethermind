...
public sealed class PbtRebuilder(PbtRocksDbPersistence target, IPbtConfig config, ILogManager logManager)
{
    ...
    private readonly bool _sortedTrieUpdater = config.SortedTrieUpdater;
        ...
        WriteFlags stagingWriteFlags)
    {
        ...
        void CommitWindow()
        {
            ...
            using (PbtPartitionBatches prepared = new())
            {
                ...
                root = TrieUpdater.UpdateRoot(new WindowStore(reader, stagingBatch, cancellationToken), root, prepared, _foldQuota, _foldFanOut, _prefixlessBranchOmission,
                    _sortedTrieUpdater, null);
                cancellationToken.ThrowIfCancellationRequested();
                stagingBatch.Commit();
            }
