...
public class PluginLoader : IPluginLoader
{
    ...
    public void Load(ILogManager logManager)
    {
        ...
        var baseDir = string.Empty.GetApplicationResourcePath();
        var pluginAssembliesDir = _pluginsDirectory.GetApplicationResourcePath();
        ...
        var assemblies = _fileSystem.Directory.GetFiles(pluginAssembliesDir, "*.dll");
        if (assemblies.Length > 0)
            if (logger.IsInfo) logger.Info($"Loading {assemblies.Length} assemblies from {pluginAssembliesDir}");

        foreach (var assemblyName in assemblies)
        {
            var pluginAssembly = _fileSystem.Path.GetFileNameWithoutExtension(assemblyName);

            try
            {
                if (logger.IsInfo) logger.Warn($"Loading assembly {pluginAssembly}");
                var assemblyPath = _fileSystem.Path.Combine(pluginAssembliesDir, assemblyName);
    ...
    {
        var order = pluginConfig.PluginOrder.Select(s => s.ToLower() + "plugin").ToList();
        _pluginTypes.Sort((f, s) =>
        {
            var fIsConsensus = typeof(IConsensusPlugin).IsAssignableFrom(f);
            var sIsConsensus = typeof(IConsensusPlugin).IsAssignableFrom(s);
            ...
            var fPos = order.IndexOf(f.Name.ToLower());
            var sPos = order.IndexOf(s.Name.ToLower());
            if (fPos == -1)
            {
                if (sPos == -1)
