...
internal class MergeOperatorAdapter(IMergeOperator inner) : MergeOperator
{
    ...
    private static unsafe nint GetResult(ArrayPoolList<byte>? data, out nint resultLength, out byte success)
    {
        ...
        using (data)
        {
            ...
            Span<byte> result = new Span<byte>(resultPtr, data.Count);
    ...
    {
        Span<byte> keyBytes = new Span<byte>((void*)key, (int)keyLength);
        RocksDbMergeEnumerator enumerator = new RocksDbMergeEnumerator(new((void*)operandsList, numOperands), new((void*)operandsListLength, numOperands));
    ...
    {
        ReadOnlySpan<byte> keyBytes = new ReadOnlySpan<byte>((void*)key, (int)keyLength);
        ...
        RocksDbMergeEnumerator enumerator = new RocksDbMergeEnumerator(existingValueBytes, hasExistingValue, new((void*)operandsList, numOperands), new((void*)operandsListLength, numOperands));
