...
public partial class DbOnTheRocks : IDb, ITunableDb, IReadOnlyNativeKeyValueStore, ISnapshottableKeyValueStore, ISortedKeyValueStore, IMergeableKeyValueStore, IKeyValueStoreWithSnapshot
{
    ...
    protected virtual void BuildOptions<T>(IRocksDbConfig dbConfig, Options<T> options, IntPtr? sharedCache, IMergeOperator? mergeOperator) where T : Options<T>
    {
        ...
        bool isUsingPlainTable = optionsAsDict.ContainsKey("plain_table_factory");
    ...
    {
        public byte[]?[] MultiGet(Span<byte[]> keys)
        {
            try
            {

                byte[][] asArray = new byte[keys.Length][];
                keys.CopyTo(asArray);
                ...
                var res = mainDb._db.MultiGet(asArray, cfs, options);
            ...
            {
                Console.Error.WriteLine($"Error in multiget {e}");
                throw;
            }
        }
