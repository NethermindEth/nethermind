...
public partial class DbOnTheRocks : IDb, ITunableDb, IReadOnlyNativeKeyValueStore, ISortedKeyValueStore, IMergeableKeyValueStore, IKeyValueStoreWithSnapshot
{
    ...
    // Striped to avoid single-cache-line contention: hit on every DB read/write across the
    // block pre-warmer's many concurrent threads.
    private readonly StripedLongCounter _totalReads = new();
    private readonly StripedLongCounter _totalWrites = new();
