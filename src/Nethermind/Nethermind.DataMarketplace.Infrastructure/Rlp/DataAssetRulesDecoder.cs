...
namespace Nethermind.DataMarketplace.Infrastructure.Rlp
{
    public class DataAssetRulesDecoder : IRlpDecoder<DataAssetRules>
    {
        ...
        {
            Nethermind.Core.Serialization.Rlp.Decoders[typeof(DataAssetRules)] = new DataAssetRulesDecoder();
        }
            ...
            RlpBehaviors rlpBehaviors = RlpBehaviors.None)
        {
            ...
            var expiry = Nethermind.Core.Serialization.Rlp.Decode<DataAssetRule>(rlpStream);
            var upfrontPayment = Nethermind.Core.Serialization.Rlp.Decode<DataAssetRule>(rlpStream);

            return new DataAssetRules(expiry, upfrontPayment);
        }

        public Nethermind.Core.Serialization.Rlp Encode(DataAssetRules item, RlpBehaviors rlpBehaviors = RlpBehaviors.None)
        {
            if (item == null)
            {
            ...
            return Nethermind.Core.Serialization.Rlp.Encode(
                Nethermind.Core.Serialization.Rlp.Encode(item.Expiry),
                Nethermind.Core.Serialization.Rlp.Encode(item.UpfrontPayment));
        }
