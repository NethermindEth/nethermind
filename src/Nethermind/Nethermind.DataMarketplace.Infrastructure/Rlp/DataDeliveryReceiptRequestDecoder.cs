...
namespace Nethermind.DataMarketplace.Infrastructure.Rlp
{
    public class DataDeliveryReceiptRequestDecoder : IRlpDecoder<DataDeliveryReceiptRequest>
    {
        ...
        {
            Nethermind.Core.Serialization.Rlp.Decoders[typeof(DataDeliveryReceiptRequest)] =
            ...
            RlpBehaviors rlpBehaviors = RlpBehaviors.None)
        {
            ...
            var unitsRange = Nethermind.Core.Serialization.Rlp.Decode<UnitsRange>(rlpStream);
            var isSettlement = rlpStream.DecodeBool();
            var receipts = Nethermind.Core.Serialization.Rlp.DecodeArray<DataDeliveryReceiptToMerge>(rlpStream);

            return new DataDeliveryReceiptRequest(number, depositId, unitsRange, isSettlement, receipts);
        }

        public Nethermind.Core.Serialization.Rlp Encode(DataDeliveryReceiptRequest item,
            RlpBehaviors rlpBehaviors = RlpBehaviors.None)
        {
            if (item == null)
            {
            ...
            return Nethermind.Core.Serialization.Rlp.Encode(
                Nethermind.Core.Serialization.Rlp.Encode(item.Number),
                Nethermind.Core.Serialization.Rlp.Encode(item.DepositId),
                Nethermind.Core.Serialization.Rlp.Encode(item.UnitsRange),
                Nethermind.Core.Serialization.Rlp.Encode(item.IsSettlement),
                Nethermind.Core.Serialization.Rlp.Encode(item.ReceiptsToMerge.ToArray()));
        }
