...
{
    private const int MaxClientIdLength = 1_024;
    private readonly HelloMessageSerializer _serializer = new();
    ...
    public void Can_do_roundtrip()
    {
        ...
        byte[] serialized = _serializer.Serialize(helloMessage);
        ...
        Assert.That(Bytes.AreEqual(serialized, expectedBytes), Is.True, "bytes");
    ...
    {
        byte[] serialized = SerializeWithClientId(new string('a', MaxClientIdLength));

        using HelloMessage deserialized = _serializer.Deserialize(serialized);
        Assert.That(deserialized.ClientId.Length, Is.EqualTo(MaxClientIdLength));
    }

    [Test]
    public void Deserialize_throws_on_client_id_above_current_limit()
    {
        byte[] serialized = SerializeWithClientId(new string('a', MaxClientIdLength + 1));

        Assert.Throws<RlpLimitException>(() => _serializer.Deserialize(serialized));
    }

    private byte[] SerializeWithClientId(string clientId)
    {
