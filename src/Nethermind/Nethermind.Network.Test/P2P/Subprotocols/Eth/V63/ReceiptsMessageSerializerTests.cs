...
namespace Nethermind.Network.Test.P2P.Subprotocols.Eth.V63
{
    ...
    public class ReceiptsMessageSerializerTests
    {
                ...
                for (int i = 0; i < receipts.Length; i++)
                {
                    ...
                    else
                    {
                        for (int j = 0; j < receipts[i].Length; j++)
                        {
                            ...
                            {
                                // todo - test equality comparer for receipts
        ...
        {
            TransactionReceipt[][] data = {new[] {Build.A.TransactionReceipt.TestObject, Build.A.TransactionReceipt.TestObject}, new[] {Build.A.TransactionReceipt.TestObject, Build.A.TransactionReceipt.TestObject}};
            Test(data);
        }
