...
namespace Nethermind.Network.Test.P2P
{
    ...
    public class SessionTests
    {
        ...
        {
            Session session = new(30312, new Node(TestItem.PublicKeyA, "127.0.0.1", 8545, false), _channel, NullDisconnectsAnalyzer.Instance, LimboLogs.Instance);
        ...
        {
            Session session = new(30312, new Node(TestItem.PublicKeyA, "127.0.0.1", 8545, false), _channel, new MetricsDisconnectsAnalyzer(), LimboLogs.Instance);
            session.Handshake(TestItem.PublicKeyA);
            session.Init(5, _channelHandlerContext, _packetSender);
            IProtocolHandler p2p = BuildHandler("p2p", 10);
            session.AddProtocolHandler(p2p);
            ...
            session = new Session(30312, new Node(TestItem.PublicKeyA, "127.0.0.1", 8545, false), _channel, new MetricsDisconnectsAnalyzer(), LimboLogs.Instance);
            session.Handshake(TestItem.PublicKeyA);
            session.Init(5, _channelHandlerContext, _packetSender);
