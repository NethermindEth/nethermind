...
{
    public enum Delivery { ByteByByte, AllAtOnce, AllAtOnceWithCorruptedHeader, BlockByBlock }
    ...
    private void Test(string frame, Delivery delivery, string expectedOutput)
    {
        ...
        IByteBuffer result = ReferenceCountUtil.ReleaseLater(delivery switch
        {
            Delivery.ByteByByte => DeliverByteByByte(frameBytes, input, zeroFrameDecoderTestWrapper),
            Delivery.AllAtOnce => DeliverAllAtOnce(frameBytes, input, zeroFrameDecoderTestWrapper),
            Delivery.AllAtOnceWithCorruptedHeader => DeliverAllAtOnceFollowedByACorruptedHeader(frameBytes, input, zeroFrameDecoderTestWrapper),
            Delivery.BlockByBlock => DeliverBlockByBlock(frameBytes, input, zeroFrameDecoderTestWrapper),
            _ => throw new System.ArgumentOutOfRangeException(nameof(delivery))
        });
