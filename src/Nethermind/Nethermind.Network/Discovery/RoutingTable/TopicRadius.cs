     ...
     public class TopicRadius : ITopicRadius
        {
            ...
            private readonly int _minSlope = 1;
            ...
            public Keccak targetForBucket(int bucket) {
                ...
                byte[] prefix = BitConverter.GetBytes(BitConverter.ToUInt64(topicHashPrefix) ^ xor);
                Keccak target;

                target = Keccak.Compute(prefix.Slice(0,8).Concat(CryptoRandom.GenerateRandomBytes(Keccak.OfAnEmptySequenceRlp.Bytes.Length - 8)).ToArray());
                return target;
            }
            ...
            public bool needMoreLookups(int a, int b, double maxValue) {
                double max = 0;
                if (a < 0) {
                    a = 0;
                }
            ...
            public (ulong, int) recalcRadius() {
                ...
                while (minRadBucket > 0 && sum < _minRightSum) {
                    ...
                    sum += b.weights[(int)TopicRadiusBucket.TopicRadiusEvent.trInside] + b.weights[(int)TopicRadiusBucket.TopicRadiusEvent.trOutside];
                }
                _minRadius = (ulong)Math.Pow(2, 64-(double)(minRadBucket)/_radiusBucketsPerBit);

                int lookupLeft = -1;
                if (needMoreLookups(0, maxBucket-_lookupWidth-1, maxValue)) {
                ...
                if (radiusLookup == -1) {
                    ...
                    if (rad > 0) {
                        newRadius = (ulong)Math.Pow(2, 64-(double)(rad)/_radiusBucketsPerBit );
                    }
                    radius = newRadius;
                }
            ...
            public LookupInfo nextTarget(bool forceRegular) {
                if (!forceRegular) {
                    ...
                    if (radiusLookup != -1) {
                        ...
                        return new LookupInfo(targetHash, topic, true);
                    }
                }
                ...
                if (rnd > radExt) {
                    ...
                    rnd = (ulong)radExt - rnd;
                }

                byte[] prefix = BitConverter.GetBytes(BitConverter.ToUInt64(topicHashPrefix, 0) ^ rnd);
                Keccak target;
                int lengthOfKeccakInBits = 256; //Keccak.OfAnEmptySequenceRlp.Bytes.Length
                target = new Keccak(prefix.Slice(0,8).Concat(CryptoRandom.GenerateRandomBytes(lengthOfKeccakInBits - 8)).ToArray());
            ...
            public void adjustWithTicket(long now, Keccak targetHash, in TicketRef t) {
                ...
                adjust(now, targetHash, t.t.node.IdHash, inside);
            }
