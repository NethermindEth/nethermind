...
namespace Nethermind.Network.Discovery.RoutingTable
{
    public interface ITicketProvider
    {
        ...
        List<TicketRef> ticketsInWindow(Topic topic);
        ...
        void registerLookupDone(LookupInfo lookup, List<Node> nodes, Func<Node, byte[]> ping);
        void searchLookupDone(LookupInfo lookup, List<Node> nodes, Func<Node, Topic, byte[]> query);
        void adjustWithTicket(long now, Keccak targetHash, in ITicket t);
        void addTicket(long localTime, byte[] pingHash, in Ticket ticket);
