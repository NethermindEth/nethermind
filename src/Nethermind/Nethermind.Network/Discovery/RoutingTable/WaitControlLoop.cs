...
namespace Nethermind.Network.Discovery.RoutingTable
{
    public class WaitControlLoop : IWaitControlLoop
    {
        ...
        public TimeSpan WaitPeriod;

        private readonly Random RandomNumberGenerator = new Random();

        public WaitControlLoop()
        {
            _wcTargetRegInterval = new TimeSpan(0, 10, 0) / _maxEntriesPerTopic;
        }
        public void registered(long time) {
            WaitPeriod = nextWaitPeriod(time);
            _lastIncoming = time;
        }
        ...
        private TimeSpan nextWaitPeriod(long time) {
            ...
            TimeSpan wp = new TimeSpan((long)(WaitPeriod.Ticks*nanosecPerTick * new TimeSpan((long)Math.Exp((_wcTargetRegInterval.Ticks-period.Ticks)/_wcTimeConst.Ticks)).TotalMilliseconds * 1000000 * nanosecPerTick));

            if (wp < _minWaitPeriod) {
                wp = _minWaitPeriod;
            }
            return wp;
        }
        ...
        public bool hasMinimumWaitPeriod() {
            return nextWaitPeriod(Stopwatch.GetTimestamp()) == _minWaitPeriod;
        }
        ...
        public TimeSpan noRegTimeout() {
            ...
            return new TimeSpan(new TimeSpan(0, 0, 1).Ticks * avgnoRegTimeout.Ticks * (long)e);
        }

        private double nextExpDouble() {
            double random_number = RandomNumberGenerator.NextDouble();
            return Math.Log10(1-random_number)/-1;
        }
    }
}