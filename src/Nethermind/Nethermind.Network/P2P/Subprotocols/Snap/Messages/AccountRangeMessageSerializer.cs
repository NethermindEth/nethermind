...
namespace Nethermind.Network.P2P.Subprotocols.Snap.Messages
{
    public class AccountRangeMessageSerializer : IZeroMessageSerializer<AccountRangeMessage>
    {
        ...
        public AccountRangeMessage Deserialize(IByteBuffer byteBuffer)
        {
            ...
            message.Proofs = rlpStream.DecodeArrayPoolList(static s => s.DecodeByteArray());

            return message;
        }

        private PathWithAccount DecodePathWithRlpData(IRlpReader stream)
        {
            stream.ReadSequenceLength();

            PathWithAccount data = new(stream.DecodeKeccak(), _decoder.Decode((RlpStream)stream));
