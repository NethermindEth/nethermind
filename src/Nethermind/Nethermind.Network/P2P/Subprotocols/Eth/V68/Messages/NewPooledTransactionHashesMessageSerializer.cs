...
namespace Nethermind.Network.P2P.Subprotocols.Eth.V68.Messages
{
        ...
        : IZeroMessageSerializer<NewPooledTransactionHashesMessage68>
    {
        ...
        private static NewPooledTransactionHashesMessage68 Deserialize(ref ValueRlpReader ctx)
        {
            ctx.ReadSequenceLength();
            ArrayPoolList<byte> types = ctx.DecodeByteArraySpan(TypesRlpLimit).ToPooledList();
            ArrayPoolList<int> sizes = ctx.DecodeArrayPoolList(static (ref ValueRlpReader c) => c.DecodeInt(), limit: SizesRlpLimit);
            ArrayPoolList<Hash256> hashes = ctx.DecodeArrayPoolList(static (ref ValueRlpReader c) => c.DecodeKeccak(), limit: HashesRlpLimit);
