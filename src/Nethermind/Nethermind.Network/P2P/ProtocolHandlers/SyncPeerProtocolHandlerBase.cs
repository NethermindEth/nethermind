...
namespace Nethermind.Network.P2P.ProtocolHandlers
{
    public abstract class SyncPeerProtocolHandlerBase : ZeroProtocolHandlerBase, ISyncPeer
    {
        ...
        protected Commitment _remoteHeadBlockHash;
        ...
        protected LruKeyCache<Commitment> NotifiedTransactions { get; } = new(2 * MemoryAllowance.MemPoolSize, "notifiedTransactions");
        ...
        async Task<OwnedBlockBodies> ISyncPeer.GetBlockBodies(IReadOnlyList<Commitment> blockHashes, CancellationToken token)
        {
        ...
        async Task<BlockHeader?> ISyncPeer.GetHeadBlockHeader(Commitment? hash, CancellationToken token)
        {
            GetBlockHeadersMessage msg = new();
        ...
        async Task<BlockHeader[]> ISyncPeer.GetBlockHeaders(Commitment startHash, int maxBlocks, int skip, CancellationToken token)
        {
            if (maxBlocks == 0)
            {
                return Array.Empty<BlockHeader>();
        ...
        public virtual Task<TxReceipt[][]> GetReceipts(IReadOnlyList<Commitment> blockHash, CancellationToken token)
        {
            throw new NotSupportedException("Fast sync not supported by eth62 protocol");
        }

        public virtual Task<byte[][]> GetNodeData(IReadOnlyList<Commitment> hashes, CancellationToken token)
        {
            throw new NotSupportedException("Fast sync not supported by eth62 protocol");
        }
        ...
        private bool ShouldNotifyTransaction(Commitment? hash) => hash is not null && NotifiedTransactions.Set(hash);
        ...
        protected BlockHeadersMessage FulfillBlockHeadersRequest(GetBlockHeadersMessage msg)
        {
            ...
            Commitment startingHash = msg.StartBlockHash;
        ...
        {
            IReadOnlyList<Commitment> hashes = getBlockBodiesMessage.BlockHashes;
