...
public class LEDBAT
{
    ...
    public void OnAck(ulong bytes_newly_acked, ulong flightSize, uint timestamp_difference_microseconds, uint nowMicros)
    {
        ...
        var delay = currentDelays.GetAvgFixed16Precision(nowMicros);
        var baseDelay = baseDelays.GetAvgFixed16Precision(nowMicros);
        ...
        var cwndSaturated = bytes_newly_acked + flightSize + MSS > WindowSize;
        ...
        if (cwndSaturated) // Calculate gain if the congestion window is saturated
        {
            ...
            var offTarget = ((TARGET << 16) - queueingDelay) / TARGET << 16;
            ...
            if (IsSlowStart)
            {
                var exponentialGain = (long)bytes_newly_acked;
        ...
        var newWindow = (uint)(WindowSize + gain);
        var maxAllowedCwnd =
            (uint)(flightSize +
    ...
    {
        var currentDelay = currentDelays.GetAvgFixed16Precision(UTPUtil.GetTimestamp());
    ...
    public void OnDataLoss(uint nowMicros)
    {
        ...
        if (_lastDataLossAdjustment / RTT != nowMicros / RTT)
        {
            ...
            WindowSize = Math.Max(WindowSize / 2, MIN_CWND * MSS);
        }
