    ...
    ) : EthereumTransactionProcessorBase(blobBaseFeeCalculator, specProvider, worldState, virtualMachine, codeInfoRepository, logManager)
{
    ...
    protected override TransactionResult ExecuteTransaction(Transaction tx, ITxTracer tracer, ExecutionOptions opts)
    {
        ...
        TransactionResult result = base.ExecuteTransaction(tx, tracer, opts);
        ...
        in TransactionSubstate substate, in EthereumGasPolicy unspentGas, in UInt256 gasPrice, int codeInsertRefunds, in EthereumGasPolicy floorGas, in EthereumGasPolicy intrinsicGasStandard)
    {
        ...
        return base.Refund(tx, header, spec, opts, substate, unspentGas, gasPrice, codeInsertRefunds, floorGas, intrinsicGasStandard);
    }
}
