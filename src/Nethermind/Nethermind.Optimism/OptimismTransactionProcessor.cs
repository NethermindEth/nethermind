    ...
    ) : EthereumTransactionProcessorBase(blobBaseFeeCalculator, specProvider, worldState, virtualMachine, codeInfoRepository, logManager)
{
    ...
    protected override GasConsumed Refund(Transaction tx, BlockHeader header, IReleaseSpec spec, ExecutionOptions opts,
        in TransactionSubstate substate, in EthereumGasPolicy unspentGas, in UInt256 gasPrice, int codeInsertRefunds, long selfDestructStateRefund, in EthereumGasPolicy floorGas, in EthereumGasPolicy intrinsicGasStandard)
    {
        ...
        return base.Refund(tx, header, spec, opts, substate, unspentGas, gasPrice, codeInsertRefunds, selfDestructStateRefund, floorGas, intrinsicGasStandard);
    }
}
