...
public class OptimismReceiptsRootCalculator : IReceiptsRootCalculator
{
    ...
    public Hash256 GetReceiptsRoot(TxReceipt[] receipts, IReceiptSpec spec, Hash256? suggestedRoot)
    {
        Hash256 SkipStateAndStatusReceiptsRoot()
        {
            ...
            try
            {
                return ReceiptTrie<OptimismTxReceipt>.CalculateRoot(spec, receipts.Cast<OptimismTxReceipt>().ToArray(), OptimismReceiptMessageDecoder.Instance);
            }
            finally
            {
        ...
        Hash256 receiptsRoot = ReceiptTrie<OptimismTxReceipt>.CalculateRoot(spec, receipts.Cast<OptimismTxReceipt>().ToArray(), OptimismReceiptMessageDecoder.Instance);
