...
public class EthereumEthApi(string l1EthApiEndpoint, IJsonSerializer jsonSerializer, ILogManager logManager) : IEthApi
{
    ...
    public Task<ReceiptForRpc[]?> GetReceiptsByHash(Hash256 blockHash) => _ethRpcClient.Post<ReceiptForRpc[]>("eth_getBlockReceipts", new object[] { blockHash });
    ...
    public Task<L1Block?> GetFinalized(bool fullTxs) => _ethRpcClient.Post<L1Block?>("eth_getBlockByNumber", BlockParameter.Finalized, fullTxs);

    public Task<L1Block?> GetSafe(bool fullTxs) => _ethRpcClient.Post<L1Block?>("eth_getBlockByNumber", BlockParameter.Safe, fullTxs);

    public async Task<ulong> GetChainId() => await _ethRpcClient.Post<ulong?>("eth_chainId") ?? throw new NullReferenceException();
}
