...
public class TxBroadcasterTests
{
    ...
    public void should_pick_best_persistent_txs_to_broadcast([Values(1, 2, 99, 100, 101, 1000)] int threshold)
    {
        ...
        Assert.That(pickedTxs, Is.EquivalentTo(expectedTxs));
    }
    ...
    public void should_add_light_form_of_blob_txs_to_persistent_txs_but_not_return_if_requested([Values(true, false)] bool isBlob)
    {
        ...
        Assert.That(_broadcaster.GetSnapshot().FirstOrDefault(), Is.EqualTo(isBlob ? new LightTransaction(tx) : tx));

        Assert.That(_broadcaster.TryGetPersistentTx(tx.Hash, out Transaction returnedTx), Is.EqualTo(!isBlob));
        Assert.That(returnedTx, Is.EqualTo(isBlob ? null : tx));
    }
    ...
    public void should_announce_details_of_full_blob_tx()
    {
        ...
        _broadcaster.Broadcast(tx, true);
        Assert.That(_broadcaster.GetSnapshot().Length, Is.EqualTo(1));
        Assert.That(_broadcaster.GetSnapshot().FirstOrDefault(), Is.EqualTo(lightTx));
        ...
        [Values(true, false)] bool useBlobTxs)
    {
        ...
        Assert.That(pickedFullTxs, Is.EquivalentTo(expectedFullTxs));
