...
/// widens the gap, so what this reports is a lower bound on the saving, not an estimate of it.
...
public class FrameTxBlobRevalidationReloadMeasurement
{
    ...
    public void Reloading_a_blob_frame_tx_prefix_costs(int blobsPerTx)
    {
        ...
        (TimeSpan fullElapsed, long fullAllocated) = TimeFull();
        (TimeSpan elidedElapsed, long elidedAllocated) = TimeElided();

        double fullPerTxUs = fullElapsed.TotalMicroseconds / SampleTxs;
        double elidedPerTxUs = elidedElapsed.TotalMicroseconds / SampleTxs;

        _report.AppendLine($"blobs per tx: {blobsPerTx}, samples: {SampleTxs}");
