...
namespace Nethermind.TxPool.Test
{
    ...
    public partial class TxPoolTests
    {
        ...
        [Test]
        // The gauge is a process-wide static and this fixture is ParallelScope.All, so any frame transaction
        // admitted alongside would move it.
        [NonParallelizable]
        public void Restored_blob_carrying_frame_tx_still_counts_against_its_payer()
        {
            ...
            Assert.That(_txPool.SubmitTx(tx, TxHandlingOptions.None), Is.EqualTo(AcceptTxResult.Accepted));
            Assert.That(Metrics.FrameTxPayersWithReservedExposure, Is.EqualTo(payersBefore + 1), "admission reserves against the payer");

            // A fresh pool over the same storage stands in for a node restart. The old pool is never disposed,
            // so its ledger entry stays counted; the restored one has to add its own on top.
            _txPool = CreatePool(txPoolConfig, GetBogotaSpecProvider(), txStorage: blobTxStorage);

            using (Assert.EnterMultipleScope())
            {
                Assert.That(_txPool.GetPendingBlobTransactionsCount(), Is.EqualTo(1), "the transaction survives the restart");
