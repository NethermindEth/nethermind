...
public class FrameTxMempoolDosMeasurement
{
    ...
    /// <summary>
    /// Where the Groth16 sweep artifacts live, overridable with <c>FRAME_GROTH16_ARTIFACTS</c>. A missing tree
    /// skips the Groth16 cases rather than failing them: the artifacts are built outside this repository.
    /// </summary>
    private const string DefaultGroth16ArtifactRoot =
        "/home/manuel/Work/Nethermind/Projects/frame-verify-gas-scaling/artifacts";
    ...
    private void MeasureFrameRejection(string shape, ulong ceiling)
    {
        ...
        Assert.That(Volatile.Read(ref Metrics.PendingTransactionsFrameTxSimulationFailed),
            Is.GreaterThan(simulationFailuresBefore), "the simulation-failure counter did not move");
    ...
    {
        string root = Environment.GetEnvironmentVariable("FRAME_GROTH16_ARTIFACTS") ?? DefaultGroth16ArtifactRoot;
    ...
    private sealed class TimingSimulator(IFrameTxPrefixSimulator inner, List<double> samples) : IFrameTxPrefixSimulator
    {
        public FrameTxSimulationResult Simulate(Transaction tx, CancellationToken token = default)
        {
            ...
            try
            {
                return inner.Simulate(tx, token);
            }
            finally
            {
