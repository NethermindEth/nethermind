...
public class BlobAndProofV2
{
    ...
    [SszVector(Ckzg.Ckzg.BytesPerBlob)]
    public Blob Blob { get; set; }


    [SszVector(Ckzg.Ckzg.CellsPerExtBlob)]
    public Proof[] Proofs { get; set; }
}

[SszSerializable]
[InlineArray(KzgPolynomialCommitments.BytesPerBlobVersionedHash)]
...
public struct BlobVersionedHash : IInlineArrayConvertable<BlobVersionedHash>
{
    ...
    {
        if (blobVersionedHash is not { Length: KzgPolynomialCommitments.BytesPerBlobVersionedHash })
        {
            throw new ArgumentException($"{nameof(BlobVersionedHash)} must be {KzgPolynomialCommitments.BytesPerBlobVersionedHash} bytes long");
        }
...
[SszSerializable]
[InlineArray(Ckzg.Ckzg.BytesPerProof)]
...
public struct Proof : IInlineArrayConvertable<Proof>
{
    ...
    {
        if (proof is not { Length: Ckzg.Ckzg.BytesPerProof })
        {
            throw new ArgumentException($"{nameof(Proof)} must be {Ckzg.Ckzg.BytesPerProof} bytes long");
        }
...
[SszSerializable]
[InlineArray(Ckzg.Ckzg.BytesPerBlob)]
...
public struct Blob : IInlineArrayConvertable<Blob>
{
    ...
    {
        if (blob is not { Length: Ckzg.Ckzg.BytesPerBlob })
        {
            throw new ArgumentException($"{nameof(Blob)} must be {Ckzg.Ckzg.BytesPerBlob} bytes long");
        }
