...
public class ForkchoiceStateV1
{
    ...
    public override string ToString() => $"ForkChoice: Head: {HeadBlockHash}, Safe: {SafeBlockHash}, Finalized: {FinalizedBlockHash}";
    public string ToString(long? headNumber, long? safeNumber, long? finalizedNumber) =>
        headNumber is null && safeNumber is null && finalizedNumber is null
            ? ToString()
            : $"ForkChoice: Head: {headNumber} ({HeadBlockHash}), Safe: {safeNumber} ({SafeBlockHash}), Finalized: {finalizedNumber} ({FinalizedBlockHash})";
}
