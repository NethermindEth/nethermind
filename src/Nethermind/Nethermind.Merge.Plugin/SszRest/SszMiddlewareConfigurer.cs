    ...
    public static void AddSszRpcEndpointHandlers(this IServiceCollection services)
    {
        foreach (SszRestEndpoint endpoint in SszRpcEndpointHandler.GetEndpoints())
            services.AddSingleton<ISszEndpointHandler>(sp =>
                SszRpcEndpointHandler.CreateHandler(
                    sp.GetRequiredService<IEngineRpcModule>(),
                    sp.GetRequiredService<ISpecProvider>(),
                    endpoint));

        services.AddSingleton<ISszEndpointHandler>(sp =>
            new ClientVersionSszHandler(sp.GetRequiredService<IEngineRpcModule>()));
        services.AddSingleton<ISszEndpointHandler>(sp =>
            new CapabilitiesSszHandler(sp.GetRequiredService<ISpecProvider>()));
    }
}
