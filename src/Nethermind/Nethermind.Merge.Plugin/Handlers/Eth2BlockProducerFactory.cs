    ...
    public class Eth2BlockProducerFactory
    {
            ...
            ILogManager logManager)
        {
            ...
            return new BlockProducerContext
            {
                ...
                TxSource = CreateTxSourceForProducer(txProcessingEnv, txPool, specProvider, logManager, miningConfig),
                ReadOnlyTxProcessingEnv = txProcessingEnv
            };
        }

        private ITxSource CreateTxSourceForProducer(ReadOnlyTxProcessingEnv processingEnv, ITxPool txPool, ISpecProvider specProvider, ILogManager logManager, IMiningConfig miningConfig) =>
            CreateTxPoolTxSource(processingEnv, txPool, miningConfig, specProvider, logManager);

        private TxPoolTxSource CreateTxPoolTxSource(ReadOnlyTxProcessingEnv processingEnv, ITxPool txPool, IMiningConfig miningConfig, ISpecProvider specProvider, ILogManager logManager)
        {
            ITxFilter txSourceFilter = CreateTxSourceFilter(miningConfig, specProvider);
            return new TxPoolTxSource(txPool, processingEnv.StateReader, logManager, txSourceFilter);
        }

        private ITxFilter CreateTxSourceFilter(IMiningConfig miningConfig, ISpecProvider specProvider) =>
            TxFilterBuilders.CreateStandardTxFilter(miningConfig, specProvider);
