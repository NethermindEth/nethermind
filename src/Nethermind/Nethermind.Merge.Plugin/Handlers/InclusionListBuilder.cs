...
internal class InclusionListBuilder(ITxPool txPool, IBlockTree blockTree, ISpecProvider specProvider, IReadOnlyStateProvider headState, IMergeConfig mergeConfig)
{
    ...
    /// for how the senders themselves are picked. Not gated on pool revalidation: the ready-tx snapshot applies
    /// no per-transaction spec check, so around a fork the draw can include entries the new spec rejects —
    /// either because the pool's background revalidation is catching up, or because the target block is the
    /// activation slot itself, whose timestamp is not known from the parent header. The cost is wasted list
    /// bytes.</remarks>
