...
public class EngineRpcCapabilitiesProvider : IRpcCapabilitiesProvider
{
    ...
    public IReadOnlyDictionary<string, (bool Enabled, bool WarnIfMissing)> GetEngineCapabilities()
    {
        if (_capabilities.IsEmpty)
        {
            ...
            #region Prague
            _capabilities[nameof(IEngineRpcModule.engine_getPayloadV4)] = (spec.DepositsEnabled || spec.ValidatorExitsEnabled, spec.DepositsEnabled || spec.ValidatorExitsEnabled);
            _capabilities[nameof(IEngineRpcModule.engine_forkchoiceUpdatedV4)] = (spec.DepositsEnabled || spec.ValidatorExitsEnabled, spec.DepositsEnabled || spec.ValidatorExitsEnabled);
            _capabilities[nameof(IEngineRpcModule.engine_newPayloadV4)] = (spec.DepositsEnabled || spec.ValidatorExitsEnabled, spec.DepositsEnabled || spec.ValidatorExitsEnabled);
            #endregion
        }
