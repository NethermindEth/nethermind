...
public interface IMergeDebugRpcModule : IRpcModule
{
    [JsonRpcMethod(Description = "Return a valid ExecutionPayload from a Block")]
    ResultWrapper<ExecutionPayloadForRpc> debug_generateNewPayload(BlockParameter blockParameter);

    [JsonRpcMethod(Description = "Return a valid ExecutionPayload from a range of block")]
    ResultWrapper<ExecutionPayloadForRpc> debug_generateNewPayload(BlockParameter blockParameterStart, BlockParameter blockParameterEnd);

    [JsonRpcMethod(Description = "Return a valid ExecutionPayload from a Block with the given transactions")]
    ResultWrapper<ExecutionPayloadForRpc> debug_generateNewPayloadWithTransactions(TransactionForRpc[] transactions);
