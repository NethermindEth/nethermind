        ...
        if (t.IsCompletedSuccessfully)
        {
            ...
            if (block is not null)
            {
                ...
                if (_logger.IsInfo)
                {
                    ...
                    {
                        _logger.Info($"Local block {blockFees.ToDecimal(null) / 1_000_000_000_000_000_000,7:N4}  {block.ToString(Block.Format.HashNumberDiffAndTx)} | {time.TotalMilliseconds,7:N2} ms");
                    }
                    else
                    {
                        _logger.Info($"Local block {blockFees.ToDecimal(null) / 1_000_000_000_000_000_000,7:N4}  {block.ToString(Block.Format.HashNumberMGasAndTx)} | {time.TotalMilliseconds,7:N2} ms");
                    }
