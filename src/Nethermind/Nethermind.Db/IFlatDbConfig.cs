...
public interface IFlatDbConfig : IConfig
{
    ...
    [ConfigItem(Description = "Delete flat storage slots, and their storage trie nodes, that belong to accounts which do not exist or have an empty storage root, and on an unwindowed archive the flat history storage rows whose account had no storage at their block. No valid state contains such entries; earlier versions left them behind when a contract was created and destroyed in the same block. The sweep runs once automatically on every database that has not been swept yet, in the background after startup, paced to take at most half of the wall clock and resuming from its cursor across restarts. Set to true to force a full pass on every start while it stays set; unset it once that pass has finished.", DefaultValue = "false")]
    bool SweepOrphanStorage { get; set; }

    [ConfigItem(Description = "Check the flat storage column, and on an unwindowed archive the flat history storage rows, for entries that belong to accounts without storage, in the background after every start, and log the counts without deleting anything.", DefaultValue = "false")]
    bool VerifyOrphanStorage { get; set; }
