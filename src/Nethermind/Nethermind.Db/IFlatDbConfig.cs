...
public interface IFlatDbConfig : IConfig
{
    ...
    [ConfigItem(Description = "Delete flat storage slots, and their storage trie nodes, that belong to accounts which do not exist or have an empty storage root, and on an unwindowed archive the flat history storage rows whose account had no storage at their block. No valid state contains such entries; earlier versions left them behind when a contract was created and destroyed in the same block. The sweep runs once automatically on every database that has not been swept yet and takes one sequential pass over the storage column; set to true to force another pass on the next start.", DefaultValue = "false")]
    bool SweepOrphanStorage { get; set; }

    [ConfigItem(Description = "Check the flat storage column for slots that belong to accounts which do not exist or have an empty storage root, on every start, and log the count without deleting anything. One sequential pass over the storage column.", DefaultValue = "false")]
