...
public interface IFlatDbConfig : IConfig
{
    ...
    [ConfigItem(Description = "Warm-up window, in seconds from startup, during which the GC pacer halves the gen1 interval (never below 1000 ms) and, when GcPaceGen2IntervalMs is set, starts a concurrent background gen2 at a quarter of that interval (never below 5000 ms), sweeping the LOH backlog of the pool-convergence allocation burst before it can race the GC heap hard limit into a blocking full collection. 0 disables the warm-up phase. Only meaningful when GcPaceIntervalMs is set.", DefaultValue = "0")]
