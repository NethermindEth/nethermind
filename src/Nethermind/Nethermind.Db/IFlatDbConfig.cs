...
public interface IFlatDbConfig : IConfig
{
    ...
    [ConfigItem(Description = "Maximum number of in-memory base snapshots before conversion to the persisted-snapshot tier kicks in. Counted as `SnapshotCount` of the in-memory repository, not a block-distance depth. Sized as a ~64 base-snapshot target plus CompactSize headroom (96 at the default CompactSize of 32), since a bulk conversion can drop the in-memory count by up to CompactSize at a boundary.", DefaultValue = "96")]
