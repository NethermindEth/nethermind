...
public interface IFlatDbConfig : IConfig
{
    ...
    [ConfigItem(Description = "Threads tracing the transactions of one covered block at the same time, each on the exact state before its transaction as the transaction index supplies it. 0 uses one per core, at most 16; 1 traces a block transaction by transaction as before. Only a node with the transaction index on is affected. Each of the debug and trace modules keeps a pool of this many processing environments, so up to twice this many can exist once both have been used; the chains of consecutive traced blocks that range traces read through are bounded to four of at most 128 blocks or 500,000 written entries each.", DefaultValue = "0")]
