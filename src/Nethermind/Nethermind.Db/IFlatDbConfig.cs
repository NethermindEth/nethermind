...
public interface IFlatDbConfig : IConfig
{
    ...
    [ConfigItem(Description = "EXPERIMENTAL / benchmark-only. Preserve the warmed Patricia account trie across consecutive writable flat-state scopes when the parent state root matches exactly.", DefaultValue = "true", HiddenFromDocs = true)]
