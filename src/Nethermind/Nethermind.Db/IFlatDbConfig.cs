...
{
    [ConfigItem(Description = "Overlay size threshold, in bytes, above which the Arena base store folds the RocksDB Account/Storage overlay into its shard tables. Approximate — tracked as bytes written since the last fold and reset on restart. 0 disables automatic folding (recommended for benchmark runs so no fold overlaps measurement; performed folds are counted by the BaseStoreFolds metric). Ignored unless BaseStore is 'Arena'.", DefaultValue = "268435456")]
