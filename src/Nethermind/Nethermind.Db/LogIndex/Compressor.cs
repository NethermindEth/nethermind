    ...
    private class Compressor : ICompressor
    {
        ...
        private void CompressValue(byte[] dbKey)
        {
            try
            {
                ...
                TryCompressValue(_storage._addressDb, dbKey);
                TryCompressValue(_storage._topicsDb, dbKey);
        ...
        private bool TryCompressValue(IDb db, byte[] dbKey)
        {
            ...
            if (db == _storage._addressDb)
                Interlocked.Increment(ref _stats.CompressedAddressKeys);
            else if (db == _storage._topicsDb)
