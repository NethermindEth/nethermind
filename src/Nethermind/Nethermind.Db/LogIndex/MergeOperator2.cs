...
partial class LogIndexStorage
{
    public class MergeOperator(ILogIndexStorage storage, ICompressor compressor, int? topicIndex) : IMergeOperator
    {
        ...
        private static void AddEnsureSorted(ReadOnlySpan<byte> key, ArrayPoolList<byte> result, ReadOnlySpan<byte> value, bool isBackwards)
        {
            ...
            var nextPos = GetValLogPos(value);
            var lastPos = result.Count > 0 ? GetValLastLogPos(result.AsSpan()) : (long?)null;
        ...
        private static long? FindNext(MergeOp op, RocksDbMergeEnumerator enumerator, ref int i)
        {
            while (i < enumerator.TotalCount && !MergeOps.Is2(op, enumerator.Get(i)))
                i++;

            if (i < enumerator.TotalCount && MergeOps.Is2(op, enumerator.Get(i), out long value))
                return value;
        ...
        private static long? Aggregate(MergeOp op, RocksDbMergeEnumerator enumerator, bool isBackwardSync)
        {
            ...
            for (var i = 0; i < enumerator.OperandsCount; i++)
            {
                if (!MergeOps.Is2(op, enumerator.GetOperand(i), out var next))
                    continue;
