...
public struct LogIndexAggregate(int firstBlockNum, int lastBlockNum)
{
    private Dictionary<Address, List<long>>? _address;
    private Dictionary<Hash256, List<long>>[]? _topic;

    public int FirstBlockNum { get; } = firstBlockNum;
    public int LastBlockNum { get; } = lastBlockNum;

    public Dictionary<Address, List<long>> Address => _address ??= new();

    public Dictionary<Hash256, List<long>>[] Topic => _topic ??= Enumerable.Range(0, LogIndexStorage.MaxTopics)
        .Select(static _ => new Dictionary<Hash256, List<long>>())
        .ToArray();
