...

{
    ...
    {
        public static IEnumerable<Commitment?> GetHashes(IEnumerable<Transaction> bundle2Txs) => bundle2Txs.Select(t => t.Hash);
        ...
        private static async Task<Commitment> SendSignedTransaction(TestMevRpcBlockchain chain, Transaction tx)
        {
            ResultWrapper<Commitment> result = await chain.EthRpcModule.eth_sendRawTransaction(EncodeTx(tx).Bytes);
        ...
        private static MevBundle SendBundle(TestMevRpcBlockchain chain, int blockNumber, BundleTransaction[] txs, bool success)
        {
            ...
            List<Commitment> revertingTxHashes = txs.Where(tx => tx.CanRevert).Select(tx => tx.Hash!).ToList();
