...
namespace Nethermind.DataMarketplace.Consumers.Infrastructure
{
    public static class NdmConsumersModule
    {
        ...
        {
            AddDecoders();
            var config = services.RequiredServices.Config;
            var dbProvider = services.RequiredServices.RocksProvider;
            ...
            var refundService = new RefundService(services.CreatedServices.BlockchainBridge,
                services.CreatedServices.AbiEncoder, services.RequiredServices.Wallet, config, logManager);
            ...
            switch (config.Persistence?.ToLowerInvariant())
            {
                case "mongo":
                    ...
                    break;
                default:
                    depositRepository = new DepositDetailsRocksRepository(dbProvider.DepositsDb,
                        depositDetailsRlpDecoder);
                    depositApprovalRepository = new ConsumerDepositApprovalRocksRepository(
                        dbProvider.ConsumerDepositApprovalsDb, depositApprovalRlpDecoder);
                    providerRepository = new ProviderRocksRepository(dbProvider.DepositsDb,
                        depositDetailsRlpDecoder);
                    receiptRepository = new ReceiptRocksRepository(dbProvider.ConsumerReceiptsDb,
                        receiptRlpDecoder);
                    sessionRepository = new ConsumerSessionRocksRepository(dbProvider.ConsumerSessionsDb,
                        sessionRlpDecoder);
                    break;
            }
            ...
            var depositService = new DepositService(services.CreatedServices.BlockchainBridge,
                services.CreatedServices.AbiEncoder, services.RequiredServices.Wallet, config, logManager);
            var consumerService = new ConsumerService(services.RequiredServices.ConfigManager, config.Id,
                ...
                config.BlockConfirmations, logManager);
