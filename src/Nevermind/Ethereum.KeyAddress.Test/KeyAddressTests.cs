...
namespace Ethereum.KeyAddress.Test
{
    public class KeyAddressTests
    {
        ...
        {
            Assembly assembly = typeof(KeyAddressTestJson).Assembly;
            string[] resourceNames = Assembly.GetExecutingAssembly().GetManifestResourceNames();
            string resourceName = resourceNames.SingleOrDefault(r => r.Contains("keyaddrtest.json"));
            using (Stream stream = assembly.GetManifestResourceStream(resourceName))
            {
                ...
                using (StreamReader reader = new StreamReader(stream))
                {
                    ...
                    KeyAddressTestJson[] testSpecs =
                        JsonConvert.DeserializeObject<KeyAddressTestJson[]>(testJson);
                    return testSpecs.Select(p => new KeyAddressTest(p.Seed, p.Key, p.Addr,
                        BigInteger.Parse(p.Signature.R), BigInteger.Parse(p.Signature.S), byte.Parse(p.Signature.V)));
                }
            }
        }
