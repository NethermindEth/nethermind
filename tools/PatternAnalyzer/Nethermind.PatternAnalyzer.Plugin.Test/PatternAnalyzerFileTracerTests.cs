...
public class PatternAnalyzerFileTracerTests : VirtualMachineTestsBase
{
...

    {
        ...
        _tracer = new PatternAnalyzerFileTracer(1, 100, _statsAnalyzer, new HashSet<Instruction>(), _fileSystem,
            _logger, 1, SortOrder.Descending, _testFileName, CancellationToken.None);
    ...
    private static IEnumerable<TestCaseData> GetTraceCases()
    {
        ...
        yield return new TestCaseData(
            new[]
            {
                Prepare.EvmCode.JUMPDEST().PushData(1000).GAS().LT().PUSHx([0, 26]).JUMPI().PushData(23).PushData(7)
                    .ADD().POP().PushData(42).PushData(5).ADD().POP().PUSHx([0, 0]).JUMP().JUMPDEST().STOP().Done
            },
