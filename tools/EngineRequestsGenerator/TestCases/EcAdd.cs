    ...
    private static byte[] PrepareCode(TestCase testCase)
    {
        switch (testCase)
        {
            case TestCase.EcAddInfinities:
                return PrepareCode(UInt256.Zero, UInt256.Zero, UInt256.Zero, UInt256.Zero);
            case TestCase.EcAdd12:
                return PrepareCode(UInt256.One, 2, UInt256.One, 2);
            case TestCase.EcAdd32ByteCoordinates:
                ...
                return PrepareCode(new UInt256(x1), new UInt256(y1), new UInt256(x2), new UInt256(y2));
            default:
                throw new ArgumentOutOfRangeException(nameof(testCase), testCase, null);
        }
    }

    private static byte[] PrepareCode(UInt256 x1, UInt256 y1, UInt256 x2, UInt256 y2)
    {
        List<byte> codeToDeploy = new();

        codeToDeploy.Add((byte)Instruction.PUSH32);     // x1
        codeToDeploy.AddRange(x1.ToLittleEndian());
