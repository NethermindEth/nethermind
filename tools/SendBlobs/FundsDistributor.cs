...
internal class FundsDistributor
{
    ...
    public FundsDistributor(RpcClient rpcClient, ulong chainId, string? keyFilePath, ILogManager logManager)
    {
    ...
    {
        string? balanceString = await _rpcClient.SendRequestAsync<string>("eth_getBalance", null, distributeFrom.Address, "latest");
        if (balanceString is null)
            throw new AccountException($"Unable to get balance for {distributeFrom.Address}");
        string? nonceString = await _rpcClient.SendRequestAsync<string>("eth_getTransactionCount", null, distributeFrom.Address, "latest");
        if (nonceString is null)
            throw new AccountException($"Unable to get nonce for {distributeFrom.Address}");

        string? gasPriceRes = await _rpcClient.SendRequestAsync<string>("eth_gasPrice") ?? "1";
        ...
        if (maxPriorityFee == 0)
        {
            maxPriorityFeePerGasRes = await _rpcClient.SendRequestAsync<string>("eth_maxPriorityFeePerGas") ?? "1";
        ...
        using (keyWriter)
        {
            foreach (PrivateKey key in privateKeys)
            {
                if (maxFee == 0)
                {
                    gasPriceRes = await _rpcClient.SendRequestAsync<string>("eth_gasPrice") ?? "1";
                ...
                string? result = await _rpcClient.SendRequestAsync<string>("eth_sendRawTransaction", null, $"0x{txRlp}");
    ...
    public async Task<IEnumerable<string>> ReclaimFunds(Address beneficiary, UInt256 maxFee, UInt256 maxPriorityFee)
    {
        ...
        {
            string? balanceString = await _rpcClient.SendRequestAsync<string>("eth_getBalance", null, signer.Address, "latest");
            if (balanceString is null)
                continue;
            string? nonceString = await _rpcClient.SendRequestAsync<string>("eth_getTransactionCount", null, signer.Address, "latest");
            if (nonceString is null)
                continue;
            ...
            if (maxPriorityFee == 0)
            {
                string? maxPriorityFeePerGasRes = await _rpcClient.SendRequestAsync<string>("eth_maxPriorityFeePerGas") ?? "1";
                maxPriorityFeePerGas = HexConvert.ToUInt256(maxPriorityFeePerGasRes);
            }
