...
internal class FundsDistributor
{
    ...
    public async Task<IEnumerable<string>> DistributeFunds(Signer distributeFrom, uint keysToMake, UInt256 maxFee, UInt256 maxPriorityFee)
    {
        ...
        ulong? nonce = await RpcHelpers.GetNonceAsync(_rpcClient, distributeFrom.Address);
        if (nonce is null)
            throw new AccountException($"Unable to get nonce for {distributeFrom.Address}");

        UInt256 gasPrice = await RpcHelpers.GetGasPriceAsync(_rpcClient);
        UInt256 maxPriorityFeePerGas = maxPriorityFee == 0
            ? await RpcHelpers.GetMaxPriorityFeePerGasAsync(_rpcClient)
            : maxPriorityFee;
        ...
        using (keyWriter)
        {
            foreach (PrivateKey key in privateKeys)
            {
                if (maxFee == 0)
                {
                    gasPrice = await RpcHelpers.GetGasPriceAsync(_rpcClient);
                    maxPriorityFeePerGas = await RpcHelpers.GetMaxPriorityFeePerGasAsync(_rpcClient);
                }
