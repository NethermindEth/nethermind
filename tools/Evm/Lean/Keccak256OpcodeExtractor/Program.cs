...
internal static class Program
{
    private static int Main(string[] args)
    {
        try
        {
            ...
            ExtractionResult result = Keccak256OpcodeProfile.Extract(
                arguments.RepoRoot,
                arguments.OutputDirectory,
                arguments.LeanOutputPath);
            Console.WriteLine(
                $"Extracted {result.SourceCount} sources, {result.AdmissionCount} exact syntax admissions, and {result.SpecializationCount} closed roots.");
