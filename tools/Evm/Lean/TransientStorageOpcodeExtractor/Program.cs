...
internal static class Program
{
    public static int Main(string[] args)
    {
        try
        {
            ...
            {
                Console.Error.WriteLine("Usage: TransientStorageOpcodeExtractor <repo-root> [output-directory] [lean-output-path]");
                return 2;
            }
            ...
            string output = args.Length >= 2
                ...
                : Path.Combine(repoRoot, TransientStorageOpcodeProfile.DefaultOutputRelativePath);
            string? lean = args.Length == 3 ? Path.GetFullPath(args[2]) : null;
            ExtractionResult result = TransientStorageOpcodeProfile.Extract(repoRoot, output, lean);
