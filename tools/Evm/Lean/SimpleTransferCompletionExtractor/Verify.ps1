...
$scratch = [System.IO.Path]::GetFullPath((Join-Path $scratchBase ("simple-transfer-completion-" + [Guid]::NewGuid().ToString("N"))))
...
$checkedLean = Join-Path $checkedIn "SimpleTransferCompletion.lean"
...
try {
    ...
    $freshLean = Join-Path $scratchGenerated "SimpleTransferCompletion.lean"
    ...
    foreach ($name in @(
        "SimpleTransferCompletion.ir.json",
        "SimpleTransferCompletion.source-manifest.json",
        "SimpleTransferCompletion.lean"
    )) {
    ...
    dotnet test --project $testProject -c $Configuration -p:SaveDiskSpace=true -p:TreatWarningsAsErrors=true --no-build -- --minimum-expected-tests 16 --no-ansi --progress off
    ...
    try {
        ...
        foreach ($leanFile in @(
            "Generated\SimpleTransferCompletion.lean",
            "Reference\SimpleTransferCompletionReference.lean",
            "Refinement\SimpleTransferCompletionRefinement.lean",
            "Vectors\SimpleTransferCompletionVectors.lean"
        )) {
