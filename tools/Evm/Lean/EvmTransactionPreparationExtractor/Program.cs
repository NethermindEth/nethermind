    ...
    private static int Main(string[] args)
    {
        try
        {
            ...
            if (arguments.Check)
            {
                ...
                Console.WriteLine("Checked source-derived transaction-preparation IR, manifest, and Lean artifacts.");
            }
            else
            {
                ExtractionResult result = Extractor.Extract(arguments.RepoRoot, arguments.OutputDirectory, arguments.LeanOutputPath);
                Console.WriteLine($"Extracted {result.SourceCount} exact source identities, {result.MemberCount} members, {result.OperationCount} operations, and {result.BranchCount} branches.");
