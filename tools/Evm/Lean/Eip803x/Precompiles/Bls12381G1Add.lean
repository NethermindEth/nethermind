...
def amsterdam : Schedule := { fixedGas := 375 }

end Schedule

def address : Nat := 11

def name : String := "BLS12_G1ADD"
...
theorem metadata_contract :
    address = 11 ∧ name = "BLS12_G1ADD" ∧ supportsCaching = true := by
  native_decide

theorem amsterdam_gas_cost (input : List Byte) :
    totalGasCost Schedule.amsterdam input = 375 := by
  rfl
