...
$inventoryName = "EXPORTED_BRANCH_THEOREMS.txt"
$inventoryBytes = [IO.File]::ReadAllBytes((Join-Path $package $inventoryName))
$inventoryCount = 2144
$inventoryHash = '466024c46460b510b09519b1b860c73970c8b4b2585c5fa2a6bcd1664e4bd133'
$namespaces = @(
...
function Read-BranchInventory([byte[]]$Bytes) {
    $digest = [Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($Bytes)).ToLowerInvariant()
    if ($digest -cne $inventoryHash) {
        throw "Branch exported-theorem inventory changed; explicit review is required."
    }
    ...
    if ($names.Count -ne $inventoryCount -or [Collections.Generic.HashSet[string]]::new([string[]]$names, [StringComparer]::Ordinal).Count -ne $names.Count) {
        throw "Branch theorem inventory must contain the complete distinct descendant export roster."
    }
    foreach ($name in $names) {
        if ($name -notmatch '^[A-Za-z_][A-Za-z0-9_]*(\.[A-Za-z_][A-Za-z0-9_]*)+$' -or
            ...
            throw "Invalid branch theorem name: $name"
        }
    }
    return ,$names
}
$expected = Read-BranchInventory $inventoryBytes

$checker = @'
open Lean Elab Command

def checkBranchAxioms (name : Name) : CommandElabM (Array Name) := do
  let dependencies ← Lean.collectAxioms name
  let allowed := ["propext", "Classical.choice", "Quot.sound"]
  for dependency in dependencies do
    unless allowed.contains dependency.toString do
      throwError "BRANCH_AXIOM_FORBIDDEN {name}: {dependency}"
  return dependencies

elab "audit_branch_export " target:ident : command => do
  let name := target.getId
  let env ← getEnv
  match env.find? name with
  | some (.thmInfo _) => pure ()
  | _ => throwError "BRANCH_AXIOM_MISSING {name}"
  let dependencies ← checkBranchAxioms name
  let result := Json.mkObj [("theorem", toJson name.toString),
    ("axioms", toJson (dependencies.map Name.toString))]
  logInfo m!"BRANCH_AXIOM_RESULT|{result.compress}"
'@
$completeness = @'
run_cmd do
  ...
  for (name, info) in env.constants do
    if namespaces.any (fun namespaceName => name.toString.startsWith (namespaceName ++ ".")) then
      if let .thmInfo _ := info then
        let _ ← checkBranchAxioms name
        unless expected.contains name.toString do
          throwError "BRANCH_AXIOM_UNEXPECTED_EXPORT {name}"
        found := found + 1
  unless found == expected.size do
    throwError "BRANCH_AXIOM_EXPORT_COUNT {found} != {expected.size}"
'@
...
function Invoke-BranchAuditLean([string]$Path) {
...
function Read-BranchAuditResults([string]$Output, [string[]]$Names) {
    $records = [Collections.Generic.Dictionary[string, object]]::new([StringComparer]::Ordinal)
    foreach ($line in ($Output -split '\r?\n')) {
        if ($line -match 'BRANCH_AXIOM_RESULT\|(\{.*\})$') {
            ...
            if (@($record.PSObject.Properties.Name).Count -ne 2 -or
                ...
                throw "Malformed, duplicate, or unexpected branch axiom result."
            }
            foreach ($dependency in $record.axioms) {
                if (@('propext', 'Classical.choice', 'Quot.sound') -cnotcontains $dependency) {
                    throw "Forbidden branch axiom result: $dependency"
                }
            }
            $records.Add($record.theorem, $record)
        }
    }
    if ($records.Count -ne $Names.Count) { throw "Missing branch axiom-audit results." }
    return ,$records
}
function Complete-BranchExports([string[]]$Names) {
    $quoted = ($Names | ForEach-Object { '"' + $_ + '"' }) -join ','
    return $completeness.Replace('__EXPECTED__', $quoted)
}

$scratch = [IO.Path]::GetFullPath((Join-Path $package (".branch-axioms-" + [Guid]::NewGuid().ToString("N"))))
if (-not $scratch.StartsWith($package + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) {
    throw "Branch axiom-audit scratch escaped the package."
}
try {
    New-Item -ItemType Directory -Path $scratch | Out-Null
    Push-Location $package
    try {
        $main = $imports + $checker + "`n" + (Complete-BranchExports $expected) + "`n" +
            (($expected | ForEach-Object { "audit_branch_export $_" }) -join "`n")
        $mainPath = Join-Path $scratch "ExportAxioms.lean"
        [IO.File]::WriteAllText($mainPath, $main, [Text.UTF8Encoding]::new($false))
        $result = Invoke-BranchAuditLean $mainPath
        if ($result.ExitCode -ne 0) { throw "Branch exported-theorem axiom baseline failed: $($result.Output)" }
        $records = Read-BranchAuditResults $result.Output $expected
        ...
        foreach ($altered in @(
            ...
            $rejected = $false
            try { $null = Read-BranchInventory ([Text.Encoding]::UTF8.GetBytes($altered + "`n")) }
            catch { $rejected = $_.Exception.Message -eq "Branch exported-theorem inventory changed; explicit review is required." }
            if (-not $rejected) { throw "Branch inventory negative control survived." }
        }

        $first = 'BRANCH_AXIOM_RESULT|{"theorem":"' + $expected[0] + '","axioms":[]}'
        foreach ($test in @(
            @{ Output = ''; Error = 'Missing branch axiom-audit results.' },
            @{ Output = $result.Output + "`n" + $first; Error = 'Malformed, duplicate, or unexpected branch axiom result.' },
            @{ Output = 'BRANCH_AXIOM_RESULT|{"theorem":"unexpected","axioms":[]}'; Error = 'Malformed, duplicate, or unexpected branch axiom result.' },
            @{ Output = 'BRANCH_AXIOM_RESULT|{"theorem":"' + $expected[0] + '"}'; Error = 'Malformed, duplicate, or unexpected branch axiom result.' },
            @{ Output = 'BRANCH_AXIOM_RESULT|{"theorem":"' + $expected[0] + '","axioms":["injected"]}'; Error = 'Forbidden branch axiom result: injected' }
        )) {
            $rejected = $false
            try { $null = Read-BranchAuditResults $test.Output $expected }
            catch { $rejected = $_.Exception.Message -eq $test.Error }
            if (-not $rejected) { throw "Branch axiom result negative control survived: $($test.Error)" }
        }
        foreach ($namespaceName in $namespaces) {
            foreach ($forbidden in @($false, $true)) {
                ...
                $body = "namespace $prefix`n$declarations`nend $prefix`n" + (Complete-BranchExports $expected)
                ...
                $negative = Invoke-BranchAuditLean $testPath
                $marker = if ($forbidden) { "BRANCH_AXIOM_FORBIDDEN $prefix.injectedExport: $prefix.injectedAxiom" }
                    else { "BRANCH_AXIOM_UNEXPECTED_EXPORT $prefix.injectedExport" }
                if ($negative.ExitCode -eq 0 -or -not $negative.Output.Contains($marker, [StringComparison]::Ordinal) -or
                    ...
                    throw "Branch lineage completeness control failed: $prefix forbidden=$forbidden`: $($negative.Output)"
                }
            }
        }

        foreach ($test in @(
            @{ Name = 'Missing'; Imports = "import Lean`n"; Body = 'audit_branch_export DoesNotExist'; Error = 'BRANCH_AXIOM_MISSING DoesNotExist' },
            @{ Name = 'Forbidden'; Imports = "import Lean`n"; Body = "axiom branchInjectedAxiom : False`ntheorem branchInjectedExport : False := branchInjectedAxiom`naudit_branch_export branchInjectedExport"; Error = 'BRANCH_AXIOM_FORBIDDEN branchInjectedExport: branchInjectedAxiom' },
            @{ Name = 'UnexpectedExport'; Imports = $imports; Body = "namespace SequentialBlockPostTransactionFinalizationExtractor.Refinement.SequentialBlockPostTransactionFinalization`ntheorem branchUnexpectedExport : True := True.intro`nend SequentialBlockPostTransactionFinalizationExtractor.Refinement.SequentialBlockPostTransactionFinalization`n" + (Complete-BranchExports $expected); Error = 'BRANCH_AXIOM_UNEXPECTED_EXPORT SequentialBlockPostTransactionFinalizationExtractor.Refinement.SequentialBlockPostTransactionFinalization.branchUnexpectedExport' },
            @{ Name = 'NestedExport'; Imports = $imports; Body = "namespace SequentialBlockPostTransactionFinalizationExtractor.Refinement.SequentialBlockPostTransactionFinalization.Nested.Audit`ntheorem injectedNestedExport : True := True.intro`nend SequentialBlockPostTransactionFinalizationExtractor.Refinement.SequentialBlockPostTransactionFinalization.Nested.Audit`n" + (Complete-BranchExports $expected); Error = 'BRANCH_AXIOM_UNEXPECTED_EXPORT SequentialBlockPostTransactionFinalizationExtractor.Refinement.SequentialBlockPostTransactionFinalization.Nested.Audit.injectedNestedExport' },
            @{ Name = 'NestedForbidden'; Imports = $imports; Body = "namespace SequentialBlockPostTransactionFinalizationExtractor.Refinement.SequentialBlockPostTransactionFinalization.Nested.Audit`naxiom injectedNestedAxiom : False`ntheorem injectedNestedExport : False := injectedNestedAxiom`nend SequentialBlockPostTransactionFinalizationExtractor.Refinement.SequentialBlockPostTransactionFinalization.Nested.Audit`n" + (Complete-BranchExports $expected); Error = 'BRANCH_AXIOM_FORBIDDEN SequentialBlockPostTransactionFinalizationExtractor.Refinement.SequentialBlockPostTransactionFinalization.Nested.Audit.injectedNestedExport: SequentialBlockPostTransactionFinalizationExtractor.Refinement.SequentialBlockPostTransactionFinalization.Nested.Audit.injectedNestedAxiom' },
            @{ Name = 'OmittedExport'; Imports = $imports; Body = (Complete-BranchExports $expected[1..($expected.Count - 1)]); Error = 'BRANCH_AXIOM_UNEXPECTED_EXPORT ' + $expected[0] }
        )) {
            ...
            if ($negative.ExitCode -eq 0 -or -not $negative.Output.Contains($test.Error, [StringComparison]::Ordinal) -or
                $negative.Output -match 'unknown (identifier|constant|module)|unexpected token|maximum recursion depth|maximum heartbeats|maximum number of steps|deterministic timeout') {
                throw "Branch axiom negative/completeness control failed: $($test.Name): $($negative.Output)"
            }
        }
        Write-Host "Verified all frozen branch theorem exports, standard-only axioms, and negative/completeness controls."
    }
