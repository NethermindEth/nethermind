...
internal static class Program
{
    private static int Main(string[] args)
    {
        try
        {
            ...
            ExtractionResult result = StackRearrangementOpcodeProfile.Extract(repoRoot, output, leanOutput);
            Console.WriteLine(
                $"Extracted {result.OpcodeCount} opcode(s), {result.SpecializationCount} specialization(s), and {result.AdmissionCount} complete syntax admission(s).");
            return 0;
        }
