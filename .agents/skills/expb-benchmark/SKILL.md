...
- Renders a temporary config (does not modify source files) by:
  ...
  - setting `resources.cpu=0` to disable the Docker CPU quota while preserving CPU affinity; quota-free runs use a separate master metrics cache from earlier quota-limited runs
