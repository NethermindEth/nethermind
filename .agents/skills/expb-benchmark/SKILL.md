...
- A multi-image dispatch (`docker_images`, or a retrospective sweep) is **one sequential campaign job**, not a per-image matrix: [`scripts/expb/sequential_driver.py`](../../../scripts/expb/sequential_driver.py) runs every image x run in order on one runner, writes `campaign.json` plus one directory per sample, and emits the `summary.md` that `multi-campaign-summary` publishes. Everything is uploaded as `expb-campaign-<run-id>-<attempt>`. A failed sample always abandons the rest of its own image; an explicit `docker_images` A/B then stops the campaign (a bad arm invalidates the comparison), while a retrospective sweep carries on with the remaining images.
...
- CPU budget experiments: `-f docker_cpu_overrides="cpu=<n|all> cpuset=<list|all> infra_cpuset=<list|all>"` (one input because `workflow_dispatch` allows at most 25; any subset of the keys, space- or semicolon-separated) overrides `resources.cpu` / `resources.cpuset` / `resources.infra_cpuset` for that dispatch only, in both the single-image job and the sequential campaign driver (the runner's config file is untouched; a missing key = configured value). The override is applied before the whole-core affinity pinning, so `cpu=<n>` picks n CPUs of the (possibly overridden) cpuset as whole cores and no Docker quota is ever applied; `cpu=all` pins the whole cpuset; `cpuset=all` leaves the client unpinned and implies `cpu=all`; `infra_cpuset=all` unpins k6, payload-server and Alloy. The single-image render step prints the effective `resources` block (the campaign keeps it in each sample's `config.json`), and while an override is active each run prints every expb container's `cpu.max` and `cpuset.cpus.effective` from the host cgroup (the campaign also records them as `cgroup_limits` in the sample's `metadata.json`), so read those rather than trusting the inputs. amd64's configured cpuset is 2-7,10-15 (6 physical cores with their SMT siblings) with `resources.cpu: 8`, so the client is pinned to 4 whole cores by default; `cpu=12` gives it all 6, and raising `cpu` past 12 changes nothing without also widening `cpuset`: the cpuset is the cap.
- Optional low-variance mode: pass `-f measurement_mode=compute-warm` to enable expb's per-block EVM warmup (`eth_simulateV1` before each measured block) together with the raised RPC gas cap it needs. It serves the measured block's reads from warm caches, which lowers both run-to-run CV (~1.8%→~0.55% on flat-realblocks) and AVG. The mode sets `EXPB_EVM_WARMUP=1` and `--JsonRpc.GasCap=1000000000000` itself and **rejects** a dispatch that also passes either by hand (`expb_env="EXPB_EVM_WARMUP=1"`, `additional_extra_flags="--JsonRpc.GasCap=..."`); without the raised cap the per-request gas budget (default 100M) is exhausted on dense blocks and the warmup `eth_simulateV1` calls fail with `-38013` (intrinsic gas), silently leaving those blocks un-warmed. It also verifies every measured block was warmed and fails the sample otherwise. Caveat: warmup minimizes cold RocksDB/storage interaction, so it is a low-variance *compute* signal, not a substitute for the default cold benchmark — don't use it when measuring storage-layer changes.
