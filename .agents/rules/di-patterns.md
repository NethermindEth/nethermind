...
## Anti pattern
- Using the form `.Add<IFoo>(ctx => new Foo(ctx.Resolve<Dep1>(), ctx.Resolve<Dep2>())` is an anti pattern. It will cause change to the wiring when `Foo` added new dependencies which add review load.
