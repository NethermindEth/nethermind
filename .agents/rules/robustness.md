...
- Outside code compiled with `EnableZkEvm=true`, do not add or retain a `try`/`finally` solely to guarantee `ArrayPool<T>.Shared.Return(array)` after an unexpected exception. The abandoned array remains GC-reclaimable, and any replacement allocation is dwarfed by the exception. Return it on the normal path; if the code deliberately throws while it still owns the array, return it immediately before throwing. A shared-array return may remain in a `finally` already required for other cleanup. Never return an array that may already have been returned or handed off — a double return silently corrupts the pool.
