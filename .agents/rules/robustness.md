...
- Return `ArrayPool<T>.Shared` rentals on the normal path and before an expected failure is handled, retried, or rethrown to a recovery boundary. Keep an existing `finally` when it covers those paths; do not replace it with catches solely for `Return`. If the code deliberately throws while it still owns the array, return it immediately before throwing. Never return an array that may already have been returned or handed off — a double return silently corrupts the pool.
