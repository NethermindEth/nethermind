...
- Use `Array.Empty<T>()` or `[]` instead of `new T[0]` — avoids allocating a new empty array each time. In contexts that require compile-time constants (attributes, default parameter values), `new T[0]` is acceptable.
