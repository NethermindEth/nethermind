...
public abstract class AmsterdamTransitionBlockChainTestFixture<TSelf> : BlockchainTestBase
{
    ...
    public static IEnumerable<BlockchainTest> LoadTests() =>
        new TestsSourceLoader(Constants.Strategy,
            $"fixtures/blockchain_tests/for_bpo2toamsterdamattime15k/{FixturePath<TSelf>()}")
        .LoadTests<BlockchainTest>();

    private static string FixturePath<T>() =>
        (typeof(T).GetCustomAttributes(typeof(AmsterdamFixturePathAttribute), false)
            is [AmsterdamFixturePathAttribute attr, ..])
            ? attr.Path
            : throw new InvalidOperationException(
                $"{typeof(T).Name} must be annotated with [AmsterdamFixturePath(...)].");
}
