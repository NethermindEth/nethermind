...
COPY src/Nethermind/Nethermind.Runner/pgo/nethermind.jit.g[z] /tmp/
RUN mkdir -p pgo && \
  if [ -f /tmp/nethermind.jit.gz ]; then \
    gzip -d -c /tmp/nethermind.jit.gz > pgo/nethermind.jit && rm /tmp/nethermind.jit.gz; \
  fi
