...
Before starting, publish the Nethermind `perf/linux-block-priority-prototype` branch (which contains the workflow and verifier), the EXPB `perf/linux-priority-prototype` branch, and a Nethermind prototype image containing the runtime change with both amd64 and arm64 manifests. Prefer an immutable image digest or pinned tag, and use that exact image for every arm. The workflow's `docker_images` input selects explicit images and `rebuild_docker=false` prevents a build. Keep `state_layout`, `payload_set`, `amount`, `delay_seconds`, `additional_extra_flags`, `flat_write_buffer_floor`, `run_count`, and profiling inputs identical.
