...
dotnet publish -c release -r $LINUX --self-contained true -p:Version=$1 -p:Commit=$2 -p:PublishSingleFile=true -p:IncludeAllContentForSelfExtract=true -o $OUT/$LIN_RELEASE
dotnet publish -c release -r $OSX --self-contained true -p:Version=$1 -p:Commit=$2 -p:PublishSingleFile=true -p:IncludeAllContentForSelfExtract=true -o $OUT/$OSX_RELEASE
dotnet publish -c release -r $WIN10 --self-contained true -p:Version=$1 -p:Commit=$2 -p:PublishSingleFile=true -p:IncludeAllContentForSelfExtract=true -o $OUT/$WIN_RELEASE
dotnet publish -c release -r $OSX_ARM64 --self-contained true -p:Version=$1 -p:Commit=$2 -p:PublishSingleFile=true -p:IncludeAllContentForSelfExtract=true -o $OUT/$OSX_ARM64_RELEASE


cp $ARM_ROCKSDB_PATH/librocksdb.so ../../rocksdb-sharp/RocksDbNative/runtimes/linux-arm64/native/librocksdb.so
dotnet publish -c release -r $LINUX_ARM64 -p:Version=$1 -p:Commit=$2 -p:PublishSingleFile=true -p:IncludeAllContentForSelfExtract=true -o $OUT/$LIN_ARM64_RELEASE
