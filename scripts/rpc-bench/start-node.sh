...
account_index_node_option() {
  case "$ACCOUNT_INDEX_PREPARE_MODE" in
    binary) return 0 ;;
    interpolation)
      printf '%s\n' "--Db.FlatAccountDbAdditionalRocksDbOptions=block_based_table_factory.index_block_search_type=kInterpolation;"
      ;;
    auto)
      printf '%s\n' "--Db.FlatAccountDbAdditionalRocksDbOptions=block_based_table_factory.index_block_search_type=kAuto;block_based_table_factory.uniform_cv_threshold=${ACCOUNT_INDEX_PREPARE_THRESHOLD};"
      ;;
...
if [[ "$ACCOUNT_INDEX_SWEEP" == "true" ]]; then
  account_index_option="$(account_index_node_option)"
  [[ -z "$account_index_option" ]] || node_args+=("$account_index_option")
fi
