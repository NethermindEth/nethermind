...
MANIFEST_LINE_PATTERN = re.compile(
    r"(?P<kind>iso|mix)\|(?P<labels>[A-Za-z0-9._|-]+)=(?P<path>.+jsonbench-summary\.md)$")


def _stage_manifest(path: Path, source_root: Path, target: Path) -> None:
    ...
    only leak runner directory layout, so the staged copy carries artifact-relative paths and
    any line that does not parse, or points outside the output root, fails staging.
    """
    lines_out = []
    for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        if not match:
            raise CorpusResultsError(f"{path.name}: line {number} does not match the manifest shape")
        entry = Path(match.group("path"))
        try:
            relative = entry.resolve().relative_to(source_root.resolve())
        except ValueError:
            raise CorpusResultsError(
                f"{path.name}: line {number} points outside the output root") from None
        lines_out.append(f"{match.group('kind')}|{match.group('labels')}={relative.as_posix()}")
    target.write_text("\n".join(lines_out) + "\n", encoding="utf-8")
    ...
    for path in sorted(source_root.rglob("*")):
        ...
        try:
            if path.name == "summary.json":
                ...
                target.parent.mkdir(parents=True, exist_ok=True)
                _stage_manifest(path, source_root, target)
                staged += 1
                continue
