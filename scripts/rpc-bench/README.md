...
The RPC workflow has a separate opt-in for the Account-index runtime-option sweep:
...
arms sequentially, each on a fresh isolated view of the same snapshot: `master` (default runtime
settings), `forced-interpolation`, then `auto-cv-0.2`, `auto-cv-0.05`, `auto-cv-0.1`, `auto-cv-0.15`,
`auto-cv-0.25`, `auto-cv-0.35`, and `auto-cv-0.5`. Forced interpolation sets only the Account index
search type; auto arms set that type and their requested uniform CV threshold. Each arm uses the
pinned 497-record corpus at 100 rps, with a 120-second discarded warm-up and a 60-second measured
cell. The node's CPU and memory resource aggregates remain separate from measured latency results.
