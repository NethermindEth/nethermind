...
def to_nethermind_optimism_cl(config):
    roles = lookup(config, ["roles"])
    addresses = lookup(config, ["addresses"])
    root = {"BatcherInboxAddress": lookup(config, ["batch_inbox_addr"])}
    nodes = { 'nodes': [] }

    return merge_all(roles, addresses, root, nodes)
...
def to_nethermind_chainspec(chain_name, l1, superchain, chain, genesis):
    ...
    nethermind = {
        ...
        "engine": {
            ...
            "OptimismCL": to_nethermind_optimism_cl(config),
        },
        "params": {
