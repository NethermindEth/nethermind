...
- Interfaces marked `[StableApi]` (`Nethermind.Core.Attributes.StableApiAttribute`) are kept more stable than the rest of the codebase; their main purpose is to give plugins something they can rely on (callers need not be plugins, nor out-of-tree). Two rules apply:
  - Any change to a `[StableApi]` interface (adding, removing, or altering members) **must be clearly described in the PR description** so reviewers can assess the impact on plugins.
  - Core code must not force-cast (`(T)x`), `as`-cast, or type-check (`x is T`) a value against a `[StableApi]` interface — a plugin may supply a decorator or an alternative implementation. The `NETH008` analyzer enforces this; reflection/discovery (`typeof`, `IsAssignableFrom`, `OfType<T>()`) is not flagged.
