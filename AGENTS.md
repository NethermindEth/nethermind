...
This file provides guidance to coding agents when working with the code in this repository.
...
- .NET SDK 10 or later (required by `global.json`)
...
dotnet test --project Nethermind.Core.Test/Nethermind.Core.Test.csproj
...
dotnet test --project <project>.csproj --filter FullyQualifiedName~ClassName
```
