...
- Every profiled run also collects a **dotnet-trace EventPipe sidecar** (`.nettrace`, in the same `dottrace-*` artifact). It carries GC pause durations, lock contention, and exception events, which no CPU profile shows — use it whenever the question is about tail latency or stalls rather than hot code, and note it is the only structured output for `timeline` runs.
