...
- CPU budget experiments: `-f docker_cpu=<n|all>`, `-f docker_cpuset=<list|all>` and `-f docker_infra_cpuset=<list|all>` override `resources.cpu` / `resources.cpuset` / `resources.infra_cpuset` for that dispatch only (the runner's config file is untouched; empty = configured value; `all` = no quota / no pinning). The render step prints the effective `resources` block and the run step prints each expb container's `cpu.max` and `cpuset.cpus.effective` from the host cgroup, so read those rather than trusting the inputs. amd64 pins the client to 2-7,10-15 (6 physical cores with their SMT siblings) under an 8-CPU quota, so raising only `docker_cpu` past 12 changes nothing: the cpuset is the cap, and the prewarmer plus TrieWarmer already fill it during execution.
