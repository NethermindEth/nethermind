...
- Metrics: two tables per payload set. "newPayload request time (k6)" is the per-payload request time from expb's pipe table (k6's time to first byte today), the figure the consensus client waits for. "Block processing (Nethermind)" is the client's own processing time from the SSE data feed (`[payload-server] client_metric` lines) with MGas/s derived from it; use it for EVM and state changes. A change that moves time between the two windows shows up as opposite deltas, so read both. Without SSE data only the request table is produced.
