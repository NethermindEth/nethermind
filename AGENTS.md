...
- [zkevm-guest.md](./.agents/rules/zkevm-guest.md) — Load when touching code the ZisK guest compiles (anything the stateless executor reaches) or when optimising for the zkVM. Covers the prover's memory cost model, the std/zkevm file split, the NativeAOT/ILC codegen traps that cost whole percentage points, the host benchmarks a shared-code change has to pass, and the ideas already measured and rejected.
