...
- CPU budget experiments: `-f docker_cpu=<n>` and `-f docker_cpuset=<list>` override the client container's `resources.cpu` / `resources.cpuset` for that dispatch only (the runner's config file is untouched; empty = configured value). Both boxes run the client on an 8-CPU quota by default, and the prewarmer plus TrieWarmer fill it during execution, so background-overlap features need this to show headroom.
